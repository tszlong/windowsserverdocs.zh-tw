 ...
 **影響：**  如果此服務停止，用戶端電腦將無法從伺服器接收 IP 位址。
 ...
 **影響：**  如果預設的網站記錄檔資料夾太大，可能會導致磁片磁碟機 C： s 的可用空間問題：
 ...
 **影響：**  如果/Remote 虛擬目錄的讀取和腳本許可權不正確，使用者將無法使用遠端 Web 存取。 當他們嘗試使用遠端 Web 存取流覽網際網路時，可能會遇到「HTTP 錯誤403.1 禁止」錯誤。
