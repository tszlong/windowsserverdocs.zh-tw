...
## <a name="using-the-tool"></a>使用工具  
 這是標準的作法，當您從 Windows Server 2011 Essentials、Windows Small Business Server 2011 Essentials 或 Windows Home Server 2011 遷移至 Windows Server Essentials 時，若要在完成遷移之後，在目的地伺服器上執行 BPA設定和資料。 您可以隨時從儀表板執行此工具。  
...
##### <a name="to-configure-the-dhcp-client-service-to-start-automatically"></a>若要設定 DHCP 用戶端服務自動啟動  
 ...
 **影響：** 服務可能沒有如預期般運作所需的許可權。 因此，使用 SQL Server 或 COM 函數的應用程式可能無法正確運作。  
 ...
 **解決方式：** 如需如何授與內建 Administrators 群組以批次工作登入許可權的相關資訊，請參閱[提供內建的系統管理員群組以批次工作登入的權利](https://technet.microsoft.com/library/jj635076)（ https://technet.microsoft.com/library/jj635076)。  
 ...
 **解決方式：** 請勿設定 Windows Server Backup 來備份非 NTFS 磁碟分割。 如需詳細資訊，請參閱[當 Windows Server 2008 電腦上的系統狀態備份失敗時，會記錄事件識別碼12290和16387（知識庫文章968128）](https://support.microsoft.com/kb/968128) （ https://support.microsoft.com/kb/968128)。  
