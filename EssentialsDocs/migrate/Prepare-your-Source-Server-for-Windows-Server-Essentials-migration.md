...
4.  [在來源伺服器上執行「遷移準備工具」](Prepare-your-Source-Server-for-Windows-Server-Essentials-migration.md#BKMK_MPT)  
  
5.  [建立可遷移企業營運應用程式的計畫](Prepare-your-Source-Server-for-Windows-Server-Essentials-migration.md#BKMK_PlanToMigrateLineOfBusinessApplications)  
...
3.  [評估來源伺服器的健全狀況](../migrate/Prepare-your-Source-Server-for-Windows-Server-Essentials-migration.md#BKMK_UseWindowsBestPracticeAnalyzer)  
  
4.  [在來源伺服器上執行「遷移準備工具」](../migrate/Prepare-your-Source-Server-for-Windows-Server-Essentials-migration.md#BKMK_MPT)  
...
1.  將來源伺服器進行完整備份。 如需有關備份 Windows Small Business Server 2011 Essentials 的詳細資訊，請參閱[深入瞭解設定伺服器備份](https://technet.microsoft.com/library/server-backup-support-1.aspx)。  
...
5. 若未列出任何群組，請按一下 **[新增]** ，然後新增未列出的每個群組。  
   ...
   您必須先安裝 Windows PowerShell 2.0 和 Microsoft 基準設定分析器2.0，才可以在來源伺服器上安裝 Windows Update 代理程式。  
  
-   若要下載並安裝 Windows PowerShell 2.0，請參閱 Microsoft 知識庫中的[文章 968929](https://go.microsoft.com/fwlink/p/?LinkId=241483) 。  
  
-   若要下載並安裝 Microsoft 基準設定分析程式2.0，請參閱 Microsoft 下載中心的[Microsoft 基準設定分析程式 2.0](https://go.microsoft.com/fwlink/p/?LinkId=241484) 。  
...
1. 在來源伺服器的 DVD 光碟機中插入 Windows Server Essentials DVD1。  
...
4. 備妥來源網域之後，「移轉準備工具」會掃描來源伺服器，以辨識出兩種類型的可能問題。  
   ...
   - **警告**在來源伺服器上發現的問題，可能會在遷移期間造成功能問題。 強烈建議您先遵循錯誤訊息中的指示修正問題，再繼續進行移轉。  
  
     在修正或瞭解所有問題之後，按 **[下一步]** 。  
  
5. 在 [移轉準備工具] 中，按一下 **[下一步完成]** 。  
  
6. 當「遷移準備工具」完成時，系統可能會提示您重新開機來源伺服器，然後才可以開始遷移至 Windows Server Essentials。  
