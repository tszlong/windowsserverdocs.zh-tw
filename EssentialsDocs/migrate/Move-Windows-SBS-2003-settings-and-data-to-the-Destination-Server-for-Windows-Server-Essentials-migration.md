---
title: "將 Windows SBS 2003 設定和資料移到目的地 Windows Server Essentials 伺服器移轉"
...
---
# <a name="move-windows-sbs-2003-settings-and-data-to-the-destination-server-for-windows-server-essentials-migration"></a>將 Windows SBS 2003 設定和資料移到目的地 Windows Server Essentials 伺服器移轉

>適用於：Windows Server 2016 Essentials 程式集 Windows Server 2012 R2、Windows Server 2012 程式集

前往設定和資料目的伺服器，如下所示：

1.  [複製到目的伺服器的資料](Move-Windows-SBS-2003-settings-and-data-to-the-Destination-Server-for-Windows-Server-Essentials-migration.md#BKMK_CopyData)  
...
6.  [允許電腦帳號地圖](Move-Windows-SBS-2003-settings-and-data-to-the-Destination-Server-for-Windows-Server-Essentials-migration.md#BKMK_MapPermittedComputers)  
...
2.  按一下**[開始]**，輸入**cmd**中的搜尋方塊中，然後按下 ENTER。  
...
##  <a name="BKMK_RemoveScripts"></a>移除舊登入指令碼（選擇性）  
 使用安裝的軟體和自訂的桌面工作的指令碼登入 Windows SBS 2003。  Windows Server Essentials 的指令碼登入的群組原則物件組合來取代 Windows SBS 2003 登入指令碼。  
  
> [!NOTE]
>  如果您修改 Windows SBS 2003 登入指令碼，您應該會重新命名的指令碼保留您的自訂項目。  
  
> [!NOTE]
>  Windows SBS 2003 登入指令碼僅適用於帳號已新增透過**新增新的使用者精靈**。  
  
#### <a name="to-remove-the-windows-sbs-2003-logon-scripts"></a>若要移除的 Windows SBS 2003 登入指令碼  
  
1.  按一下**[開始]**，指向 [**系統管理工具**，然後按一下 [ **Active Directory 使用者和電腦**。  
  
2.  在**Active Directory 使用者和電腦**，展開 [您的網路，然後按**使用者**。  
  
3.  以滑鼠右鍵按一下 [使用者名稱，請按一下**屬性**，然後按一下 [**設定檔**索引標籤。  
  
4.  Delete 到**指令碼登入**文字方塊，然後再按一下**[確定]**。  
  
5.  每個使用者的重複步驟 3、4。  
  
##  <a name="BKMK_RemoveLegacyADGPO"></a>移除舊版 Active Directory 群組原則物件（選擇性）  
 群組原則物件 (Gpo) 的 Windows Server Essentials 的更新。 有更多的 Windows SBS 2003 Gpo。 針對 Windows Server Essentials，Windows SBS 2003 Gpo 及 Windows 管理檢測 (WMI) 篩選一些必須手動刪除以防止衝突與 Windows Server Essentials Gpo 和 WMI 篩選。  
  
> [!NOTE]
>  如果修改原始 Windows SBS 2003 群組原則物件，您應該將複本儲存在不同的位置，並再加以移除 Windows SBS 2003。  
  
#### <a name="to-remove-old-group-policy-objects-from-windows-sbs-2003"></a>若要從 Windows SBS 2003 移除舊的群組原則物件  
...
6.  重複執行「步驟 5 delete 下列 Gpo 套用到您的網路：  
    ...
    -   小型企業伺服器網域密碼原則  
         ...
         我們建議您在 Windows Server Essentials 執行穩固密碼設定密碼原則。 若要設定 [密碼原則，使用儀表板，將設定為預設網域原則。 例如是 Windows SBS 2003 小型企業伺服器網域密碼原則物件，不是寫入密碼原則設定。  
    ...
    -   小型企業 Server Update Services Client 電腦原則  
         ...
         此 GPO 會出現，如果您從 Windows SBS 2003 R2 移轉。  
...
#### <a name="to-remove-wmi-filters-from-windows-sbs-2003"></a>若要移除 Windows SBS 2003 WMI 篩選  
...
3.  中的指示，請遵循**設定隨處存取**來設定您的路由器和網域名稱精靈。  
...
##  <a name="BKMK_MapPermittedComputers"></a>允許電腦帳號地圖  
 在 Windows SBS 2003，如果使用者連接到遠端的網路存取，會顯示所有網路中的電腦。 這可能包括使用者並不需要存取權限的電腦。 在 Windows Server Essentials，使用者必須明確指派到電腦，它會顯示在網路上的存取。 從 Windows SBS 2003 移轉的每個使用者帳號必須對應至一部以上的電腦。  
...
6.  按一下**電腦存取**索引標籤，然後按一下您要允許存取電腦的名稱。  
