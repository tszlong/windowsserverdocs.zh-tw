...
3.  如果您嘗試連線到 Windows Server Essentials 網路的伺服器上已啟用 Internet Explorer 增強式安全性設定，請完成下列工作；否則請跳過這個步驟。  
    ...
    > [!NOTE]
    >  將第二部伺服器連線到執行 Windows Server Essentials 的伺服器與連線到用戶端電腦不同，如下所示：  
    >   
    >  -   並沒有使用者設定檔移轉；因此，將不會移轉目前的設定檔。  
    > -   您無法使用用戶端電腦備份來備份第二部伺服器，而且也沒有可喚醒電腦來進行備份的選項。  
...
3.  使用 [Connect Computer 精靈] 將電腦連線到伺服器。 如需逐步指示，請參閱[將電腦連線到伺服器](Get-Connected-in-Windows-Server-Essentials.md#BKMK_9)。  
