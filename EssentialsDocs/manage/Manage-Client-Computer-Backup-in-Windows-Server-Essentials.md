...
### <a name="critical-system-files"></a>重要系統檔案  
 當您安裝 Windows 作業系統時，安裝程式會在它放置系統啟動和執行所需之檔案的系統磁碟機上建立資料夾。 重要系統檔案包含副檔名為 .dll、.exe、.ocx 及 .sys 的檔案。 這當中有些檔案是 True Type 字型。 此外，系統狀態檔案，例如系統登錄中，所需作業系統正常執行。  
