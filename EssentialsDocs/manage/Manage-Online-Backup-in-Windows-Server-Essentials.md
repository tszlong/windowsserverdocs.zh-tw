...
3. 在 [線上備份]  區域中，按一下 [將憑證上傳到 Azure 備份保存庫]  。  
    ...
    這會開啟**復原服務**在 Azure 管理入口網站。 如果您不已登入 Azure，您必須使用您的 Microsoft 帳戶登入。  
...
9. 使用 Azure 備份服務註冊伺服器。 如需詳細資訊，請參閱[登錄這部伺服器來進行備份](Manage-Online-Backup-in-Windows-Server-Essentials.md#BKMK_5)。  
  
10. 設定伺服器的線上備份。 如需詳細資訊，請參閱 [Configure online backup](Manage-Online-Backup-in-Windows-Server-Essentials.md#BKMK_2)。  
...
5.  在 Azure 管理入口網站頁面上，您會看到**復原服務**的左窗格中。 這是您要在其中管理儲存您從 Windows Server Essentials 的線上備份的備份保存庫。  
...
6. 在 Windows Server Essentials 中上, 傳要用於線上備份的憑證。 如需詳細資訊，請參閱[將憑證上傳到 Azure 備份保存庫](Manage-Online-Backup-in-Windows-Server-Essentials.md#BKMK_1)。  
  
7. 使用 Azure 備份保存庫註冊伺服器。 如需詳細資訊，請參閱[登錄這部伺服器來進行備份](Manage-Online-Backup-in-Windows-Server-Essentials.md#BKMK_5)。  
  
8. 設定伺服器的線上備份。 如需詳細資訊，請參閱 [Configure online backup](Manage-Online-Backup-in-Windows-Server-Essentials.md#BKMK_2)。  
