...
3.  如果儲存體空間不 t 存在，請使用新的硬碟來建立儲存空間。 您必須要有至少兩個硬碟才可建立儲存空間。  
...
##  <a name="BKMK_5"></a> 儲存空間概觀  
 儲存空間可讓您將磁碟區聚集在儲存集區中。 接著，您便可以使用集區容量建立儲存空間。 儲存空間是會顯示在儀表板 [硬碟] 索引標籤上的虛擬硬碟。 您可以使用任何其他磁碟機，例如儲存空間，因此它可以很輕易地使用在其上的檔案。 當集區容量不足時，您可以建立大型的儲存空間，以及在儲存集區中加入更多磁碟機。 如果您有兩個或多個磁碟的儲存體集區中時，您可以建立儲存空間將不會受到磁碟機故障影響的雙向鏡像？ 或甚至是兩部磁碟機失敗嗎？ 如果您建立三向鏡像儲存空間。  
