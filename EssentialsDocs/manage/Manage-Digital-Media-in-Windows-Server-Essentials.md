...
|媒體櫃|顯示媒體櫃的名稱。 媒體櫃的預設名稱叫做 [數位媒體櫃]，這是在您開啟媒體串流處理時所建立。 若要變更媒體櫃名稱，請參閱 [Rename the media library](Manage-Digital-Media-in-Windows-Server-Essentials.md#BKMK_8)。 您可以按一下 [自訂] 來自訂在媒體櫃中共用的資料夾。|  
...
4. 按一下 [確定] 以儲存您的變更。  
   ...
   如需詳細資訊，請參閱 [Allow or restrict access to a media library on the server](Manage-Digital-Media-in-Windows-Server-Essentials.md#BKMK_6)。  
...
1.  請使用下列其中一種方法來登入伺服器：  
    ...
    1.  如需登入「遠端 Web 存取」的相關資訊，請參閱 [Log on to Remote Web Access](../use/Use-Remote-Web-Access-in-Windows-Server-Essentials.md#BKMK_1)。  
...
##  <a name="BKMK_6"></a>允許或限制存取伺服器上的媒體庫  
...
##  <a name="BKMK_10"></a>啟用使用伺服器訊息區（SMB）通訊協定的媒體裝置，以存取伺服器上的共用檔案  
...
##  <a name="BKMK_CommonProcessors"></a>常見的處理器和其支援的影片設定檔  
...
##  <a name="BKMK_KnownIssues"></a>媒體檔案類型的已知問題  
