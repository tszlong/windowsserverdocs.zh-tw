...
>  在 Windows Server Essentials 和已安裝的 Windows Server Essentials 體驗角色的 Windows Server 2012 R2 中，伺服器和網路中的用戶端電腦的健康情況警示已不再顯示於 警示檢視器，但改為可檢視上的**健康情況報告**索引標籤**首頁**頁面。  
 ...
 Windows Server Essentials 會主動監控每一部電腦連接到伺服器，並發出警示系統管理員問題相關系統 s 健全狀況，包括重大更新、 缺少惡意程式碼防護，病毒定義過時用戶端上電腦和其他重要的問題，需要採取行動。 這些問題會顯示為可以啟動從伺服器儀表板或用戶端電腦的 Launchpad in Windows Server Essentials，或在 警示檢視器中的警示**健康情況報告**Windows Server Essentials 中的索引標籤。 警示預設是每 30 分鐘重新整理一次，但是您也可以按一下 [警示檢視器] 中或 [健康情況報告] 索引標籤上的 [重新整理]，來隨時評估您的網路是否有警示。  
...
>   
>  **Windows Server Essentials:** 根據預設，健康情況報告增益集與整合 Windows Server Essentials 或 Windows Server 2012 R2 與 Windows Server Essentials 體驗角色安裝，而健康情況報告顯示在**健康情況報告** 索引標籤儀表板 s**首頁**頁面。  
...
-   **存放** [詳細資料] 區段中會顯示磁碟機及其容量的清單。  
 ...
 如果您對報告中預設包含的某些資料點不感興趣，只要按一下工作窗格中的 [自訂健康情況報告設定]  ，然後按一下 [內容]  索引標籤，即可自訂報告的內容。清除核取方塊的 don t 想要在報表中看到的內容。 比方說，如果您有自己的伺服器備份計劃，不要想要看到有關伺服器備份的警告，您無法排除伺服器備份從報告藉由清除**伺服器備份**核取方塊。  
...
4.  在 [SMTP 設定] 視窗中，執行下列動作：  
    ...
    1.  在 [寄件者電子郵件地址] 中，輸入要用來傳送電子郵件警示的寄件者電子郵件地址。 此電子郵件地址會顯示為寄件者的址，在警示的通知。  
...
> [!NOTE]
>  根據預設，健康情況報告增益集與整合 Windows Server Essentials 或 Windows Server 2012 R2 與 Windows Server Essentials 體驗角色安裝，而健康情況報告顯示在**健康情況報告** 索引標籤儀表板 s**首頁**頁面。  
...
4.  在 [排程與電子郵件] 索引標籤的 [電子郵件] 區段中，執行下列動作：  
    ...
    1.  按一下 [啟用] ，然後輸入要用來傳送健康情況報告的寄件者電子郵件地址。 此電子郵件地址會顯示為寄件者的址，會以電子郵件傳送健康情況報告中。  
