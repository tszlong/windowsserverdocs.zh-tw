...
|5|工作窗格|[工作] 窗格包含工具和資訊的連結，可協助您管理特定物件 (例如使用者帳戶或電腦) 的內容，或物件類別之全域設定的內容。 [工作] 窗格分為下列兩個區段：<br /><br /> **物件工作**œ 包含連結，工具和資訊可協助您管理物件 （例如使用者帳戶或電腦） 的 [清單] 檢視中您所選取的屬性。<br /><br /> **全域工作**œ 包含連結，工具和資訊可協助您管理功能區域的全域工作。 全域工作包括新增物件、設定原則等工作。|  
|6|資訊和設定|這個區段可讓您直接存取伺服器設定，並提供您正在檢視之儀表板頁面的相關資訊說明連結。|  
|7|警示狀態|警示狀態提供有關伺服器健康情況的快速視覺指示。 按一下警示影像以檢視重大和重要的警示。<br /><br /> **注意：** 在 Windows Server Essentials 和 Windows Server 2012 R2 Standard 已安裝的 Windows Server Essentials 體驗角色，警示狀態位於**資訊和設定** 索引標籤。|  
...
|項目|功能|描述|  
|----------|-------------|-----------------|  
|1|瀏覽列|按一下瀏覽列上的區段以存取與該區段關聯的資訊和工作。 每次您開啟儀表板時，預設會顯示 [首頁]  。|  
...
|儲存體|視您目前執行的 Windows Server Essentials 版本而定，[儀表板] 的 [存放]  區段預設會包含下列各區段：<br /><br /> -**伺服器資料夾**小節包括可協助您的工作檢視和管理伺服器資料夾的屬性。 這個頁面也包含可開啟和新增伺服器資料夾的工作。<br /><br /> -**硬碟**頁面包含可協助您檢視和檢查連接到伺服器的磁碟機的健康狀態的工作。<br /><br /> -在 Windows Server Essentials 和 Windows Server 2012 R2 Standard 與 Windows Server Essentials 體驗角色啟用， **SharePoint 文件庫**頁面包含的工作，協助您管理 Office 365 中的 SharePoint 文件庫服務。<br /><br /> 管理伺服器資料夾的相關資訊，請參閱[Manage Server Folders](Manage-Server-Folders-in-Windows-Server-Essentials.md)。<br /><br /> 如需管理硬碟的資訊，請參閱[管理伺服器儲存體](Manage-Server-Storage-in-Windows-Server-Essentials.md)。|  
|應用程式|-**應用程式**區段的 Windows Server Essentials 儀表板預設包含兩個子區段。<br /><br /> 如需管理增益集應用程式的資訊，請參閱[管理的應用程式](Manage-Applications-in-Windows-Server-Essentials.md)。<br /><br /> -**增益集**子區段會顯示一份已安裝的增益集，並提供可讓您移除增益集，以及存取對於選取的增益集的其他資訊的工作。<br /><br /> - **Microsoft Pinpoint**子區段會顯示一份可從 Microsoft Pinpoint 的應用程式。|  
|Office 365|只有當 Windows Server Essentials 已與 Microsoft Office 365 整合時，才會顯示 [Office 365]  索引標籤。 這個區段包含 Office 365 訂用帳戶和系統管理員帳戶資訊。|  
  
> [!NOTE]
>  如果您安裝適用於 [Windows Server Essentials 儀表板] 的增益集，該增益集可能會建立額外的系統管理區段。 這些區段可能會出現在主導覽列或子區段索引標籤上。  
  
##  <a name="BKMK_AccessDb"></a> 存取 Windows Server Essentials 儀表板  
...
###  <a name="BKMK_Network"></a> 從連線到網路的電腦存取儀表板  
 在 Windows Server Essentials 中，在您將電腦加入網路之後，系統管理員便可使用 [啟動列] 從電腦存取伺服器 [儀表板]。  
  
> [!WARNING]
>  在 Windows Server Essentials 中，若要從用戶端電腦連線到儀表板，以滑鼠右鍵按一下系統匣圖示，，然後再從操作功能表開啟儀表板。  
...
###  <a name="BKMK_Remote"></a> 從遠端電腦存取儀表板  
 當您使用從遠端電腦時，您可以使用 「 遠端 Web 存取 」 網站來存取 Windows Server Essentials 儀表板。  
  
##### <a name="to-access-the-dashboard-by-using-remote-web-access"></a>使用遠端 Web 存取來存取儀表板  
  
1.  從遠端電腦，開啟網際網路瀏覽器 (例如 Internet Explorer)。  
  
2.  在網際網路瀏覽器網址列中輸入下列命令：**https://<DomainName\>/遠端**，其中*DomainName*是您的組織 （例如 contoso.com) 的外部網域名稱。  
  
3.  輸入您的使用者名稱和密碼以登入遠端 Web 存取網站。  
  
4.  在**電腦**的 [遠端 Web 存取] 區段**首頁**; 找出您的伺服器，然後按一下**Connect**。  
...
##  <a name="BKMK_UseSafeMode"></a> 使用安全模式  
 ...
 您可以隨時檢視安裝在伺服器上的所有增益集清單。 您也可以使用「安全模式」來開啟 [儀表板]，這會自動停用所有非 Microsoft 增益集。Windows Server Essentials 也可讓您從網路上的另一部電腦以「安全模式」存取 [儀表板]。  
