...
| 特色/功能 | 新功能或更新功能？ | 說明 |
| ---------------------------- | --------------------- | ----------------- |
| 改進記錄功能 | Windows Server 2019 中的新功能 | 工作資料夾伺服器上的事件記錄檔可以用來監視同步處理活動，以及識別失敗同步會話的使用者。 使用 Microsoft Windows-Set-syncshare/Operational 事件記錄檔中的事件識別碼4020來識別哪些使用者失敗的同步會話。 使用 Microsoft Windows-Set-syncshare/Reporting 事件記錄檔中的事件識別碼7000和事件識別碼7001，監視成功完成上傳和下載同步會話的使用者。 |
| 效能計數器 | Windows Server 2019 中的新功能 | 已新增下列效能計數器：下載的位元組數/秒、已上傳的位元組數/秒、連線的使用者、每秒下載的檔案數、每秒上傳的檔案數、每秒上傳的檔案數/秒，以及未處理的要求。 |
| 改善的伺服器效能 | Windows Server 2019 中的更新 | 針對每一部伺服器處理更多使用者所做的效能改進。 每部伺服器的限制各有不同，而且是以檔案和檔案變換的數目為基礎。 若要判斷每個伺服器的限制，應以階段將使用者新增至伺服器。 |
| 隨選檔案存取 | 已新增至 Windows 10 版本1803 | 可讓您查看及存取所有檔案。 您可以控制要儲存在電腦上並可離線使用的檔案。 其餘的檔案永遠都是可見的，而不會佔用電腦上的任何空間，但您必須連線到工作資料夾檔案伺服器才能存取它們。 |
| Azure AD 應用程式 Proxy 支援 | 已新增至 Windows 10 版本 1703、Android、iOS | 遠端使用者可以使用 Azure AD 應用程式 Proxy，安全地存取他們在工作資料夾伺服器上的檔案。 |
| 更快速地變更複寫 | Windows 10 和 Windows Server 2016 中的更新 | 在 Windows Server 2012 R2 中，將檔案變更同步處理到工作資料夾伺服器時，用戶端不會收到變更的通知，而且最多等待 10 分鐘就會收到更新。 使用 Windows Server 2016 時，工作資料夾伺服器會立即通知 Windows 10 用戶端，並立即同步處理檔案變更。 這是 Windows Server 2016 的新功能，且需要 Windows 10 用戶端。 如果您使用的是舊版用戶端，或工作資料夾伺服器是 Windows Server 2012 R2，則用戶端會每隔 10 分鐘繼續輪詢變更。 |
...
-   執行 Windows Server 2019、Windows Server 2016 或 Windows Server 2012 R2 的伺服器，以便裝載具有使用者檔案的同步共用
...
-    (選擇性) 具有 Windows Server 2012 R2 中架構延伸的 Active Directory Domain Services 樹系，以支援在使用多個檔案伺服器時，自動將電腦和裝置參照到正確的檔案伺服器。
