...
<td><p>512</p></td>
<td><p>否</p></td>
<td><p>包含的測試設定：每個目標實例 8 Lu，平均超過64個目標，以及每個目標有一個 LU 的256目標實例。</p></td>
</tr>
<tr class="odd">
...
<td><p>32</p></td>
<td><p>是</p></td>
<td><p>本機掛接的虛擬磁片&#39;不提供任何 iSCSI 特定功能，且已淘汰-如需詳細資訊，請參閱<a href="https://technet.microsoft.com/library/dn303411.aspx">Windows Server 2012 R2 中已移除或過時的功能</a>。</p></td>
</tr>
</tbody>
</table>
...
<td><p>VHD 動態格式</p></td>
<td><p>vhdx是</p>
<p>vhd是（在 Windows Server 2012 上為否）</p></td>
<td><p>是</p></td>
<td><p>&#39;不支援取消對應。</p></td>
</tr>
<tr class="odd">
...
如果您想要從應用程式伺服器建立 iSCSI 虛擬磁片上的磁片區陰影複製（VSS 檔案快照集），或想要使用需要虛擬磁碟服務（VDS）硬體的繼承應用程式（例如 Diskraid 命令）來管理 iSCSI 虛擬磁片提供者，在您想要建立快照集或使用 VDS 管理應用程式的伺服器上，安裝 iSCSI 目標儲存提供者。

ISCSI 目標儲存提供者是 Windows Server 2016、Windows Server 2012 R2 和 Windows Server 2012 中的角色服務。只要 iSCSI 目標伺服器是在 Windows Server 2012 上執行，您也可以在下列作業系統上下載並安裝[適用于下層應用程式伺服器的 Iscsi 目標儲存提供者（VDS/VSS）](http://www.microsoft.com/download/details.aspx?id=34759) ：
...
請注意，如果 iSCSI 目標伺服器是由執行 Windows Server 2012 R2 或更新版本的伺服器所主控，而您想要從遠端伺服器使用 VSS 或 VDS，則遠端伺服器也必須執行相同版本的 Windows Server，並將 iSCSI 目標儲存提供者角色 service 已安裝。 另請注意，在所有版本的 Windows 上，您應該只安裝一個版本的 iSCSI 目標儲存提供者角色服務。

如需 iSCSI 目標儲存提供者的詳細資訊，請參閱[Iscsi 目標儲存體（VDS/VSS）提供者](http://blogs.technet.com/b/filecab/archive/2012/10/08/iscsi-target-storage-vds-vss-provider.aspx)。
...
- [iSCSI 目標區塊存放裝置總覽](iscsi-target-server.md)

- [iSCSI 目標開機總覽](iscsi-boot-overview.md)
