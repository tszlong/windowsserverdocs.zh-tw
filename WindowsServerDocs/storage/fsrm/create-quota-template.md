...
**若要設定配額閾值的檔案伺服器資源管理員會產生的通知**

1.  在 **\[建立配額範本\]** 對話方塊的 **\[通知閾值\]** 下方，按一下 **\[新增\]**。 **\[新增閾值\]** 對話方塊隨即出現。

2.  若要設定產生通知的配額限制百分比：

    在 **\[當使用量達到下列數值 (%) 時，產生通知\]** 文字方塊中，輸入通知閾值的配額限制百分比 （第一個通知閾值的預設百分比是 85%）。
...
3.  若要設定電子郵件通知：
    ...
    -   若要設定訊息，請編輯提供的預設主旨列和郵件內文。 方括弧中的文字會插入關於引發通知之配額事件的變數資訊。 例如， **\[Source Io Owner\]** 變數會插入儲存檔案已達配額閾值的使用者名稱。 若要在文字中插入其他變數，請按一下 **\[插入變數\]**。
