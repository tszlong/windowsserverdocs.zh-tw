...
新的目錄名稱會依據檔案管理工作名稱和執行時間來命名。 發現已過期的檔案時，會將它移到新的目錄，同時保留其原始目錄結構。
...
5. 選擇性地在 **\[通知\]** 索引標籤上按一下 **\[新增\]** ，在工作對檔案執行動作的指定至少天數前，傳送電子郵件通知、記錄事件，或執行命令或指令碼。
   ...
   - 若要設定電子郵件通知，請按一下 **\[電子郵件訊息\]** 索引標籤，並輸入下列資訊：
     ...
     - 若要設定訊息，請編輯提供的預設主旨列和郵件內文。 方括弧中的文字會插入關於引發通知之配額事件的變數資訊。 例如， **\[原始**程式檔擁有者\]變數會插入檔案即將到期的使用者名稱。 若要在文字中插入其他變數，請按一下 **\[插入變數\]** 。
