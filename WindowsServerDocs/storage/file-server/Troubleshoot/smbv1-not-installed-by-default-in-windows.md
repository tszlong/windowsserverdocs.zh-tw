...
在 Windows 10 秋季建立者更新和 Windows Server 版本 1709 (RS3) 和更新版本中，預設不會安裝伺服器訊息區第1版 (SMBv1) 網路通訊協定。 從2007開始，SMBv2 和更新的通訊協定會取代它。 Microsoft 已公開淘汰2014中的 SMBv1 通訊協定。

從1709版開始，SMBv1 在 Windows 10 和 Windows Server 中具有下列行為 (RS3) ：

- SMBv1 現在具有可分別卸載的用戶端和伺服器子功能。
- 在全新安裝之後，windows 10 企業版、Windows 10 教育版和 Windows 10 專業版的工作站預設不再包含 SMBv1 用戶端或伺服器。
- Windows Server 2016 在全新安裝之後，預設不再包含 SMBv1 用戶端或伺服器。
- Windows 10 家用版和 Windows 10 專業版在全新安裝之後，預設不再包含 SMBv1 伺服器。
- Windows 10 家用版和 Windows 10 專業版在進行全新安裝之後，預設仍包含 SMBv1 用戶端。 如果 SMBv1 用戶端未使用15天的總計 (排除電腦) 關閉，它會自動卸載。
- Windows 10 家用版和 Windows 10 專業版的就地升級和 Insider 航班，一開始不會自動移除 SMBv1。 如果 SMBv1 用戶端或伺服器未使用超過15天， (排除) 電腦關閉的時間，則會各自自動卸載。 
- Windows 10 企業版、Windows 10 教育版和 Windows 10 專業版（適用于工作站）的就地升級和 Insider 航班不會自動移除 SMBv1。 系統管理員必須決定在這些受管理的環境中卸載 SMBv1。
...
從1809版開始，SMBv1 在 Windows 10 中具有下列其他行為 (RS5) 。 1709版的所有其他行為仍然適用：

- Windows 10 專業版在全新安裝之後，預設不再包含 SMBv1 用戶端。
- 在 Windows 10 Enterprise、Windows 10 教育版和適用于工作站的 Windows 10 Pro 中，系統管理員可以藉由開啟「SMB 1.0/CIFS 自動移除」功能來啟用自動移除 SMBv1。

  > [!NOTE]
  > Windows 10 版本 1803 (RS4) Pro 處理 SMBv1 的方式，與 Windows 10，版本 1703 (RS2) 和 Windows 10 版本 1607 (RS1) 。 此問題已在 Windows 10 1809 版 (RS5) 中修正。 您仍然可以手動卸載 SMBv1。 不過，在下列案例中，Windows 不會在15天后自動卸載 SMBv1：
...
1. 啟動 [函式探索提供者主機] 和 [函數探索資源發行集] 服務，然後將它們設定為**自動 (延遲開始) **。
...
Windows Server 2012 和更新版本的伺服器作業系統包含適用于檔案伺服器 (BPA) 的最佳做法分析程式。 如果您已遵循正確的線上指引來卸載 SMB1，執行此 BPA 將會傳回矛盾的警告訊息：
