...
> [!IMPORTANT]
> 這項測試中的四部伺服器即為範例。 您可以在每個叢集中使用 Microsoft 支援之任意數目的伺服器，目前儲存空間直接存取叢集支援 16 個，而共用存放裝置叢集支援 64 個。  
>   
...
9. 如下所示設定儲存體：  
    ...
    -   **對於儲存空間直接存取：**  
        ...
        1.  確定每個叢集只能透過部署儲存空間直接存取看到該網站的存放裝置機箱。 (https://docs.microsoft.com/en-us/windows-server/storage/storage-spaces/hyper-converged-solution-using-storage-spaces-direct) 
...
4.  設定檔案共用見證或雲端見證。  
    ...
    > [!WARNING]  
    > 如需仲裁設定的詳細資訊，請參閱[設定和管理 Windows Server 2012 容錯移轉叢集中的仲裁](http://technet.microsoft.com/library/jj612870.aspx)中的**＜見證設定＞**一節。 如需 `Set-ClusterQuorum` Cmdlet 的詳細資訊，請參閱 [Set-ClusterQuorum](http://technet.microsoft.com/library/hh847275.aspx)。  

5.  在 **Redmond** 網站中將一部磁碟新增至叢集 CSV。 若要這樣做，在 **/[存放裝置/]** 區段的 **/[磁碟/]** 節點中，使用滑鼠右鍵按一下來源磁碟，然後按一下 **/[新增至叢集共用磁碟區/]**。  

6.  使用[設定向外延展檔案伺服器](http://technet.microsoft.com/library/hh831718.aspx)中的指示，在兩個叢集上建立叢集向外延展檔案伺服器  
...
1.  在第一個叢集的任何節點上，或從遠端執行 **Grant-ClusterAccess** Cmdlet，將第一個叢集的完整存取權授與另一個叢集。  

    ```PowerShell
    Grant-SRAccess -ComputerName SR-SRV01 -Cluster SR-SRVCLUSB  
    ```  

2.  在第二個叢集的任何節點上，或從遠端執行 **Grant-ClusterAccess** Cmdlet，將第二個叢集的完整存取權授與另一個叢集。  
