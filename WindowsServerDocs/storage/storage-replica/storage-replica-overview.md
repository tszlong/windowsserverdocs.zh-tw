...
在延展式叢集，叢集-叢集之間以及伺服器對伺服器設定 （請參閱圖 1-3） 中，您可以部署儲存體複本。
...
* **客體和主機**： 儲存體複本的所有功能都會向虛擬化客體和主機型部署公開。 這表示來賓可以複寫其資料磁碟區即使在非 Windows 虛擬化平台上或公用雲端中執行，只要客體中使用 Windows Server。  
...
**測試容錯移轉**cmdlet 在 Windows Server 1709 版中推出，並也已包含在 Windows Server 2019。 這支援暫時掛接備份的目的地磁碟區的讀寫快照集、 測試、 等等。請參閱 https://aka.ms/srfaq如需詳細資訊。
...
-   儲存體複本不是備份解決方案。 相較於每日備份，某些 IT 環境基於複寫系統的零資料遺失選項，會將複寫系統部署為備份解決方案。 儲存體複本會將所有資料區塊的變更複寫到磁碟區上，無論該變更的類型為何。 如果使用者從磁碟區，刪除所有資料，儲存體複本會複寫刪除立即到另一個磁碟區，且無法復原從這兩個伺服器中移除資料。 請不要使用儲存體複本作為時間點備份解決方案的替代方案。  
