...
2. 建立兩個[可用性設定](https://ms.portal.azure.com/#create/Microsoft.AvailabilitySet-ARM)組，每個叢集的每個資源群組各一個。
    ...
    - 可用性設定組 (**azcross-如** (**SR azcross**中的) ) 

3. 建立兩個虛擬網路
   - 在**az2az**) 的第一個資源 (群組中建立[虛擬網路](https://ms.portal.azure.com/#create/Microsoft.VirtualNetwork-ARM) (**az2az-Vnet**) ，其中具有一個子網和一個閘道子網。
   - 在**azcross**) 的第二個資源 (群組中建立[虛擬網路](https://ms.portal.azure.com/#create/Microsoft.VirtualNetwork-ARM) (**azcross-VNET**) ，其中具有一個子網和一個閘道子網。

4. 建立兩個網路安全性群組
   - 在第一個資源群組中建立[網路安全性群組](https://ms.portal.azure.com/#create/Microsoft.NetworkSecurityGroup-ARM) (**az2az-NSG**) ， (**az2az**) 。
   - 在第二個資源群組中建立[網路安全性群組](https://ms.portal.azure.com/#create/Microsoft.NetworkSecurityGroup-ARM) (**azcross-NSG**) ， (**azcross**) 。
...
5. 在先前建立的資源群組中建立 Windows Server[虛擬機器](https://ms.portal.azure.com/#create/Microsoft.WindowsServer2016Datacenter-ARM)。
   ...
   在可用性設定組 (**AZ2AZ**) 中，使用虛擬網路 (**AZ2AZ-Vnet**) 和網路安全性群組 (**NSG-az2azAS1**)  (**Az2az1**、 **az2az2**) 在資源群組中建立兩部虛擬機器 **， (AZ2AZ**) 。 在建立時，將標準公用 IP 位址指派給每部虛擬機器。
      - 將至少兩個受控磁片新增至每部電腦
      - 安裝容錯移轉叢集和儲存體複本功能

   在可用性設定組 (**AZCROSS-AS**) 中，使用虛擬網路 (**AZCROSS-VNET**) 和網路安全性群組 (**NSG-AZCROSS**) ，在資源群組中建立兩個 (**azcross1**、 **azcross2**) 的虛擬機器 (**sr-iov AZCROSS**) 。 在建立時將標準公用 IP 位址指派給每部虛擬機器
      - 在每部電腦上至少新增兩個受控磁片
      - 安裝容錯移轉叢集和儲存體複本功能
...
8. 為每個叢集建立內部標準 SKU [Load Balancer](https://ms.portal.azure.com/#create/Microsoft.LoadBalancer-ARM) ， (**azlbr1**， **azlbazcross**) 。
   ...
   提供叢集 IP 位址做為負載平衡器的靜態私人 IP 位址。
      - azlbazcross => 前端 IP： 10.0.0.10 (從虛擬網路 (**azcross-VNET**) 子網中挑選未使用的 ip 位址) 
      - 為每個負載平衡器建立後端集區。 新增相關聯的叢集節點。
      - 建立健康情況探查：埠59999
      - 建立負載平衡規則：允許 HA 埠，並啟用浮動 IP。
...
9. 建立 Vnet 對 Vnet 連線的[虛擬網路閘道](https://ms.portal.azure.com/#create/Microsoft.VirtualNetworkGateway-ARM)。
   ...
   - 在第二個資源群組中建立第二個虛擬網路閘道 (**azcross-VNetGateway**)  (**sr-iov**) 
   - 閘道類型 = VPN，而 VPN 類型 = 路由式
...
13. 為這兩個叢集建立雲端見證。 在 Azure 中建立兩個[儲存體帳戶](https://ms.portal.azure.com/#create/Microsoft.StorageAccount-ARM) (**az2azcw**、**azcrosssa**) ，分別用於每個資源群組中**的每個叢集 (AZ2AZ**、 **sr-iov AZCROSS**) 。
...
15. 啟動 Windows PowerShell，然後使用 [Test-SRTopology](/powershell/module/storagereplica/test-srtopology?view=win10-ps) Cmdlet 來判斷您是否符合所有儲存體複本需求。 您可以在僅查看需求的模式中，使用此 Cmdlet 進行快速測試，還可以在評估長時間執行效能的模式中使用。
