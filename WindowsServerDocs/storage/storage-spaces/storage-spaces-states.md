...
|結束           |因為一或多個磁片磁碟機故障或遺失，虛擬磁片的復原能力會降低。 不過，遺失的磁片磁碟機包含最新的資料複本。<br><br> **動作**： <br>1. 重新連接任何遺失的磁片磁碟機，更換任何故障的磁片磁碟機，如果您是使用儲存空間直接存取，請將任何離線的伺服器上線。 <br>2. 如果您不是使用儲存空間直接存取，請使用[VirtualDisk](https://technet.microsoft.com/itpro/powershell/windows/storage/repair-virtualdisk) Cmdlet 來修復虛擬磁片。<br> 儲存空間直接存取在重新連接或更換磁片磁碟機之後，視需要自動啟動修復。|
