...
8. 若要確認叢集集建立程式是否已建立一個 SMB 共用（識別為 Volume1，或在每個叢集成員的 CSV 磁片區的基礎結構 SOFS 上，將 ScopeName 標記為基礎結構檔案伺服器名稱和兩者的路徑）：
...
12. 將管理叢集新增至叢集集合中每個節點上的本機 administrators 群組。
...
例如：已新增現有的叢集，而且它具有預先設定的虛擬機器（位於本機叢集共用磁碟區（CSV））。 VHDX 的路徑會類似 "C:\ClusterStorage\Volume1\MYVM\Virtual Hard Disks\MYVM.vhdx"。 必須完成存放裝置遷移，因為 CSV 路徑是由單一成員叢集的本機設計，因此當虛擬機器在成員叢集上進行即時移轉之後，將無法存取它們。
