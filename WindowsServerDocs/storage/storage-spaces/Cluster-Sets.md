...
ms.locfileid: "59833949"
---
# <a name="cluster-sets"></a>叢集集合

> 適用於：Windows Server Insider Preview 組建 17650 和更新版本

叢集設定為新的雲端向外延展技術，在此預覽版本，可在單一的軟體定義資料中心 (SDDC) 雲端中的叢集節點計數增加的重要性順序。 叢集組是鬆散偶合的群組多個容錯移轉叢集： 計算、 儲存體或超交集。 叢集設定技術可讓虛擬機器倍速多個叢集組和整合的儲存體命名空間內的成員叢集以支援虛擬機器流動性在集合。 

雖然叢集成員上，保留現有的容錯移轉叢集管理體驗，在叢集設定執行個體此外還提供在彙總的生命週期管理的主要使用案例。 此 Windows Server Preview 案例評估指南將提供必要的背景資訊，以及評估使用 PowerShell 的叢集設定技術的逐步指示。 

## <a name="technology-introduction"></a>技術簡介

叢集設定技術開發來符合操作軟體定義資料中心 (SDDC) 雲端規模的特定客戶要求。 可能會摘要說明在此預覽版本的叢集設定的價值主張，如下所示：  
...
叢集設定命名空間轉介 （叢集設定命名空間） SOFS 是向外延展檔案伺服器，其中每個 SMB 共用上的叢集設定命名空間 sofs 是轉介共用-型別 'SimpleReferral' 此預覽版本中新引入。  這種轉介會允許伺服器訊息區塊 (SMB) 用戶端存取 SMB 共用上的目標成員叢集 SOFS 上裝載。 叢集中設定 SOFS 是輕量級轉介機制，因此，不會參與的命名空間轉介 I/O 路徑中。 SMB 快取轉介的永久上每個用戶端節點和叢集設定命名空間以動態方式自動這些轉介會視需要更新。
...
1. 設定執行最新的 Windows Server Insider 版本的管理用戶端。
2. 此管理伺服器上安裝容錯移轉叢集工具。
...
叢集將使用下列步驟並非必要，並需要只能有一個指令。  比方說，我想要將叢集設定的虛擬機器從 CLUSTER1 移至 NODE2 CL3，CLUSTER3 上。  會使用單一命令：
...
**回答：** 叢集設定是一種新技術引進的 Windows Server Preview 中的組建，所以此情況下，不存在於舊版。 舊版作業系統為基礎的叢集無法加入叢集組。 不過，叢集作業系統輪流升級技術應該提供您要尋找這些叢集升級到 Windows Server 2019 的移轉功能。
...
**回答：** 在預覽中，叢集組已經過測試並支援多達 64 總叢集節點。 不過，叢集設定架構調整為更大的限制，而且不是硬式編碼為限制的項目。 請讓 Microsoft 知道是否更大的規模很重要而且您打算使用它的方式。
