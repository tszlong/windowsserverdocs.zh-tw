...
本主題說明如何在獨立伺服器上部署儲存空間。 如需有關如何建立叢集儲存空間的詳細資訊，請參閱[在 Windows Server 2012 R2 上部署儲存空間](</previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/mt270997(v%3dws.11)>)叢集。
...
您可以從虛擬磁碟建立一或多個磁碟區。 當您建立磁片區時，可以設定大小、磁碟機號或資料夾、檔案系統 (NTFS 檔案系統或復原檔案系統， (ReFS) # A3、配置單位大小和選擇性的磁片區標籤。
...
|HBA 考量|-建議您不要使用簡單的主機匯流排介面卡 (不支援 RAID 功能的 Hba) <br>-如果支援 RAID，Hba 必須處於非 RAID 模式，且所有 RAID 功能皆已停用<br>-介面卡不得抽象化實體磁片、快取資料，或遮蔽任何附加的裝置。 這包括由連結的集束磁碟 (just-a-bunch-of-disks，JBOD) 裝置所提供的內含服務。 |「儲存空間」是只與您可以完全停用所有 RAID 功能的 HBA 相容。|
|JBOD 機箱|-JBOD 主機殼是選擇性的<br>-建議使用 Windows Server Catalog 上列出的儲存空間認證主機殼<br>-如果您使用 JBOD 主機殼，請向存放裝置廠商確認主機殼支援儲存空間，以確保完整的功能<br>-若要判斷 JBOD 主機殼是否支援主機殼和插槽識別，請執行下列 Windows PowerShell Cmdlet：<br><br>`Get-PhysicalDisk \| ? {$_.BusType –eq "SAS"} \| fc`<br><br>如果 [ **EnclosureNumber** ] 和 [ **SlotNumber** ] 欄位包含值，則主機殼支援這些功能。||

若要規劃獨立伺服器部署所需的實體磁碟數目和復原類型，請使用下列指導方針。

|復原類型|磁碟需求|使用時機|
|---|---|---|
|**簡單**<br><br>-跨實體磁片的帶狀資料<br>-將磁片容量最大化並增加輸送量<br>-沒有復原 (無法防止磁片故障) <br><br><br><br><br><br><br>|至少需要一個實體磁碟。|請勿用來裝載無法取代的資料。 簡單空格不會因磁片失敗而受到保護。<br><br>用來裝載暫時或容易重新建立的資料以降低成本。<br><br>適用于非必要或已由應用程式提供復原的高效能工作負載。|
...
6. 在 [**選取存放集區的實體磁片**] 頁面上，執行下列動作，然後選取 **[下一步]**：
    ...
    2. 如果您想要將一或多個磁片指定為熱備件，請在 [**配置**] 底下，選取下拉箭號，然後選取 [**熱備件**]。
...
6. 在 [**選取儲存體**配置] 頁面上，選取所需的版面配置，然後選取 **[下一步]**。
    ...
    >[!NOTE]
    >如果您選取的配置沒有足夠的實體磁片，當您選取 **[下一步]** 時，就會收到錯誤訊息。 如需所要使用的版面配置和磁片需求的詳細資訊，請參閱[必要條件](#prerequisites)) 。

7. 如果您選取 [**鏡像**] 做為儲存配置，而且集區中有五個以上的磁片，則會顯示 [**設定復原設定**] 頁面。 選取下列其中一個選項：
...
9. 在 [指定的虛擬磁碟的大小]**** 頁面上，執行下列動作：
    ...
    如果您在上一個步驟中選取 [精簡布建]，請在 [**虛擬磁片大小**] 方塊中，輸入虛擬磁片大小，選取 (**MB**、 **GB**或**TB**) 的單位，然後選取 **[下一步]**。

    如果您在上一個步驟中選取了 [固定布建]，請選取下列其中一項：

      - **指定大小**

        若要指定大小，請在 [**虛擬磁片大小**] 方塊中輸入值，然後選取 (**MB**、 **GB**或**TB**) 的單位。
...
下列範例會在名為*StoragePool1*的存放集區上建立名為*VIRTUALDISK1*的 50 GB 虛擬磁片。

```PowerShell
New-VirtualDisk –StoragePoolFriendlyName StoragePool1 –FriendlyName VirtualDisk1 –Size (50GB)
```

下列範例會在名為*StoragePool1*的存放集區上建立名為*VirtualDisk1*的鏡像虛擬磁片。 磁片會使用存放集區的最大儲存體容量。

```PowerShell
New-VirtualDisk –StoragePoolFriendlyName StoragePool1 –FriendlyName VirtualDisk1 –ResiliencySettingName Mirror –UseMaximumSize
```

下列範例會在名為*StoragePool1*的存放集區上建立名為*VIRTUALDISK1*的 50 GB 虛擬磁片。 這個磁碟使用精簡佈建類型。
...
4. 在 [**指定磁片區大小**] 頁面上，輸入磁片區大小，指定 (**MB**、 **GB**或**TB**) 的單位，然後選取 **[下一步]**。
