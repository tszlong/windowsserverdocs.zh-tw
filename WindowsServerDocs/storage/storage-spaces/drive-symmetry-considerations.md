...
我們建議您盡可能使用相同型號和固件版本的磁片磁碟機。 如果您無法這麼做，請小心選取越類似的磁片磁碟機。 我們不鼓勵使用具有顯著不同效能或耐用特性的相同類型混合和比對的磁片磁碟機 (除非其中一個是快取，另一個則是容量) ，因為儲存空間直接存取會平均地分配 IO，而不會根據模型來區分。
...
如所述，伺服器 1 (10 TB) 和伺服器 2 (10 TB) 已滿。 伺服器3具有較大的磁片磁碟機，因此它的總容量會大於 (15 TB) 。 不過，若要在伺服器3上儲存更多的三向鏡像資料，也需要伺服器1和伺服器2上的複本（已滿）。 伺服器3上剩餘的 5 TB 容量無法使用–它是「*閒置*」的容量。
