...
- 就地升級：在叢集中的每部伺服器上執行**vm 時**，此選項不會產生任何 VM 停機時間，但您必須等候儲存體工作， (鏡像修復) 在每個伺服器升級後完成。

- 當 Vm 在叢集中的每部伺服器上執行**時，會進行全新的作業系統安裝**—此選項不會造成 VM 停機，但您必須等候存放裝置工作 (鏡像修復) 在每個伺服器升級後完成，而且您必須再次設定每部伺服器及其所有應用程式和角色。

- 就地升級：在叢集中的每部伺服器上**停止 vm 時**，此選項會造成 VM 停機，但您不需要等待儲存體工作 (鏡像修復) ，因此速度會更快。

- 清理-在叢集中的每部伺服器上**停止 vm 時安裝**：此選項會導致 VM 停機，但您不需要等待儲存體工作 (鏡像修復) ，因此速度會更快。
...
- ReFS 磁片區上的升級受到完整支援，不過，升級的磁片區不能受益于 Windows Server 2019 中的 ReFS 增強功能。 這些優點（例如，鏡像加速同位的增加效能）需要新建立的 Windows Server 2019 ReFS 磁片區。 換句話說，您必須使用 Cmdlet 或伺服器管理員建立新的磁片區 `New-Volume` 。 以下是一些新磁片區會取得的 ReFS 增強功能：
...
3. 一次在一部叢集伺服器上執行下列步驟：
   ...
   6. 檢查新升級的伺服器是否有最新的 Windows Server 2019 更新。 如需詳細資訊，請參閱[windows 10 和 Windows Server 2019 更新歷程記錄](https://support.microsoft.com/help/4464619/windows-10-update-history)。 組建編號 (查看 `ver` 命令) 應為17763.292 或更高版本。
...
10. 藉由執行叢集驗證來驗證叢集 (`Test-Cluster`) 並檢查叢集驗證報告。
...
3. 一次在一部叢集伺服器上執行下列步驟：
   ...
   7. 檢查新升級的伺服器是否有最新的 Windows Server 2019 更新。 如需詳細資訊，請參閱[windows 10 和 Windows Server 2019 更新歷程記錄](https://support.microsoft.com/help/4464619/windows-10-update-history)。 組建編號 (查看 `ver` 命令) 應為17763.292 或更高版本。
...
7. 藉由停止每個 VM 並使用指令程式，然後 `Update-VMVersion` 再次啟動 vm，即可選擇性地升級 vm 設定層級。
...
此選項會產生 VM 停機時間，但所需的時間可能比您在升級期間保留執行的 Vm 還短，因為您不需要等待儲存體工作 (鏡像修復) 在每個伺服器升級後完成。 雖然在升級過程中會依序重新開機個別伺服器，但叢集中的其餘伺服器仍會繼續執行。
...
3. 一次在一部叢集伺服器上執行下列步驟：
   ...
   組建編號 (查看 `ver` 命令) 應為17763.292 或更高版本。
...
8. 藉由停止每個 VM 並使用指令程式，然後 `Update-VMVersion` 再次啟動 vm，即可選擇性地升級 vm 設定層級。
...
1. 檢查叢集中的所有伺服器都執行最新的更新。
   ...
   至少要安裝 Microsoft 知識庫[文章 4487006](https://support.microsoft.com/help/4487006/windows-10-update-kb4487006) (2 月19日 2019) 。 組建編號 (查看 `ver` 命令) 應為14393.2828 或更高版本。
