...
keywords: 儲存空間直接存取，仲裁，見證，S2D，叢集仲裁，集區仲裁，叢集，集區
...
ms.locfileid: "70872128"
---
# <a name="understanding-cluster-and-pool-quorum"></a>瞭解叢集和集區仲裁
...
- 叢集**仲裁**：這會在叢集層級運作（也就是您可能會遺失節點，讓叢集保持啟動狀態）
- **集區仲裁**：這會在啟用儲存空間直接存取時在集區層級上運作（也就是您可能會遺失節點和磁片磁碟機，並讓集區保持啟動狀態）。 存放集區設計成用於叢集和非叢集的案例，這就是為什麼它們有不同的仲裁機制。
...
但是，只有當叢集中的節點總數是奇數時（例如，五個節點叢集中的三個節點），*大部分*的概念才會正常運作。 那麼，如果叢集的節點數目是偶數（例如四個節點的叢集）呢？

有兩種方式可讓叢集將*投票總數*設為奇數：

1. 首先，它可以藉由新增包含額外投票的*見證*來*啟動*一個。 這需要使用者設定。
2.  或者，它可以藉由將一個假面具節點的投票*向下*一次（視需要自動發生）。
...
動態仲裁可讓您以動態方式指派投票給節點，以避免失去大部分的投票，並允許叢集以一個節點執行（稱為「最後一位」）。 讓我們以四個節點的叢集做為範例。 假設仲裁需要3個投票。 
...
我們剛剛討論過叢集仲裁，它會在叢集層級運作。 現在，讓我們深入探索集區仲裁，它會在集區層級上運作（也就是您可能會遺失節點和磁片磁碟機，並讓集區保持啟動狀態）。 存放集區設計成用於叢集和非叢集的案例，這就是為什麼它們有不同的仲裁機制。
...
- 集區會在叢集中使用一個節點做為見證，做為系結器，讓一半的磁片磁碟機消失（此節點是集區資源擁有者）
- 集區沒有動態仲裁
- 集區不會執行自己的移除投票的版本
...
- 可能會在一部伺服器故障後存活：**是**。
- 可以存留一次伺服器失敗，然後另一個： * * 相依于 * * （如果兩個節點的三和四個都停止，但仍可存留所有其他案例，則無法繼續）。
- 一次可能會中斷兩個伺服器失敗： * * 相依于 * * （如果兩個節點都已關閉，但仍可存留所有其他案例，則無法繼續）。

### <a name="pool-quorum-recommendations"></a>集區仲裁建議

- 確定叢集中的每個節點都是對稱的（每個節點都有相同數目的磁片磁碟機）
