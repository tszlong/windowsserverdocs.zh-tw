...
- **叢集仲裁**:這會在叢集層級 （也就是您可以遺失節點且有維持叢集）
- **集區仲裁**:這會在集區層級上啟用儲存空間直接存取時 （也就是可以遺失節點和磁碟機，然後保持運作的集區）。 存放集區已設計來在叢集和非叢集的情況下，這也是為什麼它們有不同的仲裁的機制。
...
- 如果您有兩個節點，見證會**必要**。
- 您有三或四個節點，見證是否**強烈建議**。
- 如果您有網際網路存取時，使用 **[雲端見證](../../failover-clustering/deploy-cloud-witness.md)**
...
每當存活節點成功確定它們是*多數*，定義*多數*更新為只是存留者之間。 這可讓叢集失去一個節點，然後另一個，然後另一個，等等。 此概念*總數投票*調整在連續失敗之後就所謂***動態仲裁***。  
...
- 如果您有**甚至**的節點和沒有見證，數字*一個節點被歸其投票*。 例如，只有三個四個節點取得投票，因此*總投票數*為第三，將投票的兩個未回收物件會被視為多數。
- 如果您有**奇數**的節點和沒有見證，數字*都取得投票*。
- 如果您有**甚至**節點，再加上見證，數字*見證投票*，因此總計為奇數。
- 如果您有**奇數**節點，再加上見證，數字*投票見證不*。
...
上述的案例適用於沒有儲存空間直接存取啟用的一般叢集。 不過，儲存空間直接存取啟用時，叢集只能支援兩個節點失敗。 這會說明更[集區仲裁區段](#pool-quorum-overview)。

### <a name="examples"></a>範例

#### <a name="two-nodes-without-a-witness"></a>不具見證的兩個節點。 
一個節點的投票空的因此*多數*投票決定共有**1 投票**。 如果非投票節點意外關閉，存活者只有 1/1，而且叢集不受影響。 如果投票節點意外關閉，存活者具有 0/1，而且叢集將會中斷。 如果投票節點順利關機，將 「 投票 」 會轉移至其他節點，和叢集不受影響。 ***這就是為什麼很重要設定見證。***
...
這兩個節點投票，加上見證投票，因此*多數*決定共有**3 的投票**。 如果任一個節點關閉時，存活者只有 2/3，而且叢集不受影響。
...
所有的節點投票，因此*多數*決定共有**3 的投票**。 如果任何節點故障時，未回收物件都是 2/3，叢集不受影響。 叢集變成不具見證的兩個節點 – 此時，您是在案例 1 中。

![說明在案例中使用不具見證的三個節點的仲裁](media/understand-quorum/3-node-no-witness.png)

- 不會受一伺服器失敗：**是**。
- 不會受一部伺服器失敗，則另一個：**50%的機率**。
- 一次就能存活兩個伺服器失敗：**否**。 

#### <a name="three-nodes-with-a-witness"></a>含有見證的三個節點。
所有的節點投票，讓見證不一開始進行投票。 *多數*決定共有**3 的投票**。 一個失敗之後，叢集會有兩個節點，見證 – 也就是案例 2。 因此，現在的兩個節點和見證投票。
...
一個節點的投票空的因此*多數*決定共有**3 的投票**。 一個失敗之後，叢集會變成三個節點，而您是在案例 3。

![說明在案例中使用不具見證的四個節點的仲裁](media/understand-quorum/4-node-no-witness.png)

- 不會受一伺服器失敗：**是**。
- 不會受一部伺服器失敗，則另一個：**是**。
- 一次就能存活兩個伺服器失敗：**50%的機率**。 

#### <a name="four-nodes-with-a-witness"></a>含有見證的四個節點。
所有的節點投票和見證投票中，因此*多數*決定共有**5 投票**。 一次失敗之後，您已在案例 4 中。 在兩個同時失敗之後, 您直接跳到 案例 2。
...
- **[雲端見證](../../failover-clustering/deploy-cloud-witness.md)** -Blob 存取的 Azure 儲存體叢集中的所有節點。 它會維持在存在於 witness.log 檔案中，叢集資訊，但不會儲存叢集資料庫的複本。
- **檔案共用見證**– 設定執行 Windows Server 檔案伺服器的 SMB 檔案共用。 它會維持在存在於 witness.log 檔案中，叢集資訊，但不會儲存叢集資料庫的複本。
- **磁碟見證**-小型的叢集的磁碟是叢集可用的存放裝置群組中。 此磁碟是高可用性，而且可以在節點之間的容錯移轉。 它包含叢集資料庫的複本。  ***磁碟見證不支援使用儲存空間直接存取***。

## <a name="pool-quorum-overview"></a>集區仲裁概觀
...
每個 16 的磁碟機都有一個投票，第二個節點也有一票 （因為它是集區資源擁有者）。 *多數*決定共有**16 投票**。 如果三和四個節點關閉，仍正常運作的子集有 8 的磁碟機和集區資源擁有者，也就是 9/16 投票。 因此，不受影響的集區。
...
每個 16 的磁碟機都有一個投票，節點 2 也有一票 （因為它是集區資源擁有者）。 *多數*決定共有**16 投票**。 首先，7 的磁碟機故障了。 如果三和四個節點關閉，仍正常運作的子集有 7 的磁碟機和集區資源擁有者，也就是 8/16 投票。 因此，集區沒有多數，而且當機。
...
每個 24 的磁碟機都有一個投票，第二個節點也有一票 （因為它是集區資源擁有者）。 *多數*決定共有**24 投票**。 如果三和四個節點關閉，仍正常運作的子集有 8 的磁碟機和集區資源擁有者，也就是 9 月 24 投票。 因此，集區沒有多數，而且當機。

![集區仲裁 3](media/understand-quorum/pool-3.png)

- 不會受一伺服器失敗：**是**。
- 一部伺服器失敗，則另一個可以存留: * * 相依 * * （不能支撐如果三和四個兩個節點發生故障，但不會受其他所有案例。
- 一次可以存活下來兩個伺服器失敗: * * 相依 * * （不能支撐如果三和四個兩個節點發生故障，但不會受其他所有案例。
