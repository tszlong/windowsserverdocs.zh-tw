...
1. **優化不應取得寫入磁片的方式**重復資料刪除會使用後置處理模型來優化資料。 所有資料都會在未經最佳化的情況下寫入磁碟，並在稍後由重複資料刪除最佳化。

2. **優化不應變更存取語義**存取已優化磁片區上資料的使用者和應用程式，完全不知道他們存取的檔案已重復資料刪除。
...
| <a id="usage-type-backup"></a>Backup | 虛擬化備份應用程式，例如[Microsoft Data Protection Manager (DPM) ](/previous-versions/system-center/system-center-2012-R2/hh758173(v=sc.12)) | <ul><li>優先順序最佳化</li><li>預設的最佳化原則︰<ul><li>最短檔案存在時間 = 0 天</li><li>最佳化使用中檔案 = 是</li><li>最佳化部分檔案 = 否</li></ul></li><li>使用 DPM/DPM 型解決方案針對 Interop 的內部調整</li></ul> |
...
| 作業名稱 | 工作描述 | 預設排程 |
|----------|------------------|------------------|
| <a id="job-info-optimization"></a>Optimization | **優化**工作的刪除重複方式是根據磁片區原則設定，將磁片區上的資料區塊化， (選擇性地) 壓縮這些區塊，並在區塊存放區中唯一儲存區塊。 重複資料刪除使用的最佳化程序已在[重複資料刪除如何運作？](understand.md#how-does-dedup-work)詳細說明。 | 每小時一次 |
...
| <a id="dedup-term-reparse-point"></a>重新分析點 | 重新[分析點](/windows/win32/fileio/reparse-points)是特殊的標記，會通知檔案系統將 i/o 傳遞給指定的檔案系統篩選器。 當檔案的檔案資料流已最佳化時，重複資料刪除會以重新分析點取代檔案資料流，這可以讓重複資料刪除保留對該檔案的存取語意。 |
