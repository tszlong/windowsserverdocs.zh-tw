...
### <a id="modifying-job-schedules-available-settings"></a>可用的全作業設定
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>整數 0-100 (表示百分比)</td>
            <td>節流可確保工作&#39;不會干擾其他需要大量 i/o 的進程。</td>
        </tr>
        <tr>
            <td>記憶體</td>
...
## <a id="modifying-volume-settings"></a>修改重復資料刪除全磁片區設定
### <a id="modifying-volume-settings-how-to-toggle"></a>切換音量設定
...
### <a id="modifying-volume-settings-available-settings"></a>可用的全磁片區設定
<table>
    <thead>
        <tr>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>正整數，範圍 1-36</td>
            <td>修改此值的主要原因是藉由限制允許重複資料刪除在磁碟區上使用的 IO 佇列數目，減少對高 IO 工作負載的效能影響。 請注意，從預設值修改此設定可能會導致&#39;重復資料刪除的後續處理作業執行速度變慢。</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>True/false</td>
            <td>如果您的工作負載持續開啟檔案很長的時間，請啟用此設定。 如果未啟用此設定，即使工作負載有開啟的控制碼，檔案也永遠不會獲得優化，即使它偶爾&#39;會在結尾附加資料也一樣。</td>
        </tr>
        <tr>
...
## <a id="modifying-dedup-system-settings"></a>修改重復資料刪除全系統設定
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>此設定可設定一般記憶體回收工作變成<a href="advanced-settings.md#faq-full-v-regular-gc" data-raw-source="[full Garbage Collection jobs](advanced-settings.md#faq-full-v-regular-gc)">完整記憶體回收工作</a>的間隔。 設定為 n 代表每第 n<sup></sup> 次工作將會是完整記憶體回收工作。 請注意，完整記憶體回收對<a href="understand.md#usage-type-backup" data-raw-source="[Backup Usage Type](understand.md#usage-type-backup)">備份使用類型</a>的磁碟區永遠停用（無論登錄值）。 如果備份磁片區上需要完整的垃圾收集，可以使用 <code>Start-DedupJob -Type GarbageCollection -Full</code>。</td>
            <td>整數（-1 指出已停用）</td>
