...
**有兩個主要的發行管道適用於 Windows Server 客戶，長期維護通道和半年通道。** 
...
> [!Note]  
> **目前的半年通道版本是 Windows Server 版本 1809**。 如果您想將伺服器加入這個通道，就應該安裝 Windows Server 版本 1809，而您可以在 Server Core 模式下進行安裝，或是將其安裝成執行於容器中的 Nano 伺服器。 不支援從 Windows Server 2016 就地升級到 Windows Server 版本 1809，因為它們位於**不同發行通道**。 Windows Server 版本 1809 並非 Windows Server 2016 的更新，而是半年通道中的下一個 Windows Server 版本。
...
- 您需要迅速創新嗎？ 您需要優先取得最新的 Windows Server 功能嗎？ 您需要支援快節奏的混合式應用程式、DevOps 和 Hyper-V 網狀架構嗎？ 若是，您應該考慮透過安裝 **Windows Server 版本 1809** 來**加入半年通道**。 如本主題所述，您將會每年收到兩次新版本，每個版本各有 18 個月的主要生產環境支援。 您可以透過大量授權、Azure 或 Visual Studio 訂閱服務取得項目。 目前，如果您想在生產環境中執行產品，半年通道發行需要大量授權和軟體保證。
...
## <a name="obtain-preview-releases-through-the-windows-insider-program"></a>透過 Windows 測試人員計畫取得預覽版
...
## <a name="to-identify-windows-server-2019-and-windows-server-version-1809"></a>若要識別 Windows Server 2019 和 Windows Server 版本 1809
