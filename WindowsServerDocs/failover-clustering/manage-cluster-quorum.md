...
<tr class="odd">
<td>[使用一般設定]</td>
<td>叢集會自動指派投票給每個節點，並動態管理節點投票。 如果適合您的叢集，並且沒有叢集共用存放裝置可用，叢集就會選取一個磁碟見證。 建議您在大部分的情況下使用此選項，因為叢集軟體會自動選擇可為您的叢集提供最高可用性的仲裁與見證設定。</td>
</tr>
<tr class="even">
<td>[新增或變更仲裁見證]</td>
<td>您可以新增、變更或移除見證資源。 您可以設定檔案共用或磁碟見證。 叢集會自動指派投票給每個節點，並動態管理節點投票。</td>
</tr>
<tr class="odd">
<td>[進階仲裁設定與見證選取項目]</td>
<td>只有在您對於設定仲裁有應用程式特定或網站特定的需求時，才選取此選項。 您可以修改仲裁見證、 新增或移除節點投票，以及選擇叢集是否動態管理節點投票。 根據預設，會將投票指派給所有節點，並動態管理節點投票。</td>
</tr>
</tbody>
</table>
...
<tr class="odd">
<td>節點多數 (不含見證)</td>
<td>只有節點具有投票。 不設定任何仲裁見證。 叢集仲裁是使用中叢集成員資格中大部分的投票節點。</td>
</tr>
<tr class="even">
<td>節點多數 (含見證) (磁碟或檔案共用)</td>
<td>節點具有投票。 此外，仲裁見證也具有投票。 叢集仲裁是使用中叢集成員資格加上見證投票中大部分的投票節點。<br />
<br />
仲裁見證可以是指定的磁碟見證或指定的檔案共用見證。</td>
</tr>
<tr class="odd">
<td>沒有多數 (只含磁碟見證)</td>
<td>沒有節點具有投票。 只有磁碟見證具有投票。 叢集仲裁是由磁碟見證的狀態來決定。<br />
<br />
如果有一個節點可用且可與叢集存放裝置中的特定磁碟通訊，叢集就會具有仲裁。 一般而言，不建議使用，也不應該選取這種模式，因為它會使得叢集中有單一失敗點。</td>
</tr>
</tbody>
</table>
...
<tr class="odd">
<td>磁碟見證</td>
<td>- 會儲存叢集資料庫的副本的專用的 LUN<br />
- 最適合用於具有共用 （非複寫） 存放裝置叢集</td>
<td>- LUN 的大小必須至少 512 MB<br />
- 必須是叢集專用，而且未指派給叢集角色<br />
- 必須包含在叢集的儲存體，然後傳遞存放裝置驗證測試<br />
- 不能是叢集共用磁碟區 (CSV) 的磁碟<br />
- 具有單一磁碟區基本磁碟<br />
- 不需要有磁碟機代號<br />
- 可以使用 NTFS 或 ReFS 格式化<br />
- 可選擇性設定硬體 RAID 以提供容錯功能<br />
- 應從備份及病毒掃描中排除<br />
- 磁碟見證不支援使用儲存空間直接存取</td>
</tr>
<tr class="even">
<td>檔案共用見證</td>
<td>- 執行 Windows Server 檔案伺服器設定的 SMB 檔案共用<br />
- 不會儲存叢集資料庫的複本<br />
- 維護僅存在於 witness.log 檔案中的叢集資訊<br />
- 最適合用於複寫的儲存體與多站台叢集</td>
<td>- 必須有至少 5 MB 的可用空間<br />
- 必須專屬於單一叢集，並不會用來儲存使用者或應用程式的資料<br />
- 必須有寫入權限啟用叢集名稱的電腦物件<br />
<br />
以下是代管檔案共用見證之檔案伺服器的其他考量：<br />
<br />
- 單一檔案伺服器可以設定的多個叢集的檔案共用見證。<br />
- 檔案伺服器必須位於不同的叢集工作負載的站台。 如果站台至站台之間網路通訊中斷時，這可讓任何叢集站台有相同的機會繼續運作。 如果檔案伺服器位於相同站台，該網站會成為主要的站台，而且是可連線檔案共用的唯一站台。<br />
- 檔案伺服器可以執行的虛擬機器上，如果虛擬機器不裝載在相同叢集上使用檔案共用見證。<br />
- 高可用性，則可以設定個別的容錯移轉叢集上的檔案伺服器。</td>
</tr>

<tr class-"odd">
<td>雲端見證</td>
<td>- 在 Azure blob 儲存體中儲存的見證檔案<br>
-在叢集中的所有伺服器都具有可靠網際網路連線時，建議。</td>
<td>請參閱<a href="https://docs.microsoft.com/windows-server/failover-clustering/deploy-cloud-witness">部署的雲端見證</a>。</td>
<td>
</td>
</tr>
...
- 儲存空間直接存取啟用時，叢集只能支援兩個節點失敗。 這會說明多[集區仲裁區段](../storage/storage-spaces/understand-quorum.md#poolQuorum)
...
3. 在 [選取仲裁設定選項]  頁面上，選取三個設定選項的其中一個，然後完成該選項的步驟。 您可以在設定仲裁設定之前檢閱您的選擇。 如需選項的詳細資訊，請參閱[容錯移轉叢集中之仲裁的概觀](#overview-of-the-quorum-in-a-failover-cluster)稍早在本主題中。
...
執行精靈之後，**摘要**頁面隨即出現，如果您想要檢視報表的精靈所執行的工作中，選取**檢視報表**。 最新的報告將保留在*systemroot * * *\\叢集\\報告** 名稱資料夾**QuorumConfiguration.mht**。
...
<tr class="even">
<td>節點投票指派</td>
<td>- 不應該從主要站台的節點移除節點投票<em>SiteA</em><br />
- 應該從備份站台的節點移除節點投票<em>SiteB</em><br />
- 如果發生長期中斷<em>SiteA</em>，必須將投票指派給<em>SiteB</em>啟用仲裁多數在站台復原過程</td>
</tr>
<tr class="odd">
<td>動態仲裁管理</td>
<td>應該啟用</td>
...
<tr class="even">
<td>見證設定</td>
<td>- 如果有偶數數目的節點，請設定見證<em>SiteA</em><br />
- 如果需要見證，請設定只有在節點可以存取的磁碟見證或檔案共用見證<em>SiteA</em> （有時稱為非對稱式磁碟見證）</td>
</tr>
<tr class="odd">
<td>工作負載</td>
<td>使用慣用的擁有者來讓工作負載持續在 <em>SiteA</em> 的節點上執行</td>
</tr>
</tbody>
</table>
