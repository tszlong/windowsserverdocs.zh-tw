...
<p>如果您在建立叢集之前建立的叢集名稱帳戶 （叢集名稱物件） — 也就是預先設置帳戶，您必須提供<strong>建立電腦物件</strong>並<strong>讀取全部內容</strong>在網域中的電腦帳戶用於容器的權限。 您也必須停用帳戶，並且給予<strong>完全控制</strong>它將使用由系統管理員為叢集安裝的帳戶。 如需詳細資訊，請參閱 <<c0> [ 步驟的預先設置叢集名稱帳戶](#steps-for-prestaging-the-cluster-name-account)稍後在本指南中。</p></td>
</tr>
<tr class="odd">
...
<p>如果您預先設置叢集的服務或應用程式的電腦帳戶，您必須使用必要的權限來進行設定。 如需詳細資訊，請參閱 <<c0> [ 預先設置帳戶的叢集的服務或應用程式的步驟](#steps-for-prestaging-an-account-for-a-clustered-service-or-application)稍後在本指南中。</p></td>
</tr>
</tbody>
</table>
