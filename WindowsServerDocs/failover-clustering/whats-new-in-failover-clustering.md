...
- **叢集集合**
    ...
    叢集集可讓您將單一軟體定義資料中心內的伺服器數目增加 (SDDC) 解決方案，超越叢集目前的限制。 這是藉由將多個叢集分組成一個叢集集來完成，也就是多個容錯移轉叢集的鬆散結合群組：計算、儲存體和超交集。
    使用叢集集，您可以將線上虛擬機器移 (叢集集合中叢集之間的即時移轉) 。
...
- **檔案共用見證增強功能**
  ...
  - 缺少磁片見證的共用磁片磁碟機。 這可能是儲存空間直接存取超大範圍設定、SQL Server Always On 可用性群組 (AG) 或 * Exchange 資料庫可用性群組 (DAG) ，都不會使用共用磁片。
...
-   相同的 Microsoft Azure 儲存體帳戶可以用於多個叢集， (每個叢集一個 blob 檔案;叢集唯一識別碼，用來做為 blob 檔案名) 。

-   對儲存體帳戶提供非常低的持續成本， (每個 blob 檔案寫入的少量資料，僅在叢集節點的狀態變更) 時，才更新 blob 檔案一次。
...
Windows Server 2016 包含網站感知容錯移轉叢集，可根據實體位置 (網站) ，在延展的叢集中啟用群組節點。 叢集網站感知會增強叢集生命週期期間的關鍵作業，例如容錯移轉行為、放置原則、節點之間的信號，以及仲裁行為。 如需詳細資訊，請參閱[Windows Server 2016 中的網站感知容錯移轉](https://techcommunity.microsoft.com/t5/failover-clustering/site-aware-failover-clusters-in-windows-server-2016/ba-p/372060)叢集。
...
虛擬機器負載平衡是容錯移轉叢集的一項新功能，可讓您在叢集中跨節點順暢地對虛擬機器進行負載平衡。 系統會根據節點上的虛擬機器記憶體和 CPU 使用率來識別過度認可的節點。 然後，會將虛擬機器從過度認可的節點 (即時移轉) 移至具有可用頻寬 (的節點（如果適用) ）。 您可以調整平衡的能力，以確保最佳的叢集效能和使用率。 預設會在 Windows Server 2016 Technical Preview 中啟用負載平衡。 不過，啟用 SCVMM 動態優化時，會停用負載平衡。

### <a name="virtual-machine-start-order"></a><a name="BKMK_VMStartOrder"></a>虛擬機器啟動順序

虛擬機器啟動順序是容錯移轉叢集中的一項新功能，引進了虛擬機器的啟動順序協調流程 (以及叢集中) 的所有群組。 虛擬機器現在可以分組成各層，而且可以在不同層之間建立開始順序相依性。 這可確保最重要的虛擬機器 (例如網域控制站或公用程式虛擬機器) 會先啟動。 虛擬機器在其相依性的虛擬機器也已啟動之前，都不會啟動。
