...
- 顯示範圍僅限於叢集共用資料夾（不會與非叢集共用資料夾混合）：當使用者藉由指定叢集檔案伺服器的路徑來流覽共用資料夾時，顯示只會包含屬於特定檔案的共用資料夾伺服器角色。 它會排除非叢集共用資料夾，並共用位於叢集節點上的個別檔案伺服器角色的一部分。
...
- 叢集磁片一律會被辨識為叢集的一部分：無論您使用容錯移轉叢集介面、Windows Explorer 或共用和存放管理嵌入式管理單元，Windows 都會辨識磁片是否已指定為在叢集存放裝置中。 如果這類磁片已在容錯移轉叢集管理中設定為叢集檔案伺服器的一部分，則您可以使用先前所提及的任何介面，在磁片上建立共用。 如果這類磁片尚未設定為叢集檔案伺服器的一部分，您就無法在其上誤建立共用。 相反地，錯誤表示必須先將磁片設定為叢集檔案伺服器的一部分，然後才能共用它。
...
- **隔離存放裝置，每個裝置一個叢集：** 來自不同叢集的伺服器必須無法存取相同的存放裝置。 在大部分情況下，用於一組叢集伺服器的 LUN 應透過 LUN 遮罩或分區，與所有其他伺服器隔離。
...
- **管理叢集的帳戶：** 當您第一次建立叢集或將伺服器新增至叢集時，您必須使用具有該叢集中所有伺服器之系統管理員許可權的帳戶來登入網域。 帳戶不需要是網域系統管理員帳戶，但可以是位於每個叢集伺服器之 Administrators 群組中的網域使用者帳戶。 此外，如果帳戶不是網域系統管理員帳戶，則必須將位於的網域組織單位（OU）中的 [**建立電腦物件**] 和 [**讀取所有屬性**] 許可權提供給帳戶（或帳戶所屬的群組）。
...
2. 若要驗證容錯移轉叢集的電腦（例如，電腦名稱稱是節點1和節點2），請執行下列命令：
    ...
    ```
4. 若要在關閉嚮導後查看測試的結果，請參閱指定的檔案（在 SystemRoot\Cluster\Reports\)中，然後在設定中進行任何必要的變更，然後重新執行測試。
