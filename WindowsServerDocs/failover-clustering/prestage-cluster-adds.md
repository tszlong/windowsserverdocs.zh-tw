...
## <a name="step-1-prestage-the-cno-in-ad-ds"></a>步驟 1:在 AD DS 中預先設置 CNO
...
**Figure 1。範例叢集 OU @ no__t 中已停用的 CNO-0

## <a name="step-2-grant-the-user-permissions-to-create-the-cluster"></a>步驟 2:授與使用者建立叢集的許可權
...
5. 選取您剛才新增的使用者帳戶或群組，然後選取 [完全控制]旁邊的 [允許] 核取方塊。
   ...
   **Figure 2。將完全控制授與將建立叢集 @ no__t-0 的使用者或群組
6. 選取 [確定]。
...
## <a name="step-3-grant-the-cno-permissions-to-the-ou-or-prestage-vcos-for-clustered-roles"></a>步驟 3：授與 CNO 許可權給叢集角色的 OU 或預先設置 Vco

當您建立含有用戶端存取點的叢集角色時，叢集會在與 CNO 相同的 OU 中建立 VCO。 若要自動執行此動作，CNO 必須在 OU 中擁有建立電腦物件的權限。

如果您在 AD DS 中預先設定 CNO，可以執行下列任一項以建立 VCO：

- 選項 1：將[CNO 許可權授與 OU](#grant-the-cno-permissions-to-the-ou)。 如果使用此選項，叢集可以在 AD DS 自動建立 VCO。 因此，容錯移轉叢集的系統管理員不必要求您在 AD DS 預先設定 VCO，就可以建立叢集角色。

>[!NOTE]
>若要完成此選項的步驟，至少需要 **Domain Admins** 群組的成員資格或同等權限。

- 選項 2：[為叢集角色](#prestage-a-vco-for-a-clustered-role)預先設置 VCO。 如果因為組織的需求而必須預先設定叢集角色的帳戶，請使用此選項。 例如，您可能需要控制命名慣例，或控制要建立哪些叢集角色。
...
1. 在 [Active Directory 使用者和電腦] 的 [檢視] 功能表上，確定已選取 [進階功能]。
2. 以滑鼠右鍵按一下您在其中建立 CNO 的 OU，[Step 1：在 AD DS @ no__t-0 中預先設置 CNO，然後選取 [**屬性**]。
...
9. 在 [權限]底下，選取 [建立電腦物件] 核取方塊。
   ...
   **Figure 3。將 [建立電腦物件] 許可權授與 CNO @ no__t-0
