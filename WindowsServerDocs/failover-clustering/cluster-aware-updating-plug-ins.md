...
## <a name="install-a-plug-in"></a><a name="BKMK_INSTALL"></a>在中安裝外掛程式\-  
...
CAU 安裝 \(**microsoft.windowsupdateplugin**和**microsoft.hotfixplugin**\) 提供其他選項，可供您選取。\- 在 CAU UI 中，這些會出現在 [**其他選項**] 頁面上（在中設定「外掛程式\-的「更新執行」選項之後）。 如果您使用 CAU PowerShell Cmdlet，這些選項會設定為引數中的選擇性外掛程式\-。 如需詳細資訊，請參閱此主題稍後的[使用 Microsoft.WindowsUpdatePlugin](#BKMK_WUP) 與[使用 Microsoft.HotfixPlugin](#BKMK_HFP)。  
...
- 只安裝選取的一般發行版本 \(GDR\) 更新。 根據預設，中的外掛程式\-只會套用重要的軟體更新。 不需要組態。 預設設定會在每個節點上下載並安裝重要 GDR 更新。 
...
- 為了獲得最佳結果，我們建議您 \(BPA\) 執行 CAU 最佳做法分析程式，以確保叢集和更新環境已正確設定，以使用 CAU 來套用更新。 如需詳細資訊，請參閱[測試 CAU 更新整備](cluster-aware-updating-requirements.md#BKMK_BPA)。
...
- 為獲得最佳結果，建議您執行 CAU 最佳做法分析程式 \(BPA\) 模型，以確保叢集和更新環境已正確設定，以使用 CAU 來套用更新。 如需詳細資訊，請參閱[測試 CAU 更新整備](cluster-aware-updating-requirements.md#BKMK_BPA)。
- 從發行者取得更新，並將它們複製或解壓縮到伺服器訊息區 \(SMB\) 檔案共用\) \(可支援至少 SMB 2.0 且可供所有叢集節點和遠端更新協調器電腦使用的叢集（如果 CAU 在遠端 \(更新模式\-）。\) 如需詳細資訊，請參閱此主題稍後的[設定 Hotfix 根資料夾結構](#BKMK_HF_ROOT)。 
...
### <a name="configure-a-hotfix-root-folder-structure"></a><a name="BKMK_HF_ROOT"></a>設定修補程式根資料夾結構
...
### <a name="customize-the-hotfix-configuration-file"></a><a name="BKMK_CONFIG_FILE"></a>自訂修補程式設定檔  
...
### <a name="restrict-access-to-the-hotfix-root-folder"></a><a name="BKMK_ACL"></a>限制對此修補程式根資料夾的存取  
...
（選擇性）您可以停用上述的 [外掛程式]\-預設執行的檢查。 若要執行這項作業，您可以使用兩種方法：  
