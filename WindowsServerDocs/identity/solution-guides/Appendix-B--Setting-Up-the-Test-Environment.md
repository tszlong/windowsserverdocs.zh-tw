...
-   Contoso\administrator 的身分： pass@word1  
...
建立要做為網域控制站 (DC1) 的虛擬機器。 安裝虛擬機器使用 Windows Server 2012 ISO，並將它命名為 DC1。  

##### <a name="to-install-active-directory-domain-services"></a>安裝 Active Directory 網域服務  

1. 將虛擬機器連線到 ID_AD_Network。 Dc1 管理員身分登入的密碼<strong>pass@word1</strong>。  
...
12. 在 **網域控制站選項**頁面上，選取網域和樹系功能等級為 Windows Server 2012，指定 DSRM 密碼<strong>pass@word1</strong>，然後按一下 **下一步**.  
...
4. 建立下列包含指定屬性的使用者：  
   ...
   |       使用者       |  Username  |     電子郵件地址      | 部門 |      群組       | 國家/地區 |
   |------------------|------------|------------------------|------------|------------------|----------------|
...
2.  展開 [樹系: contoso.com]  ，然後展開 [網域]  ，瀏覽至 **contoso.com**，展開 [(contoso.com)]  ，然後選取 [FileServerOU]  。 以滑鼠右鍵按一下**這個網域中建立 GPO 並連結到**
...
1. 從 Windows Server 2012 ISO 建立虛擬機器使用名稱 FILE1。  

2. 將虛擬機器連線到 ID_AD_Network。  

3. 虛擬機器加入 contoso.com 網域，並再登入 FILE1 以 contoso\administrator 的身分使用密碼<strong>pass@word1</strong>。  
...
您應該安裝 Microsoft Office Filter Pack 以啟用 Ifilter 更多的 Office 檔案預設會提供比 Windows Server 2012 上。  Windows Server 2012 並沒有任何的 ifilter 就依預設，安裝 Microsoft Office 檔案和檔案分類基礎結構使用 Ifilter 來執行內容分析。  
...
4. 如果您想要定期通知特定的系統管理員的配額或檔案檢測事件，底下**預設系統管理員收件者**，輸入每個電子郵件地址，例如fileadmin@contoso.com。 使用格式account@domain，並使用分號隔開多個帳戶。  

#### <a name="create-groups-on-file1"></a>在 FILE1 上建立群組  

###### <a name="to-create-security-groups-on-file1"></a>在 FILE1 上建立安全性群組  

1. 以 contoso\administrator 的身分，使用密碼登入 FILE1: <strong>pass@word1</strong>。  
...
2.  建立指定下列詳細資料的檔案：  
    ...
    -   **Finance Memo.docx**：在文件中新增一些財務相關文字。 例如，' 誰可以存取財務文件的商務規則已變更。 財務文件現在只能由 FinanceExpert 群組的成員存取。 沒有其他部門或群組存取。 ' 在環境中實作之前必須評估此變更的影響。 請確定這份文件的每個頁面的頁尾上都有「Contoso 公司機密」的字樣。  
...
19. 在 **服務帳戶**畫面上，在**網域使用者帳戶**，按一下 **指定**，然後指定 使用者名稱 (**contoso\rms**)，和密碼 (<strong>pass@word1</strong>)，按一下  **確定**，然後按一下**下一步**。  

20. 在 [密碼編譯模式]  畫面上，按一下 [密碼編譯模式 2]  。  

21. 在 [叢集金鑰儲存]  畫面上，按一下 [下一步]  。  

22. 上**叢集金鑰密碼**畫面上，在**密碼**並**確認密碼**方塊中，輸入<strong>pass@word1</strong>，然後按一下**下一步**。  
...
28. 在 [結果]  畫面上，按一下 [關閉]  ，然後按一下 [安裝進度]  畫面上的 [關閉]  。 完成時，請先登出，然後以 contoso\rms 使用提供的密碼登入 (<strong>pass@word1</strong>)。  
...
31. 在使用者與權限 區段中，按一下**使用者和權限**，按一下**新增**，型別<strong>financeadmin@contoso.com</strong>，然後按一下**確定**。  

32. 選取 [完全控制]  ，保留 [授與擁有者 (作者) 完全控制權限永不過期]  的選取狀態。  

33. 不變更其他設定，按一下剩餘的索引標籤，然後按一下 [完成]  。 以 CONTOSO\Administrator 身分登入。  

34. 瀏覽至資料夾，C:\inetpub\wwwroot\\_wmcs\certification，選取 ServerCertification.asmx 檔案，並新增已驗證的使用者具有讀取和寫入檔案的權限。  
...
1. 在 CONTOSO\Administrator 身分登入密碼： <strong>pass@word1</strong>。  
...
7. 以 contoso\administrator 的身分和記錄檔以 CONTOSO\RMS 登入使用提供的密碼 ("pass@word1」)。  
...
7. 在指向用戶端虛擬機器上的所有使用者的桌面上建立新的快速鍵\\\FILE1\Finance 文件。  
...
## <a name="BKMK_CF"></a>跨樹系案例部署宣告的實驗室設定  
...
### <a name="BKMK_2.2"></a>設定稱為 adatum.com 的新樹系  

##### <a name="to-install-active-directory-domain-services"></a>安裝 Active Directory 網域服務  

1. 將虛擬機器連線到 ID_AD_Network。 Dc2 管理員身分登入的密碼<strong>Pass@word1</strong>。  
...
### <a name="BKMK_2.22"></a>設定 contoso.com 做為 adatum.com 信任的樹系  
...
### <a name="BKMK_2.4"></a>建立 Adatum 樹系中的其他使用者  
建立密碼的使用者 Jeff Low <strong>pass@word1</strong>，並指派公司屬性的值**Adatum**。  
...
### <a name="BKMK_2.55"></a>啟用 contoso.com 的公司資源內容  
...
### <a name="BKMK_2.6"></a>啟用 adatum.com 的動態存取控制  
...
7. 按一下 [確定]  三次，以完成並返回 Active Directory 管理中心。  
   ...
   ![解決方案指南](media/Appendix-B--Setting-Up-the-Test-Environment/PowerShellLogoSmall.gif)***<em>Windows PowerShell 對等的命令</em>***  
...
### <a name="BKMK_2.11"></a>發佈新的原則，透過群組原則  
...
2.  以滑鼠右鍵按一下 contoso.com 網域中，按一下**這個網域中建立 GPO 並連結到**  
...
### <a name="BKMK_2.12"></a>檔案伺服器上建立 Earnings 資料夾  
在 FILE1 上建立新的 NTFS 磁碟區，然後建立下列資料夾：D:\Earnings。  

> [!NOTE]  
> 系統或開機磁碟區 C: 上預設不會啟用集中存取原則。  

### <a name="BKMK_2.13"></a>設定分類，並將集中存取原則套用到 Earnings 資料夾  
