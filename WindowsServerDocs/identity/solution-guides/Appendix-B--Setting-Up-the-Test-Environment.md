...
1.  將虛擬機器連線到 ID_AD_Network。 Dc1 管理員身分登入的密碼**pass@word1**。  
...
12. 在 **網域控制站選項**頁面上，選取網域和樹系功能等級為 Windows Server 2012，指定 DSRM 密碼**pass@word1**，然後按一下 **下一步**.  
...
3.  虛擬機器加入 contoso.com 網域，並再登入 FILE1 以 contoso\administrator 的身分使用密碼**pass@word1**。  
...
1.  以 contoso\administrator 的身分，使用密碼登入 FILE1: **pass@word1**。  
...
19. 在 **服務帳戶**畫面上，在**網域使用者帳戶**，按一下 **指定**，然後指定 使用者名稱 (**contoso\rms**)，和密碼 (**pass@word1**)，按一下  **確定**，然後按一下**下一步**。  
  
20. 在 [密碼編譯模式] 畫面上，按一下 [密碼編譯模式 2]。  
  
21. 在 [叢集金鑰儲存] 畫面上，按一下 [下一步]。  
  
22. 上**叢集金鑰密碼**畫面上，在**密碼**並**確認密碼**方塊中，輸入**pass@word1**，然後按一下**下一步**。  
...
28. 在 [結果]  畫面上，按一下 [關閉] ，然後按一下 [安裝進度]  畫面上的 [關閉]  。 完成時，請先登出，然後以 contoso\rms 使用提供的密碼登入 (**pass@word1**)。  
...
31. 在使用者與權限 區段中，按一下**使用者和權限**，按一下**新增**，型別**financeadmin@contoso.com**，然後按一下**確定**。  
...
1.  在 CONTOSO\Administrator 身分登入密碼： **pass@word1**。  
...
1.  將虛擬機器連線到 ID_AD_Network。 Dc2 管理員身分登入的密碼**Pass@word1**。  
...
建立密碼的使用者 Jeff Low **pass@word1**，並指派公司屬性的值**Adatum**。  
...
7.  按一下 [確定]  三次，以完成並返回 Active Directory 管理中心。  
    ...
    ![解決方案指南](media/Appendix-B--Setting-Up-the-Test-Environment/PowerShellLogoSmall.gif)Windows PowerShell 對等命令 * * *  
...
1.  在 [HYPER-V 管理員] 中，連線至伺服器 FILE1。 使用 Contoso\Administrator 和密碼登入伺服器**pass@word1**。  
