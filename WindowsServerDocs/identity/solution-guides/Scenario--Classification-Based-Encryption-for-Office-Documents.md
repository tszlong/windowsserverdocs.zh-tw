...
|角色/功能|如何支援本案例|  
|-----------------|---------------------------------|  
|Active Directory 網域服務角色 (AD DS)|AD DS 提供一個分散式資料庫，用來儲存和管理網路資源的相關資訊，以及啟用目錄之應用程式的應用程式特定資料。 在此案例中，Windows Server 2012 中的 AD DS 引進了宣告型授權平臺，可讓您建立使用者宣告和裝置宣告、複合身分識別（使用者加上裝置宣告）、新的集中存取原則模型，以及使用授權決策中的檔案分類資訊。|  
