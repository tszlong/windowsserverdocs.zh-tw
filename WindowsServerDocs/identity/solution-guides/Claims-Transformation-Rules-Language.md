...
**LDAP**：您可以透過輕量型目錄存取協定（LDAP）在 Active Directory 中編輯宣告轉換原則。 不過，不建議您這樣做，因為原則有數個複雜的元件，而您使用的工具在將原則寫入 Active Directory 之前，可能不會驗證該原則。 這可能需要相當長的時間來診斷問題。  
...
-   宣告轉換規則集包含零個或多個規則。 每個規則都有兩個作用中的部分：**選取條件清單**和**規則動作**。 如果 [**選取條件] 清單**評估為 TRUE，則會執行對應的規則動作。  
...
1. 範例：  
   ...
   **錯誤訊息：**  
   *POLICY0002：無法剖析原則資料。*  
   @no__t 0Line 號碼：1，資料行編號：2，錯誤標記：;。行： ' c1;[] = > 問題（宣告 = c1）; '。*  
   *Parser 錯誤：'POLICY0030:語法錯誤，非預期的 '; '，應為下列其中一項： '： '。 '*  
...
2. 範例：  
   ...
   **錯誤訊息**:   
   *POLICY0011：宣告規則中沒有符合 CopyIssuanceStatement 中指定之條件標記的條件： ' c2 '。*  
...
3. 範例：  
   ...
   **錯誤訊息：**  
   *POLICY0002：無法剖析原則資料。*  
   行號：1，資料行編號：39，錯誤標記： "bool"。 行： ' c1： [type = = "x1"，value = = "1"，valuetype = = "bool"] = > Issue （宣告 = c1）; '。   
   *Parser 錯誤：'POLICY0030:語法錯誤，非預期的 ' STRING '，應為下列其中一項：' INT64_TYPE ' ' UINT64_TYPE ' ' STRING_TYPE ' ' BOOLEAN_TYPE ' ' IDENTIFIER '*  
...
4. 範例：  
   ...
   **錯誤訊息：**  
   *POLICY0002：無法剖析原則資料。*  
   @no__t 0Line 號碼：1，資料行編號：23，錯誤權杖：1.行： ' c1： [type = = "x1"，value = = 1，valuetype = = "bool"] = > Issue （宣告 = c1）; '。* @ no__t-1Parser 錯誤：'POLICY0029:未預期的輸入。 </em>  
...
5. 範例：  
   ...
   **錯誤訊息：**  
   *POLICY0002：無法剖析原則資料。*  
   @no__t 0Line 號碼：1，資料行編號：91，錯誤標記： = =。行： ' c1： [type = = "x1"，value = = "1"，*  
   *valuetype = = "boolean"] = > 問題（類型 = c1. 類型，值 = "0"，valuetype = = "boolean"）; '。*  
   *Parser 錯誤：'POLICY0030:語法錯誤，未預期的 ' = = '，應為下列其中一項： ' = '*  
