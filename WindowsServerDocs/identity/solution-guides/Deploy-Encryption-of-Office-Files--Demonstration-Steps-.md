...
Contoso 的財務部門有數個儲存文件的檔案伺服器。 這些文件可以是一般的文件或具有高商業影響 (HBI) 的文件。 例如，Contoso 將包含機密資訊的任何文件視為具有高商業影響。 Contoso 想確定他們的文件有最基本的保護，且其 HBI 文件只限制給適當的人員使用。 若要這麼做，Contoso 探索使用的檔案分類基礎結構 (FCI) 和適用於 Windows Server 2012 AD RMS。 藉著使用 FCI，Contoso 將根據文件內容來分類檔案伺服器上所有的文件，然後使用 AD RMS 套用適當的權限原則。  
...
|[使用檔案管理工作自動保護與 AD RMS 的文件](Deploy-Encryption-of-Office-Files--Demonstration-Steps-.md#BKMK_3)|建立一個檔案管理工作，它會自動使用 AD RMS 來保護具有高個人識別資訊 (PII) 的文件。 只有 FinanceAdmin 群組的成員可以存取包含高 PII 的文件。|  
|[檢視結果](Deploy-Encryption-of-Office-Files--Demonstration-Steps-.md#BKMK_4)|檢查文件的分類，觀察當您變更文件中的內容時，文件如何隨之變更。 同時確認 AD RMS 保護文件的方式。|  
|[確認 AD RMS 保護](Deploy-Encryption-of-Office-Files--Demonstration-Steps-.md#BKMK_5)|確認文件受到 AD RMS 的保護。|  
|||  
  
## <a name="BKMK_1.1"></a>步驟 1:啟用資源內容  
  
#### <a name="to-enable-resource-properties"></a>啟用資源內容  
  
1. 在 [Hyper-V 管理員] 中，連線到伺服器 ID_AD_DC1。 使用 Contoso\Administrator 和密碼登入伺服器<strong>pass@word1</strong>。  
...
![解決方案指南](media/Deploy-Encryption-of-Office-Files--Demonstration-Steps-/PowerShellLogoSmall.gif)***<em>Windows PowerShell 對等的命令</em>***  
...
此步驟說明如何建立 [高影響]  分類規則。 此規則會搜尋文件的內容，如果找到 「 Contoso 公司機密 」 字串，它會分類為具有高商業影響這份文件。 這種分類會覆寫之前指派的低商業影響分類。  
  
您也會建立 [高 PII]  規則。 這個規則會搜尋文件的內容，如果找到身分證號碼，它會將此文件分類為具有高 PII。  
  
#### <a name="to-create-the-high-impact-classification-rule"></a>建立高影響分類規則  
  
1. 在 [Hyper-V 管理員] 中，連線到伺服器 ID_AD_FILE1。 使用 Contoso\Administrator 和密碼登入伺服器<strong>pass@word1</strong>。  
...
8. 在 **描述**方塊中，輸入**判斷文件是否具有高商業影響，根據是否出現 「 Contoso 公司機密 」 字串**  
...
10. 按一下 [分類]  索引標籤。在 [選擇將內容指派給檔案的方法] 下，從下拉式清單選取 [內容分類器]  。  
...
14. 按一下 [評估類型]  索引標籤。按一下 [重新評估現有的內容值] ，按一下 [覆寫]  以覆寫現有的值，然後按一下 [確定]  以完成。  
...
2. 在桌面上，開啟名為 [規則運算式]  的資料夾，然後開啟名為 [RegEx SSN]  的文件。 反白並複製下列規則運算式字串： **^ （？ ！000) ([0-7] \d{2}| 7([0-7]\d|7[012 ([-]？)(?!00) \d\d\3 （？ ！0000) \d{4}$** 。 此步驟中稍後會用到此字串，所以將它保留在剪貼簿上。  
...
8. 按一下 [分類]  索引標籤。在 [選擇將內容指派給檔案的方法] 下，從下拉式清單選取 [內容分類器]  。  
...
11. 按一下 [參數]  下的 [設定]  。   
    在 [**分類參數**] 視窗，請在**運算式型別**清單中，選取**規則運算式**。 在 **運算式**方塊中，貼上剪貼簿中的文字： **^ （？ ！000) ([0-7] \d{2}| 7([0-7]\d|7[012 ([-]？)(?!00) \d\d\3 （？ ！0000) \d{4}$** ，然後按一下**確定**。  
  
    > [!NOTE]  
    > 這個運算式會允許無效身分證號碼。 這可讓我們在示範中使用虛構的身分證號碼。  
  
12. 按一下 [評估類型]  索引標籤。依序選取 [重新評估現有的內容值] 、[覆寫]  以覆寫現有的值，然後按一下 [確定]  以完成。  
...
## <a name="BKMK_3"></a>步驟 3:使用檔案管理工作自動保護與 AD RMS 的文件  
既然您已建立自動分類文件內容為基礎的規則下, 一個步驟是建立檔案管理工作，使用 AD RMS 自動保護特定文件分類。 在此步驟中，您將建立自動保護具有高 PII 的任何文件的檔案管理工作。 只有 FinanceAdmin 群組的成員可以存取包含高 PII 的文件。  
...
它是看看您新的自動分類和 AD RMS 保護規則實際的時間。 在這個步驟中，您會檢查文件的分類，觀察當您變更文件中的內容時，文件如何隨之變更。  
...
6. 切換至 CLIENT1。 登出任何登入的使用者登入，然後登入使用 Contoso\MReid 身分和密碼<strong>pass@word1</strong>。  
...
11. 以滑鼠右鍵按一下 [財經備忘] 文件，按一下 [內容]  。 按一下 [分類]  索引標籤。請注意，[影響] 內容現在已設定為 [高]  。 按一下 [取消]  。  
  
12. 以滑鼠右鍵按一下 [雇用核准要求] 文件，按一下 [內容]  。  
  
13. . 按一下 [分類]  索引標籤。請注意，[個人識別資訊] 內容現在已設定為 [高]  。 按一下 [取消]  。  
  
## <a name="BKMK_5"></a>步驟 5:確認 AD RMS 保護  
