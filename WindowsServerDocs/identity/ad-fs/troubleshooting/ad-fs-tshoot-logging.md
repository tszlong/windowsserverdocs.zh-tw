...
4.       使用提高的權限開啟命令提示字元並執行下列命令，以啟用稽核 auditpol.exe /set /subcategory:"Application Generated"/failure /success:enable 5。       關閉**本機安全性原則**，然後開啟 AD FS 管理嵌入式管理單元。
 
若要開啟 AD FS 管理嵌入式管理單元，按一下 開始，指向程式、 指向 系統管理工具，然後按一下 AD FS 管理。
 
6.       在 [動作] 窗格中，按一下 [編輯同盟服務屬性 7]。       在 [Federation Service 屬性] 對話方塊中，按一下 [事件] 索引標籤。8.       選取 **成功稽核**並**失敗稽核**核取方塊。
9.       按一下 [確定]。
