...
下圖顯示虛構的 Contoso 製藥公司如何將其同盟伺服器陣列與公司網路中的 SQL Server 拓撲一起部署。 它也會顯示該公司如何設定具有 DNS 伺服器存取權的周邊網路、使用相同叢集 DNS 名稱的額外 NLB 主機 \(fs.contoso.com\) 用於公司網路 NLB 叢集，以及兩個 web 應用程式 proxy \(wap1 和 wap2\)。  
...
> [!NOTE]  
> 使用 Windows Internal Database \(WID 的 AD FS 伺服器陣列\) 在主要同盟伺服器節點上以讀取\/寫入存取權提供基本資料冗余，並且唯讀取次要節點上的\-存取。  這可用於地理區域或地理位置分散的拓撲。  
>   
> 使用 WID 時，請注意下列限制：  
>   
...
AlwaysOn 可用性群組是在 SQL Server 2012 中引進，並提供新的方式來建立高可用性 SQL Server 實例。  AlwaysOn 可用性群組會結合叢集和資料庫鏡像的元素，以進行 SQL 實例層和資料庫層的冗余和容錯移轉。  與先前的高可用性選項不同的是，AlwaysOn 可用性群組不需要在資料庫層中\) 一般存放裝置 \(或存放區域網路。  
  
可用性群組是由主要複本組成，\(一組讀取\-寫入主資料庫\) 以及一到四個可用性複本 \(一組對應的次要資料庫\)。  可用性群組支援 \(主要複本\)的單一讀取\-寫入複本，以及一到四個讀取\-的唯一可用性複本。  每個可用性複本都必須位於單一 Windows Server 容錯移轉叢集 \(WSFC\) 叢集的不同節點上。  如需 AlwaysOn 可用性群組的詳細資訊，請參閱[AlwaysOn 可用性群組 \(SQL Server\)的總覽](https://technet.microsoft.com/library/ff877884.aspx)。  
...
如果您打算將 AlwaysOn 可用性群組與 SQL Server 合併式複寫搭配使用，請記下「在下列情況下使用 AD FS 與 SQL Server 合併式複寫的主要部署考慮」中所述的問題。  特別是，當包含複寫訂閱者資料庫的 AlwaysOn 可用性群組進行故障傳送時，複寫訂閱會失敗。 若要繼續複寫，複寫管理員必須手動重新設定訂閱者。  請參閱複寫訂閱者端之特定問題的 SQL Server 描述[和 AlwaysOn 可用性群組 \(SQL Server\)](https://technet.microsoft.com/library/hh882436.aspx)和 AlwaysOn 可用性群組的整體支援聲明、複寫時的複寫選項[、變更追蹤、變更資料捕獲，以及 AlwaysOn 可用性群組 \(](https://technet.microsoft.com/library/hh403414.aspx)SQL Server\)。  
  
**設定 AD FS 以使用 AlwaysOn 可用性群組**  
  
設定具有 AlwaysOn 可用性群組的 AD FS 伺服器陣列時，需要稍微修改 AD FS 部署程式：  
  
1.  您必須先建立要備份的資料庫，才能設定 AlwaysOn 可用性群組。  AD FS 會建立其資料庫，做為新 AD FS SQL Server 服務器陣列的第一個 federation service 節點的安裝和初始設定的一部分。  在 AD FS 設定中，您必須指定 SQL 連接字串，因此您將必須將第一個 AD FS 伺服器陣列節點設定為直接連接到 SQL 實例，\(這只是暫時的\)。   如需有關設定 AD FS 伺服器陣列的特定指引，包括使用 SQL server 連接字串設定 AD FS 伺服器陣列節點，請參閱[設定同盟伺服器](../../ad-fs/deployment/Configure-a-Federation-Server.md)。  
  
2.  建立 AD FS 資料庫之後，請將它們指派給 AlwaysOn 可用性群組，並在[建立和設定可用性群組 \(SQL Server\)](https://technet.microsoft.com/library/ff878265.aspx)時，使用 SQL Server 工具和程式建立一般的 TCPIP 接聽程式。  
...
下圖顯示具有合併式複寫 \(1 發行者、2個訂閱者\)的地理位置多餘 AD FS SQL Server 服務器陣列：  
  
![使用 SQL 的伺服器陣列](media/ADFSSQLGeoRedundancy3.png)  
  
**使用 AD FS 搭配 SQL Server 合併式複寫的主要部署考慮 \(在上圖中看到數位\)**  
  
-   散發者資料庫不支援與 AlwaysOn 可用性群組或資料庫鏡像搭配使用。  請參閱在複寫[、變更追蹤、變更資料捕獲和 AlwaysOn 可用性群組 \(SQL Server\)](https://technet.microsoft.com/library/hh403414.aspx)的複寫選項中，SQL Server AlwaysOn 可用性群組的支援聲明。  
  
-   當包含複寫訂閱者資料庫的 AlwaysOn 可用性群組容錯移轉時，複寫訂閱會失敗。 若要繼續複寫，複寫管理員必須手動重新設定訂閱者。  請參閱複寫訂閱者端的特定問題 SQL Server 描述[和 AlwaysOn 可用性群組 \(SQL Server](https://technet.microsoft.com/library/hh882436.aspx)使用複寫選項複寫[、\)、變更資料捕獲，以及變更追蹤 AlwaysOn 可用性群組](https://technet.microsoft.com/library/hh403414.aspx)\(SQL Server 的 AlwaysOn 可用性群組的整體支援聲明。\)  
