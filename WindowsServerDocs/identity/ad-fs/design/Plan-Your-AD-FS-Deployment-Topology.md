...
|資料庫功能|使用提取複寫的基本資料庫冗余，其中一或多部裝載讀取\-的伺服器只會在裝載資料庫讀取\/寫入複本的來源伺服器上進行變更|是|否 
|資料庫功能|使用高\-可用性解決方案的資料庫冗余，例如在資料庫層級的容錯移轉叢集或鏡像 \(僅\)**注意：** 所有 AD FS 部署拓撲都支援 AD FS 服務層級的叢集。|否|是  
