...
Active Directory 同盟服務 \(AD FS\) 的預設拓撲是同盟伺服器陣列，使用 Windows 內部資料庫 \(WID\)。 在此拓撲中，AD FS 會針對加入該伺服器陣列的所有同盟伺服器，使用 WID 做為 AD FS 設定資料庫的存放區。 伺服器陣列會複寫並維護伺服器陣列中每部伺服器之設定資料庫的 Federation Service 資料。 Windows Server 2012 R2 中的 AD FS 可讓具有100或較少信賴憑證者信任的組織，使用最多30部伺服器的 WID 來設定同盟伺服器陣列。  
