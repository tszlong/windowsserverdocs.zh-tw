...
當網際網路上的用戶端電腦嘗試存取受 Active Directory 同盟服務保護的應用程式時 @no__t 0AD FS @ no__t-1，必須先向同盟伺服器驗證。 在大部分情況下，同盟伺服器通常無法直接從網際網路存取。 因此，網際網路用戶端電腦必須改為重新導向至同盟伺服器 proxy。 您可以藉由將適當的網域名稱系統（\(DNS @ no__t-1 筆記錄新增至您的 DNS 區域或面對網際網路的區域），來完成成功的重新導向。  
  
您用來將網際網路用戶端重新導向至同盟伺服器 proxy 的方法，取決於您在周邊網路中設定 DNS 區域的方式，或如何設定您在網際網路上控制的 DNS 區域。 同盟伺服器 proxy 是用於周邊網路。 只有當 DNS 已在您控制的所有網際網路 @ no__t-0facing 區域中正確設定時，它們才會成功地將網際網路用戶端要求重新導向至同盟伺服器。 因此，您的網際網路 @ no__t-0facing 區域設定（無論您的 DNS 區域是否僅提供周邊網路或服務周邊網路和網際網路用戶端的 DNS 區域）都很重要。  
...
-   同盟伺服器 proxy 必須有 hosts 檔案中的設定，才能將同盟伺服器端點 URL 的完整功能變數名稱 @no__t 0FQDN @ no__t 解析為同盟伺服器或同盟伺服器叢集的 IP 位址。  
  
-   必須設定帳戶夥伴的周邊網路中的 DNS，才能將同盟伺服器端點 URL 的 FQDN 解析為同盟伺服器 proxy 的 IP 位址。  
  
下圖和相對應的步驟顯示如何針對指定的範例達成每一個條件。 在此圖中，Microsoft 網路負載平衡 \(NLB @ no__t 1 技術提供現有同盟伺服器陣列的單一、叢集 FQDN 和單一叢集 IP 位址。  
...
因為周邊網路中的 DNS 設定為將 fs.fabrikam.com 的所有要求解析為帳戶同盟伺服器 proxy，所以帳戶夥伴同盟伺服器 proxy 在其本機主機檔案中具有專案，可將 fs.fabrikam.com 解析為的 IP 位址實際的帳戶同盟伺服器 @no__t-連線到公司網路之同盟伺服器陣列 @ no__t-1 的0or 叢集 DNS 名稱。 如此一來，帳戶同盟伺服器 proxy 就可以將主機名稱 fs.fabrikam.com 解析為帳戶同盟伺服器，而不是解析為本身—如果它嘗試使用周邊 DNS 來查詢 fs.fabrikam.com，就會發生這種情況，因此同盟伺服器 proxy 可以與同盟伺服器通訊。  
  
### <a name="2-configure-perimeter-dns"></a>2.設定周邊 DNS  
因為用戶端電腦只會被導向至單一 AD FS 主機名稱（無論是在內部網路或網際網路上），所以使用周邊 DNS 伺服器的網際網路上的用戶端電腦必須將帳戶同盟伺服器的 FQDN \(fs 解析為周邊網路上帳戶同盟伺服器 proxy 的 IP 位址。 如此一來，當用戶端嘗試解析 fs.fabrikam.com 時，就可以將用戶端轉送至帳戶同盟伺服器 proxy，而周邊 DNS 包含一個受限的 corp.fabrikam.com DNS 區域，其中具有單一主機 \(A @ no__t-1 個 fs \(fs 的資源記錄。 fabrikam .com @ no__t-3，以及周邊網路上帳戶同盟伺服器 proxy 的 IP 位址。  
...
在此案例中，因為它會假設您將設定您控制的網際網路 DNS 區域，以解析針對特定端點 URL 所提出的要求 \(that 是，則是 no__t-1 到周邊網路中的同盟伺服器 proxy。您也必須設定周邊 DNS 中的區域，以將這些要求轉送到公司網路中的同盟伺服器。  
  
如此一來，用戶端可以在嘗試解析 fs.fabrikam.com 時轉送到帳戶同盟伺服器，就會使用單一主機設定周邊 DNS \(A @ no__t-1 資源記錄用於 fs \(fs。 fabrikam .com @ no__t-3，以及的 IP 位址公司網路上的帳戶同盟伺服器。 如此一來，帳戶同盟伺服器 proxy 就可以將主機名稱 fs.fabrikam.com 解析為帳戶同盟伺服器，而不是解析為本身—如果它嘗試使用網際網路 DNS 來查詢 fs.fabrikam.com，就會發生這種情形—讓同盟伺服器proxy 可以與同盟伺服器通訊。  
