...
下圖顯示虛構的 Contoso 製藥公司如何將其同盟伺服器陣列與公司網路中的 SQL Server 拓撲一起部署。 它也會顯示該公司如何設定具有 DNS 伺服器存取權的周邊網路、使用相同叢集 DNS 名稱的額外 NLB 主機 \(fs，以及在公司網路 NLB 叢集上使用的 no__t-1，以及兩部同盟伺服器。proxy \(fsp1 和 fsp2 新增 @ no__t-3。  
