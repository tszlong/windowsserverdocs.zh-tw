...
-   **以「第0層」系統的身分保護 AD FS** 
    ...
    AD FS 基本上是一種驗證系統。  因此，它應該被視為「第0層」系統，就像您網路上的其他身分識別系統一樣。  [Microsoft Docs](https://docs.microsoft.com/windows-server/identity/securing-privileged-access/securing-privileged-access-reference-material)具有 Active Directory 系統管理層模型的詳細資訊。 


-   **使用 [安全性設定] 嚮導，將 AD FS 特定的安全性最佳作法套用到同盟伺服器和同盟伺服器 proxy 電腦**  
...
-   **如果安全性是非常重要的考慮（例如，使用 kiosk 時），請使用權杖重新執行偵測。**  
...
-   **請仔細評估 SQL 安裝中額外的通道安全性需求。**  
    ...
    雖然加密是一項可協助確保安全性的好用工具，但還是不應該將它視為適用於所有資料或連線。 當您在決定是否要實作加密時，請考量使用者將存取資料的方式。 如果使用者是透過公用網路存取資料，可能就需要資料加密來增加安全性。 不過，如果 AD FS SQL 資料的所有存取都牽涉到安全的內部網路設定，可能就不需要加密。 任何加密的用法也都應該包含對密碼、金鑰及憑證的維護策略。  
