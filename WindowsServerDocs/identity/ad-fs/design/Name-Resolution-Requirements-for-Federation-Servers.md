...
當公司網路上的用戶端電腦嘗試存取受 Active Directory 同盟服務保護的應用程式或 Web 服務時 \(AD FS @ no__t-1，必須先向同盟伺服器進行驗證。 驗證的方法之一是讓公司網路客戶透過 Windows 整合式驗證存取本機同盟伺服器。  
  
## <a name="configure-corporate-dns"></a>設定公司 DNS  
若要透過本機同盟伺服器上的 Windows 整合式驗證進行成功的名稱解析，則必須為新主機設定在帳戶夥伴的公司網路中的網域名稱系統 \(DNS @ no__t-1，\(A @ no__t-3資源記錄，可將同盟伺服器的完整功能變數名稱 @no__t 4FQDN @ no__t-5 主機名稱解析為同盟伺服器叢集的 IP 位址。  
  
在下圖中，您可以看到特定案例如何完成這項工作。 在此案例中，Microsoft 網路負載平衡 \(NLB @ no__t-1 為現有的同盟伺服器陣列提供單一叢集 FQDN 名稱和單一叢集 IP 位址。  
