...
若要進一步瞭解如何建立風險評估外掛程式，並在 AD FS 程式中執行它，讓我們建立一個範例外掛程式，以封鎖來自特定**外部**網路 ip 的要求，並將該外掛程式向 AD FS 註冊，最後測試功能. 
...
4. 從 [ `Microsoft.IdentityServer.dll`方案瀏覽器] 中移除，如下所示：</br>
   ![模組](media/ad-fs-risk-assessment-model/risk2.png)

5. 將參考新增至`Microsoft.IdentityServer.dll`您的 AD FS，如下所示

   a.   以滑鼠右鍵按一下**方案瀏覽器**中的 [**參考**]，然後選取 [**新增參考 ...** ]</br> 
   ![模組](media/ad-fs-risk-assessment-model/risk3.png)
   
   b.   在 [**參考管理員**] 視窗中，選取 **[流覽]** 。 在 [**選取要參考的檔案 ...** ] 對話方塊中， `Microsoft.IdentityServer.dll`從您的 AD FS 安裝資料夾中選取（在我的案例中為**C:\Windows\ADFS**），然後按一下 [**新增**]。
   
   >[!NOTE]
   >在我的案例中，我會在 AD FS 伺服器本身上建立外掛程式。 如果您的開發環境是在不同的伺服器上， `Microsoft.IdentityServer.dll`請將從 AD FS 伺服器上的 AD FS 安裝資料夾複製到您的開發箱。</br> 
   
   ![模型](media/ad-fs-risk-assessment-model/risk4.png)
   
   c.   確定`Microsoft.IdentityServer.dll`已選取核取方塊之後，請在 [**參考管理員**] 視窗中按一下 **[確定**]</br>
   ![模組](media/ad-fs-risk-assessment-model/risk5.png)
...
6. 所有類別和參考現在都已準備好執行組建。   不過，因為此專案的輸出是 dll，所以必須將它安裝到 AD FS 伺服器的**全域組件快取**或 GAC 中，而且必須先簽署 dll。 這可以透過下列方式來完成：
   ...
   ![模組](media/ad-fs-risk-assessment-model/risk6.png)
   
   b.   從 [**屬性**] 頁面上，按一下左側的 [簽署]，然後**核**取標示為 **[簽署元件**] 的核取方塊。 從 [**選擇強式名稱金鑰**檔：] 下拉式功能表中，選取 [ **< 新增]。>**</br>
   ![模組](media/ad-fs-risk-assessment-model/risk7.png)

   c.   在 [**建立強式名稱金鑰] 對話方塊**中，輸入金鑰的名稱（您可以選擇任何名稱），取消核取 [**以密碼保護我的金鑰**檔] 核取方塊。 然後按一下 **[確定]** 。
   ![模組](media/ad-fs-risk-assessment-model/risk8.png)</br>
 
   d.   儲存專案，如下所示</br>
   ![模組](media/ad-fs-risk-assessment-model/risk9.png)

7. 依序按一下 [**建立**] 和 [**重建方案**] 來建立專案，如下所示</br>
   ![模組](media/ad-fs-risk-assessment-model/risk10.png)
 
   檢查畫面底部的 [**輸出] 視窗**，查看是否發生任何錯誤</br>
   ![模組](media/ad-fs-risk-assessment-model/risk11.png)
...
2. 啟動 Visual Studio 的**開發人員命令提示字元**，並移至包含**sn.exe**的目錄（在我的案例中，目錄是**C:\Program Files （x86） \Microsoft SDKs\Windows\v10.0A\bin\NETFX 4.7.2 Tools**）模型![](media/ad-fs-risk-assessment-model/risk12.png)

3. 以 **-T**參數和檔案的位置（在我的案例`SN -T “C:\extensions\ThreatDetectionModule.dll”`中） ![模型執行**SN**命令](media/ad-fs-risk-assessment-model/risk13.png)</br>
...
4. 將 dll 加入 AD FS 伺服器的**全域組件快取**中，我們的最佳做法是為您的專案建立適當的安裝程式，並使用安裝程式將檔案新增至 GAC。 另一個解決方案是在您的開發電腦上使用**Gacutil** （如需有關**Gacutil**的詳細資訊，請參閱[這裡](https://docs.microsoft.com/dotnet/framework/tools/gacutil-exe-gac-tool)提供）。  由於我的 visual studio 與 AD FS 在同一部伺服器上，因此我將使用**Gacutil** ，如下所示
   ...
   b.   執行**Gacutil**命令（在我的案例`Gacutil /IF C:\extensions\ThreatDetectionModule.dll`中![）模型](media/ad-fs-risk-assessment-model/risk14.png)
...
1. 開啟我們稍早建立的**authconfig** （在位置**C:\extensions**的案例中），並新增您想要封鎖的**外部網路 ip** 。 每個 IP 都應該位於不同的行，結尾處不應該有空格</br>
   ![模組](media/ad-fs-risk-assessment-model/risk18.png)
...
|方法 |Type|定義|
|-----|-----|-----| 
|[OnAuthenticationPipelineLoad](https://docs.microsoft.com/dotnet/api/microsoft.identityserver.public.threatdetectionframework.threatdetectionmodule.onauthenticationpipelineload?view=adfs-2019) |空位|當外掛程式載入其管線時，由 AD FS 呼叫| 
|[OnAuthenticationPipelineUnload](https://docs.microsoft.com/dotnet/api/microsoft.identityserver.public.threatdetectionframework.threatdetectionmodule.onauthenticationpipelineunload?view=adfs-2019) |空位|當外掛程式從其管線中卸載時，由 AD FS 呼叫| 
|[OnConfigurationUpdate](https://docs.microsoft.com/dotnet/api/microsoft.identityserver.public.threatdetectionframework.threatdetectionmodule.onconfigurationupdate?view=adfs-2019)| 空位|由 AD FS on config update 呼叫 |
|**屬性** |**型別** |**定義**|
...
|[ModuleIdentifier](https://docs.microsoft.com/dotnet/api/microsoft.identityserver.public.threatdetectionframework.threatdetectionmodule.moduleidentifier?view=adfs-2019)|字串 |取得外掛程式的識別碼|
...
>[!NOTE]
>上面所討論的範例外掛程式只會執行 IRequestReceivedThreatDetectionModule 介面。 不過，風險評估模型提供兩個額外的介面– IPreAuthenticationThreatDetectionModule （以執行風險評估邏輯 duing 預先驗證階段）和 IPostAuthenticationThreatDetectionModule （以實現風險驗證後階段的評估邏輯）。 以下提供兩個介面的詳細資料。 
...
此介面包含[EvaluatePostAuthentication](https://docs.microsoft.com/dotnet/api/microsoft.identityserver.public.threatdetectionframework.ipostauthenticationthreatdetectionmodule.evaluatepostauthentication?view=adfs-2019)方法，可讓您使用[RequestCoNtext RequestCoNtext](https://docs.microsoft.com/dotnet/api/microsoft.identityserver.public.threatdetectionframework.requestcontext?view=adfs-2019)、 [SecurityCoNtext SecurityCoNtext](https://docs.microsoft.com/dotnet/api/microsoft.identityserver.public.threatdetectionframework.securitycontext?view=adfs-2019)、 [ProtocolCoNtext ProtocolCoNtext](https://docs.microsoft.com/dotnet/api/microsoft.identityserver.public.threatdetectionframework.protocolcontext?view=adfs-2019)中傳遞的資訊。和[IList<Claim> additionalClams](https://docs.microsoft.com/dotnet/api/system.collections.generic.ilist-1?view=netframework-4.7.2)輸入參數，以撰寫驗證後風險評估邏輯。 
