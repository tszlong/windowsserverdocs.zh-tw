...
您可以使用`Grant-ADFSApplicationPermission` Cmdlet 來指派範圍，如下列範例所示：
...
5. 在 [**設定 WEB API** ] 畫面上，針對 [**識別碼** - **https://contoso.com/WebApp**] 輸入下列各項。  按一下 [新增]  。 按 [下一步]  。  稍後在應用程式的 web.config 檔案中，將會使用此值進行**ida： ResourceID** 。
