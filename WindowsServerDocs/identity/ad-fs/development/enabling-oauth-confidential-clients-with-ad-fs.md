...
4.  針對 [重新導向 URI] 輸入下列**內容：**  -  **https://localhost:44323** 。  按一下 [新增] 。 按 [下一步]  。

5.  在 [**設定應用程式認證**] 畫面上，勾選 [**產生共用密碼**] 和 [複製密碼]。  稍後在應用程式 web.config 檔中，將會使用此值做為**ida： ClientSecret**的值。  按 [下一步]  。

    ![AD FS Oauth](media/Enabling-Oauth-Confidential-Clients-with-AD-FS-2016/AD_FS_Confidential_4.PNG)

6. 在 [**設定 WEB API** ] 畫面上，針對 [**識別碼**] 輸入下列各項  -  **https://contoso.com/WebApp** 。  按一下 [新增] 。 按 [下一步]  。  稍後在應用程式 web.config 檔中，將會使用此值進行**ida： GraphResourceId** 。
...
3.  在頂端，選取 [ **View**  ->  **伺服器總管**]。  一旦開啟，請在 [**資料連線**] 底下，以滑鼠右鍵按一下 [ **DefaultConnection** ]，然後選取 [**修改連接**]。

    ![AD FS Oauth](media/Enabling-Oauth-Confidential-Clients-with-AD-FS-2016/AD_FS_Confidential_14.PNG)

4.  在 [**修改連接**] 的 [**資料庫檔案名] 底下 (新增或現有) **] 底下，選取 **[流覽]** 並提供**path\filename.mdf**。 在對話方塊中按一下 [**是]** 。
...
6.  在 [Advanced] 屬性上，找出 [資料來源]，然後使用下拉式) 將它從** (LocalDb\v11.0**變更為** (LocalDb) \mssqllocaldb**。
...
10. 在 Visual Studio 頂端，選取 [**查看**  ->  **其他 Windows**  ->  **套件管理員主控台**]。
