...
如果裝置未註冊，但使用者選取 [讓我保持登入] 選項，重新整理權杖的到期時間將會等於 [讓我保持登入] 的持續 SSO cookie 存留期，預設為1天，最多7天。 否則，重新整理權杖存留期等於會話 SSO cookie 存留期，預設為8小時
 ...
 如前所述，已註冊裝置上的使用者一律會取得持續性 SSO，除非已停用持續性 SSO。 針對未註冊的裝置，可以藉由啟用「讓我保持登入」 (KMSI) 功能來達到持續性 SSO。
...
當要求者從已註冊的裝置進行驗證時，AD FS 2016 會變更 PSSO，但在14天的期間內需要驗證90， ([裝置使用時間] 視窗) 。
...
[裝置使用量] 視窗預設 (14 天，) 是由 AD FS 屬性**DeviceUsageWindowInDays**所控制。

``` powershell
Set-AdfsProperties -DeviceUsageWindowInDays
```
預設的單一登入期間上限 (90 天) 由 AD FS 屬性**PersistentSsoLifetimeMins**所控管。
...
請務必注意，雖然提供較長的單一登入週期，但是當先前的登入是以主要認證而非 MFA 為基礎，但目前的登入需要 MFA 時，AD FS 會提示您進行額外的驗證 (多重要素驗證) 。  無論 SSO 設定為何，都是如此。 AD FS，當它收到驗證要求時，會先判斷是否有 SSO 內容 (例如 cookie) ，然後如果需要 MFA (例如，如果要求是來自) 外部，則會評估 SSO 內容是否包含 MFA。  如果不是，則會提示 MFA。
...
發生這種情況的原因是 Azure AD 無法判斷何時撤銷與舊認證相關的權杖 (例如已變更) 的密碼。 因此，Azure AD 必須更頻繁地檢查，以確定使用者和相關聯的權杖仍在良好的地位。
