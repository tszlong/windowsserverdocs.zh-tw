...
此外，還有一些選擇性層面。
- 如果您想要根據憑證欄位和延伸模組，以及 EKU （宣告類型 https://schemas.microsoft.com/2012/12/certificatecontext/extension/eku) 來使用宣告，請在 Active Directory 宣告提供者信任上設定額外的宣告傳遞規則。  如需可用憑證宣告的完整清單，請參閱下文。  
...
### <a name="check-if-certificate-authentication-is-enabled-in-the-ad-fs-authentication-policy"></a>檢查 AD FS 驗證原則中是否已啟用憑證驗證
AD FS 預設會在通訊埠49443上，使用與 AD FS 相同的主機名稱（例如 `adfs.contoso.com`）進行使用者憑證驗證。 您也可以使用替代 SSL 系結，將 AD FS 設定為使用埠443（預設的 HTTPS 埠）。 不過，此設定中使用的 URL 是 `certauth.<adfs-farm-name>` （例如 `certauth.contoso.com`）。 如需詳細資訊，請參閱[此連結](ad-fs-support-for-alternate-hostname-binding-for-certificate-authentication.md)。 網路連線最常見的情況是防火牆設定不正確，而且會封鎖或干擾使用者憑證驗證流量。 通常當此問題發生時，您會看到空白畫面或500伺服器錯誤。 
1)  請注意您在中設定的主機名稱和埠 AD FS
2)  確定 AD FS 或 Web 應用程式 Proxy （WAP）前方的任何防火牆都已設定為允許 AD FS 伺服器陣列的 `hostname:port` 組合。 您必須參考您的網路工程師來執行此步驟。 
