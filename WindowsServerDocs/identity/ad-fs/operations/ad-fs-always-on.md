...
3.  在 SQL Server 組態管理員中，按一下 [SQL Server 服務]，以滑鼠<instance name>按右鍵 SQL Server <instance name> （），其中是您要啟用 Always On 可用性群組的本機伺服器實例名稱，然後按一下 [屬性]。
...
6.  接下來，指定叢集類型。 可能的叢集類型取決於 SQL Server 版本和作業系統。 選擇 [WSFC]、[EXTERNAL] 或 [NONE]。 如需詳細資訊，請參閱[指定可用性組名](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/specify-availability-group-name-page?view=sql-server-ver15)頁面
