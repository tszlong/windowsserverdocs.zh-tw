...
>[!NOTE]
>協力廠商 proxy 不支援此案例，因為具有被動式 (Web 型) 要求的用戶端存取原則標頭有所限制。
...
|                                                                                 建議所有規則都包含此。                                                                                  |                                    存在 ( [類型 = = " <https://schemas.microsoft.com/2012/01/requestcontext/claims/x-ms-proxy> ]] )                                     |
...
| 此規則說明特定 Active Directory 群組中的唯一使用者 (應拒絕) 的 SID 值。 將 NOT 加入此語句表示將允許一組使用者，無論位置為何。 |             存在 ( [類型 = = " <https://schemas.microsoft.com/ws/2008/06/identity/claims/groupsid> "，值 = ~ "{允許 AD 群組} 的群組 SID 值}"] )               |
