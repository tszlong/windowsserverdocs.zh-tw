...
> [!NOTE]
> 如果您在升級至 Windows Server 2016 中的架構版本 (層級85或更高的) 之前已安裝 Azure AD Connect，您必須重新執行 Azure AD Connect 安裝並重新整理內部部署 AD 架構，以確保已設定 Msds-keycredentiallink 的同步處理規則。
...
2. 在您的 AD FS 主伺服器上，請確定您是以具有 Enterprise Admin (EA ) 許可權的 AD DS 使用者身分登入，並開啟已提升許可權的 powershell 提示字元。  然後，執行下列 PowerShell 命令：
...
1. 請確定您 AD DS 中的服務連接點存在，而且具有 (我們建立此物件的適當許可權，但再次檢查) 並不會造成損害。
2. 確定已正確設定 AD FS
...
2.  適用于已加入網域的 Windows 10 用戶端
    ...
    2. 在 AD DS 物件 (上檢查 stk 金鑰認證連結的指示是否仍然需要執行兩次？ ) 
3.  如果您在嘗試註冊已註冊裝置的 Windows 電腦時發生錯誤，但無法或尚未取消註冊裝置，您可能會在登錄中擁有裝置註冊設定的片段。  若要調查並移除此項，請使用下列步驟：
    ...
    2. 在此機碼下，GUID 表單中將會有許多子機碼。  流覽至其中有 ~ 17 個值的子機碼，並具有 "6" [已加入 MDM] 或 [13] (Azure AD 聯結的 [EnrollmentType]) 
