...
Active Directory 同盟服務現在支援使用存取控制原則範本。  藉由使用存取控制原則範本，系統管理員可以藉由將原則範本指派給一組信賴憑證者（RPs）來強制執行原則設定。 系統管理員也可以對原則範本進行更新，如果不需要使用者互動，則會自動將變更套用至信賴憑證者。  
...
|允許使用者|但是| 
| --- | --- | 
 |從**特定**網路|從**特定**網路<br /><br />從**特定**群組<br /><br />從具有**特定**信任層級的裝置<br /><br />在要求中使用**特定**宣告|  
|從**特定**群組|從**特定**網路<br /><br />從**特定**群組<br /><br />從具有**特定**信任層級的裝置<br /><br />在要求中使用**特定**宣告|  
|從具有**特定**信任層級的裝置|從**特定**網路<br /><br />從**特定**群組<br /><br />從具有**特定**信任層級的裝置<br /><br />在要求中使用**特定**宣告|  
|在要求中使用**特定**宣告|從**特定**網路<br /><br />從**特定**群組<br /><br />從具有**特定**信任層級的裝置<br /><br />在要求中使用**特定**宣告|  
|和需要多重要素驗證|從**特定**網路<br /><br />從**特定**群組<br /><br />從具有**特定**信任層級的裝置<br /><br />在要求中使用**特定**宣告|  
  
如果系統管理員選取多個條件，它們就是**和**關聯性。 動作是互斥的，而且對於一個原則規則，您只能選擇一個動作。 如果管理員選取多個例外狀況，它們會是**或**關聯性。 以下顯示幾個原則規則範例：  
  
|**原則**|**原則規則**|
| --- | --- |  
|外部網路存取需要 MFA<br /><br />允許所有使用者|**規則 #1**<br /><br />從**外部**網路<br /><br />使用 MFA 的和<br /><br />允許<br /><br />**規則 # 2**<br /><br />來自**內部**網路<br /><br />允許|  
|不允許外部存取，但非 FTE 除外<br /><br />允許已加入工作場所裝置上的 FTE 內部網路存取|**規則 #1**<br /><br />從**外部**網路<br /><br />和來自**非 FTE**群組<br /><br />允許<br /><br />**規則 #2**<br /><br />來自**內部**網路<br /><br />和來自已**加入工作場所**的裝置<br /><br />和來自**FTE**群組<br /><br />允許|  
|外部網路存取需要 MFA，但「服務管理員」除外<br /><br />允許所有使用者存取|**規則 #1**<br /><br />從**外部**網路<br /><br />使用 MFA 的和<br /><br />允許<br /><br />**服務管理群組**除外<br /><br />**規則 #2**<br /><br />總是<br /><br />允許|  
|從外部網路存取的非工作位置聯結裝置需要 MFA<br /><br />允許內部網路和外部網路存取 AD 網狀架構|**規則 #1**<br /><br />來自**內部**網路<br /><br />和來自**AD Fabric**群組<br /><br />允許<br /><br />**規則 #2**<br /><br />從**外部**網路<br /><br />和來自**未加入工作場所**的裝置<br /><br />和來自**AD Fabric**群組<br /><br />使用 MFA 的和<br /><br />允許<br /><br />**規則 #3**<br /><br />從**外部**網路<br /><br />和來自已**加入工作場所**的裝置<br /><br />和來自**AD Fabric**群組<br /><br />允許|  
...
2.  輸入 [名稱] 和 [描述]。  例如: 允許已驗證裝置但未受管理的使用者。  
...
2.  輸入 [名稱] 和 [描述]。  例如: 允許具有特定宣告和來自特定群組的使用者。  
