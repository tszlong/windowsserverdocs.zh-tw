...
如需支援的加密套件的完整清單，請參閱[TLS/SSL (SCHANNEL SSP) 中的加密套件](/windows/win32/secauthn/cipher-suites-in-schannel)。  本檔提供預設啟用的套件清單，以及預設支援但未啟用的套件。  若要設定加密套件的優先順序，請參閱設定[Schannel 加密套件的優先順序](/windows/win32/secauthn/prioritizing-schannel-cipher-suites)。
...
- [TLS/SSL (Schannel SSP) 中的加密套件](/windows/win32/secauthn/cipher-suites-in-schannel)
