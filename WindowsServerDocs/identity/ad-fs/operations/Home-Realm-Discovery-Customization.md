...
組織可以與多個宣告提供者同盟。 AD FS 現在提供了 \- box 功能，可讓系統管理員列出宣告提供者支援的尾碼，例如、、 @us.contoso.com @eu.contoso.com ，並將其啟用以後綴為 \- 基礎的探索。 使用此組態，使用者可以輸入其組織帳戶，而 AD FS 會自動選取對應的宣告提供者。

若要設定身分識別提供者 \( IDP （ \) 例如 `fabrikam` ）以使用特定電子郵件尾碼，請使用下列 Windows PowerShell Cmdlet 和語法。
...
> [!IMPORTANT]
> 請注意，如果已設定信賴憑證者的身分識別提供者清單，即使已啟用先前的設定且使用者從內部網路存取，AD FS 仍會顯示 [主領域探索 HRD] \( \) 頁面。 若要在此情況下略過 HRD，您必須確定 "Active Directory" 也會新增到此信賴憑證者的 IDP 清單。
