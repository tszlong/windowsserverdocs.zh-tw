...
2.  [步驟 2：使用裝置註冊服務設定同盟伺服器（ADFS1）](../../ad-fs/deployment/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_4)  
  
3.  [步驟 3：設定網頁伺服器（WebServ1）和範例宣告式應用程式](../../ad-fs/deployment/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_5)  
...
-   安裝 AD DS 角色服務並安裝 Active Directory Domain Services （AD DS），讓您的電腦成為 Windows Server 2012 R2 中的網域控制站。 此動作會在建立網域控制站的過程中升級您的 AD DS 架構。 如需詳細資訊與逐步指示，請參閱[https://technet.microsoft.com/ library/hh472162.aspx .aspx](https://technet.microsoft.com/library/hh472162.aspx)。  
...
- 使用者：**Robert Hatley**，認證如下：使用者名稱：**RobertH**和 password：<strong>P@ssword</strong>  
  
- 群組：**財經**  
  
如需如何在 Active Directory （AD）中建立使用者和群組帳戶的相關資訊[https://technet.microsoft.com/library/cc783323%28v=ws.10%29.aspx](https://technet.microsoft.com/library/cc783323%28v=ws.10%29.aspx)，請參閱。  
  
將 **Robert Hatley** 帳戶加入 **Finance** 群組中。 如需如何在 Active Directory 中將使用者新增至群組的詳細資訊， [https://technet.microsoft.com/library/cc737130%28v=ws.10%29.aspx](https://technet.microsoft.com/library/cc737130%28v=ws.10%29.aspx)請參閱。  
...
## <a name="BKMK_4"></a>步驟2：使用「裝置註冊服務」設定同盟伺服器 (ADFS1)  
若要設定另一部虛擬機器，請安裝 Windows Server 2012 R2，並將它連線到網域**contoso.com**。 將電腦加入網域之後，請進行設定，然後繼續安裝並設定 AD FS 角色。  
  
如需影片，請[參閱 Active Directory 同盟服務使用方法影片系列：安裝 AD FS 伺服器](https://technet.microsoft.com/video/dn469436)陣列。  
...
[Active Directory 同盟服務 How-to 影片系列：正在更新](https://technet.microsoft.com/video/adfs-updating-certificates)憑證。  
...
1. 使用網域系統管理員帳戶administrator@contoso.com登入伺服器。  
...
接下來的步驟是在 ADFS1 伺服器上設定「裝置註冊服務」。 如需影片，請[參閱 Active Directory 同盟服務使用方法影片系列：啟用裝置註冊服務](https://technet.microsoft.com/video/adfs-how-to-enabling-the-device-registration-service)。  
  
##### <a name="to-configure-device-registration-service-for-windows-server-2012-rtm"></a>為 Windows Server 2012 RTM 設定裝置註冊服務  
  
1.  > [!IMPORTANT]  
    > **下列步驟適用于 Windows Server 2012 R2 RTM 組建。**  
...
若要完成此程序，至少需要 Administrators 群組的成員資格或同等權限。 請參閱超連結 "<https://go.microsoft.com/fwlink/?LinkId=83477>" 本機和網域預設群組（<https://go.microsoft.com/fwlink/p/?LinkId=83477>）中有關使用適當帳戶和群組成員資格的詳細資料。  
...
## <a name="BKMK_5"></a>步驟3：設定網頁伺服器 (WebServ1) 與範例宣告式應用程式  
...
1. > [!NOTE]  
   ...
   使用<strong>administrator@contoso.com</strong>和密碼<strong>pass@word1</strong>登入 WebServ1。  
...
3.  編輯 **Default.aspx.cs** 檔案並停用宣告篩選。 執行此步驟的目的是要確定範例應用程式會顯示同盟伺服器發出的所有宣告。 請執行下列動作：  
...
5.  若要設定 **claimapp** 搭配您的同盟伺服器使用，請執行下列動作：  
    ...
    1.  執行 FedUtil，其位於**C:Program Files （x86） Windows Identity Foundation sdkv 3.5**。  
  
    2.  將應用程式設定位置設為**C:inetputclaimappweb.config** ，並將應用程式 URI 設為您網站 **https://webserv1.contoso.com** 的 URL/claimapp/。 按一下 [下一步]。  
  
    3.  選取 [**使用現有的 STS** ]，然後流覽至您的 AD FS **https://adfs1.contoso.com/federationmetadata/2007-06/federationmetadata.xml** 伺服器的中繼資料 URL。 按一下 [下一步]。  
...
您現在必須使用 AD FS 來保護在 web 伺服器上執行的範例應用程式。 您可以透過在您的同盟伺服器 (ADFS1) 上新增信賴憑證者信任來執行此動作。 如需影片，請[參閱 Active Directory 同盟服務使用方法影片系列：新增信賴憑證者信任](https://technet.microsoft.com/video/adfs-how-to-add-a-relying-party-trust)。  
...
用戶端必須信任用於同盟伺服器（ADFS1）的 SSL 憑證，您可以在[步驟2：使用裝置註冊服務](../../ad-fs/deployment/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_4)來設定同盟伺服器（ADFS1）。 它也必須能夠驗證憑證撤銷資訊。  
  
您也必須設定 Microsoft 帳戶並使用它來登入 Client1。  
  
## <a name="see-also"></a>另請參閱  
[Active Directory 同盟服務 How-to 影片系列：安裝 AD FS 伺服器陣列](https://technet.microsoft.com/video/dn469436)  
[Active Directory 同盟服務 How-to 影片系列：更新憑證](https://technet.microsoft.com/video/adfs-updating-certificates)  
[Active Directory 同盟服務 How-to 影片系列：新增信賴憑證者信任](https://technet.microsoft.com/video/adfs-how-to-add-a-relying-party-trust)  
[Active Directory 同盟服務 How-to 影片系列：啟用裝置註冊服務](https://technet.microsoft.com/video/adfs-how-to-enabling-the-device-registration-service)  
[Active Directory 同盟服務 How-to 影片系列：安裝 Web 應用程式 Proxy](https://technet.microsoft.com/video/dn469438)  
