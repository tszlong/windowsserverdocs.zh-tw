...
在 Active Directory 同盟服務中使用 [傳遞] 或 [篩選傳入宣告] 規則範本的 \(AD FS @ no__t-1，您可以傳遞所有具有所選宣告類型的傳入宣告。 您也可以使用選取的宣告類型來篩選傳入宣告的值。 例如，您可以使用此規則範本來建立規則，以傳送所有傳入的群組宣告。 您也可以使用此規則，只傳送使用者主體名稱 \(UPN @ no__t-1 以 @fabrikam 結尾的宣告。  
