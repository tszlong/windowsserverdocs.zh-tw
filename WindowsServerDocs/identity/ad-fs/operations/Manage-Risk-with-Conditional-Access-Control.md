...
## <a name="BKMK_1"></a>重要概念-AD FS 中的條件式存取控制
AD FS 的主要功能是發行包含一組宣告的存取權杖。 AD FS 接受並發行哪些宣告的決定是由宣告規則管理。

AD FS 中的存取控制透過發行授權宣告規則，用來發出許可或拒絕宣告，以決定使用者是否實作，或一群使用者可以或無法存取 AD FS 保護的資源。 只可以在信賴憑證者信任上設定授權規則。
...
在 Windows Server 2012 R2 中 AD FS 中，存取控制以增強了多項因素，包括使用者、 裝置、 位置和驗證資料。 這種方法之所以可行是因為授權宣告規則有多種不同的宣告類型。  換句話說，在 Windows Server 2012 R2 中 AD FS，您可以強制執行條件式存取控制，根據使用者的身分識別或群組的成員資格、 網路位置、 裝置 (無論是加入工作地點，如需詳細資訊，請參閱[從任何加入工作地點網路裝置提供 SSO 和無縫式第二個因素 Authentication Across Company Applications](../../ad-fs/operations/Join-to-Workplace-from-Any-Device-for-SSO-and-Seamless-Second-Factor-Authentication-Across-Company-Applications.md))，以及驗證狀態 （是否已執行多重要素驗證 (MFA)）。

Windows Server 2012 r2 中的 AD FS 中的條件式存取控制提供下列優點：
...
## <a name="BKMK_2"></a>使用條件式存取控制管理風險
使用可用的設定，實作條件式存取控制來管理風險的方法有很多種。

### <a name="common-scenarios"></a>常見案例
比方說，想像一個簡單案例實作特定的應用程式 （信賴憑證者信任） 的使用者的群組成員資格資料為基礎的條件式存取控制。 換句話說，您可以設定發行授權規則在同盟伺服器上讓屬於特定群組，在 AD 中的使用者網域由 AD FS 保護的特定應用程式的存取。  詳細的逐步指示 （使用 UI 和 Windows PowerShell） 來實作此案例所述[逐步解說指南：使用條件式存取控制管理風險](../../ad-fs/operations/Walkthrough-Guide--Manage-Risk-with-Conditional-Access-Control.md)。 若要完成本逐步解說中的步驟，您必須設定實驗室環境，並依照[適用於 Windows Server 2012 R2 中的 AD FS 設定實驗室環境](../../ad-fs/deployment/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md)。

### <a name="advanced-scenarios"></a>進階的案例
在 Windows Server 2012 R2 中的 AD FS 中實作條件式存取控制的其他範例包括：

-   允許此使用者的身分識別通過 MFA 驗證後，才由 AD FS 保護的應用程式的存取
...
-   允許存取要求來自工作地方聯結的裝置已註冊使用者時，才由 AD FS 保護的應用程式的存取
...
-   允許存取要求來自工作地方聯結的裝置註冊到其身分識別已通過 MFA 驗證的使用者時，才由 AD FS 保護的應用程式的存取
...
-   允許存取要求來自其身分識別已通過 MFA 驗證的使用者時，才由 AD FS 保護的應用程式的外部網路存取。
