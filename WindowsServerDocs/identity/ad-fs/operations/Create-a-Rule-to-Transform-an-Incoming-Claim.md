...
藉由使用 Active Directory 同盟服務 \(AD FS\)中的 [**轉換傳入**宣告] 規則範本，您可以選取傳入宣告、變更其宣告類型，以及變更其宣告值。 例如，您可以使用此規則範本來建立規則，以使用傳入群組宣告的相同宣告值來傳送角色宣告。 您也可以使用此規則，在具有 [系統管理員] 值的傳入群組宣告時，傳送宣告值為 [購買者] 的群組宣告，或者您只能傳送使用者主體名稱 \(UPN\) 宣告結尾是 @fabrikam。  
...
> [!NOTE]  
> 如果您要設定使用 AD FS\-發出的宣告的動態存取控制案例，請先在宣告提供者信任上建立轉換規則，然後在 [**傳入宣告類型**] 中，輸入連入宣告的名稱，或者如果先前已建立宣告描述，請從清單中選取它。 其次，在 [**傳出宣告類型**] 中，選取您想要的宣告 URL，然後在信賴憑證者信任上建立轉換規則，以發出裝置宣告。  
>   
