...
ms.locfileid: "71407571"
---
# <a name="create-a-rule-to-transform-an-incoming-claim"></a>建立規則轉換傳入宣告


藉由使用 Active Directory 同盟服務 \(AD FS @ no__t-2 中的「**轉換傳入**宣告」規則範本，您可以選取傳入宣告、變更其宣告類型，以及變更其宣告值。 例如，您可以使用此規則範本來建立規則，以使用傳入群組宣告的相同宣告值來傳送角色宣告。 您也可以使用此規則，在具有系統管理員值的連入群組宣告時，傳送宣告值為 [購買者] 的群組宣告，或者您只能傳送以 @fabrikam 結尾的 \(UPN @ no__t-1 宣告的使用者主體名稱。  
...
> [!NOTE]  
> 如果您要設定使用 AD FS @ no__t-0issued 宣告的動態存取控制案例，請先在宣告提供者信任上建立轉換規則，然後在 [**傳入宣告類型**] 中，輸入傳入宣告的名稱，或者如果宣告描述為先前已建立，請從清單中選取它。 其次，在 [**傳出宣告類型**] 中，選取您想要的宣告 URL，然後在信賴憑證者信任上建立轉換規則，以發出裝置宣告。  
>   
> 如需動態存取控制案例的詳細資訊，請參閱[動態存取控制內容藍圖](../../solution-guides/dynamic-access-control--scenario-overview.md)或搭配[使用 AD DS 宣告與 AD FS](https://technet.microsoft.com/library/hh831504.aspx)。 
