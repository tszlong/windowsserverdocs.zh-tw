...
ms.locfileid: "79320052"
---
# <a name="the-role-of-claim-rules"></a>宣告規則的角色
Active Directory 同盟服務 \(AD FS\) 中同盟服務的整體功能是發行包含一組宣告的權杖。 關於哪些宣告 AD FS 接受，然後發出問題的決定是由宣告規則所控管。

## <a name="what-are-claim-rules"></a>什麼是宣告規則？
宣告規則表示商務邏輯的實例，將會接受一或多個傳入宣告、將條件套用至 \(如果 x then y\) 並根據條件參數產生一或多個傳出宣告。 如需傳入和傳出宣告的詳細資訊，請參閱[宣告的角色](The-Role-of-Claims.md)。

當您需要實作商務邏輯來控制宣告行經宣告管線的流程時，您會使用宣告規則。 雖然宣告管線是結束\-的邏輯概念，以\-流程宣告的結束程式，但宣告規則是實際的系統管理專案，您可以用來透過宣告發布程式來自訂宣告流程。

如需有關宣告管線的詳細資訊，請參閱[宣告引擎的角色](The-Role-of-the-Claims-Engine.md)。

宣告規則可提供下列優點：

-   提供一種機制，讓系統管理員能夠針對來自宣告提供者的信任宣告，套用執行\-時間商務邏輯

-   提供系統管理員一個機制，用於定義哪些宣告會發行給信賴憑證者

-   針對想要允許或拒絕存取特定使用者的系統管理員，提供豐富且詳細的宣告\-授權功能
...
在的 [AD FS 管理] 嵌入式管理單元\-中，只能使用宣告規則範本來建立規則。 當您使用中的 貼齊\-來選取宣告規則範本之後，請輸入規則邏輯的必要資料，並將它儲存到設定資料庫中，這將會從該點向前 \(，\) 在 UI 中稱為宣告規則。

### <a name="how-claim-rule-templates-work"></a>宣告規則範本的運作方式
乍看之下，宣告規則範本似乎只是中的 [嵌入式管理\-單元] 所提供的輸入表單，用來收集傳入宣告的資料和處理特定邏輯。 不過，在更詳盡的層級中，宣告規則範本會儲存必要的宣告規則語言架構，組成必要的基礎邏輯，讓您快速建立規則而不需要知道熟知語言。

使用者介面中提供的每個範本 \(UI\) 都是根據最常需要的系統管理工作，來代表預先填入的宣告規則語言語法。 有一個規則範本，不過這是例外狀況。 此範本稱為自訂規則範本。 使用此範本不需預先填入任何語法。 而是必須使用宣告規則語言語法，在宣告規則範本表單的本文中直接撰寫宣告規則語言語法。
...
|委派授權規則集|您在信賴憑證者信任上使用的一組宣告規則，用以指定將允許作為對信賴憑證者的其他使用者的委派。<br /><br />這些規則會決定是否允許要求者模擬使用者，同時識別在傳送到信賴憑證者的權杖中的要求者。<br /><br />除非您指定委派授權規則，否則不會有任何使用者預設作為委派。|信賴憑證者信任|
