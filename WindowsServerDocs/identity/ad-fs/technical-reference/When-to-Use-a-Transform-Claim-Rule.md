...
當您需要將傳入的宣告類型對應到傳出的宣告類型，然後套用動作來根據產生的值來決定應該發生的輸出時，您可以在 Active Directory 同盟服務 \(AD FS @ no__t-1 中使用此規則。傳入宣告。 當您使用此規則時，您會根據您在規則中設定的選項之一傳遞或轉換符合下列規則邏輯的宣告，如下表中所述。  
...
|以新的 e @ no__t-1mail 尾碼取代傳入的 e @ no__t-0mail 尾碼宣告|如果傳入宣告類型等於*指定的宣告類型*，且值等於*任何尾碼值*，則會以新的傳出宣告值*指定的尾碼值*和傳出宣告類型*指定的宣告類型*轉換該宣告|  
...
您也可以使用此規則來套用條件，將以指定的 e @ no__t-0mail 尾碼值取代所有傳入宣告加上新的值。 例如，您可以在此規則設定一個條件，將所有尾碼為 sales.corp.fabrikam.com 的宣告值都變更為 fabrikam.com。  
...
您可以使用宣告規則語言，或使用 AD FS Management snap @ no__t-1in 中的「**轉換傳入**宣告」規則範本來建立此規則。 這個規則範本提供下列設定選項：  
...
-   以新的 e @ no__t-1mail 尾碼取代內送的 e @ no__t-0mail 尾碼宣告  
...
如果必須從一個以上的傳入宣告的內容建構傳出宣告，則必須改用自訂規則。 如果傳出宣告的宣告值必須根據傳入宣告值 — 但還有其他內容 — 您也必須在該內容中使用自訂規則。 如需詳細資訊，請參閱[使用宣告授權規則的時機](When-to-Use-a-Custom-Claim-Rule.md)。  
...
規則運算式也可以用於宣告轉換。 例如，下列規則會將網域 @ no__t-0USER 格式的 windows 使用者名稱宣告中的網域設定為 FABRIKAM：  
