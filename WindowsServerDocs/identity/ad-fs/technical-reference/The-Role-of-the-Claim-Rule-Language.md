...
|條件描述|條件語法範例|  
|-------------------------|----------------------------|  
|此規則有一個條件，使用正則運算式來檢查以 "@fabrikam.com" 結尾的電子郵件宣告。 如果符合的宣告位於輸入宣告中，則規則會將符合的宣告複製到輸出宣告集。|```c: [type  == "http://test/email", value  =~ "^. +@fabrikam.com$" ] => issue (claim  = c );```|  
...
例如在下圖中，會將傳入宣告加入至宣告發佈引擎所設定的輸入宣告。 當第一個自訂宣告規則執行且滿足網域使用者的準則時，宣告發布引擎會使用 add 語句來處理規則中的邏輯，並將**Editor**的值加入至輸入宣告集。 由於 Editor 的值會出現在輸入宣告集合中，因此規則2可在其邏輯中成功處理 issue 語句，並產生**Hello**的新值，這會新增到輸出宣告集和輸入宣告集，以供中的下一個規則使用。規則集。 規則3現在可以使用輸入宣告集合中出現的所有值做為輸入，以處理其邏輯。  
...
-   **Normal**—Normal 發佈陳述式可使用規則中的常值或符合條件的宣告中值，以發出宣告。 normal 發佈陳述式可以包含一或兩個下列格式：  
    ...
    -   *新宣告*：此格式會在給定各種宣告屬性的值時，建立新的宣告。 必須指定 Claim.Type，所有其他宣告屬性是選擇性的。 會忽略此格式的引數順序。  

-   **屬性存放區**—這種格式會建立宣告，包含從屬性存放區擷取的值。 您可以使用單一發行語句來建立多個宣告類型，這對於在屬性抓取期間進行網路或磁片輸入/輸出（i/o）作業的屬性存放區而言非常重要。 因此，最好限制原則引擎與屬性存放區之間的往返次數。 也可合法對指定的宣告類型建立多個宣告。 當屬性存放區傳回指定宣告類型的多個值時，發佈陳述式會自動為每個傳回的宣告值建立一個宣告。 屬性存放區實作使用 param 引數，使用這些引數所提供的值來替代查詢引數中的預留位置。 預留位置會使用與 .net 字串. Format （）函式相同的語法（ {1} {2}例如，、等等）。 此發佈格式的引數順序很重要，而且必須是下列文法中規定的順序。  
...
-   函式呼叫：函式是由識別碼所識別，而參數會以逗號分隔的運算式清單（以方括弧（"（）"）括住）來傳遞。  

-   宣告的屬性存取，格式為變數名稱點屬性名稱：指定變數估價之已識別宣告的屬性值結果。 變數必須先繫結到宣告選擇器，才能以這種方式使用。 如果變數繫結至宣告選取器的條件約束內部該相同的宣告選擇器，則不可使用該變數。  
