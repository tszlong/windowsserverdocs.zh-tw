...
例如在下圖中，會將傳入宣告加入至宣告發佈引擎所設定的輸入宣告。 當第一個自訂宣告規則執行，並符合準則的網域使用者，宣告發行引擎處理規則使用 add 陳述式和 windows 7 中的邏輯**編輯器**新增至輸入的宣告集。 由於 Editor 的值是出現在輸入的宣告集，規則 2 的成功處理 issue 陳述式，在其邏輯中的並產生新的值**Hello**，已新增到輸出宣告集和輸入宣告集，以供在規則中的下一個規則組合。 規則 3 現在可以使用所有輸入宣告集做為輸入來處理其邏輯存在的值。  
...
|屬性存放區|下列規則使用傳入宣告的值，來查詢 Active Directory 屬性存放區：|```c: [Type  == "http://test/name" ] => issue (store  = "Enterprise AD Attribute Store", types  =  ("http://test/email" ), query  = ";mail;{0}", param  = c.Value )```|  
