...
ms.locfileid: "87937797"
---
# <a name="the-role-of-the-claim-rule-language"></a>宣告規則語言的角色
Active Directory 同盟服務 (AD FS ) 宣告規則語言作為傳入和傳出宣告之行為的系統管理建立區塊，而宣告引擎則是做為定義自訂規則之宣告規則語言中邏輯的處理引擎。 如需有關宣告引擎如何處理所有規則的詳細資訊，請參閱[宣告引擎的角色](The-Role-of-the-Claims-Engine.md)。
...
您可以使用規則中的條件來檢查輸入宣告，並判斷是否應該執行規則的發佈陳述式。 條件表示必須評估為 true 來執行規則主體部分的邏輯運算式。 如果遺漏此元件，則會假設為邏輯 true;也就是說，一律會執行規則的主體。 條件部分包含一份條件清單，其中結合了結合邏輯運算子 ( "&&" ) 。 清單中的所有條件必須都評估為 true，才會將整個條件組件評估為 true。 條件可以是宣告選取運算子或彙總函式呼叫。 這兩個條件是互斥的，這表示宣告選取器和彙總函式不能合併在單一規則條件組件中。
...
|                                                                                                                   條件描述                                                                                                                    |                           條件語法範例                            |
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------|
|               此規則有一個條件，可以使用指定的宣告類型來檢查輸入宣告， ( " <http://test/name> " ) 。 如果符合的宣告位於輸入宣告，則規則會將符合的宣告複製到輸出宣告集。               |         ``` c: [type == "http://test/name"] => issue(claim = c );```          |
| 此規則有一個條件，可檢查具有指定宣告類型的輸入宣告 ( " <http://test/name> " ) 和宣告值 ( "Terry" ) 。 如果符合的宣告位於輸入宣告，則規則會將符合的宣告複製到輸出宣告集。 | ``` c: [type == "http://test/name", value == "Terry"] => issue(claim = c);``` |
...
|條件描述|條件語法範例|
|-------------------------|----------------------------|
|此規則有一個條件，使用正則運算式來檢查以 "" 結尾的電子郵件宣告 @fabrikam.com 。 如果符合的宣告位於輸入宣告中，則規則會將符合的宣告複製到輸出宣告集。|```c: [type  == "http://test/email", value  =~ "^. +@fabrikam.com$" ] => issue (claim  = c );```|

### <a name="issuance-statements"></a>發佈陳述式
自訂規則會根據發行語句來處理 (*問題*，或將您程式設計的 ) *加入*至宣告規則。 視所要的結果，可將 issue 陳述式或 add 陳述式寫入規則中，以填入輸入宣告集或輸出宣告集。 使用 add 陳述式的自訂規則只會將宣告值明確地填入輸入宣告集，而使用 issue 陳述式的自訂宣告規則會在輸入宣告集和輸出宣告集中填入宣告值。 當宣告值僅適用於宣告規則集內未來的規則使用時，這項功能可能很有用。

例如在下圖中，會將傳入宣告加入至宣告發佈引擎所設定的輸入宣告。 當第一個自訂宣告規則執行且滿足網域使用者的準則時，宣告發布引擎會使用 add 語句來處理規則中的邏輯，並將**Editor**的值加入至輸入宣告集。 由於 Editor 的值會出現在輸入宣告集合中，因此規則 2 可在其邏輯中成功處理 issue 陳述式，並產生 **Hello** 的新值，系統會其新增到輸出宣告集和輸入宣告集，供規則集內的下一個規則使用。 規則 3 現在可將輸入宣告集內出現的所有值做為輸入來處理其邏輯。
...
-   **屬性存放區**—這種格式會建立宣告，包含從屬性存放區擷取的值。 您可以使用單一發行語句來建立多個宣告類型，這對於在屬性抓取期間 (i/o ) 作業的屬性存放區而言非常重要。 因此，最好限制原則引擎與屬性存放區之間的往返次數。 也可合法對指定的宣告類型建立多個宣告。 當屬性存放區傳回指定宣告類型的多個值時，發佈陳述式會自動為每個傳回的宣告值建立一個宣告。 屬性存放區實作使用 param 引數，使用這些引數所提供的值來替代查詢引數中的預留位置。 預留位置會使用與 .NET 字串相同的語法。格式 ( ) 函式 (例如，、 {1} {2} 等等 ) 。 此發佈格式的引數順序很重要，而且必須是下列文法中規定的順序。
...
-   字串常值：以引號分隔的字串值，這兩個兩邊的 ) 字元 (。

-   運算式的字串串連：會得出由左值和右值串連所產生的字串。

-   函式呼叫：函式是以識別碼識別，而參數會以逗號分隔的運算式清單來傳遞，並以括弧括住 ( " ( ) " ) 。
...
-   \[ (如果在宣告 \_ \] 的 Properties 集合中找不到屬性 _name，此屬性就會傳回空字串。 )
