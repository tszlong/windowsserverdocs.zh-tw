...
例如，如下圖所示，當使用條件來設定規則時，只會篩選已加入 UPN 宣告類型，且結尾為的 @fabrikam.com 所有其他連入宣告，除非它們符合此準則。 這包括宣告類型為電子郵件地址的傳入宣告， \- 即使其宣告值結尾為 @fabrikam.com 。 在此情況下，只有包含值的宣告才 Nick@fabrikam.com 會傳送至信賴憑證者。  
  
![使用傳遞的時機](media/adfs2_filter.gif)  
  
## <a name="configuring-this-rule-on-a-claims-provider-trust"></a>在宣告提供者信任上設定此規則  
當您使用宣告提供者信任時，可將此規則設定為只會傳遞符合特定條件約束的宣告提供者所傳入的宣告。 例如，您可能只想要接受 \- 來自宣告提供者的電子郵件宣告，因此，您可以使用此規則範本來接受 \- 以宣告提供者的網域名稱系統 DNS 名稱結尾的電子郵件宣告 \( 類型 \) 。  
...
您可以使用宣告規則語言，或使用 [AD FS 管理] 嵌入式管理單元中的 [傳遞或篩選傳入宣告] 規則範本來建立此規則 \- 。 此規則範本提供以下設定選項：  
...
如需有關如何建立此範本的詳細指示，請參閱 AD FS 部署指南中的[建立規則以通過或篩選傳入](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/dd807060(v=ws.11))宣告。  
...
篩選可以在邏輯上與 \- ed 一起進行。 例如，下列規則將接受所有 \- 具有值的電子郵件宣告johndoe@fabrikam.com:  
...
例如，下列規則將會接受 \- 本機同盟伺服器未發出尾碼為 boeing.com 的所有電子郵件宣告：  
...
| 宣告屬性 |                                                                                                                                                                                                                                                                                                                                                  描述                                                                                                                                                                                                                                                                                                                                                  |
|----------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|      類型      |                                                                                                                                                                                        宣告類型 \( 通常表示為 Uri，會 \) 反映同盟中的夥伴與宣告中所傳達的資訊類型之間的隱含協定。 例如，HTTP： \/ \/schemas.xmlsoap.org \/ ws \/ 2005 \/ 05 \/ identity 宣告 emailaddress 類型的 \/ 宣告 \/ 會包含使用者的電子 \- 郵寄地址。                                                                                                                                                                                         |
|     值      |                                                                                                                                                                                                                                                                   宣告的值。 例如，HTTP： \/ \/schemas.xmlsoap.org \/ ws \/ 2005 \/ 05 \/ identity 宣告 emailaddress 類型的宣告，其 \/ \/ 值可能為johndoe@fabrikam.com                                                                                                                                                                                                                                                                    |
|   ValueType    |                                                                                                                                                                                                  ValueType 表示要如何解讀宣告值中包含的資訊。 ValueType 通常會設定為 HTTP： \/ \/ Www.w3.org \/ 2001 \/ XMLSchema \# string，但宣告值可能包含 Base64Binary 編碼 \( 的資料，例如影像 \) 或日期、布林值等等。                                                                                                                                                                                                  |
|     Issuer     | 簽發者代表上次發出使用者相關宣告的一方。 如果宣告是在宣告提供者同盟伺服器上取得，則所有宣告的簽發者都會設定為「本機授權」。 如果事由同盟提供者同盟伺服器接收宣告，則宣告的簽發者會設定為簽署權杖之宣告提供者的宣告提供者識別碼。 因此，在對接收自宣告提供者的宣告處理規則時，所有宣告的簽發者都會設為相同的值。 當撰寫信賴憑證者的規則時，可使用簽發者屬性來區分來自不同宣告提供者的宣告。 |
| OriginalIssuer |                                                                                                   此宣告屬性可用來傳達原本發出宣告的的同盟伺服器。 由於宣告的 issuer 屬性會設定為最後一部已簽署權杖的同盟伺服器，因此，原始簽發者在宣告已流經多部同盟伺服器的情況下很有用 \( ，例如，從同盟提供者同盟伺服器接收權杖的信賴憑證者可能會對哪些特定的宣告提供者同盟伺服器驗證使用者的感興趣\)                                                                                                   |
|   [內容]   |                                                                                                                             除了上述的五個屬性外，每個宣告也都有可儲存具名屬性的屬性包。 這些屬性不會序列化到權杖中，且只能在單一同盟伺服器的範圍內，用來傳遞宣告發行管線元件之間的資訊。 例如，在宣告提供者規則處理期間設定屬性，然後在信賴憑證者規則中加以參照。                                                                                                                              |
