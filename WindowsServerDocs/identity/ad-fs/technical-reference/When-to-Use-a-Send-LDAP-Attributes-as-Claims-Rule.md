...
ms.locfileid: "66188247"
---
# <a name="when-to-use-a-send-ldap-attributes-as-claims-rule"></a>使用「以宣告方式傳送 LDAP 屬性」規則的時機
您可以使用這項規則中 Active Directory Federation Services \(AD FS\)當您想要發出包含實際的輕量型目錄存取通訊協定的連出宣告\(LDAP\)屬性中存在的值屬性存放區，並將與每個 LDAP 屬性產生關聯的宣告類型。 如需有關屬性存放區的詳細資訊，請參閱[角色的屬性儲存](The-Role-of-Attribute-Stores.md)。  
...
當您使用傳送 LDAP 屬性作為宣告規則範本時，您可以選取屬性，從 LDAP 屬性存放區，例如 Active Directory 或 Active Directory 網域服務\(AD DS\)來將值傳送到信賴憑證者的合作對象的宣告. 這實際上會從您定義的屬性存放區中，將特定的 LDAP 屬性對應至一組可用於授權的連出宣告。  
...
如需如何建立此規則的詳細資訊，請參閱[建立規則，以宣告方式傳送 LDAP 屬性](https://technet.microsoft.com/library/dd807115.aspx)。  
  
## <a name="using-the-claim-rule-language"></a>使用宣告規則語言  
如果 Active Directory、 AD DS 或 Active Directory 輕量型目錄服務查詢\(AD LDS\)必須比較 LDAP 屬性以外**samAccountname**，您必須改為使用自訂規則。 如果沒有輸入集中沒有 Windows 帳戶名稱宣告，您也必須使用自訂規則指定要用於查詢 AD DS 或 AD LDS 的宣告。  
...
下列範例示範如何查詢使用者所**sAMAccountName**屬性，並發出電子\-電子郵件地址宣告，請使用使用者的 mail 屬性的值：  
...
下列範例示範如何依 **mail** 屬性查詢使用者，然後利用使用者的 **title** 和 **displayname** 屬性值，發出職稱和顯示名稱宣告：  
...
Active Directory 查詢必須包含使用者名稱\(網域名稱\)為最後一個參數，因此 Active Directory 屬性存放區可以查詢正確的網域。 否則支援相同的語法。  
...
-   LDAP 篩選器 — 指定這個查詢部分是為了擷取您想要查詢屬性的物件。 如需有效 LDAP 查詢的一般資訊，請參閱 RFC 2254。 當您要查詢 Active Directory 屬性存放區，且您未指定的 LDAP 篩選條件，samAccountName\= {0}查詢會假設與 Active Directory 屬性存放區需要參數，以供饋送值{0}. 否則，查詢會產生錯誤。 對於 Active Directory 以外的 LDAP 屬性存放區，您不能省略查詢的 LDAP 篩選器部分，否則查詢會產生錯誤。  
  
-   屬性規格 — 在此查詢的第二個部分中，您指定的屬性\(哪些是逗號\-分隔，如果您使用多個屬性值\)想要從篩選的物件。 您指定的屬性數目必須符合您在查詢中定義的宣告類型數目。  
  
-   Active Directory 網域 — 只有當屬性存放區是 Active Directory 時，才需要指定查詢的最後一部分 \(當您查詢其他屬性存放區時不需要。\)查詢的這個部分用來指定表單網域中的使用者帳戶\\名稱。 Active Directory 屬性存放區使用「網域」部分來決定要連接的適當網域控制站，然後執行查詢和要求屬性。  
  
### <a name="example-how-to-use-two-custom-rules-to-extract-the-manager-e-mail-from-an-attribute-in-activedirectory"></a>範例：如何使用兩個自訂規則來擷取管理員電子\-郵件從 Active Directory 中的屬性  
下列兩個自訂規則，如下所示的順序一起使用時查詢 Active Directory**管理員**使用者帳戶屬性\(規則 1\) ，然後使用該屬性查詢使用者帳戶管理員**mail**屬性\(Rule 2\)。 最後，以 "ManagerEmail" 宣告方式發出 **mail** 屬性。 在 [摘要]，規則 1 會查詢 Active Directory 並將查詢的結果傳遞至規則 2 再擷取管理員電子\-郵件值。  
  
比方說，當這些規則執行完成時會發出宣告，包含管理員電子\-corp.fabrikam.com 網域中的使用者的地址。  
...
> [!NOTE]  
> 這些規則適用於使用者的經理是與使用者相同的網域中時，才\(在此範例中的 corp.fabrikam.com\)。  
