...
基於此測試環境的目的，您可以呼叫您的根 Active Directory 網域**contoso.com**並指定<strong>pass@word1</strong>做為系統管理員密碼。
...
- 使用者：**Robert Hatley**，認證如下：使用者名稱：**RobertH**和密碼： <strong>P@ssword</strong>

- 群組：**財務**

如需有關如何建立使用者和群組帳戶在 Active Directory (AD) 中的資訊，請參閱 < [ https://technet.microsoft.com/library/cc783323%28v.aspx ](https://technet.microsoft.com/library/cc783323%28v=ws.10%29.aspx)。
...
1. > [!NOTE]
   ...
   使用的登入 WebServ1 <strong>administrator@contoso.com</strong>和 密碼<strong>pass@word1</strong>。
...
10. 在 [確認安裝選項]  頁面上，按一下 [指定替代來源路徑]  。 輸入 [Sxs] 目錄，它位於 Windows Server 2012 R2 安裝媒體的路徑。 例如 D:\Sources\Sxs。 按一下 [確定]  ，然後按一下 [安裝]  。
...
3.  編輯 **Default.aspx.cs** 檔案並停用宣告篩選。 執行此步驟的目的是要確定範例應用程式會顯示同盟伺服器發出的所有宣告。 執行下列動作：
    ...
    3.  將整個 `IF` 陳述式及其大括弧變更為註解。 表示輸入的註解"/ /"（不含引號） 在一行的開頭。
...
5.  若要設定 **claimapp** 搭配您的同盟伺服器使用，請執行下列動作：
    ...
    2.  若要設定應用程式設定位置**C:\inetput\claimapp\web.config**與應用程式 URI 設為您網站的 URL  **https://webserv1.contoso.com /claimapp/** 。 按一下 [下一步]  。
...
2.  在 [選取資料來源]  頁面上，選取 [匯入發佈到線上或區域網路的信賴憑證者相關資料]  ，輸入 **claimapp**的中繼資料 URL，然後按 [下一步]  。 執行 FedUtil.exe 會建立中繼資料 .xml 檔案。 它是位於 **https://webserv1.contoso.com/claimapp/federationmetadata/2007-06/federationmetadata.xml** 。
