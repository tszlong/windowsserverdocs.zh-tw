...
本機電腦上 **Users**、**Backup Operators**、**Power User**、**Administrators** 中的成員資格或同等成員資格，是完成這個程序的最低需求。  請參閱[本機與網域的預設群組](https://go.microsoft.com/fwlink/?LinkId=83477)中關於使用適當帳戶和群組成員資格的詳細資料。   
  
### <a name="procedure-1-to-verify-that-a-federation-server-is-operational"></a>程式1：驗證同盟伺服器是否可運作  
  
1.  若要確認已在同盟伺服器上正確設定 Internet Information Services \(IIS\)，請登入與同盟伺服器位於相同樹系中的用戶端電腦。  
  
2.  開啟瀏覽器視窗，在網址列中輸入同盟伺服器的 DNS 主機名稱，然後在新的同盟伺服器上附加 \/adfs\/fs\/federationserverservice，例如：  
