...
在 Active Directory 內，會將一組預設的高許可權帳戶和群組視為受保護的帳戶和群組。 在 Active Directory 中的大部分物件中，委派的系統管理員 (已委派許可權來管理 Active Directory 物件的使用者) 可以變更物件的許可權，包括變更許可權以允許自己變更群組的成員資格，例如。
...
SDProp 是一種處理常式，每隔60分鐘執行一次 (，) 在保存網域 PDC 模擬器 (PDCE) 的網域控制站上。 SDProp 會比較網域的 AdminSDHolder 物件使用權限與網域中受保護帳戶和群組的許可權。 如果任何受保護帳戶和群組的許可權不符合 AdminSDHolder 物件上的許可權，則會重設受保護帳戶和群組的許可權，以符合網域的 AdminSDHolder 物件。
...
值的範圍是從60到7200的秒數， (一分鐘到兩小時的) 。 若要反轉變更，請刪除 AdminSDProtectFrequency key，這會導致 SDProp 還原回60分鐘的間隔。 一般來說，您不應該在生產網域中減少此間隔，因為它可能會增加網域控制站上的 LSASS 處理負荷。 這項增加的影響取決於網域中受保護的物件數目。
...
3. 在 [**連接**] 對話方塊中，輸入保留 PDC 模擬器之網域的網域控制站名稱 (PDCE) 角色，然後按一下 **[確定]**。

   ![受保護的帳戶和群組](media/Appendix-C--Protected-Accounts-and-Groups-in-Active-Directory/SAD_10.png)

4. 請確認您已成功連線，如下列螢幕擷取畫面中的**Dn： (RootDSE) **所示 **，按一下 [** 連線] **，然後按一下**[系結]。
