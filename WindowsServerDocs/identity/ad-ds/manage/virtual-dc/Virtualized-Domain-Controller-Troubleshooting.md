...
|        **來源**        |                                                                                                                                                                                                                                    Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                                                     |
|       **低於**       |                                                                                                                                                                                                                                                         錯誤                                                                                                                                                                                                                                                          |
|       **消息**        | *<COMPUTERNAME>* 無法設定下列登錄值以啟用 DNS 更新。<br /><br />登錄機碼：%1<br /><br />登錄值：%2<br /><br />登錄值資料：%3<br /><br />錯誤碼：%4<br /><br />錯誤訊息：%5<br /><br />複製程序期間，本機電腦與複製來源電腦可能短時間內會有相同的電腦名稱。 此期間內會停用 DNS A 與 AAAA 記錄登錄，因此用戶端無法傳送要求至正在進行複製的本機電腦。 |
| **記事和解決方式** |                                                                                                                                                                                                 檢查 [應用程式] 和 [系統] 事件記錄檔。 調查可能封鎖登錄更新的協力廠商應用程式。                                                                                                                                                                                                  |
...
|**來源**|Microsoft-Windows-ActiveDirectory_DomainService|  
|**低於**|錯誤|  
|**消息**|無法設定 DSRM 開機。<br /><br />錯誤碼：%1<br /><br />錯誤訊息：%2<br /><br />當虛擬網域控制站複製失敗或虛擬網域控制站複製設定檔出現在不支援的 Hypervisor 上時，本機電腦將重新開機到 DSRM 以進行疑難排解。 設定 DSRM 開機失敗。|  
|**記事和解決方式**|檢查 [應用程式] 和 [系統] 事件記錄檔。 調查可能封鎖登錄更新的協力廠商應用程式。|  
...
|**來源**|Microsoft-Windows-ActiveDirectory_DomainService|  
|**低於**|錯誤|  
|**消息**|無法啟用關機權限。<br /><br />錯誤碼：%1<br /><br />錯誤訊息：%2<br /><br />當虛擬網域控制站複製失敗或虛擬網域控制站複製設定檔出現在不支援的 Hypervisor 上時，本機電腦將重新開機到 DSRM 以進行疑難排解。 啟用關機權限失敗。|  
...
|**來源**|Microsoft-Windows-ActiveDirectory_DomainService|  
|**低於**|錯誤|  
|**消息**|無法起始系統關機。<br /><br />錯誤碼：%1<br /><br />錯誤訊息：%2<br /><br />當虛擬網域控制站複製失敗或虛擬網域控制站複製設定檔出現在不支援的 Hypervisor 上時，本機電腦將重新開機到 DSRM 以進行疑難排解。 起始系統關機失敗。|  
|**記事和解決方式**|檢查 [應用程式] 和 [系統] 事件記錄檔。 調查可能封鎖權限使用方式的協力廠商應用程式。|  
...
|        **來源**        |                                                                  Microsoft-Windows-ActiveDirectory_DomainService                                                                  |
|       **低於**       |                                                                                       錯誤                                                                                       |
|       **消息**        | *<COMPUTERNAME>* 無法建立或修改下列複製的 DC 物件。<br /><br />其他資料：<br /><br />物件：<br /><br />%1<br /><br />錯誤值：%2<br /><br />%3 |
...
|        **來源**        |                                                                                                                                                                                               Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                |
|       **低於**       |                                                                                                                                                                                                                    錯誤                                                                                                                                                                                                                     |
|       **消息**        |                                                                                                                     *<COMPUTERNAME>* 無法建立下列複製的 DC 物件，因為該物件已存在。<br /><br />其他資料：<br /><br />來源 DC：<br /><br />%1<br /><br />物件：<br /><br />%2                                                                                                                     |
...
|**來源**|Microsoft-Windows-ActiveDirectory_DomainService|  
|**低於**|錯誤|  
|**消息**|上一個虛擬網域控制站複製作業失敗。 這是上次複製作業後第一次重新開機，因此這應該是複製重試。 但是，虛擬網域控制站複製設定檔不存在，也未偵測到虛擬機器世代識別碼變更。 開機進入 DSRM。<br /><br />上一個虛擬網域控制站複製作業失敗：%1<br /><br />虛擬網域控制站複製設定檔存在：%2<br /><br />偵測到虛擬機器產生識別碼變更：%3|  
...
|注意事項與解決方法|預期會在使用獨立的 MSA (非群組 MSA) 時發生。 請勿依照事件建議來移除帳戶 - 它的內容不正確。 使用卸載-Uninstall-adserviceaccount- [https://technet.microsoft.com/library/hh852310](https://technet.microsoft.com/library/hh852310)。<br /><br />獨立的 MSA (在 Windows Server 2008 R2 中首次發行) 已在 Windows Server 2012 中使用群組 MSA (gMSA) 來取代。 GMSA 支援複製。|  
...
|-|-|-|  
|**事件識別碼**|**來源**|**消息**|  
|**16654**|Directory-Services-SAM|帳戶識別碼 (RID) 集區成為無效。 在下列預期情況中可能會發生此問題：<br /><br />1. 從備份還原網域控制站。<br /><br />2. 在虛擬機器上執行的網域控制站會從快照集還原。<br /><br />3. 系統管理員已手動將集區失效|  
...
|**1074**|User32|進程 C:\Windows\system32\lsass.exe （DC2）已代表使用者 NT AUTHORITY\SYSTEM 起始電腦 DC2 的重新開機，原因如下：作業系統：重新設定（已規劃）<br /><br />原因代碼：0x80020004<br /><br />關機類型：重新啟動<br /><br />註解：「|  
...
|-|-|-|  
|**事件識別碼**|**來源**|**消息**|  
|**1202**|ADWS 執行個體事件|這個電腦目前裝載指定的目錄執行個體，但是 Active Directory Web 服務無法對它提供服務。 Active Directory Web 服務會定期重試這個作業。<br /><br />目錄實例： NTDS<br /><br />目錄實例 LDAP 埠：389<br /><br />目錄實例 SSL 埠：636|  
...
|**1100**|ADWS 執行個體事件|已載入在 Active Directory Web 服務之設定檔的 <appsettings> 區段中指定的值，而且未發生錯誤。|  
|**1200**|ADWS 執行個體事件|Active Directory Web 服務現在正在服務指定的目錄執行個體。<br /><br />目錄實例： NTDS<br /><br />目錄實例 LDAP 埠：389<br /><br />目錄實例 SSL 埠：636|  
...
|  **13565**   |   NtFrs    |                                                                         檔案複寫服務正在使用其他網域控制站的資料來初始化系統磁碟區。 除非這個處理程序完成，否則電腦 DC2 無法變成網域控制站。 系統磁碟區接著會被視為 SYSVOL 加以共用。<br /><br />如果要檢查 SYSVOL 共用，請在命令提示字元下輸入：<br /><br />net share<br /><br />當檔案複寫服務完成初始化程序，SYSVOL 共用將會出現。<br /><br />系統磁碟區初始化會花費一些時間。 花費的時間視系統磁碟區資料量、其他網域控制站可用性，以及網域控制站之間的複寫間隔而定。                                                                         |
|  **13501**   |   NtFrs    |                                                                                                                                                                                                                                                                                                                                                                                                   檔案複寫服務正在啟動。                                                                                                                                                                                                                                                                                                                                                                                                   |
|  **13553**   |   NtFrs    |                                                                                                                                                                          檔案複寫服務成功地將這台電腦加入下列複本集：<br /><br />"DOMAIN SYSTEM VOLUME (SYSVOL SHARE)"<br /><br />這個事件的相關資訊顯示在下方：<br /><br />電腦 DNS 名稱是 *<Domain Controller FQDN>*<br /><br />複本集成員名稱為 *<Domain Controller>*<br /><br />複本集根路徑為 *<path>*<br /><br />複本臨時目錄路徑為 *<path>*<br /><br />複本工作目錄路徑為 *<path>*                                                                                                                                                                           |
|  **13520**   |   NtFrs    |                檔案複寫服務已將 <path>中的預先存在檔案移至 *<path>* \ NtFrs_PreExisting___See_EventLog。<br /><br />檔案複寫服務可能會隨時刪除 *<path>* \ NtFrs_PreExisting___See_EventLog 中的檔案。 您可以將檔案複製到 *<path>* \ NtFrs_PreExisting___See_EventLog，以將檔案從刪除中儲存。 如果檔案已經存在其他的複寫協力電腦上，將檔案複製到 c:\windows\sysvol\domain 可能導致名稱衝突。<br /><br />在某些情況下，檔案複寫服務可能會將檔案從 *<path>* \ NtFrs_PreExisting___See_EventLog 複製到 *<path>* ，而不是從其他複寫夥伴複寫檔案。<br /><br />藉由刪除 *<path>* \ NtFrs_PreExisting___See_EventLog 中的檔案，可以隨時復原空間。」                 |
|  **13508**   |   NtFrs    | 檔案複寫服務無法啟用從 *\\\\<Domain Controller FQDN>* 到 *<Domain Controller>* *<path>* 的複寫，請使用<br /><br />DNS 名稱 *\\\\<Domain Controller FQDN>* 。 FRS 將繼續重試。<br /><br />出現此警告可能是由下列原因導致。<br /><br />[1] FRS 無法從這部電腦正確解析 DNS 名稱 *\\\\<Domain Controller FQDN>* 。<br /><br />[2] FRS 未在 *\\\\<Domain Controller FQDN>* 上執行。<br /><br />[3] 這個複本在 Active Directory 網域服務上的拓撲資訊尚未複寫到所有網域控制站上。<br /><br />這個事件記錄檔訊息會在每次連線時出現一次。在問題解決之後，您會看到其他的事件記錄檔訊息，指出連線已經建立。 |
...
|   **1314**   |    DFSR    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  DFS 複寫服務已順利設定偵錯記錄檔。<br /><br />其他資訊：<br /><br />Debug 記錄檔路徑： C:\Windows\debug                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
|   **6102**   |    DFSR    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            DFS 複寫服務已順利登錄 WMI 提供者                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
|   **1206**   |    DFSR    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 DFS 複寫服務已順利連絡網域控制站 DC2.corp.contoso.com 以存取設定資訊。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
|   **1210**   |    DFSR    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    DFS 複寫服務已順利設定連入複寫要求的 RPC 接聽程式。<br /><br />其他資訊：<br /><br />埠： 0 "                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|   **4614**   |    DFSR    | DFS 複寫服務已初始化位於本機路徑 C:\Windows\SYSVOL\domain 的 SYSVOL，而且正在等候執行初始複寫。 複寫的資料夾會維持在初始同步狀態，直到它和協力電腦完成複寫為止。 如果伺服器已經開始升級至網域控制站，網域控制站會在此問題解決之後才通告並開始運作。 若指定的協力電腦也處於初始同步狀態，或此伺服器或同步協力電腦發生共用違規，也會發生此情形。 若此事件是發生於將 SYSVOL 從檔案複寫服務 (FRS) 移轉至 DFS 複寫時，則只有在解決問題之後才會對外複寫變更。 這會導致此伺服器上的 SYSVOL 資料夾與其他網域控制站不同步。<br /><br />其他資訊：<br /><br />複寫的資料夾名稱： SYSVOL 共用<br /><br />複寫資料夾識別碼： *<GUID>*<br /><br />複寫組名：網域系統磁碟區<br /><br />複寫群組識別碼： *<GUID>*<br /><br />成員識別碼： *<GUID>*<br /><br />唯讀：0 |
|   **4604**   |    DFSR    |                                                                                                                                                                                                                                                                      DFS 複寫服務已順利初始化位於本機路徑 C:\Windows\SYSVOL\domain 的 SYSVOL 複寫資料夾。 此成員已完成與協力電腦 dc1.corp.contoso.com 的 SYSVOL 初始同步處理作業。  若要檢查 SYSVOL 共用是否存在，請開啟命令提示視窗，然後輸入 "net share"。<br /><br />其他資訊：<br /><br />複寫的資料夾名稱： SYSVOL 共用<br /><br />複寫資料夾識別碼： *<GUID>*<br /><br />複寫組名：網域系統磁碟區<br /><br />複寫群組識別碼： *<GUID>*<br /><br />成員識別碼： *<GUID>*<br /><br />同步夥伴： *<domain controller FQDN>*                                                                                                                                                                                                                                                                       |
...
|        **來源**        |                                                                                                                                                                                                                                                                                                                                                             Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                                                                                                                                                                             |
|       **低於**       |                                                                                                                                                                                                                                                                                                                                                                              資訊                                                                                                                                                                                                                                                                                                                                                                              |
|       **消息**        |                                                                                      *<COMPUTERNAME>* 停止用來複寫 SYSVOL 資料夾的 FRS 或 DFSR 服務。<br /><br />服務名稱：%1<br /><br />Active Directory 偵測到主控網域控制站的虛擬機器已還原至先前狀態。 *<COMPUTERNAME>* 必須在本機 SYSVOL 複本上初始化非權威還原。 若要這樣做，請停止用來複寫 SYSVOL 資料夾的 FRS 或 DFSR 服務，然後使用適當的登錄機碼與值啟動它以觸發還原。 重新啟動 FRS 或 DFSR 服務時，將記錄事件 2187。                                                                                       |
| **記事和解決方式** |                                                                                                                                                                                                                                                                                                                           預期會在還原快照時發生。 此網域控制站上的所有 SYSVOL 資料會都由協力電腦 DC 的複本來取代。                                                                                                                                                                                                                                                                                                                           |
|       **事件識別碼**       |                                                                                                                                                                                                                                                                                                                                                                                  2186                                                                                                                                                                                                                                                                                                                                                                                   |
|        **來源**        |                                                                                                                                                                                                                                                                                                                                                             Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                                                                                                                                                                             |
|       **低於**       |                                                                                                                                                                                                                                                                                                                                                                                  錯誤                                                                                                                                                                                                                                                                                                                                                                                  |
|       **消息**        | *<COMPUTERNAME>* 無法停止用來複寫 SYSVOL 資料夾的 FRS 或 DFSR 服務。<br /><br />服務名稱：%1<br /><br />錯誤碼：%2<br /><br />錯誤訊息：%3<br /><br />Active Directory 偵測到主控網域控制站的虛擬機器已還原至先前狀態。 *<COMPUTERNAME>* 必須在本機 SYSVOL 複本上初始化非權威還原。 若要這樣做，請停止用來複寫 SYSVOL 資料夾的 FRS 或 DFSR 複寫服務，然後使用適當的登錄機碼與值啟動它以觸發還原。 *<COMPUTERNAME>* 無法停止目前正在執行的服務，也無法完成非權威還原。 請手動執行非權威還原。 |
...
|        **來源**        |                                                                                                                                                                                                                                       Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                                                        |
|       **低於**       |                                                                                                                                                                                                                                                        資訊                                                                                                                                                                                                                                                         |
|       **消息**        | *<COMPUTERNAME>* 啟動用來複寫 SYSVOL 資料夾的 FRS 或 DFSR 服務。<br /><br />服務名稱：%1<br /><br />Active Directory 偵測到主控網域控制站的虛擬機器已還原至先前狀態。 *<COMPUTERNAME>* 需要在本機 SYSVOL 複本上初始化非權威還原。 若要這樣做，請停止用來複寫 SYSVOL 資料夾的 FRS 或 DFSR 服務，然後使用適當的登錄機碼與值啟動它以觸發還原。 |
| **記事和解決方式** |                                                                                                                                                                                                     預期會在還原快照時發生。 此網域控制站上的所有 SYSVOL 資料會都由協力電腦 DC 的複本來取代。                                                                                                                                                                                                      |
...
|        **來源**        |                                                                                                                                                                                                                                                                                                                                                                              Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                                                                                                                                                                                              |
|       **低於**       |                                                                                                                                                                                                                                                                                                                                                                                                   錯誤                                                                                                                                                                                                                                                                                                                                                                                                   |
|       **消息**        | *<COMPUTERNAME>* 無法啟動用來複寫 SYSVOL 資料夾的 FRS 或 DFSR 服務。<br /><br />服務名稱：%1<br /><br />錯誤碼：%2<br /><br />錯誤訊息：%3<br /><br />Active Directory 偵測到主控網域控制站的虛擬機器已還原至先前狀態。 *<COMPUTERNAME>* 必須在本機 SYSVOL 複本上初始化非權威還原。 若要這樣做，請停止用來複寫 SYSVOL 的 FRS 或 DFSR 服務，然後使用適當的登錄機碼與值啟動它以觸發還原。 *<COMPUTERNAME>* 無法啟動用來複寫 SYSVOL 資料夾的 FRS 或 DFSR 服務，也無法完成非權威還原。 請手動執行非權威還原，然後重新啟動服務。 |
| **記事和解決方式** |                                                                                                                                                                                                                                                                                                                                                                   檢查 [系統]、[FRS] 及 [DFSR] 事件記錄檔，以取得進一步資訊。                                                                                                                                                                                                                                                                                                                                                                    |
...
|        **來源**        |                                                                                                                                                                                                                                                                                      Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                                                                                                      |
|       **低於**       |                                                                                                                                                                                                                                                                                                       資訊                                                                                                                                                                                                                                                                                                       |
|       **消息**        | *<COMPUTERNAME>* 在非權威還原期間，設定下列登錄值以初始化 SYSVOL 複本：<br /><br />登錄機碼：%1<br /><br />登錄值：%2<br /><br />登錄值資料：%3<br /><br />Active Directory 偵測到主控網域控制站的虛擬機器已還原至先前狀態。 *<COMPUTERNAME>* 必須在本機 SYSVOL 複本上初始化非權威還原。 若要這樣做，請停止用來複寫 SYSVOL 資料夾的 FRS 或 DFSR 服務，然後使用適當的登錄機碼與值啟動它以觸發還原。 |
| **記事和解決方式** |                                                                                                                                                                                                                                                    預期會在還原快照時發生。 此網域控制站上的所有 SYSVOL 資料會都由協力電腦 DC 的複本來取代。                                                                                                                                                                                                                                                    |
...
|        **來源**        |                                                                                                                                                                                                                                                                                                                                                                                                          Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                                                                                                                                                                                                                           |
|       **低於**       |                                                                                                                                                                                                                                                                                                                                                                                                                               錯誤                                                                                                                                                                                                                                                                                                                                                                                                                                |
|       **消息**        | *<COMPUTERNAME>* 無法在未授權的還原期間，設定下列登錄值以初始化 SYSVOL 複本：<br /><br />登錄機碼：%1<br /><br />登錄值：%2<br /><br />登錄值資料：%3<br /><br />錯誤碼：%4<br /><br />錯誤訊息：%5<br /><br />Active Directory 偵測到主控網域控制站角色的虛擬機器已還原至先前狀態。 *<COMPUTERNAME>* 必須在本機 SYSVOL 複本上初始化非權威還原。 若要這樣做，請停止用來複寫 SYSVOL 資料夾的 FRS 或 DFSR 服務，然後使用適當的登錄機碼與值啟動它以觸發還原。 *<COMPUTERNAME>* 無法設定上述登錄值，也無法完成非權威還原。 請手動執行非權威還原。 |
| **記事和解決方式** |                                                                                                                                                                                                                                                                                                                                                                       檢查 [應用程式] 和 [系統] 事件記錄檔。 調查可能封鎖登錄更新的協力廠商應用程式。                                                                                                                                                                                                                                                                                                                                                                       |
...
|        **來源**        |                                                                                                                                                                                                                                                                                                                                                                                                Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                                                                                                                                                                                                                |
|       **低於**       |                                                                                                                                                                                                                                                                                                                                                                                                                 資訊                                                                                                                                                                                                                                                                                                                                                                                                                 |
|       **消息**        | *<COMPUTERNAME>* 偵測到虛擬機器世代識別碼已變更。 該變更表示虛擬網域控制站已還原至先前狀態。 *<COMPUTERNAME>* 將會執行下列作業，以保護已還原的網域控制站免于可能的資料分歧，並使用重複的 sid 來保護安全性主體的建立：<br /><br />建立新的呼叫識別碼<br /><br />使目前的 RID 集區無效<br /><br />下次執行輸入複寫時將驗證 FSMO 角色的擁有權。 在此期間，若網域控制站持有 FSMO 角色，該角色將無法使用。<br /><br />啟動 SYSVOL 複寫服務還原作業。<br /><br />啟動複寫將還原的網域控制站維持在最新的狀態。<br /><br />要求新的 RID 集區。 |
...
|        **來源**        |                                                                             Microsoft-Windows-ActiveDirectory_DomainService                                                                             |
|       **低於**       |                                                                                                  錯誤                                                                                                  |
|       **消息**        | *<COMPUTERNAME>* 無法在虛擬網域控制站還原至先前狀態之後使目前的 RID 集區無效。<br /><br />其他資料：<br /><br />錯誤碼：%1<br /><br />錯誤值：%2 |
...
|        **來源**        |                                                                                                                                                                                                                                                     Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                                                                      |
|       **低於**       |                                                                                                                                                                                                                                                                          錯誤                                                                                                                                                                                                                                                                           |
|       **消息**        | *<COMPUTERNAME>* 無法刪除 DFSR 資料庫。<br /><br />其他資料：<br /><br />錯誤碼：%1<br /><br />錯誤值：%2<br /><br />Active Directory 偵測到主控網域控制站的虛擬機器已還原至先前狀態。 *<COMPUTERNAME>* 必須在本機 SYSVOL 複本上初始化非權威還原。 對於 DFSR，若要這樣做，您必須停止 DFSR 服務、刪除 DFSR 資料庫，然後重新啟動該服務。 在重新啟動時，DFSR 將重建資料庫並開始進行初始同步。 |
...
|  **16654**   | Directory-Services-SAM  | 帳戶識別碼 (RID) 集區成為無效。 在下列預期情況中可能會發生此問題：<br /><br />1. 從備份還原網域控制站。<br /><br />2. 在虛擬機器上執行的網域控制站會從快照集還原。<br /><br />3. 系統管理員已手動將集區失效。<br /><br />如需相關資訊，請參閱<https://go.microsoft.com/fwlink/?LinkId=226247>。 |
...
|--------------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **事件識別碼** | **來源** |                                                                                                                                                                                           **消息**                                                                                                                                                                                            |
|   **103**    |   ESENT    |        DFSRs （1360） \\\\.\C： \System Volume Information\DFSR\database<em>_<GUID></em>\dfsr.db：資料庫引擎已停止實例（0）。<br /><br />異常關機：0<br /><br />內部計時順序：[1] 0.000、[2] 0.000、[3] 0.000、[4] 0.000、[5] 0.141、[6] 0.000、[7] 0.000、[8] 0.000、[9] 0.000、[10] 0.000、[11] 0.016、[12] 0.000、[13] 0.000、[14] 0.000、[15] 0.000。         |
|   **102**    |   ESENT    |                                                                                                                    DFSRs （532） \\\\.\C： \System Volume Information\DFSR\database<em>_<GUID></em>\dfsr.db：資料庫引擎（6.02.8189.0000）正在啟動新的實例（0）。                                                                                                                    |
|   **105**    |   ESENT    |                                      DFSRs （532） \\\\.\C： \System Volume Information\DFSR\database<em>_<GUID></em>\dfsr.db：資料庫引擎已啟動新的實例（0）。 (時間=0 秒)<br /><br />內部計時順序：[1] 0.000、[2] 0.000、[3] 0.000、[4] 0.000、[5] 0.000、[6] 0.000、[7] 0.000、[8] 0.000、[9] 0.031、[10] 0.000、[11] 0.000。                                      |
|              |            | DFSRs （532） \\\\.\C： \System Volume Information\DFSR\database<em> _<GUID></em>\dfsr.db：資料庫引擎已建立新的資料庫（1、\\\\.\C： \System Volume Information\DFSR\database<em>_ <GUID></em>\dfsr.db）。 (時間=0 秒)<br /><br />內部計時順序：[1] 0.000、[2] 0.000、[3] 0.016、[4] 0.062、[5] 0.000、[6] 0.016、[7] 0.000、[8] 0.000、[9] 0.015、[10] 0.000、[11] 0.000。 |
...
|   **1210**   |    DFSR    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    DFS 複寫服務已順利設定連入複寫要求的 RPC 接聽程式。<br /><br />其他資訊：<br /><br />埠：0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
|   **4614**   |    DFSR    | DFS 複寫服務已初始化位於本機路徑 C:\Windows\SYSVOL\domain 的 SYSVOL，而且正在等候執行初始複寫。 複寫的資料夾會維持在初始同步狀態，直到它和協力電腦完成複寫為止。 如果伺服器已經開始升級至網域控制站，網域控制站會在此問題解決之後才通告並開始運作。 若指定的協力電腦也處於初始同步狀態，或此伺服器或同步協力電腦發生共用違規，也會發生此情形。 若此事件是發生於將 SYSVOL 從檔案複寫服務 (FRS) 移轉至 DFS 複寫時，則只有在解決問題之後才會對外複寫變更。 這會導致此伺服器上的 SYSVOL 資料夾與其他網域控制站不同步。<br /><br />其他資訊：<br /><br />複寫的資料夾名稱： SYSVOL 共用<br /><br />複寫資料夾識別碼： *<GUID>*<br /><br />複寫組名：網域系統磁碟區<br /><br />複寫群組識別碼： *<GUID>*<br /><br />成員識別碼： *<GUID>*<br /><br />唯讀：0 |
|   **4604**   |    DFSR    |                                                                                                                                                                                                                                                                   DFS 複寫服務已順利初始化位於本機路徑 C:\Windows\SYSVOL\domain 的 SYSVOL 複寫資料夾。 此成員已完成與協力電腦 dc1.corp.contoso.com 的 SYSVOL 初始同步處理作業。  若要檢查 SYSVOL 共用是否存在，請開啟命令提示視窗，然後輸入 "net share"。<br /><br />其他資訊：<br /><br />複寫的資料夾名稱： SYSVOL 共用<br /><br />複寫資料夾識別碼： *<GUID>*<br /><br />複寫組名：網域系統磁碟區<br /><br />複寫群組識別碼： *<GUID>*<br /><br />成員識別碼： *<GUID>*<br /><br />同步夥伴： *<partner domain controller FQDN>*                                                                                                                                                                                                                                                                    |
...
|  **13520**   |   NTFRS    | 檔案複寫服務已將 *<path>* 中的預先存在檔案移至 *<path>* \ NtFrs_PreExisting___See_EventLog。<br /><br />檔案複寫服務可能會隨時刪除 *<path>* \ NtFrs_PreExisting___See_EventLog 中的檔案。 您可以將檔案複製到 *<path>* \ NtFrs_PreExisting___See_EventLog，以將檔案從刪除中儲存。 如果檔案已經存在於其他複寫協力電腦上，將檔案複製到 *<path>* 可能會導致名稱衝突。<br /><br />在某些情況下，檔案複寫服務可能會將檔案從 *<path>* \ NtFrs_PreExisting___See_EventLog 複製到 *<path>* ，而不是從其他複寫夥伴複寫檔案。<br /><br />藉由刪除 *<path>* \ NtFrs_PreExisting___See_EventLog 中的檔案，可以隨時復原空間。 |
...
|  **13554**   |   NTFRS    |                                                                                                                                                                                                                     檔案複寫服務成功地將下列連線加入複本集中：<br /><br />"DOMAIN SYSTEM VOLUME (SYSVOL SHARE)"<br /><br />來自 " *<partner domain controller FQDN>* " 的輸入<br /><br />輸出至「 *<partner domain controller FQDN>* 」<br /><br />後續的事件記錄檔訊息中會有其他的資訊。                                                                                                                                                                                                                     |
...
|-|-|-|  
|**事件識別碼**|**來源**|**消息**|  
|**327**|ESENT|ntfrs (1424) 資料庫引擎已中斷連結資料庫 (1，c:\windows\ntfrs\jet\ntfrs.jdb)。 (時間=0 秒)<br /><br />內部計時順序：[1] 0.000、[2] 0.015、[3] 0.000、[4] 0.000、[5] 0.000、[6] 0.516、[7] 0.000、[8] 0.000、[9] 0.000、[10] 0.000、[11] 0.063、[12] 0.000。<br /><br />恢復快取：0|  
|**103**|ESENT|ntfrs (1424) 資料庫引擎已停止執行個體 (0)。<br /><br />異常關機：0<br /><br />內部計時順序：[1] 0.000、[2] 0.000、[3] 0.000、[4] 0.000、[5] 0.000、[6] 0.000、[7] 0.000、[8] 0.000、[9] 0.031、[10] 0.000、[11] 0.016、[12] 0.000、[13] 0.000、[14] 0.047、[15] 0.000。|  
...
|**103**|ESENT|ntfrs (3000) 資料庫引擎已停止執行個體 (0)。<br /><br />異常關機：0<br /><br />內部計時順序：[1] 0.000、[2] 0.000、[3] 0.000、[4] 0.000、[5] 0.000、[6] 0.000、[7] 0.000、[8] 0.000、[9] 0.000、[10] 0.000、[11] 0.000、[12] 0.000、[13] 0.015、[14] 0.000、[15] 0.000。|  
|**102**|ESENT|ntfrs (3000) 資料庫引擎 (6.02.8189.0000) 正在啟動新的執行個體 (0)。|  
...
|**103**|ESENT|ntfrs (3000) 資料庫引擎已停止執行個體 (0)。<br /><br />異常關機：0<br /><br />內部計時順序：[1] 0.000、[2] 0.000、[3] 0.000、[4] 0.000、[5] 0.078、[6] 0.000、[7] 0.000、[8] 0.000、[9] 0.125、[10] 0.016、[11] 0.000、[12] 0.000、[13] 0.000、[14] 0.000、[15] 0.000。|  
|**102**|ESENT|ntfrs (3000) 資料庫引擎 (6.02.8189.0000) 正在啟動新的執行個體 (0)。|  
