...
- 連線**能力：** 此測試會判斷網域控制站是否在 DNS 中註冊、是否可透過<system>ping</system>命令連線，以及具有輕量型目錄存取協定/遠端程序呼叫（LDAP/RPC）連線能力。 如果網域控制站上的連線能力測試失敗，則不會對該網域控制站執行其他測試。 連線能力測試會在執行任何其他 DNS 測試之前自動執行。
- **基本服務：** 此測試會確認下列服務正在執行，且可在已測試的網域控制站上使用： DNS 用戶端服務、Net Logon 服務、金鑰發佈中心（KDC）服務，以及 DNS 伺服器服務（如果 DNS 已安裝在網域控制站上）。
...
4. 在詳細資料窗格中，確認有下列資源記錄：名為 Dsa_Guid 的別名（CNAME）資源記錄。 _msdcs。<placeholder>Dns_Domain_Name</placeholder>和對應的主機（a）資源記錄做為 Dns 伺服器的名稱。

如果未註冊別名（CNAME）資源記錄，請確認動態更新是否正常運作。 請使用下一節中的測試來確認動態更新。
    
## <a name="verifying-dynamic-update"></a>正在驗證動態更新
    
如果基本 DNS 測試顯示資源記錄不存在於 DNS 中，請使用動態更新測試來判斷 Net Logon 服務為何無法自動註冊資源記錄。 若要確認 Active Directory 網域區域已設定為接受安全動態更新，並執行測試記錄（_dcdiag_test_record）的註冊，請使用下列程式。 測試記錄會在測試之後自動刪除。
...
6. 在命令提示字元中，輸入下列命令，然後按 ENTER： `dcdiag /test:dns /v /s:<DCName>` </br></br>以 &lt;DCName&gt;取代網域控制站的 [辨別名稱]、[NetBIOS 名稱] 或 [DNS 名稱]。 檢查測試的輸出，以確定已通過 DNS 測試。 如果您未在網域控制站上啟用 IPv6，您應該預期測試的主機（AAAA）資源記錄部分失敗，這是未啟用 IPv6 時的正常情況。
