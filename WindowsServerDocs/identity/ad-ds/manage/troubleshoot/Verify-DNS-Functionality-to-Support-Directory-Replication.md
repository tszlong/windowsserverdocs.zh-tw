...
## <a name="running-the-domain-controller-basic-dns-testtitle"></a>執行網域控制站基本 DNS 測試 @ no__t-0

基本 DNS 測試會檢查 DNS 功能的下列層面：


- **技術**此測試會判斷網域控制站是否在 DNS 中註冊、是否可透過<system>ping</system>命令連線，以及具有輕量型目錄存取協定/遠端程序呼叫（LDAP/RPC）連線能力。 如果網域控制站上的連線能力測試失敗，則不會對該網域控制站執行其他測試。 連線能力測試會在執行任何其他 DNS 測試之前自動執行。
- **基本服務：** 此測試會確認下列服務正在執行，且可在已測試的網域控制站上使用：DNS 用戶端服務、Net Logon 服務、金鑰發佈中心（KDC）服務，以及 DNS 伺服器服務（如果 DNS 安裝在網域控制站上）。
- **DNS 用戶端設定：** 此測試會確認可連線到 DNS 用戶端電腦上所有網路介面卡上的 DNS 伺服器。
- **資源記錄註冊：** 此測試會確認每個網域控制站的主機（A）資源記錄都已在用戶端電腦上設定的至少一個 DNS 伺服器上註冊。
- **區域和啟動授權（SOA）：** 如果網域控制站正在執行 DNS 伺服器服務，此測試會確認 Active Directory 網域區域的 Active Directory 網域區域和啟動授權（SOA）資源記錄存在。
- **根區域：** 檢查根（.）區域是否存在。
...
</br></br>以 &lt;DCName @ no__t-1 取代網域控制站的實際辨別名稱、NetBIOS 名稱或 DNS 名稱。 或者，您可以輸入/e：而不是/s：，藉以測試樹系中的所有網域控制站。 /F 參數會指定一個檔案名，在上一個命令中是 dcdiagreport .txt。 如果您想要將檔案放在目前工作目錄以外的位置，您可以指定檔案路徑，例如/f： c:reportsdcdiagreport.txt。

5. 在 [記事本] 或類似的文字編輯器中開啟 dcdiagreport。 若要在 [記事本] 中開啟檔案，請在命令提示字元中輸入 Notepad dcdiagreport，然後按 ENTER。 如果您將檔案放在不同的工作目錄中，請包含檔案的路徑。 例如，如果您將檔案放在 c:reports 中，請輸入 notepad c:reportsdcdiagreport.txt，然後按 ENTER 鍵。
6. 流覽至靠近檔案底部的摘要資料表。 
</br></br>請注意 [摘要] 資料表中報告「警告」或「失敗」狀態的所有網域控制站名稱。  藉由搜尋字串 "DC：來尋找詳細的分類區段，嘗試判斷是否有問題的網域控制站DCName，"其中 DCName 是網域控制站的實際名稱。

如果您看到所需的明顯設定變更，請適當地加以設定。 例如，如果您發現其中一個網域控制站有明顯不正確的 IP 位址，您可以修正它。 然後，重新執行測試。

若要驗證設定變更，請視需要使用/e：或/s：交換器重新執行 Dcdiag/test： DNS/v 命令。 如果您未在網域控制站上啟用 IP 版本6（IPv6），您應該預期測試的主機（AAAA）驗證部分失敗，但如果您未在網路上使用 IPv6，則不需要這些記錄。
...
### <a name="to-verify-resource-record-registrationtitle"></a>若要驗證資源記錄註冊 @ no__t-0
...
- **別名（CNAME）：** 尋找複寫協力電腦的全域唯一識別碼（GUID）型資源記錄
- **主機（A）：** 包含網域控制站 IP 位址的主機資源記錄
- **LDAP SRV：** 尋找 LDAP 伺服器的服務（SRV）資源記錄
- **GC SRV**：找出通用類別目錄伺服器的服務（SRV）資源記錄
- **PDC SRV**：尋找主域控制站（PDC）模擬器操作主機的服務（SRV）資源記錄

您可以使用下列程式，單獨驗證別名（CNAME）資源記錄註冊。

### <a name="to-verify-alias-cname-resource-record-registration"></a>驗證別名（CNAME）資源記錄註冊
...
4. 在 [詳細資料] 窗格中，確認有下列資源記錄：名為 Dsa_Guid. _msdcs 的別名（CNAME）資源記錄。<placeholder>Dns_Domain_Name</placeholder>和對應的主機（a）資源記錄，以取得 Dns 伺服器的名稱。

如果未註冊別名（CNAME）資源記錄，請確認動態更新是否正常運作。 請使用下一節中的測試來確認動態更新。
    
## <a name="verifying-dynamic-update"></a>正在驗證動態更新
    
如果基本 DNS 測試顯示資源記錄不存在於 DNS 中，請使用動態更新測試來判斷 Net Logon 服務為何無法自動註冊資源記錄。 若要確認 Active Directory 網域區域已設定為接受安全動態更新並執行測試記錄（_dcdiag_test_record）的註冊，請使用下列程式。 測試記錄會在測試之後自動刪除。

### <a name="to-verify-dynamic-updatetitle"></a>若要驗證動態更新 @ no__t-0
...
2. 在命令提示字元中，輸入下列命令，然後按 ENTER： `dcdiag /test:dns /v /s:<DCName> /DnsDynamicUpdate`
   </br></br>以 &lt;DCName @ no__t-1 取代網域控制站的辨別名稱、NetBIOS 名稱或 DNS 名稱。 或者，您可以輸入/e：而不是/s：，藉以測試樹系中的所有網域控制站。 如果您未在網域控制站上啟用 IPv6，您應該預期測試的主機（AAAA）資源記錄部分失敗，這是未啟用 IPv6 時的正常情況。
...
如果 DNS 資源記錄並未出現在來源網域控制站的 DNS 中，您已驗證動態更新，而您想要立即註冊 DNS 資源記錄，您可以使用下列程式手動強制註冊。 網域控制站上的 Net Logon 服務會註冊網域控制站必須位於網路上的 DNS 資源記錄。 DNS 用戶端服務會註冊別名（CNAME）記錄指向的主機（A）資源記錄。

### <a name="to-register-dns-resource-records-manuallytitle"></a>手動註冊 DNS 資源記錄 @ no__t-0
...
6. 在命令提示字元中，輸入下列命令，然後按 ENTER： `dcdiag /test:dns /v /s:<DCName>` </br></br>以 &lt;DCName @ no__t-1 取代網域控制站的辨別名稱、NetBIOS 名稱或 DNS 名稱。 檢查測試的輸出，以確定已通過 DNS 測試。 如果您未在網域控制站上啟用 IPv6，您應該預期測試的主機（AAAA）資源記錄部分失敗，這是未啟用 IPv6 時的正常情況。
