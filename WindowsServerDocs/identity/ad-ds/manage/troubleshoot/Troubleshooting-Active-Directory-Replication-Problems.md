...
ms.locfileid: "87941617"
---
# <a name="troubleshooting-active-directory-replication-problems"></a>進行 Active Directory 複寫問題疑難排解

>適用於：Windows Server 2016、Windows Server 2012 R2、Windows Server 2012

Active Directory 複寫問題可以有數個不同的來源。 例如，網域名稱系統 (DNS) 問題、網路問題或安全性問題，都可能導致 Active Directory 複寫失敗。
...
在理想的情況下，[目錄服務] 事件記錄檔中的紅色 (錯誤) 和黃色 () 警告，會建議在來源或目的地網域控制站上導致複寫失敗的特定條件約束。 如果事件訊息建議解決方案的步驟，請嘗試執行事件中所述的步驟。 Repadmin 工具和其他診斷工具也提供可協助您解決複寫失敗的資訊。
...
如果複寫錯誤是由嘗試複寫的網域控制站所報告，而該網域控制站已內建在預備網站中，而且目前在最後生產網站中等待其部署 (遠端網站（例如分公司) ），您就可以考慮這些複寫錯誤。 若要避免長時間將網域控制站與複寫拓撲隔開，這會造成連續錯誤，直到網域控制站重新連線為止，請考慮一開始將這類電腦新增為成員伺服器，然後使用 [從媒體安裝] (IFM) 方法來安裝 Active Directory Domain Services (AD DS) 。 您可以使用 Ntdsutil 命令列工具來建立安裝媒體，以儲存在卸載式媒體上， (CD、DVD 或其他媒體) 並寄送到目的地網站。 然後，您可以使用安裝媒體在網站的網域控制站上安裝 AD DS，而不需要使用複寫。

### <a name="hardware-failures-or-upgradestitle"></a>硬體故障或升級</title>

如果由於硬體故障而導致複寫問題 (例如，主機板、磁片子系統或硬碟) 失敗，請通知伺服器擁有者，讓硬體問題能夠解決。

定期硬體升級也會導致網域控制站無法服務。 請確定您的伺服器擁有者有一個很好的系統，可以事先進行這類中斷的溝通。

### <a name="firewall-configuration"></a>防火牆設定

根據預設，Active Directory 複寫遠端程序呼叫 () Rpc 會透過 RPC 端點對應程式（ (RPCSS) 埠135），以動態方式在可用埠上發生。 請確定已正確設定 [具有 Advanced Security 的 Windows 防火牆] 和其他防火牆，以允許進行複寫。 如需指定 Active Directory 複寫和埠設定之通訊埠的詳細資訊，請參閱[Microsoft 知識庫中的文章 224196](https://go.microsoft.com/fwlink/?LinkId=22578)。
...
- 複寫拓撲：網域控制站在 AD DS 中必須有對應到 (WAN) 或虛擬私人網路 (VPN) 連線的網站間連結。 如果您在 AD DS 中為網路的實際網站拓朴不支援的複寫拓撲建立物件，則需要設定錯誤拓撲的複寫會失敗。

## <a name="general-approach-to-fixing-problems"></a>修正問題的一般方法

若要修正複寫問題，請使用下列一般方法：

1. 每天監視複寫健全狀況，或使用 Repadmin.exe 每日取出複寫狀態。
...
4. 如果在伺服器連線到網路時，無法正常移除 AD DS，請使用下列其中一種方法來解決問題：
   ...
   - 強制 AD DS 移除目錄服務還原模式 (DSRM) 、清理伺服器中繼資料，然後重新安裝 AD DS。
   - 重新安裝作業系統，並重建網域控制站。
...
對於樹系中的每個網域控制站，試算表會顯示來源複寫協力電腦、複寫上次發生的時間，以及每個命名內容的上次複寫失敗時間 (目錄磁碟分割) 。 藉由使用 Excel 中的 [自動篩選]，您只可以查看工作網域控制站的複寫健康情況、僅失敗的網域控制站，或最小或最新的網域控制站，而且您可以看到複寫協力電腦已成功複寫。
...
|上次嘗試于 <日期時間> 失敗，並出現「目標帳戶名稱不正確」。|此問題可能與連線能力、DNS 或驗證問題有關。 如果這是 DNS 錯誤，本機網域控制站無法解析其複寫協力電腦 (GUID) DNS 名稱的全域唯一識別碼。|修正複寫 DNS 查詢問題 (事件識別碼1925、2087、2088) 修正複寫安全性問題修正複寫連線問題 (事件識別碼 1925) |
...
|1925 NTDS KCC|嘗試為可寫入的目錄磁碟分割建立複寫連結失敗。 視錯誤而定，此事件可能會有不同的原因。| 修正複寫連線問題 (事件識別碼 1925) 修正複寫 DNS 查閱問題 (事件識別碼1925、2087、2088) |
