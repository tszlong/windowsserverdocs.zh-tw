...
- **認證竊取的吸引**人-認證竊取攻擊是攻擊者一開始取得網路上電腦的特殊許可權存取權，然後使用自由可用的工具，從其他已登入的帳戶會話中解壓縮認證。 本節包含下列內容：
  ...
  - **許可權提升和傳播**特有的帳戶、伺服器和基礎結構元件通常是針對 Active Directory 攻擊的主要目標。 這些帳戶分別是：
...
| 修補應用程式。 | 戰術 | 預防性 |
| 修補作業系統。 | 戰術 | 預防性 |
| 部署並立即更新所有系統上的防毒軟體和反惡意程式碼軟體，並監視是否嘗試移除或停用它。 | 戰術 | 兩者 |
| 監視敏感的 Active Directory 物件以進行修改嘗試，並針對可能表示嘗試洩露的事件進行 Windows。 | 戰術 | 偵測性 |
| 針對可存取敏感性資料的使用者，保護和監視帳戶 | 戰術 | 兩者 |
| 防止未經授權的系統使用強大的帳戶。 | 戰術 | 預防性 |
| 排除高許可權群組中的永久成員資格。 | 戰術 | 預防性 |
| 在需要時，執行控制項以授與特殊許可權群組中的暫時成員資格。 | 戰術 | 預防性 |
| 執行安全的系統管理主機。 | 戰術 | 預防性 |
| 在網域控制站、管理主機和其他機密系統上使用應用程式允許清單。 | 戰術 | 預防性 |
| 識別重要資產，並設定其安全性和監視的優先順序。 | 戰術 | 兩者 |
| 執行最低許可權、以角色為基礎的存取控制來管理目錄、其支援的基礎結構，以及已加入網域的系統。 | 策略 | 預防性 |
| 找出舊版的系統和應用程式。 | 戰術 | 預防性 |
| 解除委任舊版系統和應用程式。 | 策略 | 預防性 |
| 為自訂應用程式實施安全的開發生命週期程式。 | 策略 | 預防性 |
| 執行設定管理、定期審查合規性，並使用每個新的硬體或軟體版本來評估設定。 | 策略 | 預防性 |
| 使用嚴格的安全性和監視需求，將重要資產遷移至初始化樹系。 | 策略 | 兩者 |
| 簡化終端使用者的安全性。 | 策略 | 預防性 |
| 使用主機型防火牆來控制和保護通訊的安全。 | 戰術 | 預防性 |
| 修補裝置。 | 戰術 | 預防性 |
| 為 IT 資產實行以商務為中心的生命週期管理。 | 策略 | N/A |
| 建立或更新事件復原計畫。 | 策略 | N/A |
