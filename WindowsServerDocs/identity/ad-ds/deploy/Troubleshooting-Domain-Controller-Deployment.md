...
## <a name="built-in-logs-for-troubleshooting"></a>內建的記錄檔用於疑難排解
...
|網域控制站的安裝/升級|-   %systemroot%\debug\dcpromo.log<br /><br />-   %systemroot%\debug\dcpromo*.log<br /><br />事件檢視器 \windows 記錄 \ 系統<br /><br />事件檢視器 \windows 記錄 \ 應用程式<br /><br />-事件 viewer\Applications and services 記錄檔 \ 目錄服務<br /><br />-事件 viewer\Applications and services 記錄檔 \ 檔案複寫服務<br /><br />-事件 viewer\Applications and services 記錄檔 \dfs 複寫|  
...
|伺服器管理員 ADDSDeployment Windows PowerShell 部署引擎|-事件 viewer\Applications and services 記錄檔 \microsoft\windows\directoryservices-deployment\|  
...
## <a name="troubleshooting-events-and-error-messages"></a>疑難排解事件和錯誤訊息
...
|     21     |                             此網域控制站保有 FSMO 角色、是一個 GC，以及 (或) 是一部 DNS 伺服器                             |                                                                                                      新增 **-demoteoperationmasterrole**使用時 **-forceremoval**。                                                                                                      |
...
|     50     | 在降級期間，偵測到最後一個網域控制站 (即使它並不是)，或已指定最後一個網域控制站 (但它並不是) |        除非網域控制站確實是**網域中最後一個網域控制站**( **-lastdomaincontrollerindomain**)，否則請勿指定此項目。 使用 **-ignorelastdcindomainmismatch**覆寫，如果這確實是最後一個網域控制站，而且有虛設網域控制站中繼資料        |
|     51     |                                          此網域控制站上有應用程式磁碟分割存在                                          |                                                                                              請指定**移除應用程式磁碟分割**( **-removeapplicationpartitions**)                                                                                               |
|     52     |            遺失必要的命令列引數 (也就是命令列中必須指定回應檔案)             |                                                                                              *只看過的 dcpromo / 自動安裝，已被取代。請參閱較舊的文件*                                                                                              |
...
|     61     |        您無法在尚未主控 DNS 的現有網域中，安裝具有 DNS 的 Active Directory 網域服務         |                                                                                                                      *不可能發生此錯誤*                                                                                                                      |
|     62     |                                         回應檔案中沒有 [DCInstall] 區段                                          |                                                                                             *只看過的 dcpromo / 自動安裝，已被取代。請參閱較舊的文件。*                                                                                              |
...
|     68     |                                                  複寫協力電腦無效                                                  |                                                                             使用 repadmin.exe 或**Get Set-adreplication\\**  \* Windows PowerShell 來驗證夥伴網域控制站健康情況                                                                              |
|     69     |                                    必要的連接埠已由其他應用程式使用                                     |                                                                                    請使用 **netstat.exe anob** 尋找錯誤地指派給保留 AD DS 連接埠的處理程序                                                                                     |
|     70     |                                          樹系根網域控制站必須是 GC                                          |                                                                                              *只看過的 dcpromo / 自動安裝，已被取代。請參閱較舊的文件*                                                                                              |
...
|     87     |                                      有類型衝突的網域控制站帳戶存在                                      |   如果不是要嘗試附加到未佔用的網域控制站，請先將電腦重新命名之後再升級。 您必須附加至未佔用的網域控制站帳戶使用 **-useexistingaccount**和正確唯讀或可寫入引數，根據帳戶類型    |
...
|     91     |                                             無法偵測處理程序是否為 wow64                                             |                                                                                                  *不可能發生此錯誤，作業系統是 64 位元*                                                                                                  |
|     92     |                                                  不支援 Wow64 處理程序                                                  |                                                                                                  *不可能發生此錯誤，作業系統是 64 位元*                                                                                                  |
...
|     96     |                                                 無法解除安裝 DS 二進位檔                                                  |                                                                                              *只看過的 dcpromo / 自動安裝，已被取代。請參閱較舊的文件*                                                                                              |
|     97     |                      樹系功能等級版本高於子網域作業系統版本                       |                                                                                           提供與樹系功能等級相同或更高的子網域功能                                                                                            |
|     98     |                                        元件二進位安裝/解除安裝正在進行中。                                        |                                                                                              *只看過的 dcpromo / 自動安裝，已被取代。請參閱較舊的文件*                                                                                              |
...
## <a name="known-issues-and-common-support-scenarios"></a>已知的問題和常見的支援案例
...
|解決方式和注意事項|移除 AD DS 角色時，也會移除 DNS 伺服器角色或將 DNS 伺服器服務設定為停用。 請記得將 DNS 用戶端指向其本身之外的其他伺服器。 如果使用 Windows PowerShell，請在將伺服器降級之後執行以下項目：<br /><br />程式碼-解除安裝 windowsfeature dns<br /><br />或<br /><br />程式碼-設定服務 dns starttype 停用<br />停止服務的 dns|  
...
|---------|------------------------------------------------------------------|  
|問題|傳回錯誤：<br /><br />程式碼-無法驗證媒體路徑。 呼叫 「 GetDatabaseInfo"與"2"的引數例外狀況。 資料夾不是有效的。|  
|解決方式和注意事項|您必須將 IFM 檔案儲存在本機磁碟，而非遠端 UNC 路徑。 此刻意區塊可防止因為網路中斷而導致伺服器只有部分升級。|  

|問題|在網域控制站升級期間顯示兩次 DNS 委派警告|  
|---------|-------------------------------------------------------------------------|  
|問題|傳回警告*兩次*使用 ADDSDeployment Windows PowerShell 升級時：<br /><br />程式碼-「 無法建立此 DNS 伺服器的委派，因為找不到授權的上層區域，或它不會執行 Windows DNS 伺服器。 如果您要使用現有的 DNS 基礎結構整合，您應該手動建立此 DNS 伺服器的委派，以確保來自網域以外的可靠的名稱解析的上層區域中。 否則，不不需要任何動作。 」|  
|解決方式和注意事項|忽略。 ADDSDeployment Windows PowerShell 會在先決條件檢查時第一次顯示警告，然後在設定網域控制站時再顯示一次。 如果您不想設定 DNS 委派，請使用引數：<br /><br />Code - -creatednsdelegation:$false<br /><br />請*勿*略過先決條件檢查，以隱藏此訊息|  

|問題|在設定期間指定 UPN 或非網域認證時傳回誤導錯誤|  
|---------|--------------------------------------------------------------------------------------------|  
|問題|伺服器管理員傳回錯誤：<br /><br />程式碼-呼叫 「 DNSOption 」 與 「 6 」 引數例外狀況<br /><br />ADDSDeployment Windows PowerShell 傳回錯誤：<br /><br />程式碼-驗證失敗的使用者權限。 您必須提供此使用者帳戶所屬的網域名稱。|  
...
|---------|--------------------------------------------------------------------|  
|問題|使用 ADDSDeployment Windows PowerShell 升級時傳回錯誤：<br /><br />Code -  Test.VerifyDcPromoCore.DCPromo.General.74<br /><br />無法驗證的網域控制站升級的必要條件。 指定的網域功能等級無效|  
...
|解決方式和注意事項|這是伺服器管理員的限制。 如需意見反應，請使用 ADDSDeployment Windows PowerShell Cmdlet：<br /><br />Code - Uninstall-addsdomaincontroller|  

|||  
|-|-|  
|問題|「從媒體安裝驗證」不會偵測針對可寫入網域控制站提供的 RODC 媒體，反之亦然。|  
|問題|使用 IFM 升級新的網域控制站且提供給 IMF 的媒體不正確時 (例如提供 RODC 媒體給可寫入網域控制站，或提供 RWDC 媒體給 RODC ) [驗證] 按鈕都不會傳回錯誤。 之後，升級會失敗並顯示錯誤：<br /><br />程式碼-嘗試設定此電腦的網域控制站時發生錯誤。 <br />唯讀 DC 從媒體安裝升級無法啟動，因為不允許指定之來源資料庫。 只有從其他 Rodc 的資料庫可以用於 RODC 的 IFM 升級。|  
|解決方式和注意事項|驗證時只會驗證 IFM 的整體完整性。 請勿提供錯誤的 IFM 類型給伺服器。 請先重新啟動伺服器，然後使用正確的媒體再次嘗試升級。|  

|||  
|-|-|  
|問題|預先建立之電腦帳戶中的 RODC 升級失敗|  
|問題|使用 ADDSDeployment Windows PowerShell 來升級具備分段電腦帳戶的新 RODC 時，收到錯誤：<br /><br />程式碼-無法使用指定的具名參數解析參數集。    <br />InvalidArgument:ParameterBindingException<br />    + FullyQualifiedErrorId:AmbiguousParameterSet,Microsoft.DirectoryServices.Deployment.PowerShell.Commands.Install|  
|解決方式和注意事項|請勿提供已在預先建立的 RODC 帳戶中定義的參數。 它們包括：<br /><br />Code - -readonlyreplica<br />-installdns<br />-donotconfigureglobalcatalog<br />-sitename<br />-installdns|  

|||  
|-|-|  
|問題|取消選取/選取 [必要時自動重新啟動目的地伺服器] 時未執行任何動作|  
|問題|如果選取 （或不選取） [伺服器管理員] 選項**必要時自動重新啟動每個目的地伺服器**whendemoting 透過移除角色的網域控制站，伺服器一律會重新啟動，無論選擇為何。|  
|解決方式和注意事項|這是刻意設計的。 無論此設定為何，降級處理程序都會重新啟動伺服器。|  

|||  
|-|-|  
|問題|Dcpromo.log 顯示「[錯誤] 伺服器檔案設定安性失敗，發生 2 項失敗」|  
|問題|網域控制站降級完成且未發生問題，但是檢查 dcpromo 記錄時顯示錯誤：<br /><br />程式碼-[error] 伺服器檔案上設定安全性失敗，2|  
|解決方式和注意事項|請忽略，預期會發生此錯誤且沒有實質作用。|  

|||  
|-|-|  
|問題|先決條件 adprep 檢查失敗，發生「 無法執行 Exchange 架構衝突檢查 」錯誤|  
|問題|嘗試將 Windows Server 2012 網域控制站升級到現有的 Windows Server 2003、Windows Server 2008 或 Windows Server 2008 R2 樹系時，先決條件檢查失敗並發生錯誤：<br /><br />程式碼-AD 準備的必要元件驗證失敗。 無法執行 Exchange 架構衝突檢查網域 *<domain name>* (例外狀況： RPC 伺服器不存在)<br /><br />adprep.log 顯示錯誤：<br /><br />程式碼-Adprep 無法擷取資料，從伺服器 *<domain controller>*<br /><br />透過 Windows Management Instrumentation (WMI)。|  
|解決方式和注意事項|新的網域控制站無法透過 DCOM/RPC 通訊協定對現有網域控制站存取 WMI。 到目前為止，有三個原因造成此問題：<br /><br />-現有的網域控制站防火牆規則封鎖存取<br /><br />-網路服務帳戶缺少的 「 登入為服務 」 (SeServiceLogonRight) 權限，現有的網域控制站<br /><br />-停用的 NTLM 網域控制站上使用中所述的安全性原則[NTLM 驗證的限制簡介](https://technet.microsoft.com/library/dd560653(WS.10).aspx)|  

|||  
|-|-|  
|問題|建立新的 AD DS 樹系一律會顯示 DNS 警告|  
|問題|在新的網域控制站上為其本身建立新的 AD DS 樹系及建立 DNS 區域時，您一定會收到警告訊息：<br /><br />DNS 設定中偵測到程式碼-發生錯誤。 <br />沒有任何使用此電腦的 DNS 伺服器回應的逾時間隔內。<br />(錯誤碼 0x000005B4"ERROR_TIMEOUT")|  
|解決方式和注意事項|忽略。 這個警告是在新樹系之根網域中第一個網域控制站上刻意設計的，以方便您指向現有 DNS 伺服器和區域。|  

|||  
|-|-|  
|問題|Windows PowerShell-whatif 引數傳回不正確的 DNS 伺服器資訊|  
|問題|如果您在設定具備隱含或明確 **-installdns： $true** 的網域控制站時使用 **-whatif** 引數，會顯示以下輸出結果：<br /><br />程式碼-「 DNS 伺服器：否 」|  
|解決方式和注意事項|忽略。 已正確安裝及設定 DNS。|  

|||  
|-|-|  
|問題|升級之後登入失敗，並顯示 [儲存體不足，無法處理此命令。 ]|  
|問題|在您升級新的網域控制站之後，接著登出並嘗試以互動方式登入時，您收到錯誤：<br /><br />程式碼-沒有足夠的存放裝置可供處理此命令|  
...
|解決方式和注意事項|您有多個 AD DS 站台且至少有一個站台遺失子網路。這個新的網域控制站隸屬於那些子網路的其中之一。 您必須從 [站台名稱] 下拉式功能表手動選取子網路。 您也應該使用 DSSITE.MSC 檢閱所有 AD 站台，或使用以下 Windows PowerShell 命令來尋找遺失子網路的所有站台：<br /><br />Code - get-adreplicationsite -filter \* -property subnets &#124; where-object {!$_.subnets -eq "\*"} &#124; format-table name|  

|||  
|-|-|  
|問題|升級或降級失敗，且顯示 [無法啟動服務] 訊息|  
|問題|如果您嘗試升級、降級，或複製網域控制站，會收到錯誤：<br /><br />程式碼-服務無法啟動，可能是因為它是已停用或它有沒有與其相關聯的啟用的裝置"(0x80070422)<br /><br />可能是互動，事件，或寫入記錄檔 (例如 dcpromoui.log 或 dcpromo.log) 錯誤|  
...
|-|-|  
|問題|如果您搭配下列範例回應檔案使用 dcpromo /unattend 來升級網域控制站：<br /><br />Code -<br /><br />[DCInstall]<br />NewDomain=Forest<br /><br />ReplicaOrNewDomain=Domain<br /><br />NewDomainDNSName=corp.contoso.com<br /><br />SafeModeAdminPassword=Safepassword@6<br /><br />DomainNetbiosName=corp<br /><br />DNSOnNetwork = Yes<br /><br />AutoConfigDNS=Yes<br /><br />RebootOnSuccess=NoAndNoPromptEither<br /><br />RebootOnCompletion=No<br /><br />*DomainLevel=0*<br /><br />*ForestLevel=0*<br /><br />升級失敗且 dcpromoui.log 中會顯示下列錯誤：<br /><br />Code - dcpromoui EA4.5B8 0089 13:31:50.783       Enter CArgumentsSpec::ValidateArgument DomainLevel<br /><br />dcpromoui EA4.5B8 008A 13:31:50.783 DomainLevel 值為 0<br /><br />dcpromoui EA4.5B8 008B 13:31:50.783 結束代碼為 77<br /><br />dcpromoui EA4.5B8 008 C 13:31:50.783 指定的引數無效。<br /><br />dcpromoui EA4.5B8 008 D 13:31:50.783 關閉記錄<br /><br />dcpromoui EA4.5B8 0032 13:31:50.830 結束代碼為 77<br /><br />等級 0 為 Windows 2000，Windows Server 2012 不支援。|  
|解決方式和注意事項|請勿使用過時的 dcpromo /unattend，而且它允許您指定之後會失敗的無效設定。 此行為是預期的行為且是設計使然。|  

|問題|升級 「 擱置 」，在建立 NTDS 設定物件，永遠無法完成|  
|-|-|  
|問題|如果您升級複本 DC 或 RODC，升級就會達到 「 建立 NTDS 設定物件 」 和一直沒有繼續執行或完成。 記錄檔也停止更新。|  
|解決方式和注意事項|這是因提供了與內建網域系統管理員帳戶密碼相同之內建本機系統管理員帳戶的認證而產生的已知問題。 這會導致核心安裝程式引擎失敗，但不會發生錯誤，而是會無限期等待 (odd 迴圈)。 這預期行為-雖然不想要的行為。<br /><br />修正伺服器：<br /><br />1.將它重新開機。<br /><br />1.在 AD 中，刪除該伺服器的成員電腦帳戶 （它還無法成為 DC 帳戶）<br /><br />1.在該伺服器上，將它從網域強制退出<br /><br />1.在該伺服器上移除 AD DS 角色。<br /><br />1.重新開機<br /><br />1.重新新增 AD DS 角色並重新嘗試升級，確保永遠提供***domain\admin***格式的認證以 DC 升級，而不只是內建的本機系統管理員帳戶|  
