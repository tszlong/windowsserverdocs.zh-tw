...
|--|--|
| 徵兆 | 使用 IFM 升級新的網域控制站且提供給 IMF 的媒體不正確時 (例如提供 RODC 媒體給可寫入網域控制站，或提供 RWDC 媒體給 RODC ) [驗證] 按鈕都不會傳回錯誤。 之後，升級會失敗並顯示錯誤：<p>程式碼-嘗試將此電腦設定為網域控制站時發生錯誤。 <br />因為不允許指定的源資料庫，所以無法啟動 Read-Only DC 的從媒體安裝升級。 只有來自其他 Rodc 的資料庫可用於 RODC 的 IFM 升級。 |
