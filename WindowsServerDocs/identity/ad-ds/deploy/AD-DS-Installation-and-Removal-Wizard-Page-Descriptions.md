...
-   建立新的 Active Directory 樹系時，請不要使用與外部 DNS 名稱相同的名稱。 例如，如果您的網際網路 DNS URL 為 http: \/ /contoso.com，您必須選擇不同的名稱，為您的內部樹系以避免未來的相容性問題。 這個名稱必須是唯一而且不像是會用在網路流量的名稱，如 corp.contoso.com。  
