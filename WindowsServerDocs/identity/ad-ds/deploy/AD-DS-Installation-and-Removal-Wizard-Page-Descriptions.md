...
-   必須執行 Adprep/domainprep，才能將執行 Windows Server 2012 的第一個網域控制站新增至現有的網域。 此命令必須由您安裝執行 Windows Server 2012 的網域控制站之網域的 Domain Admins 群組成員執行。 若要成功完成這個命令，執行命令的電腦與網域的基礎結構主機之間必須連線。  
