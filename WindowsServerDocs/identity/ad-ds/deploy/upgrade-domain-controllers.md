...
1. 確認目標伺服器符合系統需求。
1. 驗證應用程式相容性。
1. 瞭解移至 Windows Server 2016 的建議
1. 確認安全性設定。 如需詳細資訊, 請參閱[與 Windows Server 2016 中的 AD DS 相關的已淘汰功能和行為變更](../../../get-started/deprecated-features.md)。
1. 檢查要執行安裝的電腦與目標伺服器的連線。
1. 檢查必要操作主機角色的可用性：
   - 若要在現有網域和樹系中安裝執行 Windows Server 2016 的第一個 DC, 執行安裝的電腦必須連線到**架構主機**才能執行 adprep/forestprep 和基礎結構主機, 以便執行 adprep/domainprep.
   - 若要在已擴充樹系架構的網域中安裝第一個 DC, 您只需要連接到**結構主機**。
   - 若要安裝或移除現有樹系中的網域, 您需要連接到**網域命名主機**。
   - 任何網域控制站安裝也需要**RID 主機**的連線。
   - 如果您要在現有樹系中安裝第一個唯讀網域控制站, 您需要連接到每個應用程式目錄分割的**結構主機**, 也稱為非網域命名內容或 NDNC。

### <a name="installation-steps-and-required-administrative-levels"></a>安裝步驟和必要的系統管理層級

下表提供完成這些步驟的升級步驟和許可權需求的摘要
...
如需 Windows Server 2016 新功能的詳細資訊, 請參閱[Windows server 2016 的新](../../../get-started/what-s-new-in-windows-server-2016.md)功能。

## <a name="supported-in-place-upgrade-paths"></a>支援的就地升級路徑

執行64位版本的 Windows Server 2012 或 Windows Server 2012 R2 的網域控制站可以升級至 Windows Server 2016。 僅支援64位版本升級, 因為 Windows Server 2016 僅隨附于64位版本。
...
如需所支援升級路徑的詳細資訊, 請參閱[支援的升級路徑](../../../get-started/supported-upgrade-paths.md)

## <a name="adprep-and-domainprep"></a>Adprep 和域

如果您要就地將現有的網域控制站升級至 Windows Server 2016 作業系統, 您必須手動執行 adprep/forestprep 和 adprep/domainprep。  Adprep/forestprep 只需要在樹系中執行一次。  在您要升級至 Windows Server 2016 的網域控制站所在的每個網域中, 都必須執行 Adprep/domainprep 一次。

如果您要升級新的 Windows Server 2016 伺服器, 則不需要手動執行。  這些會整合到 PowerShell 中, 並伺服器管理員體驗。

如需執行 adprep 的詳細資訊, 請參閱執行[adprep](https://technet.microsoft.com/library/dd464018.aspx)
...
您必須先移除 windows 2000 網域控制站, 才能將 Windows Server 2016 網域控制站新增至您的樹系。 在這種情況下，請考慮下列工作流程：

1. 安裝執行 Windows Server 2003 或更新版本的網域控制站。 可以在評估版的 Windows Server 上部署這些網域控制站。 此步驟也需要針對該作業系統版本執行 adprep 做為必要條件。
...
將網域功能等級設定為某個值之後, 就無法回復或降低網域功能等級, 但有下列例外狀況:

- 當您將網域功能等級提高至 Windows Server 2016 時, 如果樹系功能等級是 Windows Server 2012 或更低版本, 您可以選擇將網域功能等級回復為 Windows Server 2012 或 Windows Server 2012 R2。
...
## <a name="administration-of-windows-server-2016-servers"></a>Windows Server 2016 伺服器的管理

使用適用于 Windows 10 的遠端伺服器管理工具來管理網域控制站和執行 Windows Server 2016 的其他伺服器。 您可以在執行 Windows 10 的電腦上執行 Windows Server 2016 遠端伺服器管理工具。
...
1. 在**伺服器管理員**的 [新增**角色及功能**] 底下, 在新的 Windows Server 2016 上安裝**Active Directory Domain Services** 。 這會自動在 2012 R2 樹系和網域上執行 adprep。

   ![升級](media/Upgrade-Domain-Controllers-to-Windows-Server-2016/upgrade3.png)

1. 在**伺服器管理員**中, 按一下黃色三角形, 然後從下拉式下按一下 [將**伺服器升級為網域控制站**]。

   ![升級](media/Upgrade-Domain-Controllers-to-Windows-Server-2016/upgrade4.png)

1. 在 [**部署**設定] 畫面上, 選取 [**將網域控制站新增至現有樹**系], 然後按 [下一步]。
...
1. 在 PowerShell 視窗中, 使用移動 ADDirectoryServerOperationMasterRole 來移動 FSMO 角色。 您可以輸入每個 OperationMasterRole 的名稱, 或使用數位來指定角色。 如需詳細資訊, 請參閱[移動-ADDirectoryServerOperationMasterRole](https://technet.microsoft.com/library/hh852302.aspx)
...
1. 前往 Windows Server 2016 伺服器以確認角色已移動, 在**伺服器管理員**的 [**工具**] 底下, 選取 [**適用于 Windows PowerShell 的 Active Directory 模組**]。 `Get-ADDomain`使用和`Get-ADForest` Cmdlet 來查看 FSMO 角色持有者。
...
1. 降級和移除 Windows Server 2012 R2 網域控制站。 如需降級 dc 的詳細資訊, 請參閱將[網域控制站和網域降級](../../ad-ds/deploy/Demoting-Domain-Controllers-and-Domains--Level-200-.md)
1. 一旦伺服器降級並移除之後, 您就可以將樹系功能和網域功能等級提升至 Windows Server 2016。
...
- [安裝 Active Directory Domain Services &#40;層級100&#41;](../../ad-ds/deploy/Install-Active-Directory-Domain-Services--Level-100-.md)
