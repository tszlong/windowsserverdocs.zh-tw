...
1. 在 PowerShell 視窗中，使用移動 ADDirectoryServerOperationMasterRole 來移動 FSMO 角色。 您可以輸入每個 OperationMasterRole 的名稱，或使用數位來指定角色。 如需詳細資訊，請參閱[移動-ADDirectoryServerOperationMasterRole](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/dd464018(v=ws.10))
...
1. 前往 Windows Server 2016 伺服器以確認角色已移動，在**伺服器管理員**的 [**工具**] 底下，選取 [**適用于 Windows PowerShell 的 Active Directory 模組**]。 使用 `Get-ADDomain` 和 `Get-ADForest` Cmdlet 來查看 FSMO 角色持有者。
