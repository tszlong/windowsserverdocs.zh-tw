...
知識一致性檢查程式（KCC）會自動建立連線物件，但也可以手動建立。 KCC 所建立的連線物件會出現在 [Active Directory 網站和服務] 嵌入式管理單元中，如 **<automatically generated>** ，而且在正常作業條件下會被視為適當。 系統管理員所建立的連線物件會以手動方式建立連線物件。 手動建立的連線物件是由系統管理員在建立時指派的名稱所識別。 當您修改 **@no__t 1**連線物件時，會將它轉換成系統管理員修改的連線物件，並以 GUID 的形式顯示該物件。 KCC 不會對手動或修改的連線物件進行變更。  
...
對於 Windows Server 2003 網域控制站，您可以從 Windows Server 2003 分公司部署指南（[https://go.microsoft.com/fwlink/?LinkID=28523](https://go.microsoft.com/fwlink/?LinkID=28523)）使用如 Adlb 的工具來重新平衡工作負載。  
