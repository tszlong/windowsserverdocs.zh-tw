...
ms.locfileid: "59848099"
---
# <a name="whats-new-in-hyper-v-on-windows-server-2016"></a>在 Windows Server 2016 上的 HYPER-V 中最新消息

>適用於：Microsoft HYPER-V Server 2016、windows Server 2016
  
這篇文章說明在 Windows Server 2016 和 Microsoft HYPER-V Server 2016 的新功能和變更功能的 HYPER-V。 若要使用虛擬機器使用 Windows Server 2012 R2 建立和移動或匯入到 Windows Server 2016 執行 HYPER-V 的伺服器上的新功能，您必須手動升級虛擬機器設定版本。 如需相關指示，請參閱 <<c0> [ 升級的虛擬機器版本](deploy/Upgrade-virtual-machine-version-in-Hyper-V-on-Windows-or-Windows-Server.md)。  
  
以下是這篇文章中包含的內容，以及功能是新的或更新。  

## <a name="BKMK_standby"></a>與連線待命相容\(新\)
使用 Always on/always Connected (AOAC) 電源模式的電腦上安裝 HYPER-V 角色時**連線待命**電源狀態現已推出。  
  
## <a name="BKMK_device"></a>不連續的裝置指派\(新\) 
...
## <a name="BKMK_hot"></a>熱新增和移除網路介面卡和記憶體\(新\) 
...
## <a name="BKMK_Mgmt"></a>HYPER-V 管理員改善\(更新\) 
  
-   **備用認證支援**-您現在可以使用一組不同的認證在 HYPER-V 管理員中當您連接到另一部 Windows Server 2016 或 Windows 10 的遠端主機。 您也可以儲存這些認證可讓您更輕鬆地再次登入。  
  
-   **管理舊版**-使用 HYPER-V 管理員在 Windows Server 2016 和 Windows 10 中，您可以管理 Windows Server 2012 中，Windows 8、 Windows Server 2012 R2 和 Windows 8.1 上執行 HYPER-V 的電腦。  
...
## <a name="BKMK_IS"></a>透過 Windows Update 的整合服務傳遞\(更新\) 
...
## <a name="BKMK_linux"></a>Linux 安全開機\(新\) 
...
## <a name="BKMK_nested"></a>巢狀虛擬化\(新\) 
這項功能可讓您使用做為 HYPER-V 主機的虛擬機器，並建立該虛擬化主機的虛擬機器。 這可以是特別適用於開發和測試環境。 若要使用巢狀虛擬化，您將需要：  
  
-   若要在至少執行 Windows Server 2016 或實體的 HYPER-V 主機和虛擬化的主機上的 Windows 10。  
...
## <a name="BKMK_networking"></a>網路功能\(新\) 
新的網路功能包括：  
...
## <a name="BKMK_HyperVRollingUpgrades"></a>復原 HYPER-V 叢集升級\(新\) 
您現在可以新增 HYPER-V 叢集到執行 Windows Server 2016 與執行 Windows Server 2012 R2 節點的節點。 這可讓您升級叢集，而不需要停機。 叢集會在 Windows Server 2012 R2 的功能層級執行，直到您升級叢集中所有節點，然後使用 Windows PowerShell cmdlet 來更新叢集功能等級[Update-clusterfunctionallevel](https://docs.microsoft.com/powershell/module/failoverclusters/Update-ClusterFunctionalLevel)。  
  
> [!IMPORTANT]  
> 更新叢集功能等級之後，您無法將它返回到 Windows Server 2012 R2。  
  
針對 Windows Server 2012 R2 的功能等級，與執行 Windows Server 2012 R2 和 Windows Server 2016 的節點的 HYPER-V 叢集，請注意下列各項：  
  
-   管理叢集、 HYPER-V 和虛擬機器從執行 Windows Server 2016 或 Windows 10 的節點。  
  
-   您可以在所有 HYPER-V 叢集中的節點之間都移動虛擬機器。  
  
-   若要使用新的 HYPER-V 功能，所有節點都必須都執行 Windows Server 2016，而且必須升級叢集功能等級。  
...
## <a name="BKMK_shared"></a>共用虛擬硬碟\(更新\)
...
## <a name="BKMK_shielded"></a>受防護的虛擬機器\(新\)
...
## <a name="BKMK_StartOrder"></a>啟動叢集虛擬機器的優先順序\(新\)
...
.  
## <a name="BKMK_QoS"></a>儲存體服務品質 (QoS)\(更新\)
...
## <a name="BKMK_Config"></a>虛擬機器設定檔案格式\(更新\)
...
## <a name="BKMK_ConfgVersion"></a>虛擬機器設定版本\(更新\)
版本代表虛擬機器的組態，以儲存狀態，以及快照集檔案的 HYPER-V 版本的相容性。 第 5 版的虛擬機器與 Windows Server 2012 R2 相容，並可在 Windows Server 2012 R2 和 Windows Server 2016 上執行。 Windows Server 2012 R2 上，在 HYPER-V 中執行不會與 Windows Server 2016 中引進的版本的虛擬機器。   
  
如果您移動或匯入到執行 Windows Server 2016 的 HYPER-V 從 Windows Server 2012 R2 的伺服器的虛擬機器，虛擬機器的設定不自動更新。 這表示您可以將虛擬機器移回執行 Windows Server 2012 R2 的伺服器。 但是，這也表示您無法使用新的虛擬機器功能，直到您手動更新虛擬機器組態的版本。  
...
## <a name="BKMK_Containers"></a>Windows 容器\(新\) 
...
## <a name="BKMK_PowerShellDirect"></a>Windows PowerShell Direct\(新\)  
