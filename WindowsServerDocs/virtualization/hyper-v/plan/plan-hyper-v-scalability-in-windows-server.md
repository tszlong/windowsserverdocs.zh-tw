...
|Memory|第2代的 12 TB; <br>1 TB 的層代1|檢閱特定作業系統的需求，以判定需求量下限及建議的需求量。|
|序列 (COM) 埠|2|無。|
...
|虛擬處理器|240代表層代 2;<br>適用于層代1的 64;<br>320可供主機 OS (根磁碟分割) |客體作業系統支援的虛擬處理器數目可能較低。 如需詳細資訊，請參閱針對特定作業系統發佈的資訊。|
...
|每一伺服器執行的虛擬機器|1024|無。|
|儲存體|受限於主機作業系統支援的功能。 Hyper-V 並無強制限制。|**注意：** 當使用 SMB 3.0 時，Microsoft 支援網路連接存放裝置 (NAS) 。 不支援 NFS 型存放裝置。|
