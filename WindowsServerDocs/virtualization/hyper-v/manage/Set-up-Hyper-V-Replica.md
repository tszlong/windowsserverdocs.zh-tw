...
**測試容錯移轉**:如果您想要執行測試容錯移轉以滑鼠右鍵按一下主要虛擬機器，然後選取**複寫** > **測試容錯移轉**。 如果設定，請挑選最新或其他復原點。 將建立新的測試虛擬機器，並將其啟動次要站台上。 您已完成測試之後，請選取**停止測試容錯移轉**清除選項，則複本虛擬機器上。 請注意，您只能執行一個虛擬機器測試容錯移轉一次。 [深入了解](https://blogs.technet.com/b/virtualization/archive/2012/07/26/types-of-failover-operations-in-hyper-v-replica.aspx)。  
