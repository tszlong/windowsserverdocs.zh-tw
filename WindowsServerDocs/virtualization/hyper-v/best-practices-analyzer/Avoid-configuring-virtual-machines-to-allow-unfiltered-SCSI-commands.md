...
@no__t 0Bypassing SCSI 命令篩選會帶來安全性風險。只有在與客體作業系統中執行的存放裝置應用程式相容時，才需要啟用此設定。下列虛擬機器已設定為允許未篩選的 SCSI 命令： *  
  
@no__t 0list 的虛擬機器名稱 >  
  
## <a name="resolution"></a>解析度  
  
@no__t 0Contact 存放裝置廠商，以判斷是否需要此設定。此外，如果管理作業系統或其他客體作業系統遭到入侵或呈現不尋常的行為，請重新設定虛擬機器以封鎖命令。 *  
