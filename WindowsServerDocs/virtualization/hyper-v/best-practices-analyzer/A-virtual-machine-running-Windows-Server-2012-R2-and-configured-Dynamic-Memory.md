---
title: 執行 Windows Server 2012 R2 並設定動態記憶體的虛擬機器應該使用建議的記憶體設定值
...
ms.locfileid: "71366606"
---
# <a name="a-virtual-machine-running-windows-server-2012-r2-and-configured-with-dynamic-memory-should-use-recommended-values-for-memory-settings"></a>執行 Windows Server 2012 R2 並設定動態記憶體的虛擬機器應該使用建議的記憶體設定值
...
## <a name="issue"></a>問題  
*一或多部虛擬機器已設定為使用動態記憶體，且小於建議用於 Windows Server 2012 R2 的記憶體數量。*  
...
*使用 Hyper-v 管理員將最小記憶體增加到至少 256 MB，啟動記憶體至少為 512 MB，最大記憶體為 2 GB （針對此虛擬機器）。*  
