---
title: 執行 Windows Vista，並使用動態記憶體設定的虛擬機器應該用於記憶體設定的建議的值
...
ms.locfileid: "59850579"
---
# <a name="a-virtual-machine-running-windows-vista-and-configured-with-dynamic-memory-should-use-recommended-values-for-memory-settings"></a>執行 Windows Vista，並使用動態記憶體設定的虛擬機器應該用於記憶體設定的建議的值
...
## <a name="issue"></a>問題  
*一或多個虛擬機器會設定為使用動態記憶體少於建議適用於 Windows Vista 的記憶體數量。*  
...
*使用 HYPER-V 管理員] 或 [Windows PowerShell 來增加至少 256 MB，為至少 512 MB 的啟動記憶體和最大記憶體為至少 1 GB 的記憶體下限。*  
