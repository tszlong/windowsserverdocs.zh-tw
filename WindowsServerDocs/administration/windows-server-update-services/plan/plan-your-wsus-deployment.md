...
|工作|描述|
|----|--------|
|[1.1. 審查考慮和系統需求](#11-review-considerations-and-system-requirements)|檢查考量及系統需求清單，確保您已準備好部署 WSUS 所需的所有硬體及軟體。|
...
|[1.6. 規劃 WSUS 效能考慮：背景智慧型傳送服務](#16-plan-wsus-performance-considerations)|規劃 WSUS 設計以取得最佳效能。|
...
-   啟用 WSUS 角色的伺服器硬體需求會與硬體需求合併在一起。 WSUS 的最小硬體需求：
    ...
    -   **處理器：** 1.4 ghz x64 處理器（建議使用 2 ghz 或更快）

    -   **記憶體：** 除了伺服器和所有其他服務或軟體所需的 RAM，WSUS 還需要額外的 2 GB RAM。

    -   **可用磁碟空間：** 10 GB （建議使用 40 gb 或以上）

    -   **網路介面卡：** 100 百萬位元/秒（Mbps）或更高
...
1.  如果上游伺服器已設定為以語言子集下載更新檔案：在 [WSUS 設定精靈] 中，按一下 [只下載下列語言的更新 (上游伺服器只支援以星號標示的語言)]，然後選取您要更新的語言。

> [!NOTE]
> 即使您希望下游伺服器下載與上游伺服器相同的語言，您仍然應該執行這個動作。

2. 如果上游伺服器已設定為以所有語言下載更新檔案：在 [WSUS 設定精靈] 中，按一下 [以所有語言下載上游伺服器支援的更新]。
