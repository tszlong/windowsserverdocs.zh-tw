---
title: 規劃 WSUS 部署
description: Windows Server Update Service （WSUS）主題-部署規劃程式的總覽，其中包含相關主題的連結
...
ms.locfileid: "71369843"
---
# <a name="plan-your-wsus-deployment"></a>規劃 WSUS 部署

>適用於：Windows Server （半年通道）、Windows Server 2016、Windows Server 2012 R2、Windows Server 2012

做出重要決定是部署 Windows Server Update Services (WSUS) 的第一步，例如決定 WSUS 部署案例、選擇網路拓撲以及了解系統需求。 下列檢查清單摘要說明準備部署的相關步驟。

|工作|描述|
|----|--------|
|[1.1。審查考慮和系統需求 @ no__t-0|檢查考量及系統需求清單，確保您已準備好部署 WSUS 所需的所有硬體及軟體。|
|[1.2。選擇 WSUS 部署案例 @ no__t-0|決定要使用哪個 WSUS 部署案例。|
|[1.3。選擇 WSUS 儲存策略 @ no__t-0|決定哪個 WSUS 儲存策略最適合您的部署。|
|[1.4。選擇 WSUS 更新語言 @ no__t-0|決定要安裝哪種 WSUS 更新語言。|
|[1.5。規劃 WSUS 電腦群組 @ no__t-0|規劃部署將使用的 WSUS 電腦群組方式。|
|[1.6。規劃 WSUS 效能考慮：背景智慧型傳送服務 @ no__t-0|規劃 WSUS 設計以取得最佳效能。|
|[1.7。規劃自動更新設定 @ no__t-0|規劃如何針對您的案例設定自動更新設定。|
...
-   啟用 WSUS 角色的伺服器硬體需求會與硬體需求合併在一起。 WSUS 的最小硬體需求：
    ...
    -   **Processor**1.4 ghz x64 處理器（建議使用 2 Ghz 或更快）

    -   **快閃記憶體**除了伺服器和所有其他服務或軟體所需的 RAM，WSUS 還需要額外的 2 GB RAM。

    -   **可用的磁碟空間：** 10 GB （建議使用 40 GB 或以上）

    -   **網路介面卡：** 100百萬位元/秒（Mbps）或更高

-   軟體需求：

    -   如需檢視報表，WSUS 需要 [Microsoft 報表檢視器可轉散發套件 2008](https://www.microsoft.com/download/details.aspx?id=6576)。 在 Windows Server 2016 上，WSUS 需要[Microsoft Report Viewer 執行時間 2012](https://www.microsoft.com/download/details.aspx?id=35747)

-   如果您安裝的角色或軟體更新需要在安裝完成時重新啟動伺服器，請重新啟動伺服器，然後再啟用 WSUS 伺服器角色。

-   Microsoft .NET Framework 4.0 必須安裝在將安裝 WSUS 伺服器角色的伺服器上。
...
1. 無法視需要從 Microsoft Update 安裝已使用功能隨選安裝移除其裝載的伺服器角色。 您必須在嘗試安裝這類伺服器角色時提供安裝來源，或在群組原則中設定功能的來源。

2. Windows 用戶端版本無法視需要從網站上安裝 .NET 3.5。 伺服器角色的考量同樣適用於 .NET 3.5。

   > [!NOTE]
   > 設定功能隨選安裝來源不包含 WSUS。 如需如何設定功能的相關資訊，請參閱在[Windows Server 中設定功能隨選](https://technet.microsoft.com/library/jj127275.aspx)安裝。

3. 執行 Windows 10 版本1709或1803版的企業裝置，無法直接從 WSUS 安裝任何功能。 若要視需要安裝功能，請[建立功能檔案（並存存放區）](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj127275%28v=ws.11%29#create-a-feature-file-or-side-by-side-store) ，或從下列其中一個來源取得隨選套件功能：
   - [大量授權服務中心](https://www.microsoft.com/licensing/servicecenter)（VLSC）-必須存取 VL
   - OEM 入口網站-需要 OEM 存取
   - MSDN 下載-需要 MSDN 訂用帳戶

     您可以使用[DISM 命令列選項](https://docs.microsoft.com/windows-hardware/manufacture/desktop/dism-operating-system-package-servicing-command-line-options)來安裝個別取得的隨選功能套件。
...
> [!NOTE]
> SQL Server Express 2008 R2 的資料庫大小限制為 10 GB。 這個資料庫大小對 WSUS 而言應該很足夠，但是使用這個資料庫並沒有比使用 WID 有任何明顯的好處。 WID 資料庫的 RAM 記憶體需求最低為 2 GB，超過標準的 Windows Server 系統需求。
...
最基本的 WSUS 部署是由公司防火牆內的伺服器所組成，可在私人內部網路上服務用戶端電腦。 WSUS 伺服器會連線到 Microsoft Update 下載更新。 這稱為「同步處理」。 在同步處理期間，WSUS 會判斷上次同步處理後是否有任何新的更新。 如果您是第一次同步處理 WSUS，所有更新都可以下載。

> [!NOTE]
> 第一次同步處理可能需要 1 小時。 後續的所有同步處理都會快速許多。

根據預設，WSUS 伺服器會為 HTTP 通訊協定使用連接埠 80、為 HTTPS 通訊協定使用連接埠 443，從 Microsoft 取得更新。 如果您的網路與網際網路之間設有公司防火牆，您必須在與 Microsoft Update 直接通訊的伺服器上開啟這些連接埠。 如果您打算使用自訂埠來進行這種通訊，則必須改為開啟這些埠。 您可以設定多部 WSUS 伺服器與父 WSUS 伺服器進行同步處理。 根據預設，WSUS 伺服器會為 HTTP 通訊協定使用連接埠 8530、為 HTTPS 通訊協定使用連接埠 8531，提供用戶端工作站的更新。

### <a name="multiple-wsus-servers"></a>多部 WSUS 伺服器
系統管理員可以部署多部執行 WSUS 的伺服器，以同步處理其組織內部網路中的所有內容。 您可能只會將一部伺服器公開到網際網路，這會是從 Microsoft Update 下載更新的唯一伺服器。 此伺服器會設定為上游伺服器，下游伺服器會在該來源中同步處理。 在適用的情況下，伺服器可以位於地理位置分散的各個網路，為所有用戶端電腦提供最佳的連線能力。
...
> [!NOTE] 
> 建議您不要建立三層以上的 WSUS 伺服器階層。 每增加一個階層，就會增加在連線的伺服器上傳播更新的時間。 雖然階層沒有理論上的限制，但只有有五個層級階層的部署已由 Microsoft 測試過。
>
> 此外，下游伺服器必須與上游伺服器同步處理來源位於相同版本或舊版的 WSUS。
...
1.  有助於降低 WAN 連結的使用率，並改善應用程式的回應能力。 若要啟用 WSUS 伺服器服務的內容 BranchCache 加速功能，請在伺服器和用戶端上安裝 BranchCache 功能，並確認 BranchCache 服務已經啟動。 不需要其他任何步驟。
...
網路負載平衡 (NLB) 提高了 WSUS 網路的可靠性及效能。 您可以設定多部 WSUS 伺服器，以共用執行 SQL Server 的單一容錯移轉叢集，例如 SQL Server 2008 R2 SP1。 在這個設定中，您必須使用完整的 SQL Server 安裝而不是 WSUS 提供的 Windows 內部資料庫安裝，而且必須在所有 WSUS 前端伺服器安裝資料庫角色。 您也可以讓所有 WSUS 伺服器使用分散式檔案系統 (DFS) 儲存內容。

適用于 nlb 的**wsus 設定：** 相較于 NLB 的 wsus 3.2 設定，已不再需要特殊的安裝程式呼叫和參數來設定適用于 NLB 的 wsus。 您只需要設定每部 WSUS 伺服器，請記住下列考慮。
...
如果網路包含從其他位置登入網路的行動使用者，您可以設定 WSUS 讓漫遊使用者從地理位置最靠近他們的 WSUS 伺服器更新用戶端電腦。 例如，您可以在每個區域部署一個 WSUS 伺服器，並針對每個區域使用不同的 DNS 子網。 所有用戶端電腦都可以導向至相同的 WSUS 伺服器，這會在每個子網中解析成最接近的實體 WSUS 伺服器。
...
WSUS 要求每個 WSUS 伺服器都要有一個資料庫。 WSUS 支援使用位於 WSUS 伺服器以外不同電腦上的資料庫，但有一些限制。 如需支援的資料庫清單和遠端資料庫限制，請參閱本指南中的「1.1 檢查初始考慮和系統需求」一節。
...
> [!NOTE]
> 請不要透過直接存取資料庫的方式來管理 WSUS。 直接運算元據庫可能會造成資料庫損毀。 這種毀損不一定馬上看得出來，但是將無法升級到下一版產品。 您可以使用 WSUS 主控台或 WSUS 應用程式開發介面 (API) 來管理 WSUS。
...
-   [備份和還原 WSUS 資料並備份您的伺服器](https://technet.microsoft.com/library/dd939904(WS.10).aspx)

-   [重新編制 WSUS 資料庫的索引](https://technet.microsoft.com/library/dd939795(WS.10).aspx)
...
這個選項需要伺服器擁有足夠的磁碟空間以儲存所需的所有更新。 WSUS 至少需要 20 GB 才能在本機儲存更新;不過，我們建議以經過測試的變數為基礎的 30 GB。
...
WSUS 組態精靈中的 [選擇語言] 頁面能讓您取得所有語言或語言子集的更新。 選取語言子集可以節省磁碟空間，但請務必選擇 WSUS 伺服器的所有下游伺服器和用戶端電腦所需的所有語言。

以下是您在設定此選項之前，應該記住的更新語言的一些重要注意事項：
...
在要求語言的用戶端電腦上，更新將會顯示為 [不適用]。 若要避免這個情況，請確定您的 WSUS 伺服器同步處理選項中已包含所有的作業系統語言。 您可以前往 WSUS 管理主控台的 [**電腦**] 視圖，並依作業系統語言排序電腦，以查看所有的作業系統語言。 不過，如果 Microsoft 應用程式有多種語言（例如，如果在某些使用英文版 Windows 8 的電腦上安裝法文版的 Microsoft Word），您可能會想要包含更多語言。
...
電腦一律會指派給 [**所有電腦**] 群組，而且它們仍會指派給 [未指派的**電腦**] 群組，直到您將它們指派給另一個群組為止。 電腦可以屬於多個群組。
...
因為可以將電腦指派到多個群組，所以可能為相同的電腦多次核准單一更新。 不過，只能部署更新一次，而且 WSUS 伺服器會解決任何衝突。 若要繼續進行前一個範例，如果將 computerA 指派給薪資群組和 Accounts 賬款群組，而同時核准這兩個群組的 Update1，則只會部署一次。
...
在下列 [更新服務] 主控台階層窗格的文字範例中，針對名為 WSUS-01 的 WSUS 伺服器，將名為桌上型電腦和伺服器的電腦群組新增到預設的 [**所有電腦**] 群組。 桌上型電腦和伺服器群組都位於相同的階層式層級。
...
在此範例中，桌上型電腦分支（桌面 L2）底下的群組兩個層級，其優先順序高於伺服器分支底下的群組一層（伺服器 L1）。 因此，若電腦在 Desktops-L2 和 Servers-L1 群組皆具有成員資格，則 Desktops-L2 群組的所有動作皆優先於為 Servers-L1 群組指定的動作。
...
在部署 WSUS 之前，您應該仔細規劃一些領域，讓您可以獲得最佳效能。 這些重要的方面包括：
...
如果部署連線的 WSUS 伺服器階層，建議不要建立太多層的巢狀伺服器。 如果您啟用延後下載，而下游伺服器要求上游伺服器未核准的更新，下游伺服器的要求會在上游伺服器上強制進行下載。 接著下游伺服器會在後續同步處理中下載更新。 在層數過多的 WSUS 伺服器階層中，會在要求、下載，然後透過伺服器階層傳遞更新時，發生延遲。 根據預設，將更新儲存在本機時會啟用延後下載。 您可以手動變更這個選項。
...
這項功能有時稱為「差異傳遞」，因為它只會下載兩個檔案版本之間的差異（差別）。 快速安裝檔案大於散佈到用戶端電腦的更新，因為快速安裝檔案包含每個檔案要更新的所有可能版本。
...
1.  使用背景智慧型傳送服務 (BITS) 節流。 BITS 頻寬限制可以日期時間進行控制，但是它們會套用到所有使用 BITS 的應用程式。 若要瞭解如何控制 BITS 節流，請參閱[群組原則](https://msdn.microsoft.com/library/windows/desktop/aa362844(v=vs.85).aspx)。
