---
title: 規劃 WSUS 部署
description: Windows Server Update Service (WSUS) 主題-規劃程序的相關主題的連結部署的概觀
...
ms.locfileid: "66222901"
---
# <a name="plan-your-wsus-deployment"></a>規劃 WSUS 部署

>適用於：Windows Server （半年通道），Windows Server 2016 中，Windows Server 2012 R2 中，Windows Server 2012

做出重要決定是部署 Windows Server Update Services (WSUS) 的第一步，例如決定 WSUS 部署案例、選擇網路拓撲以及了解系統需求。 下列檢查清單摘要說明在準備部署所涉及的步驟。
...
-   啟用 WSUS 角色的伺服器硬體需求會與硬體需求合併在一起。 WSUS 的最小硬體需求：
    ...
    -   **處理器：** 1.4 ghz x64 處理器 (2 Ghz 或更快建議)

    -   **記憶體：** 超過所需的伺服器和所有其他服務或軟體，WSUS 還需要額外的 2 GB 的 RAM。

    -   **可用磁碟空間：** 10 GB (40 GB 或更高的建議使用)

    -   **網路介面卡：** 100 mb / 秒 (Mbps) 或更新版本

-   軟體需求：

    -   如需檢視報表，WSUS 需要 [Microsoft 報表檢視器可轉散發套件 2008](https://www.microsoft.com/download/details.aspx?id=6576)。 在 Windows Server 2016 中，WSUS 還需要[Microsoft 報表檢視器執行階段 2012年](https://www.microsoft.com/download/details.aspx?id=35747)

-   如果您安裝的角色或軟體更新需要在安裝完成時重新啟動伺服器，請重新啟動伺服器，然後再啟用 WSUS 伺服器角色。

-   必須安裝 WSUS 伺服器角色的伺服器上安裝 Microsoft.NET Framework 4.0。
...
1.  無法視需要從 Microsoft Update 安裝已使用功能隨選安裝移除其裝載的伺服器角色。 您必須在您嘗試安裝這類伺服器角色中，階段提供安裝來源，或設定功能隨群組原則中的來源。

2.  Windows 用戶端版本無法視需要從網站上安裝 .NET 3.5。 伺服器角色的考量同樣適用於 .NET 3.5。

    > [!NOTE]
    > 設定功能隨選安裝來源並不包含 WSUS。 如需如何設定功能的資訊，請參閱[設定的功能，在 Windows Server 中以隨](https://technet.microsoft.com/library/jj127275.aspx)。

3. 執行 Windows 10 1709年版或版本 1803、 企業版裝置無法安裝任何功能，視需要直接從 WSUS。 依需求、 安裝的功能[建立功能檔案 （並排顯示存放區）](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj127275%28v=ws.11%29#create-a-feature-file-or-side-by-side-store)或從下列來源之一取得隨選封裝上的功能：
    - [Volume Licensing Service Center](https://www.microsoft.com/licensing/servicecenter) (VLSC)-VL 存取是否必要
    - OEM 入口網站-OEM 存取必要
    - MSDN 訂用帳戶則需要 MSDN 下載-

    可以使用安裝個別取得隨選套件的功能[DISM 命令列選項](https://docs.microsoft.com/windows-hardware/manufacture/desktop/dism-operating-system-package-servicing-command-line-options)。
...
> [!NOTE]
> SQL Server Express 2008 R2 的資料庫大小限制為 10 GB。 這個資料庫大小對 WSUS 而言應該很足夠，但是使用這個資料庫並沒有比使用 WID 有任何明顯的好處。 WID 資料庫有標準的 Windows Server 系統需求超過 2 gb 最小 RAM 記憶體需求。
...
3.  資料庫伺服器必須位於相同的 active directory 網域與 WSUS 伺服器，或者必須與 WSUS 伺服器的 active directory 網域的信任關係。

4.  WSUS 伺服器和資料庫伺服器必須位於相同的時區，或同步處理至相同的國際標準時間 （格林威治標準時間） 來源。
...
最基本的 WSUS 部署是由公司防火牆內部的私人內部網路上提供用戶端電腦的伺服器所組成。 WSUS 伺服器會連線到 Microsoft Update 下載更新。 這稱為「同步處理」  。 在同步處理期間，WSUS 會判斷上次同步處理後是否有任何新的更新。 如果您是第一次同步處理 WSUS，所有更新都可以下載。

> [!NOTE]
> 第一次同步處理可能需要 1 小時。 後續的所有同步處理都會快速許多。

根據預設，WSUS 伺服器會為 HTTP 通訊協定使用連接埠 80、為 HTTPS 通訊協定使用連接埠 443，從 Microsoft 取得更新。 如果您的網路與網際網路之間設有公司防火牆，您必須在與 Microsoft Update 直接通訊的伺服器上開啟這些連接埠。 如果您打算使用自訂連接埠進行這項通訊，您必須改為開啟這些連接埠。 您可以設定多部 WSUS 伺服器與父 WSUS 伺服器進行同步處理。 根據預設，WSUS 伺服器會為 HTTP 通訊協定使用連接埠 8530、為 HTTPS 通訊協定使用連接埠 8531，提供用戶端工作站的更新。

### <a name="multiple-wsus-servers"></a>多部 WSUS 伺服器
系統管理員可以部署多部執行其組織的內部網路中的所有內容同步都處理的 WSUS 伺服器。 您可能會公開到網際網路，這會是唯一的伺服器，從 Microsoft Update 下載更新的只有一部伺服器。 此伺服器是設定為上游伺服器的下游伺服器同步處理的來源。 在適用的情況下，伺服器可以位於地理位置分散的各個網路，為所有用戶端電腦提供最佳的連線能力。
...
> [!NOTE] 
> 建議您不要建立三層以上的 WSUS 伺服器階層。 每增加一個階層，就會增加在連線的伺服器上傳播更新的時間。 雖然理論上沒有限制的階層架構，Microsoft 都已經過測試有五個層級深度的階層架構的部署。
>
> 此外，下游伺服器必須位於相同的版本或較早版本的 WSUS 做為上游伺服器同步處理來源。
...
1.  有助於降低 wan 使用率並提升應用程式回應速度。 若要啟用 WSUS 伺服器服務的內容 BranchCache 加速功能，請在伺服器和用戶端上安裝 BranchCache 功能，並確認 BranchCache 服務已經啟動。 不需要其他任何步驟。
...
**適用於 NLB 的 WSUS 安裝：** 相較之下 WSUS 3.2 安裝 NLB，特殊的安裝程式呼叫和參數都不再需要 NLB 設定 WSUS。 您只需要設定每個 WSUS 伺服器，並記住下列考量。
...
如果網路包含從其他位置登入網路的行動使用者，您可以設定 WSUS 讓漫遊使用者從地理位置最靠近他們的 WSUS 伺服器更新用戶端電腦。 比方說，您可能會部署一部 WSUS 伺服器的每個區域，並針對每個區域使用不同的 DNS 子網路。 所有用戶端電腦都可以導向到相同的 WSUS 伺服器會解析至最接近的實體 WSUS 伺服器的每個子網路中。
...
WSUS 要求每個 WSUS 伺服器都要有一個資料庫。 WSUS 支援使用位於 WSUS 伺服器以外不同電腦上的資料庫，但有一些限制。 如需支援的資料庫和遠端資料庫限制，請參閱本節"1.1 檢查初始考量及系統需求，「 本指南中。
...
> [!NOTE]
> 請不要透過直接存取資料庫的方式來管理 WSUS。 直接操控資料庫可能會造成資料庫損毀。 這種毀損不一定馬上看得出來，但是將無法升級到下一版產品。 您可以使用 WSUS 主控台或 WSUS 應用程式開發介面 (API) 來管理 WSUS。
...
> [!NOTE]
> Windows Server 2008 中引進了 Windows 內部資料庫 (WID)。
...
-   [備份和還原 WSUS 資料以及備份伺服器](https://technet.microsoft.com/library/dd939904(WS.10).aspx)

-   [WSUS 資料庫重新建立索引](https://technet.microsoft.com/library/dd939795(WS.10).aspx)
...
這個選項需要伺服器擁有足夠的磁碟空間以儲存所需的所有更新。 至少，WSUS 需要 20 GB 來儲存在本機; 更新不過，我們建議根據已測試的變化的 30 GB。
...
WSUS 組態精靈中的 [選擇語言]  頁面能讓您取得所有語言或語言子集的更新。 選取語言子集可以節省磁碟空間，但選擇所需的所有下游伺服器和用戶端電腦的 WSUS 伺服器的所有語言的重要事項。

以下是更新語言的一些重要事項，您應該記住之前設定此選項：
...
在要求語言的用戶端電腦上，更新將會顯示為 [不適用]  。 若要避免這個情況，請確定您的 WSUS 伺服器同步處理選項中已包含所有的作業系統語言。 您可以看到所有的作業系統語言，方法是前往**電腦**WSUS 管理主控台，並根據作業系統語言排序電腦的檢視。 不過，您可能想要包含更多語言，如果有多個語言的 Microsoft 應用程式 （例如，如果在某些使用英文版的 Windows 8 的電腦上安裝法文版本的 Microsoft Word。
...
> [!NOTE]
> 如果 WSUS 伺服器是在複寫模式中執行，這部伺服器上就不能建立電腦群組。 複本伺服器用戶端電腦需要的所有電腦群組，必須在 WSUS 伺服器階層的根 WSUS 伺服器上建立。 如需複寫模式的詳細資訊，請參閱 < 管理 WSUS 複本伺服器[管理 WSUS 複本伺服器](https://technet.microsoft.com/library/dd939893(WS.10).aspx)WSUS 3.0 SP2 操作手冊 》 中。

電腦一律指派到**的所有電腦**群組，而且在都會持續指派到**未指派的電腦**群組之前將它們指派給另一個群組。 電腦可以屬於多個群組。
...
因為可以將電腦指派到多個群組，所以可能為相同的電腦多次核准單一更新。 不過，只能部署更新一次，而且 WSUS 伺服器會解決任何衝突。 若要繼續上一個範例中，如果 computerA 指派到 「 薪資 」 群組和帳款 」 群組，而且這兩個群組核准 Update1，它將會部署一次。
...
在 Update Services 主控台階層窗格下列文字範例中，名為 wsus-01 的 WSUS 伺服器，電腦群組名為桌上型電腦，伺服器已新增為預設值**的所有電腦**群組。 桌上型電腦和伺服器群組是在相同的階層式層級。
...
在此範例中，群組分支下兩層桌面的電腦 (Desktops L2) 會有較高的優先順序高於伺服器分支 (Servers L1) 下的一層的群組。 因此，若電腦在 Desktops-L2 和 Servers-L1 群組皆具有成員資格，則 Desktops-L2 群組的所有動作皆優先於為 Servers-L1 群組指定的動作。
...
有一些您必須審慎規劃以便擁有最佳化效能，部署 WSUS 之前的區域。 這些重要的方面包括：
...
如果部署連線的 WSUS 伺服器階層，建議不要建立太多層的巢狀伺服器。 如果啟用延後的下載且下游伺服器要求上游伺服器沒有核准的更新，那麼下游伺服器的要求會強制上游伺服器下載。 接著下游伺服器會在後續同步處理中下載更新。 在層數過多的 WSUS 伺服器階層中，會在要求、下載，然後透過伺服器階層傳遞更新時，發生延遲。 根據預設，將更新儲存在本機時會啟用延後下載。 您可以手動變更這個選項。
...
有時這項功能被稱為 「 差異傳送 」，因為它會下載只之間的差異 （差異） 檔案的兩個版本。 快速安裝檔案大於散佈到用戶端電腦的更新，因為快速安裝檔案包含每個檔案要更新的所有可能版本。
