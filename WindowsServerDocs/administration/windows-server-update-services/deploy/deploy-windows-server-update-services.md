...
description: Windows Server Update Service （WSUS）主題-部署程式的總覽，並連結至完成這項工作的四個步驟
...
在 WSUS 實作中，網路中至少要有一部 WSUS 伺服器必須連線到 Microsoft Update，以取得可用的更新資訊。 您可以根據網路安全性和設定，決定有多少其他伺服器直接連接到 Microsoft Update。  

本指南提供規劃和部署 Windows Server Update Service 的概念資訊。  
