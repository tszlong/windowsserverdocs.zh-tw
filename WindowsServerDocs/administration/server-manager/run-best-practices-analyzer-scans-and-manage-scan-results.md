...
|嚴重性等級|描述|
|---------|--------|
|Error|當角色未滿足最佳做法規則的條件時，就會傳回錯誤結果，而且可以預期會發生功能問題。|
|內容|當角色滿足最佳做法規則的條件時，就會傳回資訊結果。|
...
## <a name="BKMK_BPAscan"></a>執行角色的最佳做法分析程式掃描
...
#### <a name="BKMK_singlerole"></a>使用 Windows PowerShell Cmdlet 掃描單一角色
...
5.  輸入下列任一個命令，開始進行特定角色的 BPA 掃描。 如果有多個角色，可以使用逗點分隔模型識別碼。
    ...
    **範例：** `Invoke-BPAmodel -modelId Microsoft/Windows/DNSServer,Microsoft/Windows/FileServices`
    ...
    執行此 Cmdlet 時，若未指定模型識別碼，會將 `Get-BPAmodel` Cmdlet 傳回的所有模型傳送到 `Invoke-BPAmodel` Cmdlet 中，開始掃描已新增至伺服器管理員伺服器集區之伺服器上的所有可用模型。

#### <a name="BKMK_allroles"></a>使用 Windows PowerShell Cmdlet 掃描所有角色
...
##### <a name="BKMK_gui"></a>使用 GUI 包含掃描結果
...
##### <a name="BKMK_cmdlets"></a>使用 Windows PowerShell Cmdlet 來包含掃描結果
...
2.  輸入下列命令然後按 **Enter**，以從模型掃描包含特定結果。
    ...
    命令的第二部分（在第一個管線字元（ **|** ）之後）會篩選**get-bparesult**指令程式的結果，只取得結果欄位值（以*功能變數名稱*、相符專案表示）的掃描結果以引號括住的文字。
...
#### <a name="BKMK_recentPS"></a>若要使用 Windows PowerShell 來查看角色最近掃描的結果
...
#### <a name="BKMK_formats"></a>以不同格式從 Windows PowerShell 會話中查看或儲存 BPA 結果
