...
最佳做法分析程式（BPA）是 Windows Server 2012 R2、Windows Server 2012 和 Windows Server 2008 R2 中提供的伺服器管理工具。 BPA 可以掃描安裝在執行 Windows Server 2012 或 Windows Server 2008 R2 的受管理伺服器上的角色，並將最佳做法違規報告給系統管理員，藉此協助系統管理員減少最佳做法違規。
...
|Policy(Windows Intune 說明：原則)|原則規則適用于識別可能需要修改的群組原則或 Windows 登錄設定，才能以最佳且安全的方式操作角色。|
|操作|操作規則用於識別角色執行企業中規定的工作時可能發生的失敗情形。|
...
5.  輸入下列任一個命令，開始進行特定角色的 BPA 掃描。 如果有多個角色，可以使用逗點分隔模型識別碼。
    ...
    **範例：** `Invoke-BPAmodel -modelId Microsoft/Windows/DNSServer,Microsoft/Windows/FileServices`
    ...
    執行此 Cmdlet 時，若未指定模型識別碼，會將 `Get-BPAmodel` Cmdlet 傳回的所有模型傳送到 `Invoke-BPAmodel` Cmdlet，開始掃描已新增至伺服器管理員伺服器集區的伺服器上可用的所有模型。
...
您可以將 `Set-BPAResult` Cmdlet 與 `Exclude` 參數搭配使用，排除掃描結果。 如同伺服器管理員中的 [最佳做法分析程式] 圖格，您可以排除個別的結果物件，也可以排除欄位（例如，類別、標題和嚴重性）等於或包含指定值的一組結果。 例如，您可以從模型的一組掃描結果中排除所有 [效能] 結果。
...
1.  以提高的使用者權限開啟 Windows PowerShell 工作階段。
...
##### <a name="to-include-scan-results-by-using-windows-powershell-cmdlets"></a><a name=BKMK_cmdlets></a>使用 Windows PowerShell Cmdlet 來包含掃描結果
...
2.  輸入下列命令然後按 **Enter**，以從模型掃描包含特定結果。
    ...
    命令的第二部分（在第一個管道字元（ **|** ）之後）會篩選 get-bparesult 指令**程式**的結果，只取得結果欄位（以功能變數名稱*表示）的*值符合引號中文字的掃描結果。
...
#### <a name="to-view-results-of-the-most-recent-scan-of-a-role-by-using-windows-powershell"></a><a name=BKMK_recentPS></a>若要使用 Windows PowerShell 來查看角色最近掃描的結果
...
#### <a name="to-view-or-save-bpa-results-from-windows-powershell-sessions-in-different-formats"></a><a name=BKMK_formats></a>以不同格式從 Windows PowerShell 會話中查看或儲存 BPA 結果
...
-   在 Windows PowerShell 中，每個 BPA 結果都會與下列類似。
    ...
    -   若要將 BPA 結果匯出到逗點分隔值（CSV）文字檔，請執行下列 Cmdlet，其中*path*代表您要儲存 CSV 結果的路徑與文字檔名稱。 CSV 結果可以匯入 Microsoft Excel，或是在試算表或方格中顯示資料的其他程式。
...
[Windows Server 技術中心上的最佳做法分析程式內容](https://go.microsoft.com/fwlink/p/?LinkId=241597)
在[伺服器管理員磚中篩選、排序及查詢資料](filter-sort-and-query-data-in-server-manager-tiles.md)
