...
|縮圖列|描述|
|---------|--------|
|管理能力|伺服器的管理性包括數個量值：不論伺服器是線上或離線、是否可存取，以及是否要將資料回報給伺服器管理員，登入本機電腦的使用者是否具有足夠的使用者權限來存取或管理遠端伺服器，無論遠端伺服器是否正在執行遠端系統管理它所需的所有軟體，或是伺服器的設定方式，讓它能夠使用伺服器管理員進行查詢和管理。 伺服器管理員可以從執行 Windows Server 2003 的伺服器收集的管理性資料，就是伺服器為線上或離線。 如需管理性狀態錯誤及其解決方式的詳細資訊，請參閱[伺服器管理員論壇](https://docs.microsoft.com/answers/topics/windows-server-manager.html)。|
