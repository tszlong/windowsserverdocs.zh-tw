...
*縮圖*會顯示在每個角色的 [伺服器管理員] 儀表板上 (角色的縮圖會反映針對執行角色之伺服器管理員集區中所有伺服器所收集的資料) 、針對每個伺服器群組、[**所有伺服器**] 群組 (伺服器管理員集區) 中的所有伺服器，以及本機伺服器。 伺服器管理員從受管理的伺服器取得資料之後，就會針對在伺服器集區中的伺服器上執行的角色自動建立縮圖。

如果伺服器管理員主控台在遠端伺服器管理工具的一部分的用戶端電腦上執行，則不會有**本機伺服器**縮圖。

縮圖會顯示角色、伺服器及伺服器群組的狀態與管理性的快速檢視。 當事件、效能計數器、最佳做法分析程式結果、服務或一般管理性問題符合您在 [**詳細資料檢視**] 對話方塊中按一下 [縮圖列] 開啟的條件時，縮圖示題欄會變更色彩 (和反白顯示的數位會顯示在左邊界) 。 下表描述縮圖中顯示的資料。

|縮圖列|描述|
|---------|--------|
|管理能力|伺服器的管理性包括數個量值：不論伺服器是線上或離線、是否可存取，以及是否要將資料回報給伺服器管理員，登入本機電腦的使用者是否具有足夠的使用者權限來存取或管理遠端伺服器，無論遠端伺服器是否正在執行遠端系統管理它所需的所有軟體，或是伺服器的設定方式，讓它能夠使用伺服器管理員進行查詢和管理。 伺服器管理員可以從執行 Windows Server 2003 的伺服器收集的管理性資料，就是伺服器為線上或離線。 如需管理性狀態錯誤及其解決方式的詳細資訊，請參閱[伺服器管理員論壇](/answers/topics/windows-server-manager.html)。|
...
7.  在 [事件識別碼]**** 欄位中，輸入您要警示的特定事件的事件識別碼。 您可以輸入一個範圍的事件識別碼（以破折號 (**-**) ），然後在您要排除的事件識別碼或事件識別碼範圍之前輸入虛線，從範圍中排除事件識別碼。 例如，值 **1,3,5-99,-76** 表示會針對事件識別碼 1 和 3，以及 5 到 99 之間的所有事件識別碼 (但事件識別碼 76 除外) 引發警示。
...
6.  指定要在選取的頁面上，依照 [效能]**** 磚中資源 [CPU 使用率]**** 和 [可用記憶體]**** 的圖表來顯示的時間間隔。 預設期間為一天。 按一下 **[儲存]** 。
    ...
    > [!NOTE]
    > 對於已開啟[動態記憶體](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/ff817651(v=ws.10))的虛擬機器，提高效能警示閾值會導致誤報的正面警示。
...
如需 Windows server 2012 和更新版本的 Windows Server （包括 Server Performance Advisor 3.0）適用之先進效能監視和分析工具的詳細資訊，請參閱 MSDN 上的[效能](/previous-versions/windows/hardware/design/dn614608(v=vs.85))。
...
1.  在伺服器管理員主控台中，開啟儀表板以外的任何頁面，如 [任何角色或群組] 首頁)  (。
...
2.  在 [**服務詳細資料檢視**] 對話方塊中，選取您想要收到警示之服務的 [啟動類型]。 預設會選取 [**自動 (延遲開始) **和 [**自動**]。
