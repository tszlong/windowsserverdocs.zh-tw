...
7.  在 [事件識別碼]**** 欄位中，輸入您要警示的特定事件的事件識別碼。 您可以輸入以破折號（）分隔的事件識別碼範圍 **-** ，然後在您要排除的事件識別碼或事件識別碼範圍之前輸入虛線，以從範圍中排除事件識別碼。 例如，值 **1,3,5-99,-76** 表示會針對事件識別碼 1 和 3，以及 5 到 99 之間的所有事件識別碼 (但事件識別碼 76 除外) 引發警示。
...
6.  指定要在選取的頁面上，依照 [效能]**** 磚中資源 [CPU 使用率]**** 和 [可用記憶體]**** 的圖表來顯示的時間間隔。 預設期間為一天。 按一下 [檔案] 。
