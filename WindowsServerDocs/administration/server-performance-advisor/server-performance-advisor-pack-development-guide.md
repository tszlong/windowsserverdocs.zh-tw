...
* Framework 版本： **frameworkversion** （預設為3.0）
...
若要收集登錄設定，請指定值名稱的完整路徑： HKEY\_本機\_機\\MyKey\\MyValue

若要收集登錄機碼下的所有設定，請指定登錄機碼的完整路徑： HKEY\_本機\_機\\MyKey\\

若要收集登錄機碼及其子機碼下的所有值（以遞迴方式收集登錄資料），請針對最後一個路徑分隔符號使用兩個反斜線： HKEY\_本機\_機\\MyKey\\\\

若要從遠端電腦收集登錄資訊，請在登錄路徑開頭包含電腦名稱稱： HKEY\_本機\_機\\MyKey\\MyValue
...
query | NVarchar （4000） NOT Null | 布建中繼資料中的原始查詢字串
...
可能有兩個實例： **\_總計**和**0 個 C： D：** ，輸出可能如下所示：

時間戳記 | CategoryName | CounterName | _Total 的實例值 | 實例值為 0 C： D：
---- | ---- | ---- | ---- | ----
...
欄位名稱 | SQL 資料類型 | 說明
---- | ---- | ---- | ----
時間戳記 | datetime2 （3） NOT Null | 在 UNC 中收集的日期時間
CategoryName | NVarchar （200） NOT Null | 類別名稱
...
規則名稱和建議是索引鍵，且在其範圍內是唯一的。 兩個規則都不能有相同的名稱，而且一個規則內沒有兩個建議可以具有相同的名稱。 當您撰寫 SQL 腳本報表時，這些名稱會非常重要。 您可以呼叫 \[dbo\]。\[SetNotification\] API 來設定規則狀態。
...
### <a href="" id="bkmk-ui-section"></a>各節
...
您可以為每個單一值定義資料類型。 **類型**的允許輸入是在 **&lt;datatype/&gt;** 中定義。 最終的報告看起來可能像這樣：
...
**&lt;值/&gt;** 的**caption**屬性會顯示在第一個資料行中。 [值] 資料行中的值會在未來由腳本報表透過 \[dbo\]設定。\[SetSingleValue\]。 **&lt;值/&gt;** 的**description**屬性會顯示在工具提示中。 工具提示通常會向使用者顯示資料的來源。 如需工具提示的詳細資訊，請參閱[工具提示](#bkmk-tooltips)。
...
清單值名稱必須是全域唯一的。 這個名稱會成為臨時表的名稱。 在上述範例中，將會在執行環境初始化階段建立名為 \#NetworkAdapterInformation 的資料表，其中包含所描述的所有資料行。 類似于單一值名稱，清單值名稱也會當做自訂視圖名稱的一部分使用，例如 vwNetworkAdapterInformation。

&lt;資料行/&gt; 的 @type 是由 &lt;datatype/&gt; 所定義

最終報表的模擬 UI 看起來可能如下所示：

**實體網路介面卡資訊**

識別碼 | 名稱 | 在工作列搜尋方塊中輸入 | 速度（Mbps） | MAC 位址
--- | --- | --- | --- | ---
 | <br> | | |
 | | | |


&lt;column/&gt; 的**caption**屬性會顯示為數據行名稱，而 &lt;資料行/&gt; 的**description**屬性則會顯示為對應資料行標頭的工具提示。 工具提示通常會向使用者顯示資料的來源。 如需詳細資訊，請參閱[工具提示](#bkmk-tooltips)。
...
CounterName | InstanceName | 平均 | 總和
--- | :---: | :---: | :---:
...
* CounterName （處理器時間百分比）/InstanceName （\_總計）/平均

* CounterName （處理器時間百分比）/實例名稱（CPU0）/平均

* CounterName （處理器時間百分比）/InstanceName （\_總計）/總和
...
在上一個範例中，系統管理員可能只對 InstanceName \_總計或 CPU1 的索引鍵感興趣。 篩選器可以定義如下：
...
主要報表腳本是藉由結合**name**和**reportScript**屬性來命名。 在下列範例中，它會 \[ServerPerformanceAdvisor. CoreOS. V2\]。\[ReportScript\]。
...
正在執行 \[dbo\]。\[DebugReportScript\] 會傳回多個結果集，包括：
...
|-----------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------|-----------------------------------|
| <ul><li>ProvisionMetadata 中的名稱</li><li>已儲存程序</li><li>函式</li><li>視圖名稱</li><li>臨時表名稱</li></ul> | <ul><li>參數名稱</li><li>區域變數</li></ul> | 針對所有 SQL 保留關鍵字使用 |
...
* 若發生錯誤，請考慮使用**SET 交易\_ABORT ON**來終止進程。
...
2. 發生合併衝突時，會遵循下列規則：
   ...
   2. 採用一組超級效能計數器。 例如，with **Process （\*）\\% processor time**和**process （\*）\\\*，\\Process （\*）\\** \\* 會傳回更多資料，因此**進程（\*）\\% Processor time**和**process （\*）** \\\\* 會從合併的資料收集器集合中移除。
...
若要尋找**FriendlyName**值，請開啟 [登錄編輯程式] 並流覽至 [登錄] 設定，方法是將**HKEY\_本機\_機\\SYSTEM\\CurrentControlSet\\Enum\\** 與上一個範例中的每一行。 例如： **HKEY\_本機\_機\\SYSTEM\\CurrentControlSet\\Enum\\ ROOT\\\*IPHTTPS\\0000**。
...
在此範例中，您先在 managementpaths 底下新增 WMI 查詢，並定義索引鍵名稱**NetworkAdapter**。 然後您會新增登錄機碼，並使用 **$ （NetworkAdapter. PNPDeviceID）** 語法來參考**NetworkAdapter** 。
...
對於 WMI 資料收集器，每個 WMI 物件都有許多附加的屬性。 任何類型的 WMI 物件一律會有三個屬性： \_\_命名空間、\_\_類別，以及 \_\_RELpath。
...
當 SPA 從目標伺服器收集資料時，它會動態地以其參考資料收集器（登錄機碼/WMI）的實際收集資料來取代模式 $ （\*.\*），例如：
...
registry | 登錄： &lt;registerKey&gt; | 登錄： HKLM\SOFTWARE\Microsoft<br>\\ASP.NET\\Rootver
組態檔 | Read-configfile： &lt;Filepath&gt;\[;Xpath： &lt;Xpath&gt;\]<br>**注意**<br>Xpath 是選擇性的，只有在檔案是 xml 檔案時才有效。 | Read-configfile： windir%\\System32\\inetsrv\config\\Applicationhost.config<br>Xpath： configuration&frasl;System.webserver<br>&frasl;HTTPProtocol&frasl;@allowKeepAlive
ETW | ETW： &lt;提供者/&gt;（關鍵字） | ETW： Windows 核心追蹤（進程、網路）

### <a name="table-collation"></a>資料表定序

當 advisor 套件變得更複雜時，您可以建立自己的變數資料表或臨時表，將中繼結果儲存在報表腳本中。

定序字串資料行可能會有問題，因為您所建立的資料表定序可能與 SPA 架構所建立的定序不同。 如果您將不同資料表中的兩個字串資料行相互關聯，可能會看到定序錯誤。 若要避免這個問題，您應該一律將資料行定序的字串定義為**SQL\_採用 latin1-general\_General\_CP1\_CI\_，如同**您定義資料表一樣。
...
--- | --- | ---
guid | GUID | 提供者 GUID
工作階段 | 字串 | ETW 會話名稱（選擇性，只有核心事件才需要）
keywordsany | 十六進位 | 任何關鍵字（選擇性，沒有0x 前置詞）
keywordsAll | 十六進位 | 所有關鍵詞（選擇性）
內容 | 十六進位 | 屬性（選擇性）
level | 十六進位 | Level （選擇性）
...
ThreadId | BigInt NOT Null | 執行緒 ID
時間戳記 | datetime2 NOT Null | 時間戳記
