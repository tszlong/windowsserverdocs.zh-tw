---
title: shadow
description: Shadow 的參考文章，可讓您從遠端控制遠端桌面工作階段主機伺服器上另一位使用者的使用中會話。
ms.topic: article
...
-   除了目前的會話) 之外，您還可以從遠端控制自己的會話 (，但您必須具備 [完全控制] 許可權或 [遠端控制] 特殊存取權限，才能遠端控制另一個會話。
...
-   當您想要結束遠端控制 (遮蔽) 時，請 \* \* 從數位鍵臺上僅) 使用 CTRL + (。
