---
title: dir
description: Dir 的參考主題，它會顯示目錄的檔案和子目錄清單。
...
- 您可以使用萬用字元（**&#42;** 或 **？**）來代表檔案名的一個或多個字元，並顯示檔案或子目錄的子集。
  ...
  **星號（\*）：** 使用星號取代任何字元字串，例如：  
  ...
  - **dir read\*.txt**會列出目前目錄中開頭為 read 且副檔名開頭為 .txt 的所有檔案，例如 .txt、txt1 或. txt_old。
  - **目錄讀取\*。\* **列出目前目錄中所有副檔名開頭為 [讀取] 的所有檔案。
  ...
  您可能預期輸入**dir t97\\*** 會傳回 t97 檔案。 不過，鍵入**dir t97\\*** 會傳回這兩個檔案，因為星號萬用字元會使用其簡短名稱對應 T97B4 ~ 1 .txt 來比對 txt2 到 t97 .txt。 同樣地，輸入**del\\t97*** 會同時刪除這兩個檔案。
...
- 指定檔案名排序
  ...
  如果您指定一個以上的*SortOrder*值， **dir**會依照第一個準則排序檔案名，然後依第二個準則，依此類推。 例如，如果您針對*SortOrder*使用 **/o**搭配**e**和 **-s**值（藉由使用 **/o： e-s**或 **/oe-s**）， **dir**會依副檔名排序目錄和檔案的名稱，最大的是第一個，然後顯示最終的結果。 依副檔名的字母排序會導致不具有副檔名的檔案名先出現，然後是目錄名稱，再加上副檔名為的檔案名。
- 使用重新導向符號和管道

  當您使用重新導向符號（**>**）將**目錄**輸出傳送至檔案或**|** 管道（）以將**目錄**輸出傳送至另一個命令時，請使用 **/a：-d**和 **/b**只列出檔案名。 您可以使用*FileName*搭配 **/b**和 **/s**來指定**dir**是要搜尋目前目錄及其子目錄中所有符合*FileName*的檔案名。 **Dir**只會列出它所找到的每個檔案名的磁碟機號、目錄名稱、檔案名和副檔名（每行一個路徑）。 在您使用管道將**目錄**輸出傳送至另一個命令之前，您應該在 Autoexec nt 檔案中設定 TEMP 環境變數。
