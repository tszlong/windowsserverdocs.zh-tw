...
- 使用重新導向符號和管道
  ...
  當您使用重新導向符號（ **>** ）將**目錄**輸出傳送至檔案或管道（ **|** ）以將**dir**輸出傳送至另一個命令時，請使用 **/a：-d**和 **/b**只列出檔案名。 您可以使用*FileName*搭配 **/b**和 **/s**來指定**dir**是要搜尋目前目錄及其子目錄中所有符合*FileName*的檔案名。 **Dir**只會列出它所找到的每個檔案名的磁碟機號、目錄名稱、檔案名和副檔名（每行一個路徑）。 在您使用管道將**目錄**輸出傳送至另一個命令之前，您應該在 Autoexec nt 檔案中設定 TEMP 環境變數。
