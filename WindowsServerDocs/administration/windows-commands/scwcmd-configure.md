...
```
若要使用 webadmin 帳號憑證，針對 file webpolicy.xml 設定電腦的安全性原則，請輸入：
```
scwcmd configure /m:172.16.0.0 /p:webpolicy.xml /u:webadmin
```
若要在清單上的所有電腦上設定安全性原則，campusmachines.xml 最多100個執行緒，請輸入：
```
scwcmd configure /i:campusmachines.xml /t:100
```
若要在 WebServers OU 中的所有電腦上，使用 DomainAdmin 帳戶的認證來設定 webpolicy.xml 的安全性原則，請輸入：
```
