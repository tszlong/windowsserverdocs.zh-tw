...
|      -adaccountorgroup       | 新增磁片區的安全識別碼（SID）型身分識別保護裝置。  您也可以使用 **-sid**作為此命令的縮寫版本。 **重大**根據預設，您無法使用 WMI 或 manage-bde 從遠端新增 ADAccountOrGroup 保護裝置。  如果您的部署需要能夠從遠端新增此保護裝置，您必須啟用限制委派。 |
