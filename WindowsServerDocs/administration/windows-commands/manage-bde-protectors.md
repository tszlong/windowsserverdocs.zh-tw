...
description: Manage-bde 保護裝置命令的參考文章，它會管理用於 BitLocker 加密金鑰的保護方法。
ms.topic: article
...
| 參數 | 描述 |
| ----------- | ----------- |
| -get | 顯示磁片磁碟機上已啟用的所有金鑰保護方法，並提供其類型和識別碼， (識別碼) 。 |
| -新增 | 新增使用其他 **-add**參數所指定的金鑰保護方法。 |
| -delete | 刪除 BitLocker 所使用的金鑰保護方法。 除非使用選擇性**的刪除**參數來指定要刪除的保護裝置，否則會從磁片磁碟機移除所有金鑰保護裝置。 刪除磁片磁碟機上的最後一個保護裝置時，會停用磁片磁碟機的 BitLocker 保護，以確保不會不慎遺失資料的存取權。 |
| -disable | 停用保護，讓任何人都能在磁片磁碟機上提供不安全的加密金鑰來存取加密的資料。 不會移除任何金鑰保護裝置。 除非使用選擇性的**停**用參數來指定重新開機計數，否則會在下一次啟動 Windows 時繼續保護。 |
...
#### <a name="additional--add-parameters"></a>其他-加入參數
...
| -adaccountorgroup | 為磁片區新增 (SID) 型身分識別保護裝置的安全識別碼。 您也可以使用 **-sid**作為此命令的縮寫版本。 **重要事項：** 根據預設，您無法使用 WMI 或 manage-bde 從遠端新增 ADaccountorgroup 保護裝置。 如果您的部署需要能夠從遠端新增此保護裝置，您必須啟用限制委派。 |
...
| --------- | ----------- |
| `<drive>` | 表示後面接著冒號的磁碟機號。 |
| rebootcount | 指定作業系統磁片區的保護已暫停，並會在 Windows 重新開機**rebootcount**參數中指定的次數後繼續。 指定**0**會無限期暫停保護。 如果未指定此參數，BitLocker 保護會在 Windows 重新開機之後自動繼續。 您也可以使用 **-rc**做為此命令的縮寫版本。 |
| -computername | 指定 manage-bde.exe 將用來修改另一部電腦上的 BitLocker 保護。 您也可以使用 **-cn**做為此命令的縮寫版本。 |
| `<name>` | 代表要修改 BitLocker 保護的電腦名稱稱。 接受的值包括電腦的 NetBIOS 名稱和電腦的 IP 位址。 |
| -? 或/？ | 在命令提示字元中顯示簡短說明。 |
| -help 或-h | 在命令提示字元中顯示完整的說明。 |
...
若要新增憑證金鑰保護裝置（由憑證檔案識別）到磁片磁碟機 E，請輸入：

```
manage-bde -protectors -add E: -certificate -cf c:\File Folder\Filename.cer
```

若要將**adaccountorgroup**金鑰保護裝置（依網域和使用者名稱識別）新增至磁片磁碟機 E，請輸入：

```
manage-bde -protectors -add E: -sid DOMAIN\user
```

若要停用保護，直到電腦重新開機3次為止，請輸入：

```
manage-bde -protectors -disable C: -rc 3
```

若要刪除磁片磁碟機 C 上所有 TPM 和以啟動金鑰為基礎的金鑰保護裝置，請輸入：
