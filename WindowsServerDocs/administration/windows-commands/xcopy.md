...
**第.** 若要將 \Reports 目錄中的檔案更新為 \Rawdata 目錄中自1993年12月29日起變更的檔案，請輸入：

```
xcopy \rawdata \reports /d:12-29-1993
```

**4gb.** 若要更新上一個範例中存在於 \Reports 中的所有檔案（不論日期為何），請輸入：

```
xcopy \rawdata \reports /u
```

**第.** 若要取得上一個命令所要複製的檔案清單（也就是不會實際複製檔案），請輸入：
...
**7.** 若要將 \Customer 目錄和所有子目錄複寫到網路磁碟機 H：上的目錄 \\ @ no__t-1Public\Address，請保留唯讀屬性，並在 H：上建立新檔案時出現提示，請輸入：

```
xcopy \customer h:\public\address /s /e /k /p
```

**utf-7.** 若要發出先前的命令，請確定**xcopy**會建立 \Address 目錄（如果不存在），並隱藏當您建立新的目錄時所顯示的訊息，新增 **/i**命令列選項，如下所示：

```
xcopy \customer h:\public\address /s /e /k /p /i
```

**8.** 如果發生錯誤，您可以建立 batch 程式來執行**xcopy**作業，並使用批次**if**命令來處理結束代碼。 例如，下列 batch 程式會針對**xcopy**來源和目的地參數使用可取代的參數：
...
**9.** 這個範例中的所有非空白目錄，加上名稱符合以星號符號指定之模式的檔案。
...
在上述範例中，這個特定的來源參數值 **。 \\toc\*.yml**會複製相同的3個檔案，即使它的兩個路徑字元已移除 **\\** 。 不過，如果已從來源參數移除星號萬用字元，則不會複製任何檔案，而是 **\\toc. yml**。
