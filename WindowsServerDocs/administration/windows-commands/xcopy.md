...
|/t|複製 (的子目錄結構，也就是樹狀) ，而不是檔案。 若要複製空的目錄，您必須包含 **/e**命令列選項。|
...
|/n|使用 NTFS 短檔案或目錄名稱來建立複本。 當您從 NTFS 磁片區將檔案或目錄複寫到 FAT 磁片區，或當 FAT 檔案系統命名慣例 (也就是*目的地*檔案系統上必須有8.3 個) 字元時，就需要 **/n** 。 *目的地*檔案系統可以是 FAT 或 NTFS。|
...
- 使用 **/z**
  ...
  如果您在複製階段期間遺失連線 (例如，如果伺服器離線會將連線) ，則在您重新建立連接之後就會繼續進行。 **/z**也會顯示每個檔案完成複製作業的百分比。
...
- 附加檔案
  ...
  若要附加檔案，請針對 [目的地] 指定單一檔案，但針對 [來源 (的多個檔案]，使用 [萬用字元] 或 [file1 + file2 + file3 格式]) 。
...
**5.** 若要取得前一個命令所要複製的檔案清單 (也就是，在不) 實際複製檔案的情況下，請輸入：
