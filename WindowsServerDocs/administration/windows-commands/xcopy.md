...
|/a|只複製*來源*檔案具有其封存檔案的屬性集。 **/ a**不會修改原始程式檔的封存檔案屬性。 如需有關如何設定使用的封存檔案屬性資訊**attrib**，請參閱[其他參考](#additional-references)。|
|/m|複本*來源*檔案具有其封存檔案的屬性集。 不同於 **/a**， **/m**關閉所指定來源中的檔案中的封存檔案屬性。 如需有關如何設定使用的封存檔案屬性資訊**attrib**，請參閱[其他參考](#additional-references)。|
...
|/exclude:FileName1[+[FileName2][+[FileName3]( \)]|指定檔案的清單。 必須指定至少一個檔案。 每個檔案會包含搜尋字串，其中每個檔案中的個別行上的字串。</br>若其中任一字串比對要複製檔案的絕對任何的路徑部分，該檔案將不複製。 比方說，指定字串**obj**目錄下的所有檔案中都排除**obj**或使用的所有檔案 **.obj**延伸模組。|
...
- 使用**xcopy**命令，以設定封存屬性*目的地*檔案
  ...
  **Xcopy**命令會建立檔案以封存屬性集，不論是否已將此屬性設定原始程式檔中。 如需有關檔案屬性及**attrib**，請參閱[其他參考](#additional-references)。
...
- 結束代碼**xcopy**
  ...
  若要處理序所傳回的結束代碼**xcopy**，使用**ErrorLevel**參數**如果**批次程式中的命令列。 如需範例的批次程式處理序結束碼使用之**如果**，請參閱[其他參考](#additional-references)。 下表列出每個結束代碼和描述。  
...
**2.** 若要在上述範例中包含的任何系統或隱藏的檔案，新增<strong>/h</strong>命令列選項，如下所示：
