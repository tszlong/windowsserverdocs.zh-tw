...
description: 建立磁碟分割擴充的參考主題，這會在具有焦點的磁片上建立延伸磁碟分割。
...
|  參數  |                                                                                                                             描述                                                                                                                              |
|-------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|  容量\=<n>  |                                                  指定磁碟分割的大小（以 mb \(為\)單位）。 如果沒有指定大小，磁碟分割會繼續進行，直到延伸磁碟分割中沒有其他可用空間為止。                                                  |
| 投影\=<n> |                     指定分割區的建立\(位移\)，以 kb 為單位。 如果沒有指定位移，分割區會從磁片上可用空間的開頭開始，夠大而足以容納新的資料分割。                      |
| 對齊\=<n>  | 將所有資料分割範圍對齊最接近的對齊界限。 通常與硬體 RAID 邏輯單元編號\(LUN\)陣列搭配使用，以改善效能。 <n>這是從磁片開始\(到\)最接近對齊界限的 kb kb 數。 |
