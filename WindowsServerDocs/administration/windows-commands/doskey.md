...
| /macros | 顯示所有**doskey**宏的清單。 您可以使用重新導向符號 (`>`) 搭配 **/macros** ，將清單重新導向至檔案。 您可以將 **/macros**縮寫為 **/m**。 |
...
| /exename =`<exename>` | 指定程式 (也就是**doskey**宏執行所在的可執行檔) 。 |
...
- 下表列出**doskey**編輯金鑰及其功能：
  ...
  | END | 將插入點移到行尾。 |
  | ESC | 從顯示中清除命令。 |
  | F1 | 將一個字元從範本中的資料行複製到命令提示字元視窗中的相同資料行。  (範本是一個記憶體緩衝區，其中包含您輸入的最後一個命令。 )  |
  ...
  | F5 | 將範本複製到目前的命令列。 |
  | F6 | 將檔案結尾字元放 (CTRL + Z) 在目前的插入點位置。 |
  | F7 | 在對話方塊中顯示 (，) 此程式儲存在記憶體中的所有命令。 使用向上鍵和向下鍵來選取您想要的命令，然後按 ENTER 執行命令。 您也可以記下命令前面的連續數位，並將此數位與 F9 鍵搭配使用。 |
...
- 您可以使用 Doskey.exe 來建立可執行一或多個命令的宏。 下表列出當您定義宏時，可以用來控制命令作業的特殊字元。
  ...
  | 字元 | 描述 |
  |---------- | ----------- |
  | `$G` 或 `$g` | 重新導向輸出。 使用其中一個特殊字元，將輸出傳送至裝置或檔案，而不是畫面。 此字元相當於輸出 () 的重新導向符號 `>` 。 |
  | `$G$G` 或 `$g$g` | 將輸出附加至檔案結尾。 使用上述任一雙字元將輸出附加至現有的檔案，而不是取代檔案中的資料。 這些雙字元相當於輸出 () 的附加重新導向符號 `>>` 。 |
