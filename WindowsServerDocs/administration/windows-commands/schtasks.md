...
|排程類型|修飾詞|日期值 (/d)|描述|
|-------------|--------|---------------|-----------|
|每週|1 - 52|MON - SUN[,MON - SUN...] | *|選擇性。 預設值為星期一。 萬用字元值 （*） 表示每一天。|
...
「 一週的 day"的排程是每週排程的變化。 在每週排程中， **/sc 每週**是必要參數。 **/月**（修飾詞） 參數是選擇性的並指定每個執行的工作之間的週數。 預設值 **/月**為 1 （每週）。 **/D**參數，這是選擇性的在一週當中的指定天數或所有天數 （*） 上執行工作的排程。 預設為 MON （星期一）。 每個 [天] 選項 (**/d \*** ) 就相當於排程每日工作。
...
在此排程類型 **/sc 每月**參數， **/月**（修飾詞） 參數，而 **/d** （天） 參數所需。 **/月**（修飾詞） 參數會指定一週執行工作。 **/D**參數會指定一週的星期幾。 （您可以指定此排程類型之當週的某一天）。此排程也有選擇性 **/m** （月） 參數，可讓您針對特定的幾個月或每月排程的工作 (*)。預設值 **/m**參數為每個月 (*)。
...
**/M** （月） 參數是選用此排程類型和預設值是每個月 （*）。 **Schtasks**不會讓您排程的工作不會發生在所指定月份中日期 **/m**參數。 不過，如果省略 **/m**參數，而不會執行工作的工作不會出現在每個月，例如第 31 天，則工作的日期，這是在較短的幾個月的排程。 若要排程的工作在當月最後一天，使用最後一天的排程類型。
...
|/ru {[\<Domain>\]<User> | 系統}|指定要變更工作所執行的使用者帳戶。 指定的本機系統帳戶，有效的項目都是""，"NT AUTHORITY\SYSTEM"或"SYSTEM"。</br>當您變更使用者帳戶時，您也必須變更使用者密碼。 如果命令具有 **/ru**參數，但不是 **/rp**參數**schtasks**會提示輸入新密碼。</br>以本機系統帳戶的權限執行的工作不需要或提示輸入密碼。|
...
|詞彙|定義|
|----|----------|
|/tn {\<工作名稱 >| *}|必要。 識別要刪除的工作。</br>**\<工作名稱 >** -刪除具名的工作。</br>**<\*>** -刪除電腦上的所有排程的工作。|
...
```
在回應中， **SchTasks.exe**會顯示下列確認訊息。 若要刪除的工作，請按下 Y **。** 若要取消命令，請輸入**n**:
```
...
下列命令會刪除所有工作，從本機電腦，包括其他使用者已排程工作的排程。 它會使用 **/tn \*** 參數來代表電腦上的所有工作和 **/f**參數來抑制確認訊息。
```
...
|/fo {TABLE| 清單| CSV}|指定輸出格式。 **資料表**是預設值。|
