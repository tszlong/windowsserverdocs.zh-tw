...
description: Wbadmin start sysrecovery 的參考文章，它會使用您所指定的參數， (裸機復原) 執行系統修復。
ms.topic: article
...
|-restoreAllVolumes|從選取的備份復原所有磁片區。 如果未指定此參數，則只會復原包含 [系統狀態] 和 [作業系統] 元件)  (磁片區的重要磁片區。 當您需要在系統修復期間復原非重要磁片區時，這個參數很有用。|
