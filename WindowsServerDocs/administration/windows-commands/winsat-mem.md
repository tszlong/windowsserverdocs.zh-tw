...
|-do\<n>|指定來源緩衝區結尾與目的地緩衝區開頭之間的距離（以位元組為單位）。 預設值為64個位元組。 可允許的目的地位移上限為16MB。 指定不正確目的地位移會導致錯誤。</br>注意：零是的有效值 **\<n>** ，但負數不是。|
...
- 使用32MB 緩衝區大小，並將結果以 XML 格式儲存至檔案**memtest.xml**，以執行至少4秒且不超過12秒的評量。
  ```
