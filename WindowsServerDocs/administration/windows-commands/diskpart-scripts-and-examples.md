...
使用 Diskpart `/s` 來執行腳本，以自動執行 disk @ no__t-1related 工作，例如建立磁片區或將磁片轉換成動態磁碟。 如果您使用自動安裝或 Sysprep 工具（不支援建立開機磁碟區以外的磁片區）部署 Windows，則編寫這些工作的腳本會很有用。  
...
當 DiskPart 啟動時，會在命令提示字元中顯示 DiskPart 版本和電腦名稱稱。 根據預設，如果 DiskPart 在嘗試執行腳本工作時遇到錯誤，DiskPart 會停止處理腳本，並顯示錯誤碼 @no__t 0unless 您指定了**noerr**參數 @ no__t-2。 不過，不論您是否使用了**noerr**參數，DiskPart 一律會在遇到語法錯誤時傳回錯誤。 **Noerr**參數可讓您執行有用的工作，例如使用單一腳本刪除所有磁片上的所有分割區，而不論磁片總數為何。  
  
## <a name="see-also"></a>另請參閱  
[抽樣使用 Windows PE 和 DiskPart @ no__t-2 來設定 UEFI @ no__t-0gpt @ no__t-1Based 硬碟磁碟分割  
[抽樣設定 BIOS @ no__t-0MBR @ no__t-使用 Windows PE 和 DiskPart @ no__t-2 1Based 硬碟磁碟分割  
