...
|    /u [\<Domain > \] @ no__t-2     |                                                          使用指定之使用者帳戶的許可權來執行命令。 如果您未使用 **/u**選項，預設會使用系統許可權。                                                           |
...
```
若要將目錄與開啟的檔案名 "C:\TestShare @ no__t-0 中斷連接，不論是誰在存取它，請輸入：
```
...
|  /u [\<Domain > \] @ no__t-2   |                                                          使用指定之使用者帳戶的許可權來執行命令。 如果您未使用 **/u**選項，預設會使用系統許可權。                                                           |
|       /p [\<Password >]       |                                               指定 **/u**選項中所指定使用者帳戶的密碼。 如果您未使用 **/p**選項，則會在命令執行時顯示密碼提示。                                                |
| [/fo {TABLE \| 清單 \| CSV}] |                             以指定的格式顯示輸出。 *格式*的有效值為：</br>目錄在資料表中顯示輸出。</br>名單顯示清單中的輸出。</br>CSV以逗點分隔值格式顯示輸出。                              |
