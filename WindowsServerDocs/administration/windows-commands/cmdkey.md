---
title: cmdkey
description: Cmdkey 命令的參考主題，它會建立、列出及刪除已儲存的使用者名稱和密碼或認證。
...
| 參數 | 描述 |
| ---------- | ----------- |
| /add`<targetname>` | 將使用者名稱和密碼加入清單中。<p>需要可識別此專案`<targetname>`將關聯之電腦或功能變數名稱的參數。 |
| /一般`<targetname>` | 將一般認證加入清單中。<p>需要可識別此專案`<targetname>`將關聯之電腦或功能變數名稱的參數。 |
