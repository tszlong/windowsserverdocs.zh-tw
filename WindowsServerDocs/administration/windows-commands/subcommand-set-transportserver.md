...
description: 適用于子命令集的 Windows 命令主題-TransportServer，可設定傳輸伺服器的配置設定。
...
|[/ObtainIpv4From： {Dhcp &#124; Range}]|設定 IPv4 位址的來源，如下所示：<p>-[/start： <IP address>] 設定 IP 位址範圍的開頭。 這是必要的，只有在此選項設定為 [**範圍**] 時才有效。<br />-[/End： <IP address>] 設定 IP 位址範圍的結尾。 這是必要的，只有在此選項設定為 [**範圍**] 時才有效。<br />-[/startPort： <port>] 設定埠範圍的開頭。<br />-[/EndPort： <port>] 設定埠範圍的結尾。|
