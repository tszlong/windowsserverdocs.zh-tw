...
| `/deleterodfsrmember [<read_only_domain_controller_name>]` | 刪除對應至指定唯讀網域控制站之 DFS 複寫的全域 AD DS 設定，或如果未指定的值，則刪除所有唯讀網域控制站之 DFS 複寫的全域 AD DS 設定 `<read_only_domain_controller_name>` 。<p>使用此選項只會在唯讀網域控制站上自動刪除失敗時，手動刪除 AD DS 設定，並在從備妥狀態復原至啟動狀態時，長時間停止唯讀網域控制站。 |
| /? | 在命令提示字元顯示說明。 |
...
- Active Directory 複寫會將全域狀態複寫到網域中的其他網域控制站，但因為複寫延遲，如果您 `dfsrmig /getglobalstate` 在 PDC 模擬器以外的網域控制站上執行，可能會出現不一致的情況。
...
`dfsrmig /getmigrationstate`當所有網域控制站上的本機遷移狀態符合全域遷移狀態時，從命令輸出：
...
`dfsrmig /getmigrationstate`當某些網域控制站上的本機遷移狀態不符合全域遷移狀態時，從命令輸出。
