...
若要對*每個使用者*原則執行*移除*作業，您必須擁有安全描述項中該物件集的**寫入**或**完整控制**許可權。 如果您的 [**管理審核及安全性記錄** (SeSecurityPrivilege) ] 使用者權限，也可以執行*移除*作業。 不過，此許可權允許執行整體*移除*作業所需的其他存取權。
...
| 參數 | 描述 |
| ------- | -------- |
| /user | 針對要刪除其每個使用者稽核原則的使用者，指定 (SID) 或使用者名稱的安全識別碼。 |
