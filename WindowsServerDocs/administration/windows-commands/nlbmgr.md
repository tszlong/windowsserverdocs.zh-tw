...
| 參數 | 描述 |
| --------- | ----------- |
| /noping | 防止「網路負載平衡管理員」在嘗試透過 Windows Management Instrumentation (WMI) 來與主機進行連線之前，先進行 ping。 如果您已在所有可用的網路介面卡上停用 (ICMP) 的網際網路控制訊息通訊協定，請使用此選項。 如果「網路負載平衡管理員」嘗試連線到無法使用的主機，您會在使用此選項時遇到延遲。 |
