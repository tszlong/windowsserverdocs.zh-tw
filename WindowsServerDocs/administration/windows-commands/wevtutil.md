...
|{im \| 安裝-資訊清單}\<Manifest>|從資訊清單安裝事件發行者和記錄。 如需事件資訊清單和使用此參數的詳細資訊，請參閱 Microsoft 開發人員網路 (MSDN) 網站 () 的 Windows 事件記錄檔 SDK [https://msdn.microsoft.com](../../index.yml) 。|
|{um \| 卸載-資訊清單}\<Manifest>|從資訊清單中卸載所有發行者和記錄。 如需事件資訊清單和使用此參數的詳細資訊，請參閱 Microsoft 開發人員網路 (MSDN) 網站 () 的 Windows 事件記錄檔 SDK [https://msdn.microsoft.com](../../index.yml) 。|
...
|    ab\<Auto>     |                                                                                                                                   指定記錄自動備份原則。 \<Auto>可以是 true 或 false。 如果此值為 true，則記錄會在達到大小上限時自動備份。 如果此值為 true，則使用 **/rt**選項指定的保留 () 也必須設定為 true。                                                                                                                                    |
|   /ms\<MaxSize>   |                                                                                                                                                                        設定記錄檔的大小上限（以位元組為單位）。 記錄大小下限為1048576個位元組 () 1024KB，記錄檔一律為64KB 的倍數，因此您輸入的值將會相應地四捨五入。                                                                                                                                                                         |
...
|   /ca\<Channel>   |                                                                                                                   設定事件記錄檔的存取權限。 \<Channel>是使用安全描述項定義語言 (SDDL) 的安全描述項。 如需 SDDL 格式的詳細資訊，請參閱 Microsoft 開發人員網路 (MSDN) 網站 ([https://msdn.microsoft.com](../../index.yml)) 。                                                                                                                    |
