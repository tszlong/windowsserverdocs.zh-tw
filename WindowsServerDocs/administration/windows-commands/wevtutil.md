...
|{sl \| set-log}\<Logname> [/E：\<啟用的>] [/i\<：隔離>] [/lfn\<： Logpath>] [/Rt\<：保留>] [/ab\<：自動>] [/ms\<： MaxSize>] [/L\<： Level>] [/k\<：關鍵字>] [/ca\<：通道>] [/c\<： Config>]|修改指定之記錄檔的設定。|
|{ep \|列舉-發行者}|顯示本機電腦上的事件發行者。|
|{gp \| get-publisher}\<Publishername> [/Ge：\<Metadata>] [/gm：\<Message>] [/f：\<Format>]]|顯示指定之事件發行者的設定資訊。|
|{im \|安裝-資訊清單}\<資訊清單>|從資訊清單安裝事件發行者和記錄。 如需事件資訊清單和使用此參數的詳細資訊，請參閱 Microsoft 開發人員網路（MSDN）網站（[https://msdn.microsoft.com](https://msdn.microsoft.com)）上的 Windows 事件記錄檔 SDK。|
|{um \|卸載-資訊清單}\<資訊清單>|從資訊清單中卸載所有發行者和記錄。 如需事件資訊清單和使用此參數的詳細資訊，請參閱 Microsoft 開發人員網路（MSDN）網站（[https://msdn.microsoft.com](https://msdn.microsoft.com)）上的 Windows 事件記錄檔 SDK。|
|{qe \|查詢-事件}\<路徑> [/Lf：\<Logfile>] [/sq：\<Structquery>] [/q：\<查詢>] [/Bm：\<書簽>] [/sbm：\<Savebm>] [/rd：\<方向>] [/f：\<格式>] [/l：\<地區設定>] [/c\<：計數>] [/e\<：元素>]|從事件記錄檔、記錄檔或使用結構化查詢讀取事件。 根據預設，您會提供\<路徑> 的記錄檔名稱。 不過，如果您使用 **/lf**選項，則\<path> 必須是記錄檔的路徑。 如果您使用 **/sq**參數， \<path> 必須是包含結構化查詢之檔案的路徑。|
|{gli \| get-loginfo}\<Logname> [/Lf：\<Logfile>]|顯示事件記錄檔或記錄檔的狀態資訊。 如果使用 **/lf**選項， \<Logname> 是記錄檔的路徑。 您可以執行**wevtutil el**來取得記錄檔名稱的清單。|
|{epl \|匯出-記錄}\<路徑> \<Exportfile> [/lf：\<Logfile>] [/sq：\<Structquery>] [/Q：\<查詢>] [/ow：\<覆寫>]|從事件記錄檔、記錄檔或使用結構化查詢，將事件匯出至指定的檔案。 根據預設，您會提供\<路徑> 的記錄檔名稱。 不過，如果您使用 **/lf**選項，則\<path> 必須是記錄檔的路徑。 如果您使用 **/sq**選項， \<path> 必須是包含結構化查詢之檔案的路徑。 \<Exportfile> 是將儲存所匯出事件之檔案的路徑。|
...
|{cl \|清除-記錄}\<Logname> [/Bu：\<Backup>]|從指定的事件記錄檔清除事件。 您可以使用 **/bu**選項來備份已清除的事件。|

## <a name="options"></a>選項。

|       選項       |                                                                                                                                                                                                                                                                 說明                                                                                                                                                                                                                                                                  |
|--------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    /f：\<格式>    |                                                                                                                                                               指定輸出應該是 XML 或文字格式。 如果\<Format> 是 xml，則輸出會以 xml 格式顯示。 如果\<Format> 是 Text，則會顯示不含 XML 標記的輸出。 預設為 Text。                                                                                                                                                                |
|   /e：\<啟用的>    |                                                                                                                                                                                                                                         啟用或停用記錄檔。 \<啟用的> 可以是 true 或 false。                                                                                                                                                                                                                                          |
|  /i：\<隔離>   | 設定記錄隔離模式。 \<隔離> 可以是系統、應用程式或自訂的。 記錄檔的隔離模式會決定記錄檔是否與相同隔離類別中的其他記錄共用會話。 如果您指定系統隔離，目標記錄檔將至少與系統記錄共用寫入權限。 如果您指定應用程式隔離，目標記錄檔將至少與應用程式記錄檔共用寫入權限。 如果您指定自訂隔離，則也必須使用 **/ca**選項提供安全描述項。 |
|  /lfn：\<Logpath>   |                                                                                                                                                                                                           定義記錄檔名稱。 \<Logpath> 是檔案的完整路徑，其中的事件記錄檔服務會儲存此記錄檔的事件。                                                                                                                                                                                                           |
...
|    /ab：\<自動>     |                                                                                                                                   指定記錄自動備份原則。 \<Auto> 可以是 true 或 false。 如果此值為 true，則記錄會在達到大小上限時自動備份。 如果此值為 true，則保留（使用 **/rt**選項指定）也必須設定為 true。                                                                                                                                    |
|   /ms：\<MaxSize>   |                                                                                                                                                                        設定記錄檔的大小上限（以位元組為單位）。 記錄大小的最小值是1048576個位元組（1024KB），而記錄檔一律是64KB 的倍數，因此您輸入的值將會相應地四捨五入。                                                                                                                                                                         |
|    /l：\<層級>     |                                                                                                                                                                     定義記錄檔的層級篩選。 \<層級> 可以是任何有效的層級值。 此選項只適用于具有專用會話的記錄。 您可以藉由將設定<Level>為0來移除層級篩選。                                                                                                                                                                      |
...
|    /c：\<Config>    |                                                                                                                                  指定設定檔案的路徑。 此選項將會從 Config> 中\<定義的設定檔讀取記錄檔屬性。 如果您使用此選項，則不得指定<Logname>參數。 記錄檔名稱會從設定檔案中讀取。                                                                                                                                   |
...
|   /e：\<元素>    |                                                                                                                                                           在 XML 中顯示事件時包含根項目。 \<元素> 是您想要在根項目內的字串。 例如， **/e： root**會產生 XML，其中包含根項目配對\<根>。 </root>                                                                                                                                                            |
|  /ow：\<覆寫>  |                                                                                                                                                                 指定應該覆寫匯出檔案。 \<覆寫> 可以是 true 或 false。 若為 true，且中<Exportfile>指定的匯出檔案已經存在，則會覆寫該檔案而不進行確認。                                                                                                                                                                 |
|   /bu：\<備份>    |                                                                                                                                                                                                      指定將儲存已清除之事件的檔案路徑。 在備份檔案的名稱中包含 .evtx 副檔名。                                                                                                                                                                                                       |
|    /r：\<遠端>    |                                                                                                                                                                                            在遠端電腦上執行命令。 \<遠端> 是遠端電腦的名稱。 **Im**和**um**參數不支援遠端操作。                                                                                                                                                                                            |
|   /u：\<Username>   |                                                                                                                                                                          指定不同的使用者登入遠端電腦。 \<Username> 是以 domain\user 或 user 格式的使用者名稱。 只有在指定 **/r**選項時，才適用此選項。                                                                                                                                                                          |
|   /p：\<Password>   |                                                                                                                                               指定使用者的密碼。 如果使用 **/u**選項，但未指定此選項，或\<密碼> 為 *，則會提示使用者輸入密碼。只有\* \*在指定/u* \*選項時，才適用此選項。                                                                                                                                                |
|     /a：\<Auth>     |                                                                                                                                                                                             定義用來連接到遠端電腦的驗證類型。 \<Auth> 可以是預設、Negotiate、Kerberos 或 NTLM。 預設值為 Negotiate。                                                                                                                                                                                              |
|  /uni：\<Unicode>   |                                                                                                                                                                                                             以 Unicode 顯示輸出。 \<Unicode> 可以是 true 或 false。 如果<Unicode>為 true，則輸出為 Unicode。                                                                                                                                                                                                             |
