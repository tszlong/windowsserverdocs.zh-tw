---
title: winrs
description: 適用于 winrs 的 Windows 命令主題，可讓您從遠端系統管理及執行程式。
...
|           參數            |                                                                                                                                                                                    描述                                                                                                                                                                                     |
|--------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|      /remote：\<端點 >       |                                                                                          使用 NetBIOS 名稱或標準連接來指定目標端點：<p>-   <url>： [\<transport >：//]\<目標 > [：\<埠 >]<p>如果未指定，則會使用 **/r： localhost** 。                                                                                          |
...
|      /timeout：\<秒數 >       |                                                                                                                                                                             此選項已被取代。                                                                                                                                                                             |
|       /目錄：\<路徑 >       |                                                                                            指定遠端 shell 的起始目錄。<p>如果未指定，則遠端 shell 將會在環境變數 **% USERPROFILE%** 定義的使用者主目錄中啟動。                                                                                             |
| /environment：\<字串 > =<value> |                                                                          指定要在 shell 啟動時設定的單一環境變數，這可讓您變更 shell 的預設環境。<p>這個參數的多次出現必須用來指定多個環境變數。                                                                          |
