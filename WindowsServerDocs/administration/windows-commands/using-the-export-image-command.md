---
title: 匯出-影像
description: 匯出-影像的 Windows 命令主題，它會將現有的映射從映射存放區匯出到另一個 Windows 映像（.wim）檔案。
...
|[/Overwrite： {Yes &#124;否&#124; append}]|判斷當/Filepath. 中已經存在具有該名稱的現有檔案時，是否會覆寫 **/DestinationImage**選項中指定的檔案。<p>-    **[是]** 會導致覆寫現有的檔案。<br />-   **否** （預設選項）會導致發生錯誤（如果已經有同名的檔案存在）。<br />-   **附加**會導致產生的映射附加為現有 .wim 檔案內的新映射。|
...
使用 [映射]](using-the-get-image-command.md)命令
