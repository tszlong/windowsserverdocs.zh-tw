...
|/WdsUnattend|這些設定會控制 Windows 部署服務用戶端的自動安裝行為。 您可以使用下列選項來設定設定：<p>-[/Policy： {Enabled &#124; Disabled}]-指定是否要使用自動安裝。<br />-[/CommandlinePrecedence： {Yes &#124; No}]-指定在用戶端安裝期間，將會使用以/Unattend 選項直接傳遞至 Windows 部署服務用戶端的 Autounattend.xml 檔案（如果存在於用戶端）或自動安裝檔案（而不是映射自動安裝檔案）。 預設設定為 [**否**]。<br />-[/File： <Relative path> /Architecture： {x86 &#124; ia64 &#124; x64}]-指定自動安裝檔案的檔案名、路徑和架構。|
