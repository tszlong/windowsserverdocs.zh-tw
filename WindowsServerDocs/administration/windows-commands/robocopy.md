...
|    \<File >     | 指定要複製的檔案。 如有需要，您可以 **&#42;** 使用萬用字元（或 **？** ）。 如果未指定**File**參數， **\*. \\** \* 當做預設值使用。 |
...
|/z|在可重新開機的模式中複製檔案。|
|/b|以備份模式複製檔案。|
|/zb|使用可重新開機模式。 如果存取被拒，此選項會使用備份模式。|
...
|/dcopy： \<copyflags @ no__t-1|定義要為目錄複寫哪些內容。 預設值為 DA。 選項為 D = data、A = attributes 和 T = 時間戳記。|
...
|/mon： \<N >|會監視來源，並在偵測到超過*N*個變更時再次執行。|
...
|/min： \<N >|指定最小檔案大小（以排除小於*N*個位元組的檔案）。|
...
|選項|描述|
|------|-----------|
|/r： \<N >|指定失敗複本上的重試次數。 *N*的預設值是1000000（1000000重試）。|
...
|/nfl|指定不記錄檔案名。|
|/ndl|指定不記錄目錄名稱。|
|/np|指定複製作業的進度（到目前為止複製的檔案或目錄數目）將不會顯示。|
