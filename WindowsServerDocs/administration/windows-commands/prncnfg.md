...
| `{+|-}`發佈 | 指定此印表機是否應在 active directory 中發佈。 如果您發行印表機，其他使用者可以根據其位置和功能（例如色彩列印和裝訂）來搜尋它。 |
| `{+|-}`隱藏 | 保留的函式。 |
...
| `{+|-}`enabledevq | 指定不符合印表機安裝程式的列印工作（例如，從多工緩衝處理到非 PostScript 印表機的 PostScript 檔案）是否應保留在佇列中，而不是列印。 |
