...
|（@no__t 0Set >）|必要。 指定一或多個檔案、目錄或文字字串，或要在其上執行命令的值範圍。 需要括弧。|
...
- 使用批次參數
  ...
  下列屬性適用于**for**命令：  
  - **For**命令會以指定集合中的每個文字字串取代 **%** <em>變數</em>或 **@no__t 5**<em>變數</em>，直到指定的命令處理所有檔案為止。
...
- **針對**使用其他形式的
  ...
  - 反覆運算和檔案剖析
    ...
    *Set*引數會指定一個或多個檔案名。 在移至*集合*中的下一個檔案之前，會先開啟、讀取和處理每個檔案。 若要覆寫預設剖析行為，請指定*ParsingKeywords*。 這是加上引號的字串，其中包含一或多個用來指定不同剖析選項的關鍵字。
    ...
    |     usebackq      |                                                                                             指定要：執行反向加上引號的字串做為命令、使用單引號的字串做為常值字串，或者，對於包含空格的長檔名，允許 *\<Set @ no__t-2*中的檔案名，分別以雙引號括住。                                                                                              |
...
- 剖析字串
  ...
  您可以在立即字串上使用**for/f**剖析邏輯，方法是以雙引號（*不含*"usebackq"）或單引號（*含*"usebackq"）括住 *\<LiteralString @ no__t-* -例如（"MyString"）或（'MyString '）。 *\<LiteralString @ no__t-2*會視為檔案輸入的一行。 剖析以雙引號括住 *\<LiteralString @ no__t-2*時，會將命令符號（例如 **\\ \& \|** \> \< \^）視為一般字元。
- 剖析輸出

  您可以使用**for/f**命令來剖析命令的輸出，方法是將括在括弧中的 *\<command @ no__t*加上引號。 它會被視為命令列，它會傳遞至子 Cmd.exe。 輸出會捕獲到記憶體中，並剖析成檔案。
