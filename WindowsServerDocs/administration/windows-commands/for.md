...
-   使用批次參數
    ...
    下列屬性套用至**針對**命令：  
    -   **For** siguiente reemplaza **%* * * 變數*或 **%%* * * 變數*與指定的命令處理的所有檔案之前指定集合中每個文字字串。
...
-   指定檔案的群組
    ...
    *設定*參數可代表一組檔案或數個檔案群組。 您可以使用萬用字元 (**&#42;** 並 **？**) 指定的檔案設定。 有效的檔案集如下：  
    ...
    (ar??1991.* ap??1991.*)
    ```  
    當您使用**針對**命令時中的第一個值*設定*取代 **%* * * 變數*或 **%%* * * 變數*，然後指定命令會處理此值。 這會繼續直到所有檔案 （或檔案群組） 對應至*設定*處理值。
-   使用**中**並**請勿**關鍵字
