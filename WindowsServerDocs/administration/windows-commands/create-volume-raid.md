...
description: 建立磁片區 raid 命令的參考主題，它會使用三個或多個指定的動態磁碟建立 RAID-5 磁片區。
...
| 磁片 =`<n>,<n>,<n>[,<n>,...]` | 要在其中建立 RAID-5 磁片區的動態磁碟。 您至少需要三個動態磁碟以建立一個 RAID-5 磁碟區。 每個磁片上配置了`size=<n>`等於的空間量。 |
