...
若要建立基準，您應該測量各種系統負載點所需的平均耗電量，從閒置到100% （最大輸送量）以產生負載線。 下圖顯示三個範例設定的載入行：
...
如果您的伺服器需要超低延遲、不穩定的 CPU 頻率（例如，以進行可重複的測試）或最高效能層級，則您可能不希望處理器切換至較低的效能狀態。 對於這類伺服器，您可以使用下列命令，將最低處理器效能狀態上限為100%：
