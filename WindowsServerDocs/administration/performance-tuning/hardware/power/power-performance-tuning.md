...
ms.locfileid: "71370545"
---
# <a name="power-and-performance-tuning"></a>電源與效能調整

在企業和資料中心環境中，能源效率日益重要，並且在混合設定選項方面增加了另一組取捨。

Windows Server 2016 已針對各種客戶工作負載的最小效能影響，優化了絕佳的能源效率。 [Windows Server 平衡電源計劃的處理器電源管理（PPM）調整](processor-power-management-tuning.md)說明用來在 windows server 2016 中調整預設參數的工作負載，並提供自訂 tunings 的建議。
...
您可以使用此計量來設定符合電源與效能之間取捨的實際目標。 相較之下，跨資料中心節省 10% 能源的目標，無法捕獲相對於效能的影響，反之亦然。

同樣地，如果您將伺服器調整為增加 5% 的效能，並導致 10% 的能源消耗較高，則總結果可能或可能無法接受您的商務目標。 「能源效率」度量可讓您以更明智的決策為依據，而不是僅限電源或效能計量。
...
判斷您的伺服器是否支援計量和預算的其中一種方式，就是查看[Windows Server Catalog](http://www.windowsservercatalog.com)。 如果您的伺服器模型符合 Windows 硬體認證計畫中新的增強電源管理合格規定，保證會支援計量和預算功能。
...
| 省電 | 限制效能以節省能源並降低營運成本。 不建議在不進行徹底測試的情況下，確定效能是否足夠。 | 具有有限功率預算和熱條件約束的部署 | 以最大百分比（如果支援）的上限處理器頻率，並啟用其他省電功能。 |


這些電源計劃存在於 Windows 中，可用於替代目前（AC）和直接目前（DC）的系統，但我們會假設伺服器一律使用 AC 電源來源。
...
Intel Turbo 加速和 AMD Turbo CORE 技術是一種功能，可讓處理器在最有用（也就是在系統負載較高）時達到額外效能。 不過，這項功能會增加 CPU 核心能源耗用量，因此 Windows Server 2016 會根據使用中的電源原則和特定的處理器實現來設定 Turbo 技術。
...
若為 P 狀態控制項，則會停用、啟用（當要求名義效能時，可以在硬體上使用 Turbo）和有效率（只有在執行 EPB 暫存器時才可使用 Turbo）的選項。

針對以 CPPC 為基礎的控制項，選項會停用、有效率地啟用（Windows 會指定要提供的正確 Turbo 量），以及積極的（Windows 會要求「最大效能」來啟用 Turbo）。
...
若要針對目前所選方案以外的電源計劃設定此值，您可以使用別名，例如\_配置上限（省電）、\_配置最小值（高效能）\_和配置平衡（平衡）來取代配置\_目前。 以所需的別名取代前面所示的 setactive 命令中的「配置目前」，以啟用該電源計劃。
...
處理器會非常快速地在效能狀態（P-狀態）之間變更，以符合提供給需求，盡可能在必要時提供效能並節省能源。 如果您的伺服器具有特定的高效能或最小耗電量需求，您可以考慮設定**最低處理器效能狀態**參數或**最大處理器效能狀態**參數。

**最低處理器效能狀態**和**最大處理器效能狀態**參數的值會以最大處理器頻率的百分比表示，其值為0–100。

如果您的伺服器需要超低延遲、不穩定的 CPU 頻率（例如，以進行可重複的測試）或最高效能層級，則您可能不希望處理器切換至較低的效能狀態。 對於這類伺服器，您可以使用下列命令，將最低處理器效能狀態上限為 100%：
...
如果您的伺服器需要較低的能源消耗，您可能會想要將處理器效能狀態的上限設為最大值的百分比。 例如，您可以使用下列命令，將處理器的最大頻率限制為 75%：
...
核心停車是 Windows Server 2008 R2 中引進的功能。 處理器電源管理（PPM）引擎和排程器會共同運作，以動態調整可執行執行緒的核心數目。 PPM 引擎會為要排程的執行緒選擇最小的核心數目。
...
如果您的伺服器有特定的核心停車需求，您可以使用**處理器效能核心停車最大核心**參數或**處理器效能核心停車最小核心數，控制可供公園使用的核心數目**Windows Server 2016 中的參數。
...
這些參數的值是範圍0–100中的百分比。 **處理器效能核心停車最大核心**數參數會控制可以隨時離開的核心百分比上限（可用於執行執行緒），而**處理器效能核心的停車最小核心**參數控制可被離開之核心的最小百分比。 若要關閉核心停車，請使用下列命令，將**處理器效能核心停車最小核心**參數設定為 100%：
...
若要將可排程核心的數目減少到最大計數的 50%，請將**處理器效能核心停車最大核心**參數設定為50，如下所示：
...
公用程式散發是 Windows Server 2016 中的演算法優化，其設計目的是為了改善某些工作負載的電源效率。 它會追蹤無法移動的 CPU 活動（也就是 Dpc、插斷或嚴格相似化為的執行緒），並根據假設，所有可移動的工作都可以在所有已離開的核心上平均散發，來預測每個處理器的未來工作。
