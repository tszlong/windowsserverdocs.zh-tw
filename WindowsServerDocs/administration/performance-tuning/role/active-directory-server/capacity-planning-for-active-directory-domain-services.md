...
> [!NOTE]
> 重點在於資料庫的讀取，因為這通常是最嚴苛的元件，所以可以將相同的邏輯套用至寫入記錄檔，方法是以 LogicalDisk （ *\<NTDS Log\>* ） \Avg Disk sec/Write 和 LogicalDisk （ *\<NTDS Log\>* ） \ writes/sec）取代：
>
> - LogicalDisk （ *\<NTDS\>* ） \Avg Disk sec/Read 指出目前的儲存體是否有適當的大小。  如果結果大致等於磁片類型的磁片存取時間，則 LogicalDisk （ *\<NTDS\>* ） \ 讀取/秒是有效的量值。  查看後端儲存體的製造商規格，但 LogicalDisk （ *\<NTDS\>* ） \Avg Disk sec/Read 的良好範圍大致如下：
>   - 7200–9到12.5 毫秒（毫秒）
>   - 10000–6到10毫秒
>   - 15000–4到6毫秒
>   - SSD –1到3毫秒
...
- 特別是新增/優化 RAM，將會降低讀取 i/o （LogicalDisk （ *\<NTDS\>* ） \ Reads/Sec）的數量。 這表示儲存體解決方案可能不需要與一開始計算的健全。  可惜的是，比這個一般語句更具體的專案，是取決於用戶端負載的環保，而且無法提供一般指引。  最佳選項是在優化 RAM 之後調整儲存體大小。
...
- LogicalDisk （ *\<NTDS Database Drive\>* ） \ 每秒在尖峰期間的傳輸數15分鐘期間
...
針對現有的環境，由於先前已討論過儲存體大小調整，因此假設儲存體現在已適當調整大小，因此處理器負載的相關資料是有效的。 再次重申，確保系統中的瓶頸不是存放裝置的效能，這是很重要的。 當瓶頸存在且處理器正在等候時，一旦移除瓶頸，就會發生閒置狀態。  隨著處理器等候狀態的移除，根據定義，CPU 使用率會增加，因為它不再需要等待資料。 因此，收集效能計數器「Logical Disk （ *\<NTDS Database Drive\>* ） \Avg Disk sec/Read」和「Process （lsass） \\ % Processor Time」。 \\如果「邏輯磁片（） \Avg Disk sec/Read」超過10到15毫秒，則「進程（lsass）% 處理器時間」中的資料會是人為低的 *\<NTDS Database Drive\>* 值，這是 Microsoft 支援用來針對儲存體相關的效能問題進行疑難排解的一般閾值。 如前所述，建議的取樣間隔為15、30或60分鐘。 任何較少的專案通常會太大，以致于良好的測量。任何較大的東西都會非常順暢地進行每日查看。
...
此外，如果網站中的應用程式和用戶端使用尋找網域控制站的最佳做法（也就是使用[DsGetDcName](https://docs.microsoft.com/windows/win32/api/dsgetdc/nf-dsgetdc-dsgetdcnamea)函式），則用戶端應該相對平均分散，因為有任何數目的因素。
...
「進程 \\ 處理器時間百分比」效能物件計數器會加總應用程式的所有線程花費在 CPU 上的時間總計，並除以已通過的系統時間總量。 這是因為多 CPU 系統上的多執行緒應用程式可能會超過100% 的 CPU 時間，而且其轉譯方式與「處理器資訊 \\ % 處理器公用程式」截然不同。 實際上，「進程（lsass） \\ % 處理器時間」可視為在100% 執行以支援處理常式需求所需的 cpu 計數。 值為200% 時，表示需要2個 Cpu，分別是100%，以支援完整的 AD DS 負載。 雖然在100% 容量上執行的 CPU 是最符合成本效益的，但從 Cpu 和電源和能源耗用量的金錢觀點來看，有許多原因，例如附錄 A 中的詳細說明，當系統在100% 執行時，會有更佳的多執行緒系統回應能力。
...
> *New_MaxConcurrentApi_setting* &ge;（*semaphore_acquires*  + *semaphore_time*時）&times; *average_semaphore_hold_time* &divide; *time_collection_length*
...
因此，有關更多或更快處理器的細節會對應用程式行為造成高度主觀，在 AD DS 的情況下是非常環保，甚至是在環境內與伺服器之間的差異。 這就是為什麼發行項中稍早的參考不太精確，而且計算中包含了安全性邊界的原因。 進行預算驅動的購買決策時，建議您先將40% （或所需的環境）的處理器使用優化，再考慮購買更快的處理器。 在更多處理器上增加的同步處理可減少線性進展中更多處理器的真正優勢（2個 &times; 處理器數目提供少於2個 &times; 可用的額外計算能力）。
...
- 最後一種情況是，雖然它可能會讓您的細節更大，但匯流排已滿，但道路並不擁擠。 雖然附件在進入和關閉匯流排時仍有問題，但在匯流排的道路之後，旅程也會很有效率。 這是新增更多匯流排（對來賓的邏輯處理器）來改善來賓效能的唯一案例。
...
1. 若要判斷修飾詞，請使用下列方程式：
   >（（*目標平臺每個核心分數值*） &times;（*基準平臺的每個核心 MHz*）&divide;（（*每個核心的基準分數值*） &times; （*目標平臺的每個核心 MHz*）
...
- **PCI 匯流排–** 這是一個經常被忽略的元件。 在此範例中，這不會是瓶頸;不過，隨著系統相應增加，它可能會成為瓶頸。 如需參考，32位 PCI 匯流排在33Mhz 運作，可在理論上傳輸 133 MB/秒的資料。 以下是方程式：
  ...
  66Mhz 64 位 PCI 匯流排可支援理論上限（64位 &divide; 每個位元組的8位 &times; 66 Mhz） = 528 MB/秒。此外，其他任何裝置（例如網路介面卡、第二個 SCSI 控制器等等）將會減少頻寬共用時可用的頻寬，而且裝置會爭用有限的資源。
...
> *每個主軸* &times; 的最大 IOPS2 &times; 個主軸 [（*% 讀取*  +  *% 寫入*） &divide; （*% 讀取*+ 2 &times; *% 寫入*）] = *IOPS 總計*

RAID 1 + 0 的行為與 RAID 1 完全相同，關於讀取和寫入的費用。 不過，i/o 現在會在每個鏡像集合中等量。 If

> *每個主軸* &times; 的最大 IOPS2 &times; 個主軸 [（*% 讀取*  +  *% 寫入*） &divide; （*% 讀取*+ 2 &times; *% 寫入*）] = *Total i/o*

在 RAID 1 集合中，當 RAID 1 集的多重性（*N*）為等量時，可以處理的總 i/o 會 &times; 每個 RAID 1 設定為 N 個 i/o：

> *N* &times; {*每個主軸2主軸的最大 IOPS* &times; &times; [（*%* 讀取  +  *% 寫入*） &divide; （*% 讀取*+ 2 &times; *% 寫入*）]} = *iops 總數*
...
> *每個主軸* &times; 的 IOPS（*主軸*–1） &times;[（*%* 讀取  +  *% 寫入*） &divide; （*% 讀取*+ 4 &times; *% 寫入*）] = *IOPS 總計*
...
在任何配管設計中，會將多個耗盡摘要放入主清空。 如果任何專案停止了其中一個耗盡或連接點，則只會備份該連接點背後的東西。 在儲存案例中，這可能是多載交換器（SAN/NAS/iSCSI 案例）、驅動程式相容性問題（錯誤的驅動程式/HBA 固件/storport.sys 組合），或備份/防毒軟體/磁碟重組。 若要判斷存放裝置「管道」是否夠大，必須測量 IOPS 和 i/o 大小。 每個聯合都會將它們相加，以確保有足夠的「管道直徑」。
...
針對正常作業狀況，儲存體規劃目標會將要求的等候時間最小化，從磁片傳回 AD DS。 這基本上表示未處理和擱置 i/o 的數目小於或等於磁片的路徑數目。 有多種方法可以測量這種情況。 在效能監視案例中，一般建議是 LogicalDisk （ *\<NTDS Database Drive\>* ） \Avg Disk sec/Read 小於20毫秒。 所需的操作閾值必須較低，最好是盡可能接近儲存體的速度，在2到6毫秒（.006 second）範圍內，視儲存體類型而定。
