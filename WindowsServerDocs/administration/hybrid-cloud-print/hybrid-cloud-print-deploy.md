...
- 步驟 3：安裝 Azure 應用程式 Proxy (AAP) 和 Kerberos 限制委派 (KCD)
- 步驟 4：設定必要的 MDM 原則
- 步驟 5：發佈共用的印表機
...
### <a name="step-3---install-azure-application-proxy-aap-with-kerberos-constrained-delegation-kcd"></a>步驟 3： 安裝 Azure 應用程式 Proxy (AAP) 與 Kerberos 限制委派 (KCD)
...
3. Active Directory 在電腦上，請移至**工具]-> [使用者和電腦**
    - 選取的機器，執行應用程式 Proxy 連接器
    - 以滑鼠右鍵按一下並選取**內容-> 委派** 索引標籤
    - 選取**信任這台電腦所委派指定的服務。**
    - 選取**使用任何驗證通訊協定。**
    - 在下**的這個帳戶可以呈送委派的認證的服務**
        - 新增執行服務 （MopriaDiscoveryService 和 MicrosoftEnterpriseCloudPrint 服務） 的電腦的 SPN 身分識別值
            - Spn，輸入該電腦，SPN 也就是「 主機 /\<MachineName\>。\<網域\>"<br>
                `HOST/appServer.Contoso.com`
4. 返回至 AAD 租用戶管理入口網站，並新增應用程式 proxy
   - 移至**企業應用程式** 索引標籤
   - 按一下 **新的應用程式**
   - 選取 **在內部部署應用程式**並填寫各欄位
       - 名稱：任何您想要的名稱
   ...
   - 建立之後，請回到**企業應用程式** -> **所有應用程式**，選取您剛才建立的新應用程式
   - 移至**單一登入**，確定 「 單一登入模式 」 設定為 [整合式 Windows 驗證]
   - 上述步驟 3.3 中所指定的 spn 設定 」 內部的應用程式 SPN"
   - 請確定 「 委派的登入身分識別 」 已設定為 「 使用者主體名稱 」

5. 重複上述的 4，企業雲端列印服務和內部的 URL 提供給您的企業雲端列印服務
6. 回到 Azure AD 租用戶管理入口網站並移至**應用程式註冊**並選取 原生用戶端應用程式-> 設定
    - 選取**必要權限**
        - 新增 2 個新您剛才建立的 proxy 應用程式
        - 授與這些 2 個應用程式的委派權限
        - 一旦已加入兩個 proxy 應用程式，請按一下 [授與權限]。  選取 [是] 當系統提示您核准要求。

7. Mopria 雲端服務和企業雲端列印服務電腦啟用 IIS 中的 Windows 驗證
    - 請確定已安裝 Windows 驗證功能：
        1. 開啟 [伺服器管理員]
        2. 按一下 **管理**
        3. 按一下 **新增角色及功能**
        4. 選取**角色型或功能型安裝**
        5. 選取的伺服器
        6. 在 Web 伺服器 (IIS)-> Web 伺服器-> 安全性，請選取  **Windows 驗證**
        7. 您完成安裝之前，請按下一步
    - 啟用 IIS 中的 Windows 驗證：
        1. 開啟 Internet Information Services (IIS) 管理員
        2. 每個服務/站台：
            1.  選取的服務/站台
            2.  按兩下 **驗證**
            3.  按一下  **Windows 驗證**然後按一下**啟用**下**動作**
...
- 找到企業雲端列印原則的群組，並設定以下的指導方針的原則：
  ...
  - MopriaDiscoveryResourceId = Mopria 探索服務 Azure 應用程式 Proxy 的步驟 3.4 中建立的外部 URL (必須完全相同包括結尾的 /)
  - CloudPrintResourceId = 企業雲端列印服務 Azure 應用程式 Proxy 的建立步驟 3.5 中的外部 URL (必須完全相同包括結尾的 /)
...
- OMA-URI
    ...
    - `MopriaDiscoveryResourceId = ./Vendor/MSFT/Policy/Config/EnterpriseCloudPrint/MopriaDiscoveryResourceId`
        - 值 = Mopria 探索服務 Azure 應用程式 Proxy 的步驟 3.4 中建立的外部 URL (必須完全相同包括結尾的 /)
    - `CloudPrintResourceId = ./Vendor/MSFT/Policy/Config/EnterpriseCloudPrint/CloudPrintResourceId`
        - 值 = 企業雲端列印服務 Azure 應用程式 Proxy 的建立步驟 3.5 中的外部 URL (必須完全相同包括結尾的 /)
...
5. Windows 10 Fall Creator Update 機器上，開啟提升權限的 Windows PowerShell 命令提示字元
   1. 執行下列命令
      ...
      - Publish-CloudPrinter
        ...
        - DiscoveryEndpoint = Mopria 探索服務 Azure 應用程式 Proxy 的步驟 3.4 中建立的外部 URL
        - PrintServerEndpoint = 企業雲端列印服務 Azure 應用程式 Proxy 的建立步驟 3.5 中的外部 URL
...
- 開啟網頁瀏覽器並移到 https://&lt;services 機器端點 &gt; /mcs/services (的探索端點的外部 URL)
