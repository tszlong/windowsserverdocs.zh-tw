...
如需 Microsoft 在安全通道 SSP 中執行 TLS 和 SSL 的詳細資訊，請參閱[TLS/SSL 技術參考 (2003) ](/previous-versions/windows/it-pro/windows-server-2003/cc784149(v=ws.10))。
...
將 TLS 連接到伺服器的裝置經常因工作階段逾時而需要重新連線。 Windows 8.1 和 Windows Server 2012 R2 現在支援 RFC 5077， (TLS 會話繼續，而不需要伺服器端狀態) 。 此修改為 Windows Phone 和 Windows RT 裝置提供：
...
-   預設會關閉傳送受信任簽發者清單的行為： SendTrustedIssuerList 登錄機碼的預設值現在預設為 0 (關閉) ，而不是1。
...
雖然安全通道 SSP 支援的受信任憑證授權單位單位清單大小上限 (16 KB) 維持與 Windows Server 2008 R2 相同，但在 Windows Server 2012 中，用戶端驗證簽發者會有新的專用憑證存放區，因此訊息中不會包含不相關的憑證。
...
如果使用的「**受信任的根憑證授權**單位」存放區包含根 (自我簽署) 和憑證授權單位單位 (CA) 簽發者憑證，則依預設只會將 Ca 簽發者憑證傳送至伺服器。
...
如需使用 PowerShell 提供者來管理憑證的相關資訊，請參閱 [Windows 中的 AD CS 管理 Cmdlet](/powershell/module/adcsadministration/?view=winserver2012-ps)。
...
### <a name="tls-support-for-server-name-indicator-sni-extensions"></a><a name="BKMK_SNI"></a>伺服器名稱指標的 TLS 支援 (SNI) 延伸模組
