...
此適用于 IT 專業人員的參考包含傳輸層安全性 (TLS) 通訊協定、安全通訊端層 (SSL) 通訊協定，以及資料包傳輸層安全性 (以安全通道安全性支援提供者) SSP (執行的) 通訊協定的相關資訊。

這些通訊協定提供了一種方法，可使用憑證型驗證和對稱式加密金鑰，保護在不受信任網路上的應用程式之間傳送的資料。
