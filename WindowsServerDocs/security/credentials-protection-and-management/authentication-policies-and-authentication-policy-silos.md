...
驗證原則定址接收器和伴隨的原則提供一個方法，將高權限認證限制在只與選取的使用者、電腦或服務相關的系統。 您可以使用 Active Directory 管理中心和 Active Directory 的 Windows PowerShell Cmdlet，在 Active Directory Domain Services （AD DS）中定義和管理定址接收器。
...
驗證原則定義帳戶類型的 Kerberos 通訊協定票證授與票證 (TGT) 的存留期屬性以及驗證存取控制條件。 原則建置於上，並控制稱為「驗證原則定址接收器」的 AD DS 容器。
...
Active Directory 帳戶類型會將呼叫者的角色判斷為下列其中一項：

-   **使用者**

    使用者應該一律是 Protected Users 安全性群組的成員，它預設會拒絕使用 NTLM 進行驗證的嘗試。

    您可以設定原則，將使用者帳戶的 TGT 存留期設為較短的值，或限制使用者帳戶可以登入的裝置。 您可以在驗證原則中設定豐富的運算式，以控制使用者和其裝置必須符合才能向服務進行驗證的條件。
...
### <a name="BKMK_HowKerbUsed"></a>如何搭配使用 Kerberos 通訊協定與驗證原則定址接收器和原則
...
當驗證原則處於 audit 模式，且網域控制站收到票證授與服務要求的網域帳戶時，網域控制站會根據要求的票證許可權屬性憑證，檢查是否允許驗證。（PAC）資料，它會在失敗時記錄一則警告訊息。 PAC 包含各種類型的授權資料，包括使用者所屬的群組、使用者具有的權限以及套用到使用者的原則。 此資訊用來產生使用者的存取權杖。 如果它是強制驗證原則，允許驗證使用者、裝置或服務，則網域控制站會根據要求的票證 PAC 資料，檢查是否允許驗證。 如果失敗，網域控制站會傳回錯誤訊息，並記錄事件。
...
### <a name="BKMK_HowRestrictingSignOn"></a>限制使用者登入的運作方式
...
當使用者登入 Windows 或在應用程式的認證提示中輸入其網域認證時，Windows 預設會將未防護的 AS-REQ 傳送給網域控制站。 如果使用者是從不支援防護的電腦傳送要求，例如執行 Windows 7 或 Windows Vista 的電腦，則要求會失敗。
...
允許帳戶，因為該帳戶符合驗證原則所設定的存取控制條件，而且具有 TGT 的使用者嘗試連線到服務（例如，開啟的應用程式需要驗證服務，由服務的 SPN 識別），會發生下列順序：
...
## <a name="BKMK_ErrorandEvents"></a>關聯的錯誤和資訊事件訊息
