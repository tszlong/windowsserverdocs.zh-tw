...
如果控制流程防護檢查失敗，在執行階段，Windows Server 2016 立即終止程式，來間接呼叫了無效的位址會嘗試任何攻擊的重大。 控制流程防護 Device Guard 提供重要的額外的保護。 如果遭入侵的允許清單的應用程式，它就是能夠執行未檢查的 Device Guard 的情況，因為檢測會看到應用程式已經過簽署，並會被視為 Device Guard 的受信任。
