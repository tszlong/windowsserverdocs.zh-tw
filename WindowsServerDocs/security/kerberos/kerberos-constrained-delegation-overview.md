...
如需這些延伸模組的執行資訊[, 請參閱\] \[ms-chap:Kerberos 通訊協定延伸：MSDN 中的 Service for User 和限制](https://msdn.microsoft.com/library/cc246071(PROT.10).aspx)委派通訊協定規格。
...
您必須在執行 Windows Server 之前作業系統的前端和後端網域之間, 于使用者帳戶網域中的所有域2012控制器上套用下列修補程式:以 Windows Server 2008 R2 為基礎之網域控制站的環境中, 以資源為基礎的限制 https://support.microsoft.com/en-gb/help/2665790/resource-based-constrained-delegation-kdc-err-policy-failure-in-enviro) 委派 KDC_ERR_POLICY 失敗 (。
