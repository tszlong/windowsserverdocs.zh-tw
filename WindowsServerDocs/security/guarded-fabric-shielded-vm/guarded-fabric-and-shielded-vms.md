...
為了協助防範遭到入侵的虛擬化網狀架構，Windows Server 2016 Hyper-v 引進了受防護的 Vm。 受防護的 VM 是第2代 VM (在具有虛擬 TPM 的 Windows Server 2012 和更新版本) 上受到支援，會使用 BitLocker 加密，而且只能在網狀架構中狀況良好且已核准的主機上執行。 受防護的 VM 和受防護網狀架構可讓雲端服務提供者或企業私人雲端系統管理員為租用戶 VM 提供更安全的環境。
...
|將偵錯工具 (連結至 VM 進程) <sup>1</sup>| 支援          | 停用 (無法啟用) |

<sup>1</sup>因為 VM 的工作者進程 ( # A1) 是受保護的進程 LIGHT (PPL) ，所以直接附加至進程的傳統偵錯工具（例如 WinDbg.exe）會遭到封鎖。
...
2. **主機要求證明。** 受防護主機會要求證明。 證明模式取決於「主機守護者服務」：
    ...
    - **TPM 信任證明**： hyper-v 主機傳送的資訊包括：
      ...
      - 在主機上套用的程式碼完整性 (CI) 原則的相關資訊。

        主機啟動後便會發生證明，之後每隔 8 小時發生。 如果因為某些原因而主機在 VM 嘗試啟動時沒有證明憑證，這也會觸發證明。
...
4. **證明憑證傳送給主機。** 假設證明成功，會將健康情況憑證傳送至主機，並將主機視為「受防護」 (授權) 執行受防護的 Vm。 主機會使用健康情況憑證來授權金鑰保護服務以安全地發行和受防護的 VM 搭配使用的金鑰
