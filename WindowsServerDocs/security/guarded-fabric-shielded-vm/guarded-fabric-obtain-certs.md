...
SSL 憑證屬性 | 必要值
-------------------------|---------------
主體名稱             | HGS 叢集的名稱 (稱為「分散式網路名稱」或「虛擬電腦物件 FQDN) 。 這將是您提供的 HGS 服務名稱與 `Initialize-HgsServer` hgs 功能變數名稱的串連。
主體別名 | 如果您要使用不同的 DNS 名稱來連線到 HGS 叢集 (例如，如果它是在負載平衡器) 後方，請務必在您的憑證要求的 SAN 欄位中包含這些 DNS 名稱。
