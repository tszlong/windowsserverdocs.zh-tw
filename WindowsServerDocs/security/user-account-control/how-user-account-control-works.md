...
屬於 Administrators 群組成員的使用者可以在使用標準使用者存取權杖的情況下，登入、流覽網頁及閱讀電子郵件。 當系統管理員需要執行需要系統管理員存取權杖的工作時，Windows Server 2012 會自動提示使用者進行核准。 這個提示稱為提高權限提示，使用本機安全性原則嵌入式管理單元 (Secpol.msc) 或群組原則，即可設定其行為。
...
當標準使用者嘗試執行需要使用者系統管理存取權杖的工作時，就會顯示認證提示。 您可以使用本機安全性原則嵌入式管理單元 (Secpol.msc) 或群組原則，設定此標準使用者預設提示行為。 系統管理員也可以藉由設定 [使用者帳戶控制] 來提供他們的認證：系統管理員在管理核准模式原則設定值中的提高許可權提示行為，以提示輸入認證。
...
UAC 提高權限提示會依應用程式標示不同顏色，讓您可以立即識別應用程式的潛在安全性風險。 當應用程式嘗試以系統管理員的完整存取權杖執行時，Windows Server 2012 會先分析可執行檔以判斷其發行者。 應用程式會先根據可執行檔的發行者，分成三個類別：Windows Server 2012、發行者已驗證（已簽署）和發行者未通過驗證（不帶正負號）。 下圖說明 Windows Server 2012 如何判斷要向使用者呈現哪些色彩提高許可權提示。

以下是提高權限提示的色彩編碼：

-   紅色背景與紅色盾牌圖示：應用程式被群組原則封鎖，或來自封鎖的發行者。

-   藍色背景與藍色和金色盾牌圖示：應用程式是 Windows Server 2012 系統管理應用程式，例如 [控制台] 專案。

-   藍色背景與藍色盾牌圖示：應用程式是使用 Authenticode 簽署，而且受到本機電腦的信任。

-   具有黃色盾牌圖示的黃色背景：應用程式未簽署或已簽署，但尚未受到本機電腦信任。
...
將提示導向安全桌面，可以進一步保護提高權限處理程序。 根據預設，在 Windows Server 2012 中，同意和認證提示會顯示在安全桌面。 只有 Windows 處理程序可以存取安全桌面。 針對較高的安全性層級，建議您保留 @no__t 0User 帳戶控制：提示啟用提高許可權 @ no__t-0 原則設定時，切換到安全桌面。

可執行檔要求提高權限時，互動式桌面 (又稱為使用者桌面) 會切換至安全桌面。 安全桌面會將使用者桌面變暗，並顯示提高權限提示，您必須先回應該提示，才能繼續後續動作。 當使用者按一下 [是] 或 [否] 時，桌面會切換回使用者桌面。

惡意程式碼可以呈現安全桌面的仿造，但在使用者帳戶控制時：[管理核准模式中系統管理員的提高許可權提示的行為] 原則設定會設定為 [提示要求同意]，如果使用者在仿造上按一下 [是]，惡意程式碼就無法提高許可權。 如果原則設定是設定為 [提示認證]，則模仿認證提示的惡意程式碼可能可以向使用者收集認證。 不過，惡意程式碼不會得到提高後的權限而且系統會採取其他保護措施，讓惡意程式碼即使使用蒐集到的密碼也無法控制使用者介面。
...
|提升 ActiveX 安裝|如果未安裝 ActiveX，系統會檢查 UAC 滑桿等級。 如果已安裝 ActiveX，@no__t 0User 帳戶控制：[提示提高許可權時切換到安全桌面] @ no__t-0 群組原則設定已核取。|
|檢查 UAC 滑桿等級|UAC 現在有 4 個通知等級可以選擇，還有一個用於選擇通知等級的滑桿：<br /><br /><ul><li>高<br /><br />    如果滑桿設定為 [一律通知]，系統會檢查是否已經啟用安全桌面。</li><li>中等<br /><br />    如果滑杆設定為 [**預設-只有在程式嘗試變更我的電腦時才通知我**]，@no__t 1User 帳戶控制：只會檢查已簽署和已驗證 @ no__t-0 原則設定的可執行檔：<br /><br /><ul><li>如果啟用該原則設定，在允許執行指定的可執行檔之前，會先強制執行公開金鑰基礎架構 (PKI) 憑證路徑驗證。</li><li>如果沒有啟用該原則設定 (預設值)，在允許執行指定的可執行檔之前，不會強制執行 PKI 憑證路徑驗證。 @No__t 0User 帳戶控制：[提示提高許可權時切換到安全桌面] @ no__t-0 群組原則設定已核取。</li></ul></li><li>低<br /><br />    如果滑桿是設定成 [程式嘗試變更我的電腦時才通知我 (不要將桌面變暗)]，則會呼叫 CreateProcess。</li><li>永不通知<br /><br />    如果滑杆設定為 [**永遠不要通知我**]，當程式嘗試安裝或嘗試在電腦上進行任何變更時，UAC 提示永遠不會通知。 **重要事項：**   不建議使用這項設定。 此設定與設定 @no__t 0User 帳戶控制的方式相同：系統管理員在管理核准模式 @ no__t-0 原則設定中的提高許可權提示的行為，可在**不提示的情況下進行提升**。</li></ul>|
|安全桌面已啟用|@No__t 0User 帳戶控制：核取 [提示提高許可權時切換到安全桌面] @ no__t-0 原則設定：<br /><br />-如果已啟用安全桌面，則所有提高許可權要求都會移至安全桌面，無論系統管理員和標準使用者的提示行為原則設定為何。<br />-如果未啟用安全桌面，則所有提高許可權要求都會移至互動式使用者的桌面，並會使用系統管理員和標準使用者的每個使用者設定。|
...
Windows Server 2012 UAC 與舊版 Windows 之間的變更。 新的滑杆永遠不會完全關閉 UAC。 新的設定將會：
...
> [!IMPORTANT]
> 若要完全停用 UAC，您必須停用原則 **User 帳戶控制：在管理員核准模式中執行所有系統管理員 @ no__t-0。
...
-   版本控制資源欄位包含下列關鍵字：廠商、公司名稱、產品名稱、檔案描述、原始檔案名、內部名稱和匯出名稱。
...
> [!NOTE]
> 使用者帳戶控制：偵測應用程式安裝，並提示提高許可權原則設定必須啟用，安裝程式才可偵測安裝程式。 這項設定預設為啟用，且可以使用本機安全性原則嵌入式管理單元 (Secpol.msc) 在本機進行設定，或使用群組原則 (Gpedit.msc) 為網域、OU 或特定群組進行設定。
