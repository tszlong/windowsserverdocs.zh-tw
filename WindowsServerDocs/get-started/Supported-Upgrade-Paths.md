...
- **安裝** 是讓您的硬體有新作業系統的基本概念。 特別是， **全新安裝** 需要刪除之前的作業系統。 如需安裝 Windows Server 2016 的相關資訊，請參閱 [System Requirements and Installation Information for Windows Server 2016](https://technet.microsoft.com/windows-server-docs/get-started/system-requirements--and-installation) (Windows Server 2016 的系統需求與安裝資訊)。 如需安裝其他 Windows Server 版本的相關資訊，請參閱 [Windows Server 安裝與升級](https://technet.microsoft.com//windowsserver/dn527667)。
...
針對**Windows Server 2016 Essentials**:在命令中輸入零售、 大量授權或 OEM 金鑰，即可轉換成完整的零售版**slmgr.vbs**。
...
針對 Windows Server 2016 Standard，您可以將系統轉換到 Windows Server 2016 Datacenter，如下所示：從提升權限的命令提示字元，使用命令 **DISM /online /Get-CurrentEdition**判斷目前的版本名稱。 這會是針對 Windows Server 2016 Standard `ServerStandard`。 執行命令**DISM /online /enable-feature /Get-TargetEditions**若要取得您可以升級至版本的識別碼。 記下此版本的識別碼，以縮寫格式的版本名稱。 然後執行**DISM /online /Set-Edition:\<版本識別碼\>/productkey:-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX /AcceptEula**，提供您的目標和其零售版產品金鑰的版本識別碼。 伺服器將重新啟動兩次。
