...
您可以使用 Get-IISServerManager 傳回的 IISServerManager 物件來建立虛擬目錄，這會公開 .NET Microsoft.Web.Administration.ServerManager API。 在此範例中，這些命令會存取 Sites 集合的 "Default Web Site" 元素及 Applications 區段的根應用程式元素 ("/")。 接著會呼叫該應用程式元素之 VirtualDirectories 集合的 Add() 方法，來建立新的目錄：  
...
3.  在 Nano Server 上，使用下列命令，將憑證匯入「我的」存放區：  
