...
|\/xpr \[\<Activation&nbsp;ID\>] |顯示產品的啟用到期日。 根據預設，這是指目前的 Windows 版本且主要用於 KMS 用戶端，因為 MAK 和零售版啟用沒有期限。<br />指定 \<**Activation ID**\> 參數會顯示與該啟用識別碼相關聯之指定版本的啟用到期日。此操作不需要提升的權限。 |
