...
Microsoft 會定期發行 Windows Server 的兩個更新。 為了確保您的伺服器可以收到日後的更新 (包括安全性更新)，請務必持續更新這些伺服器。 如需已發行更新的完整清單，請查看 [Windows 10 及 Windows Server 2016 更新記錄](https://support.microsoft.com/help/4000825/windows-10-windows-server-2016-update-history)。

本節包含 advanced 疑難排解主題和連結，可協助您解決 Windows Server 的問題。 其他主題則會在可用時加入。

## <a name="troubleshoot-activation"></a>針對啟用進行疑難排解
...
## <a name="troubleshoot-startup-and-restart"></a>針對啟動和重新開機進行疑難排解
...
- [分頁檔案簡介](/windows/client-management/introduction-page-file)
...
- [事件識別碼41的 Advanced 疑難排解「系統已重新開機，而未先正常關機」](/windows/client-management/troubleshoot-event-id-41-restart)
...
- [設定電腦以進行疑難排解](../identity/ad-ds/manage/troubleshoot/configuring-a-computer-for-troubleshooting.md)

## <a name="troubleshoot-ad-fs"></a>針對 AD FS 進行疑難排解
...
- [AD FS 疑難排解-Fiddler-WS-同盟](../identity/ad-fs/troubleshooting/ad-fs-tshoot-fiddler-ws-fed.md)
...
## <a name="troubleshoot-converged-nic"></a>針對融合式 NIC 進行疑難排解

- [針對融合式 NIC 設定進行疑難排解](../networking/technologies/conv-nic/cnic-app-troubleshoot.md)

## <a name="troubleshoot-dfsr"></a>疑難排解 DFSR
...
## <a name="troubleshoot-dns"></a>針對 DNS 進行疑難排解

- [針對網域名稱系統 (DNS) 問題進行疑難排解](../networking/dns/troubleshoot/troubleshoot-dns-data-collection.md)
...
- [從主動容錯移轉叢集成員資格中移除節點時發生問題](./problem-nodes-failover-cluster.md)
- [從 VMWare ESX 上的容錯移轉叢集成員資格中移除的節點](./nodes-failover-cluster-vmware.md)
- [IaaS 與 SQL AlwaysOn - 微調容錯移轉叢集網路閾值](./iaas-sql-failover-cluster.md)

## <a name="troubleshoot-dhcp"></a>針對 DHCP 進行疑難排解
...
- [使用受防護網狀架構診斷工具進行疑難排解](../security/guarded-fabric-shielded-vm/guarded-fabric-troubleshoot-diagnostics.md)
- [疑難排解主機守護者服務](../security/guarded-fabric-shielded-vm/guarded-fabric-troubleshoot-hgs.md)
- [疑難排解主機守護者服務](../security/guarded-fabric-shielded-vm/guarded-fabric-troubleshoot-hosts.md)
...
- [疑難排解新增進入點](../remote/remote-access/ras/multisite/troubleshoot/troubleshooting-adding-entry-points.md)
...
## <a name="troubleshoot-nano-server"></a>針對 Nano Server 進行疑難排解
...
- [儲存空間直接存取-常見問題](../storage/storage-spaces/storage-spaces-direct-faq.md)
...
## <a name="troubleshoot-rds-session-connectivity"></a>針對 RDS 會話連線進行疑難排解
...
- [用戶端無法連線，而且沒有看到可用的授權錯誤](../remote/remote-desktop-services/troubleshoot/rdp-error-no-licenses-available.md)
- [使用者無法驗證，或必須驗證兩次](../remote/remote-desktop-services/troubleshoot/cannot-authenticate-or-must-authenticate-twice.md)
- [連接時，使用者會收到遠端桌面服務目前忙碌中的訊息](../remote/remote-desktop-services/troubleshoot/remote-desktop-service-currently-busy.md)
