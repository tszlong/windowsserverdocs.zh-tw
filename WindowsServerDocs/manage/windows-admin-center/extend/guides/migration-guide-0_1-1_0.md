...
## <a name="1-learn-about-new-controls-with-the-dev-guide-extension"></a>1.瞭解使用 Dev Guide 擴充功能的新控制項
...
* 選取 [ **Advanced** ] 索引標籤
* 在 [*實驗金鑰*] 底下，按一下 [**新增**]
* 在上一個步驟所建立的空白欄位中，輸入 ```msft.sme.shell.devguide``` 的新值
* 按一下 [**儲存並重載**]
...
* 選取其中一個索引標籤，以流覽 Dev Guide 的內容
    * **登錄**包含用於*管理 As*和*通知*案例的程式碼範例
    * **Controls**包含 SDK 中每個可用控制項的範例
    * **Ray**包含可用的轉換器和格式器函式的範例
    * **式樣**包含 SDK 中可用的 CSS 樣式範例
...
修改 ```package.json``` 中的下面這一行，以 ```next``` 版本為目標（SDK 1.0）：
...
### <a name="update--rxjs-function-use-for-observable-objects"></a>Update ```rxjs``` 函式用於可觀察物件
...
### <a name="resolve-runtime-issues-with-map-and-filter-functions-on-observable-objects"></a>解決可觀察物件上 ```.map()``` 和 @no__t 1 函式的執行時間問題

如果編譯器無法正確識別 ```observable``` 物件的類型，則來自 ```array``` 物件的 ```.map()``` 和 @no__t 2 函式可能會對應到您的物件，而在執行時間造成錯誤。  請確定您的函式傳回的 @no__t 0 物件指定明確的資料類型，以避免發生此問題。
...
* 執行 ```ng lint --fix```，以修正常見的不起毛問題
* 重複執行 ```gulp build```，以漸進方式修正 ```gulp build``` 可以自動解決的問題

## <a name="9-build-and-serve-your-project"></a>9.建立專案並為其提供服務
...
* 選取 [ **Advanced** ] 索引標籤
* 在 [*實驗金鑰*] 底下，按一下 [**新增**]
* 在上一個步驟所建立的空白欄位中，輸入 ```msft.sme.shell.personalization``` 的新值
* 按一下 [**儲存並重載**]
