...
3. 您也可以設定下列其他選項：
   - **易記名稱** – 您要連線之電腦的易記名稱。 您可以使用任何字串，但如果您未指定易記名稱，則會顯示電腦名稱。
   ...
   - **切換滑鼠按鈕** - 使用此選項即可交換滑鼠左鍵功能，以供滑鼠右鍵使用。 (如果遠端電腦是針對慣用左手的使用者而設定，但您使用右手的滑鼠，則這特別有用)。
...
2. 輸入下列資訊：
   ...
   - **使用者名稱** - 您要連線的遠端桌面閘道所要使用的使用者名稱和密碼。 您也可以選取 [使用桌面使用者帳戶]  ，以使用針對遠端桌面連線所使用的相同認證。
...
- **顯示桌面預覽** - 可讓您在連線到桌面之前，在連線中心查看其預覽。 根據預設，這設定為 [開啟]  。
- **捏合以縮放** - 可讓您使用捏合以縮放手勢。 如果您要透過遠端桌面使用的應用程式支援多點觸控 (在 Windows 8 中引進)，請將此設定 [關閉]  。
- **協助改善遠端桌面** - 傳送匿名資料給 Microsoft。 我們會使用此資料來改善用戶端。 您可以深入了解我們如何處理這個匿名的私人資料，請參閱[遠端桌面用戶端隱私權聲明](https://www.microsoft.com/privacystatement/RemoteApp/Default.aspx)。 根據預設，此設定為 [開啟]  。
- **顯示** - 顯示器有兩個全域設定：
