...
3. 新增[電腦](#add-a-pc)或[工作區](#add-a-workspace)。 您可以使用連線直接連線至 Windows 電腦和遠端資源，以使用 RemoteApp 程式、工作階段型桌面，或使用 RemoteApp 和桌面連線在內部部署發佈的虛擬桌面。 在公司環境中通常可以使用這項功能。
...
2. 輸入下列資訊：
   ...
   - **使用者名稱** – 用來存取遠端電腦的使用者名稱。 您可以使用下列格式：*user_name*、*domain\user_name* 或 `user_name@domain.com`。 您也可以視需要選取 [必要時詢問]  ，以在必要時提示輸入使用者名稱和密碼。
3. 您也可以設定下列其他選項：
...
2. 在 [摘要 URL] 欄位中，輸入所要新增摘要的 URL。 這可以是 URL 或電子郵件地址。
   - 如果您使用 URL，請使用系統管理員提供給您的帳戶。 此 URL 通常是 <https://rdweb.wvd.microsoft.com>。
   - 如果您使用電子郵件地址，請輸入您的電子郵件地址。 這會告訴用戶端搜尋與您的電子郵件地址相關聯的 URL (如果系統管理員是這樣設定伺服器的話)。
3. 點選 [下一步]  。
4. 出現提示時，請提供您的認證。
...
2. 請點 [新增閘道]  。
3. 輸入下列資訊：
   - **閘道名稱** - 您想要用來作為閘道的電腦名稱。 這可以是 Windows 電腦名稱、網際網路網域名稱或 IP 位址。 您也可以將連接埠資訊新增至伺服器名稱 (例如：**RDGateway:443** or **10.0.0.1:443**)。
