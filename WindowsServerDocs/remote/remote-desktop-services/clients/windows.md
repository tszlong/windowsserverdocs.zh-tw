---
title: 在 Windows 上開始使用遠端桌面
description: 適用於 Windows 的遠端桌面用戶端的基本設定步驟。
ms.custom: na
...
---
# <a name="get-started-with-remote-desktop-on-windows"></a>在 Windows 上開始使用遠端桌面
...
> [!NOTE]
> - 想知道適用於 Windows 用戶端的新版本嗎？ 請參閱 [Windows 上的遠端桌面有什麼新功能？](windows-whatsnew.md)
> - 您可以在任意版本的 Windows 10 上執行該用戶端。
...
3. 您也可以點選 [顯示較多]  來設定其他選項：
   ...
   - **切換滑鼠按鈕** - 使用此選項即可交換滑鼠左鍵功能，以供滑鼠右鍵使用 (假如遠端電腦是針對慣用左手的使用者而設定的，但您使用慣用右手的滑鼠，則這特別有用)。
...
應用程式設定
- **顯示桌面預覽** - 可讓您在連線到桌面之前，在連線中心查看其預覽。 根據預設，會將此選項設定為 [開啟]  。
...
3. 輸入下列資訊：
   ...
   - **密碼** - 您所指定的使用者密碼。 這可以保持空白，以便在連線期間提示輸入密碼。
4. 點選 [儲存]  。
...
## <a name="navigate-the-remote-desktop-session"></a>巡覽遠端桌面工作階段
...
連線列可讓您存取其他巡覽控制項。 根據預設，連線列位於畫面頂端的中間。 點選該列並向左或右拖曳以移動它。
...
> [!TIP]
> 隨時都歡迎提出問題和意見。 不過，請「勿」使用本文結尾的留言功能來張貼需要疑難排解協助的要求。 請改為前往[遠端桌面用戶端論壇](https://social.technet.microsoft.com/forums/windowsserver/en-us/home?forum=winrdc) \(英文\)，並開始新的對話。 有功能建議嗎？ 請使用[意見反應中樞](feedback-hub://?tabid=2&contextid=605)告知我們。
