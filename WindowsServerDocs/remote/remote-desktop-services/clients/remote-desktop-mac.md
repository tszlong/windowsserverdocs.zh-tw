...
2. 輸入下列資訊：
   ...
   - **使用者帳戶**-新增您用來存取遠端電腦的使用者帳戶。
      - 電腦或本機帳戶，已加入 Active Directory (AD)，使用其中一種格式： *user_name*， *domain\user_name*，或*user_name@domain.com*。
      - 針對 Azure Active Directory (AAD) 加入的電腦，使用其中一種格式：*AzureAD\user_name*或是*AzureAD\user_name@domain.com*。
      - 您也可以選擇是否需要密碼。
