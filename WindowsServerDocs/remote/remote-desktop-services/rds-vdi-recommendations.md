...
|-------------------|----------|--------------|
|**個人化**| | |
|透過將您的輸入資料傳送給 Microsoft，將您的語音、輸入與筆跡輸入個人化。|    開啟| 關閉|
|將鍵入和筆跡資料傳送給 Microsoft，以改進辨識與建議平台。|  開啟| 關閉|
|讓應用程式使用您的廣告識別碼來體驗各種應用程式。|  開啟| 關閉|
|讓 Skype (如果已安裝) 協助您與通訊錄中的朋友聯繫，並驗證您的行動電話號碼。 SMS 和數據可能會收取費用。|    開啟| 關閉|
|**位置**| | |
|開啟 [尋找我的裝置]，並讓 Windows 和應用程式要求存取您的位置，包括位置歷程記錄| 開啟| 關閉|
|連線能力與錯誤報告| | |
|自動連線到建議的已開啟熱點。 並非所有網路都是安全的。|    開啟| 關閉|
|自動連線可暫時開放熱點以查看是否可以使用付費的網路服務。| 開啟| 關閉|
|將完整的診斷與使用狀況資料傳送到 Microsoft。 關閉此功能僅會傳送基本資料。| 開啟| 關閉|
|**瀏覽器、保護與更新**| | |
|使用 SmartScreen 線上服務協助防範惡意內容，以及 Windows 瀏覽器和市集應用程式在網站中載入的下載項目|    開啟| 開啟 (如果沒有網際網路存取權，則設定為 [關閉])。
|使用頁面預測改進閱讀體驗、加快瀏覽速度，以及改善 Windows 瀏覽器中的整體體驗。 您瀏覽的資料將會傳送到 Microsoft。| 開啟| 關閉|
|從網際網路上取得更新，並將更新傳送到其他電腦，以加速應用程式與 Windows Update 下載|   開啟| 關閉|
...
|設定|預設值|用於 VDI 的建議值|  
|-------------------|----------|--------------|
|取得來自 App 與其他寄件者的通知| 開啟| 關閉|
|在鎖定畫面上顯示通知。|    開啟| 關閉|
|在鎖定畫面上顯示鬧鐘、提醒與 VoIP 來電。|   開啟| 關閉|
|當您使用 Windows 時，顯示提示、秘訣和建議。|    開啟| 關閉|
...
|顯示常用應用程式|開啟|關閉|
|顯示最近新增的應用程式|開啟|關閉|
|在 [開始] 或工作列上的捷徑清單中顯示最近開啟的項目|開啟|關閉|
...
|設定|預設值|用於 VDI 的建議值|  
|-------------------|----------|--------------|
|讓應用程式使用我的廣告識別碼來提供跨應用程式體驗 (關閉此功能將重設您的識別碼)|  開啟| 關閉|
|透過讓網站存取我的語言清單以提供當地相關內容|開啟|關閉|
|讓我其他裝置上的 App 在此裝置上開啟 App 並繼續執行|開啟|關閉|
...
||不允許此電腦做為 BITS 對等快取用戶端|Enabled|
||不允許此電腦做為 BITS 對等快取伺服器|Enabled|
||允許 BITS 對等快取|已停用|
|BranchCache||
||開啟 BranchCache|已停用|
|作用區驗證||
...
|-------------------|----------|--------------|
|裝置安裝||
||當裝置上已安裝標準驅動程式時，不要傳送 Windows 錯誤報告|Enabled|
||在通常會提示建立系統還原點的裝置活動期間，防止建立系統還原點|Enabled|
||防止從網際網路擷取裝置中繼資料|Enabled|
||當裝置驅動程式在安裝期間要求其他軟體時，防止 Windows 傳送錯誤報告|Enabled|
||在裝置安裝期間關閉 [找到新硬體] 註解方塊|Enabled|
...
||停用說明秘訣|        Enabled|
|檔案總管|||
||不顯示「已安裝新應用程式」通知|     Enabled|
|遊戲總管|||
||關閉遊戲資訊下載|     Enabled|
||關閉遊戲更新|        Enabled|
||關閉 [遊戲] 資料夾中追蹤最後一次執行遊戲時間的功能|     Enabled|
|家用群組|||
||防止電腦加入家用群組|        Enabled|
|Internet Explorer|||
...
||關閉附加元件效能通知|        Enabled|
||關閉瀏覽器地理位置|     Enabled|
||關閉 [重新開啟上一次的瀏覽工作階段]|        Enabled|
||關閉所有使用者安裝之提供者的建議|        Enabled|
||開啟建議的網站|       已停用|
...
||關閉電話號碼偵測|Enabled|
|**多媒體**|||
||允許 Internet Explorer 播放使用替代轉碼器的媒體檔案|已停用|
...
|**RSS 摘要**|||
||防止自動探索摘要和網頁快訊|Enabled|
|**搜尋**|||
||允許 Cortana|        已停用|
||允許在鎖定畫面上使用 Cortana|      已停用|
||允許搜尋和 Cortana 使用定位|     已停用|
||不允許 Web 搜尋|      Enabled|
||不要在 [搜尋] 中搜尋網路或顯示網路搜尋結果|        Enabled|
||防止從控制台新增 UNC 位置到索引|     Enabled|
||防止對離線檔案快取中的檔案編製索引|        Enabled|
|**市集**|||
||關閉更新至最新版 Windows 的服務|Enabled|
|**Windows 錯誤報告**|||
||自動傳送 OS 產生的錯誤報告的記憶體傾印|       已停用|
||停用 Windows 錯誤報告|      Enabled|
|**Windows Installer**|||
||控制基準檔案快取的大小上限|  已啟用，然後使用 [選項]  區域中的調整方塊，將 [基準檔案快取大小上限]  設定為 *5*。|
||關閉系統還原檢查點建立功能|      Enabled|
|**Windows Mail**|||
||關閉社群功能|Enabled|
|**Windows Media Player**|||
||不要顯示 [安裝第一次使用] 對話方塊|       Enabled|
||防止媒體共用|        Enabled|
|**Windows 行動中心**|||
||關閉 Windows 行動中心|Enabled|
|**Windows 可靠性分析**|||
||設定可靠性 WMI 提供者|已停用|
|**Windows Update**|||
||允許立即安裝自動更新|       Enabled|
||移除對所有 Windows Update 功能的存取|     Enabled|
...
|**檔案總管**|||
||關閉縮圖快取處理|       Enabled|
||關閉在 [檔案總管] 搜尋方塊中顯示最近搜尋項目|        Enabled|
||關閉在隱藏的 thumbs.db 檔案中快取縮圖|      Enabled|
