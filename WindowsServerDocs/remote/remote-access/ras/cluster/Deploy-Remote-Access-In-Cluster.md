...
Windows Server 2016 和 Windows Server 2012 將 DirectAccess 與遠端存取服務 \(RAS @ no__t-1 VPN 合併成一個遠端存取角色。 您可以在許多企業案例中部署遠端存取。 本總覽提供企業案例的簡介，以在使用 Windows 網路負載平衡的叢集中部署多部遠端存取服務器 \(NLB @ no__t-1 或使用外部負載平衡器 \(ELB @ no__t-3，例如 F5Big @ no__t-4IP。  

## <a name="BKMK_OVER"></a>案例描述  
叢集部署會將多部遠端存取服務器收集成單一單位，然後使用外部虛擬 IP，透過 DirectAccess 或 VPN 連線到內部公司網路的遠端用戶端電腦，將其作為單一連絡人點。 @no__0VIP @ no__t-1-遠端存取叢集的位址。  使用 Windows NLB 或外部負載平衡器，將流量負載平衡至叢集 \(such 為 F5 Big @ no__t-1IP @ no__t-2。  
...
-   在 ELB 部署中，如果需要向外管理，則 DirectAccess 用戶端無法使用 @ no__t-0Teredo。 只有 IPHTTPS 可以用於 end @ no__t-0to @ no__t-1end 通訊。  

-   請確定已安裝所有已知的 NLB @ no__t-0ELB 修補程式。  
...
-   高可用性。 叢集會提供 always @ no__t-0on 存取的高可用性。 如果叢集中有一部伺服器當機，遠端使用者可以透過叢集中的其他伺服器，繼續連接公司網路。 叢集中的所有伺服器都具有一組相同的叢集虛擬 IP \(VIP @ no__t-1 個位址，同時仍會為每部伺服器維護唯一的私人 IP 位址。  

-   簡化 @ no__t-0of @ no__t-1management。 叢集可讓多部伺服器做為單一實體進行管理。 共用的設定可以輕鬆設定到叢集中的每一部伺服器。 您可以從叢集中的任何伺服器管理遠端存取設定，或使用遠端伺服器管理工具 \(RSAT @ no__t-1 從遠端進行管理。 此外，整個叢集可以從單一「遠端存取管理」主控台進行監視。  
...
|---------|-----------------|  
|遠端存取角色|這個角色是利用伺服器管理員主控台安裝和解除安裝。 它同時包含 DirectAccess （先前是 Windows Server 2008 R2 中的功能），以及路由及遠端存取服務 \(RRAS @ no__t-1，先前是網路原則與存取服務 \(NPAS @ no__t-3 的角色服務伺服器角色。 遠端存取角色包含兩個元件：<br /><br />-Always On VPN 和路由及遠端存取服務 \(RRAS @ no__t-1 VPN-DirectAccess 和 VPN 會在 [遠端存取管理] 主控台中一起管理。<br />-RRAS 路由-在舊版路由及遠端存取主控台中管理 RRAS 路由功能。<br /><br />相依性如下所示：<br /><br />-Internet Information Services \(IIS @ no__t-1 Web 服務器-設定網路位置伺服器和預設 Web 探查時，需要這項功能。<br />-Windows 內部資料庫-用於遠端存取服務器上的本機帳戶處理。|  
...
-   針對外部 Load Balancer 案例，需要專用硬體 \( i. e. F5 BigIP @ no__t-1。  
...
-   除了單一伺服器的軟體需求以外，還有一些叢集 @ no__t-0specific 需求：  

    -   在每部叢集伺服器上，IP @ no__t-0HTTPS 憑證主體名稱必須符合 ConnectTo 位址。 叢集部署支援在叢集伺服器上混合使用萬用字元和非 @ no__t 0wildcard 憑證。  

    -   如果網路位置伺服器安裝在遠端存取伺服器上，則在叢集中的每一部伺服器上，網路位置伺服器憑證必須要有相同的主體名稱。 此外，網路位置伺服器憑證的名稱不可以和 DirectAccess 部署中的任何伺服器名稱相同。  

    -   IP @ no__t-0HTTPS 和網路位置伺服器憑證必須使用與單一伺服器所發行之憑證相同的方法來發出。 例如，如果單一伺服器使用公開憑證授權單位單位 \(CA @ no__t-1，則叢集中的所有伺服器都必須具有公用 CA 所發行的憑證。 或者，如果單一伺服器針對 IP @ no__t-1HTTPS 使用自我 @ no__t-0signed 憑證，則叢集中的所有伺服器都必須執行同樣的動作。  
...
-   在具有單一網路介面卡的 IPv4 @ no__t-0only 部署中設定 DirectAccess 之後，以及在預設的 DNS64 \(the IPv6 位址（包含 "：3333：："）之後，就會在網路介面卡上自動設定 \)，並嘗試啟用透過 [遠端存取管理] 主控台載入 @ no__t-3balancing 會導致提示使用者提供 IPv6 DIP。 如果提供 IPv6 DIP，設定會在按一下 [認可] 之後失敗，並發生錯誤：參數不正確。  

    解決此問題：  

    1.  從 [備份和還原遠端存取設定](https://gallery.technet.microsoft.com/Back-up-and-Restore-Remote-e157e6a6)下載備份和還原指令碼。  

    2.  使用已下載的腳本備份 @ no__t-0RemoteAccess 備份您的遠端存取 Gpo  

    3.  嘗試啟用負載平衡，直到它失敗所在的步驟。 在 [啟用負載平衡] 對話方塊中，展開詳細資料區域，然後在 [詳細資料] 區域中，按右 @ no__t-0click，然後按一下 [**複製腳本**]。  
    ...
    8.  如果 Cmdlet 因為輸入值 @ no__t-1 而執行時失敗 \(not，請執行命令 Restore @ no__t-2RemoteAccess，並遵循指示來確保保留原始設定的完整性。  
