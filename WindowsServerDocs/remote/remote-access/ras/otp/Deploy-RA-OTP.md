...
在此案例中，已啟用 DirectAccess 的遠端存取服務器已設定為使用兩個\-因素一次性密碼 \(OTP\) 驗證來驗證 DirectAccess 用戶端使用者，以及標準 Active Directory 認證。  
...
2.  [使用 OTP 驗證規劃遠端存取](https://docs.microsoft.com/windows-server/remote/remote-access/ras/otp/plan/plan-remote-access-with-otp-authentication)。 除了針對單一伺服器所需的規劃以外，OTP 還需要規劃 Microsoft 憑證授權單位單位 \(CA\) 和 OTP 的憑證範本;和 RADIUS\-啟用的 OTP 伺服器。 規劃也可能包含安全性群組的需求，讓特定使用者免于受到強式 \(OTP 或智慧卡\) 驗證。 如需多重\-樹系環境中 OTP 設定的相關資訊，請參閱[設定多樹系部署](../../ras/multi-forest/Configure-a-Multi-Forest-Deployment.md)。  
...
1.  DirectAccess 用戶端會透過基礎結構通道\)，進入網域認證來存取 DirectAccess 基礎結構伺服器 \(。  如果因為特定的 IKE 失敗而無法連接內部網路，用戶端電腦上的「工作地方連線」會通知使用者務必準備好認證。 在執行 Windows 7 的用戶端電腦上，會出現 pop\-要求智慧卡認證。  
  
2.  輸入 OTP 認證之後，它們會透過 SSL 傳送至遠端存取服務器，以及簡短\-的智慧卡登入憑證的要求。  
  
3.  遠端存取服務器會起始以 RADIUS\-為基礎的 OTP 伺服器來驗證 OTP 認證。  
...
2.  除了單一伺服器的軟體需求以外，還有一些 OTP\-特定需求：  
  
    1.  IPsec 驗證的 CA-在 OTP 部署中，必須使用 CA 所簽發的 IPsec 機器憑證來部署 DirectAccess。 在部署的 OTP 中，不支援將遠端存取伺服器當作 Kerberos Proxy，負責驗證工作。 需要內部 CA。  
  
    2.  CA 進行 OTP 驗證-必須在 Windows 2003 Server 或更新\) 版本上執行的 Microsoft 企業 CA \(，才能發行 OTP 用戶端憑證。 為了驗證 IPsec 而負責簽發憑證的同一個 CA，也是可以使用的。 CA 伺服器必須可透過第一個基礎結構通道來使用。  
    ...
    5.  請注意以下幾點：  
        ...
        2.  在遠端存取設定期間，指定安全性群組中的使用者可以豁免兩個\-因素驗證，因此只能以 username\/密碼進行驗證。  
...
-   「遠端存取」會使用探查機制來驗證 RADIUS\-型 OTP 伺服器的連線能力。 在某些情況下，這可能會導致 OTP 伺服器發出錯誤。 若要避免這個問題，請在 OTP 伺服器上執行下列作業：  
    ...
    -   建立與遠端存取伺服器上針對探查機制而設定的使用者名稱和密碼相符的使用者帳戶。 此使用者名稱不應定義 Active Directory 使用者。  
        ...
        -   HKEY\_本機\_電腦\\軟體\\Microsoft\\DirectAccess\\OTP\\RadiusProbeUser  
  
        -   HKEY\_本機\_電腦\\軟體\\Microsoft\\DirectAccess\\OTP\\ RadiusProbePass  
