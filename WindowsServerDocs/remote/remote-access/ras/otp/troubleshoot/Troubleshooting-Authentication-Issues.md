...
**案例**。 使用者無法使用 OTP 進行驗證，發生錯誤：「驗證因內部錯誤而失敗」
 ...
 (用戶端事件記錄檔) **收到錯誤**。 CA 伺服器上使用者的 OTP 憑證註冊 <username> 失敗 <CA_name>、要求失敗、失敗的可能原因：無法解析 ca 伺服器名稱、無法透過第一個 DirectAccess 通道存取 ca 伺服器，或無法建立 ca 伺服器的連線。
...
**案例**。 使用者無法使用 OTP 進行驗證，發生錯誤：「驗證因內部錯誤而失敗」
 ...
 (用戶端事件記錄檔) **收到錯誤**。 CA <CA_name> 的憑證註冊失敗。 要求未依照 OTP 簽署憑證的預期方式簽署，或使用者沒有註冊的許可權。

**原因**

使用者所提供的一次性密碼正確，但頒發證書授權單位 (CA) 拒絕發行 OTP 登入憑證。 憑證要求可能未正確地以正確的 EKU 簽署 (OTP 登錄授權單位應用程式原則) ，或使用者沒有對 DA OTP 範本的「註冊」許可權。
...
如果找不到有效的憑證，請刪除不正確憑證 (如果存在) 然後 `gpupdate /Force` 從提高許可權的命令提示字元執行或重新開機用戶端電腦，重新註冊電腦憑證。

## <a name="missing-ca-that-issues-otp-certificates"></a>缺少簽發 OTP 憑證的 CA
**案例**。 使用者無法使用 OTP 進行驗證，發生錯誤：「驗證因內部錯誤而失敗」

 (用戶端事件記錄檔) **收到錯誤**。 因為 DA 伺服器未傳回發行 CA 的位址，所以無法完成 OTP 驗證。
...
2.  如果未設定任何 Ca：
    ...
    1.  使用命令 `Set-DAOtpAuthentication` 或 [遠端存取管理] 主控台來設定頒發 DIRECTACCESS OTP 登入憑證的 ca。

    2.  套用新的設定，並強制用戶端重新整理 DirectAccess GPO 設定，方法是 `gpupdate /Force` 從提高許可權的命令提示字元執行，或重新開機用戶端電腦。
...
使用檢查已設定的 DirectAccess 伺服器位址 `Get-DirectAccess` ，並更正位址（如果不正確的話）。

`gpupdate /force`從提升許可權的命令提示字元執行，或重新開機用戶端電腦，以確定最新的設定已部署在用戶端電腦上。

## <a name="failed-to-generate-the-otp-logon-certificate-request"></a>無法產生 OTP 登入憑證要求
**案例**。 使用者無法使用 OTP 進行驗證，發生錯誤：「驗證因內部錯誤而失敗」

 (用戶端事件記錄檔) **收到錯誤**。 無法初始化 OTP 驗證的憑證要求。 無法產生私密金鑰，或使用者 <username> 無法存取網域控制站上 <OTP_template_name> 的憑證範本。
...
**案例**。 使用者無法使用 OTP 進行驗證，發生錯誤：「驗證因內部錯誤而失敗」
 ...
 (用戶端事件記錄檔) **收到錯誤**。 無法建立與網域控制站的連線以因應 OTP 驗證的目的。 錯誤碼： <error_code>。
...
**案例**。 使用者無法使用 OTP 進行驗證，發生錯誤：「驗證因內部錯誤而失敗」
 ...
 (用戶端事件記錄檔) **收到錯誤**。 使用遠端存取服務器的 OTP 驗證 ( # B0 DirectAccess_server_name>) 使用者 (<username>) 需要使用者提供挑戰。
...
1.  執行 PowerShell Cmdlet 並檢查的值，以檢查已設定的 OTP 簽署憑證範本名稱 `Get-DAOtpAuthentication` `SigningCertificateTemplateName` 。

2.  使用 [憑證] MMC 嵌入式管理單元，確認電腦上已有從這個範本註冊的有效憑證。

3.  如果沒有這類憑證存在，請刪除過期的憑證 (如果有的話) 並根據此範本註冊新憑證。
...
-   <username>無法使用 OTP 驗證使用者。 請確定已在 Active Directory 中為使用者名稱定義 UPN。 錯誤碼： <error_code>。

-   <username>無法使用 OTP 驗證使用者。 請確定已在 Active Directory 中為使用者名稱定義了 DN。 錯誤碼： <error_code>。
...
使用者沒有使用者主體名稱 (UPN) 或在使用者帳戶中正確設定的辨別名稱 (DN) 屬性，這些屬性對於 DirectAccess OTP 是否正常運作都是必要的。
...
**案例**。 使用者無法使用 OTP 進行驗證，發生錯誤：「驗證因內部錯誤而失敗」
 ...
 (用戶端電腦) **收到錯誤**。 Windows 驗證您的認證時，發生錯誤。 請再試一次，或要求您的系統管理員尋求協助。
