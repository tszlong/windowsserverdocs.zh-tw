...
- [步驟1：執行單一伺服器遠端存取部署](../../multisite/configure/Step-1-Implement-a-Single-Server-Remote-Access-Deployment.md)。 安裝和設定單一遠端存取服務器。 如需指示，請參閱[使用 Advanced Settings 部署單一 DirectAccess 伺服器](../../../directaccess/single-server-advanced/deploy-a-single-directaccess-server-with-advanced-settings.md)。
