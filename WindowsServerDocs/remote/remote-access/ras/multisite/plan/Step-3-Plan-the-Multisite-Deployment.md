...
-   如果已將 ConnectTo 位址指定為 IP 位址，而不是 DNS 名稱) 或 ip-HTTPs URL 的 FQDN，請在 [主體] 欄位中指定遠端存取服務器的外部介面卡的 IPv4 位址 (。
...
-   如果您在多網站部署中支援執行 Windows 7 的用戶端電腦，請在 [CRL 發佈點] 欄位中，指定連線到網際網路的 DirectAccess 用戶端可存取的 CRL 發佈點。 執行 Windows 8 (的用戶端並不需要這麼做，因為這些用戶端) 上的 IP-HTTPS 已停用 CRL 撤銷檢查。
...
網路位置伺服器網站可以裝載在遠端存取服務器或組織中的另一部伺服器上。 如果您將網路位置伺服器裝載在遠端存取服務器上，當您部署遠端存取時，就會自動建立網站。 如果您在組織中執行 Windows 作業系統的另一部伺服器上裝載網路位置伺服器，您必須確定已安裝 Internet Information Services (IIS) ，才能建立網站。
...
-   所有進入點都必須設定具有相同主體名稱的網路位置伺服器憑證 (符合網路位置伺服器 URL) 。

-   您必須先建立進入點的網路位置伺服器基礎結構 (DNS 和憑證設定) ，才能新增進入點。
...
3.  如果用戶端電腦位於 Web Proxy (且無法使用 DNS 解析) ，或如果全域伺服器負載平衡器 FQDN 未解析為任何已設定的全域伺服器負載平衡器 IP 位址，則會使用 HTTPS 探查將進入點自動選取至所有進入點的 ip-HTTPs Url。 用戶端將會連接到先回應的伺服器。
...
3. 針對每個進入點，內部網路中應該有一到三個路由：
   ...
   4. 所有上述路由都必須路由傳送至遠端存取服務器的內部介面卡上的 IPv6 位址 (或負載平衡進入點) 的內部虛擬 IP (VIP) 位址。
...
如果您的組織使用含有網站特定 IPv6 首碼的 Active Directory 拓撲 (例如，內部資源 FQDN app.corp.com 同時裝載于北美洲和歐洲，且每個位置都有網站特定的 IP 位址) 這不會使用遠端存取主控台預設設定，而且不會針對每個進入點設定網站特定的 IPv6 首碼。 如果您想要啟用此選擇性案例，您必須使用特定的 IPv6 首碼來設定每個進入點，用戶端電腦應該將這些前置詞連接到特定的進入點。 以下列方式來執行此動作：
...
不需要重新安裝 DirectAccess 部署，就能從僅 IPv4 轉換成僅 IPv6 的公司網路。 若要在轉換期間維護用戶端連線能力，您可以使用另一個 DirectAccess 部署。 當第一個轉換階段完成時，需要進行雙重部署 (僅限 IPv4 的網路升級至 IPv4 + IPv6) ，而您想要準備未來轉換到僅 IPv6 的公司網路 orto 利用原生 IPv6 連線能力的優點。 雙重部署的說明請見下列一般步驟：
