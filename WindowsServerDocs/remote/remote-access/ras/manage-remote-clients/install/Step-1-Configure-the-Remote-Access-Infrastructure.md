...
根據您是否決定將遠端存取服務器放在邊緣，或在網路位址轉譯後方 (NAT) 裝置，在具有 IPv4 和 IPv6 的環境中進行單一伺服器部署時，需要下列網路介面位址設定。 您可以使用 [Windows 網路和共用中心]**** 中的 [變更介面卡設定]**** 設定所有 IP 位址。
...
如果遠端存取服務器有兩張網路介面卡 (一個用於網域設定檔，另一個用於公用或私人設定檔) ，但您使用的是單一網路介面卡拓撲，建議如下：
...
-   **Teredo 流量**
    ...
    使用者資料包協定 (UDP) 目的地埠3544輸入，以及 UDP 來源埠3544輸出。 為遠端存取服務器上的兩個網際網路對向連續公用 IPv4 位址套用此豁免。
...
遠端存取服務器和所有 DirectAccess 用戶端上都需要憑證，才能使用 IPsec 驗證。 憑證必須由 (CA) 的內部憑證授權單位單位所發行。 遠端存取服務器和 DirectAccess 用戶端必須信任簽發根和中繼憑證的 CA。
...
-   **私用**
    ...
    憑證是以您在設定[憑證範本](assetId:///6a5ec5c1-d653-47b1-a567-cc485004e7bc#ConfigCertTemp)中建立的憑證範本為基礎。 它需要憑證撤銷清單 (CRL) 發佈點，可以從可公開解析的 FQDN 存取。
...
-   憑證主體名稱應該是可從外部解析的完整功能變數名稱 (ip-HTTPs) 的 ip-HTTPs URL (僅供遠端存取服務器 IP-HTTPS 連線使用的 ConnectTo 位址) 。
...
3.  在 [**新增主機**] 對話方塊中，在 [**名稱 (使用父系網域名稱如果空白) ** ] 方塊中，輸入網路位置伺服器網站的 DNS 名稱 (這是 DirectAccess 用戶端用來連線到網路位置伺服器) 的名稱。 在 [ **IP 位址**] 方塊中，輸入網路位置伺服器的 IPv4 位址，然後按一下 [**新增主機**]，再按一下 **[確定]**。

4.  在 [**新增主機**] 對話方塊的 [**名稱 (使用父系網域名稱，如果空白) ** ] 方塊中，輸入 web 探查的 DNS 名稱 (預設 web 探查的名稱是 directaccess directaccess-webprobehost) 。 在 [IP 位址]**** 方塊中，輸入 Web 探查的 IPv4 位址，然後按一下 [新增主機]****。
...
-   **ISATAP**
     ...
     (ISATAP) 的站上自動通道定址通訊協定會使用通道，讓 DirectAccess 用戶端透過 IPv4 網際網路連線到遠端存取服務器，將 IPv6 封包封裝在 IPv4 標頭內。 「遠端存取」會使用它，透過內部網路提供對 ISATAP 主機的 IPv6 連線能力。 在非原生 IPv6 網路環境中，遠端存取服務器會自動將自己設定為 ISATAP 路由器。 需要 ISATAP 名稱的解析支援。
...
3.  如果您在將伺服器加入網域時也要變更電腦名稱稱，請在 [**電腦名稱稱**] 方塊中輸入電腦的名稱。 在 [**成員隸屬**] 底下，按一下 [**網域**]，然後輸入您要加入伺服器的功能變數名稱， (例如 corp.contoso.com) ]，然後按一下 **[確定]**。
...
網路位置伺服器應該位於具有高可用性的伺服器上，而且需要 DirectAccess 用戶端所信任的有效安全通訊端層 (SSL) 憑證。
