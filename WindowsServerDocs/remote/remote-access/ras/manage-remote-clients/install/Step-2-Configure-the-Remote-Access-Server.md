...
若要佈建用戶端電腦以使用 DirectAccess，它必須屬於所選取的安全性群組。 設定 DirectAccess 之後，就會布建安全性群組中的用戶端電腦，以接收 (Gpo) 的 DirectAccess 群組原則物件，以進行遠端系統管理。
...
2.  用戶端電腦可以 (ConnectTo 位址連接的遠端存取服務器公用 URL) 
...
5.  在 [**首碼**設定] 頁面上 (只有在內部網路) 中偵測到 ipv6 時，才會顯示此頁面。 如果您的部署需要額外的首碼，請設定內部網路的 IPv6 首碼、要指派給 DirectAccess 用戶端電腦的 IPv6 首碼，以及要指派給 VPN 用戶端電腦的 IPv6 首碼。
