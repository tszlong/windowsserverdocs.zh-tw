...
1.  在 [遠端存取] 伺服器上，開啟 [遠端存取管理] 主控台：在 [**開始**] 畫面上，輸入**RAMgmtUI.exe**，然後按 enter。 如果出現 [**使用者帳戶控制**] 對話方塊，請確認它所顯示的動作就是您所需的動作，然後按一下 [**是**]。
...
若要啟用所有電腦的遠端存取 (不只是) 的筆記本和膝上型電腦，以及啟用 Windows 7 用戶端的遠端存取：
