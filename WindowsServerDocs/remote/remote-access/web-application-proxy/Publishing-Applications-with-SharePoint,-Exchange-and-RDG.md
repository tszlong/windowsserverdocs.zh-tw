...
您必須將應用程式設定為支援 Kerberos 驗證。 此外，您必須將服務主體名稱（SPN）註冊至執行 web 服務的帳戶。 您可以在網域控制站或後端伺服器上執行此動作。 在負載平衡的 Exchange 環境中，這需要使用替代的服務帳戶，請參閱設定[負載平衡的用戶端存取伺服器的 Kerberos 驗證](/exchange/configuring-kerberos-authentication-for-load-balanced-client-access-servers-exchange-2013-help)
...
1.  使用 RDG 進行 Web 應用程式 Proxy 預先驗證的運作方式，是將 Internet Explorer 所取得的預先驗證 cookie 傳遞至遠端桌面連線用戶端（mstsc.exe）。 然後，遠端桌面連線用戶端（mstsc.exe）會使用此方法。 然後，遠端桌面連線用戶端會使用此來作為驗證證明。

    下列程式會告知集合伺服器將必要的自訂 RDP 屬性包含在傳送給用戶端的遠端應用程式 RDP 檔案中。 這些會告訴用戶端需要預先驗證，並將預先驗證服務器位址的 cookie 傳遞給遠端桌面連線用戶端（mstsc.exe）。 與停用 Web 應用程式 Proxy 應用程式上的 HttpOnly 功能搭配使用，可讓遠端桌面連線用戶端（mstsc.exe）利用透過瀏覽器取得的 Web 應用程式 Proxy cookie。

    RD Web 存取伺服器的驗證仍會使用 RD Web 存取表單登入。 這會提供最少的使用者驗證提示，因為 RD Web 存取登入表單會建立用戶端認證存放區，供遠端桌面連線用戶端（mstsc.exe）用於後續的遠端應用程式啟動。
...
4.  https://rdg.contoso.com/在 Web 應用程式 Proxy 中發佈網站的根目錄（例如）。 將預先驗證設定為 AD FS 並使用您在上面建立的信賴憑證者信任。 這會讓/rdweb 和/rpc 使用相同的 Web 應用程式 Proxy 驗證 cookie。
...
7.  在集合伺服器上設定相關的 RDG 集合，讓遠端桌面連線用戶端（mstsc.exe）知道 rdp 檔案中需要預先驗證。
    ...
    -   在 Windows Server 2008 R2 中：
        ...
        5.  完成後 **，按一下 [** 套用]。
            ...
            這會告知集合伺服器將自訂 RDP 屬性包含在傳送給用戶端的 RDP 檔案中。 這些會告訴用戶端需要預先驗證，並將預先驗證服務器位址的 cookie 傳遞給遠端桌面連線用戶端（mstsc.exe）。 這會結合在 Web 應用程式 Proxy 應用程式上停用 HttpOnly，讓遠端桌面連線用戶端（mstsc.exe）利用透過瀏覽器取得的 Web 應用程式 Proxy 驗證 cookie。
