...
在此步驟中，您將安裝網路原則伺服器 (NPS) 之 VPN 伺服器所傳送的連線要求的處理：
...
使用 Windows PowerShell 執行此程序、 系統管理員身分執行 Windows PowerShell，請輸入下列 cmdlet:
...
3.  在 [選取安裝類型，請確認**角色型或功能型安裝**已選取，然後選取**下一步]** 。

4.  在 選取目的地伺服器，請確認**從伺服器集區選取伺服器**已選取。

5.  在 [伺服器集區，請確定本機電腦是已選取，然後選取**下一步]** 。

6.  在選取伺服器角色**角色**，選取**網路原則與存取服務**。 會開啟一個對話方塊，要求如果它應該加入所需的網路原則與存取服務的功能。

7.  選取 [**將功能加入**，然後選取**下一步]**

8.  在 [選取功能] 中，選取**下一步**，並在 [網路原則與存取服務，檢閱提供的資訊，然後選取**下一步]** 。

9.  在 [選取角色服務] 中，選取**網路原則伺服器**。

10. 如需網路原則伺服器所需的功能，請選取**將功能加入**，然後選取**下一步**。

11. 在 確認安裝選項，選取**需要時自動重新啟動目的地伺服器**。

12. 選取  **是**若要確認已選取，然後選取**安裝**。
    
    [安裝進度] 頁面會顯示安裝程序期間的狀態。 此程序完成時，訊息 」 上安裝成功*ComputerName*"的訊息，其中*ComputerName*是您安裝網路原則伺服器的電腦名稱。
...
1.  在 [伺服器管理員] 中，選取**工具**，然後選取**網路原則伺服器**。 NPS 主控台隨即開啟。

2.  在 NPS 主控台中，以滑鼠右鍵按一下**NPS （本機）** ，然後選取**在 Active Directory 中的註冊伺服器**。
...
5. 在 **位址 （IP 或 DNS）** ，請輸入 NAS IP 位址或 FQDN。
     
     如果您輸入的 FQDN，請選取**確認**如果您想要確認名稱正確，且對應至有效的 IP 位址。
...
6. 在 **共用祕密**，執行：
    ...
    3. 請重新輸入共用的密碼，在 確認共用密碼。

7. 選取 [確定]。  VPN 伺服器會出現在清單中的 NPS 伺服器上設定的 RADIUS 用戶端。
...
4. 在 指定撥號或 VPN 伺服器的 RADIUS 用戶端中，選取您在上一個步驟中加入 VPN 伺服器的名稱。 例如，如果您的 VPN 伺服器 NetBIOS 名稱 RAS1，請選取**RAS1**。
...
6. 在設定驗證方法，完成下列步驟：
    ...
    2. 選取 **可延伸驗證通訊協定**核取方塊來選取它。

    3. 在 （根據存取和網路設定的方法） 的類型，選取**Microsoft:受保護的 EAP (PEAP)** ，然後選取**設定**。
      
        編輯受保護的 EAP 內容 對話方塊隨即開啟。

    4. 選取 **移除**若要移除的保護密碼 (EAP-MSCHAP v2) 的 EAP 類型。

    5. 選取 **新增**。 [新增 EAP] 對話方塊隨即開啟。

    6. 選取 **智慧卡或其他憑證**，然後選取**確定**。

    7. 選取 **確定**以關閉 編輯受保護的 EAP 內容。
...
8. 在指定的使用者群組 中，完成下列步驟：
    ...
    2. 請輸入**VPN 使用者**，然後選取**確定**。

    3. 選取 **\[下一步\]** 。

9. 在指定的 IP 篩選器，選取**下一步**。

10. 在指定的加密設定 中，選取**下一步**。 不進行任何變更。

    這些設定僅適用於此案例中不支援的 Microsoft 點對點加密 (MPPE) 連線。
