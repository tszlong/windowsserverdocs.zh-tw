...
11. 在 [屬性] 中，選取 [**安全性**] 索引標籤，然後執行：
    ...
    d. 在 [**伺服器名稱**] 中，輸入您組織/公司網路上 NPS 伺服器 (FQDN) 的完整功能變數名稱。
...
15. 選取 [ **IPv4** ] 索引標籤，然後執行下列動作：
    ...
    e.  (選擇性) 如果您使用 DHCP，請選取 [**介面卡**]，然後在結果清單中選取連線到內部周邊網路的 Ethernet 介面卡。

16.  (選擇性) *如果您要設定 vpn 連線的條件式存取*，請從 [**憑證**] 下拉式清單的 [ **SSL 憑證**系結] 底下，選取 [vpn 伺服器驗證]。

17.  (選擇性) *如果您要設定 VPN 連線的條件式存取*，請在 NPS MMC 中，展開 [原則] [ ** \\ 網路原則**]，然後執行：
...
19. 選取 [ **WAN 迷你埠] (SSTP) **然後選取 [**設定**]。 [設定裝置-WAN 迷你埠 (SSTP) ] 對話方塊隨即開啟。
    ...
    a. 清除 [**遠端存取] ([僅限輸入) **和指定**撥號路由連接] ([輸入] 和 [輸出) ** ] 核取方塊。
...
[步驟4。安裝和設定網路原則伺服器 (NPS) ](vpn-deploy-nps.md)：在此步驟中，您會使用 Windows PowerShell 或伺服器管理員新增角色及功能嚮導，安裝網路原則伺服器 (NPS) 。 您也可以設定 NPS，以處理從 VPN 伺服器接收的連線要求的所有驗證、授權和帳戶管理責任。
