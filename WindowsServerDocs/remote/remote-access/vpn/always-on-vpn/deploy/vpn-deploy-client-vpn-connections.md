...
在此步驟中，您可以深入了解 ProfileXML 選項以及結構描述，並設定 Windows 10 用戶端電腦通訊的 VPN 連線與該基礎結構。 
...
## <a name="bkmk_profile"></a>手動建立範本的連線設定檔
...
6.  記錄的值**憑證發給**並**簽發者**。<p>您即將推出的 VPN 範本組態中使用這些值。 比方說，如果伺服器的 FQDN 是 nps01.corp.contoso.com 主機名稱是 NPS01，憑證名稱根據伺服器-例如 nps01.corp.contoso.com FQDN 或 DNS 名稱。
...
### <a name="configure-the-template-vpn-profile-on-a-domain-joined-client-computer"></a>網域上設定 VPN 設定檔範本\-聯結用戶端電腦

既然您已加入網域的用戶端電腦上設定 VPN 設定檔範本的必要資訊。 您使用的使用者帳戶的類型\(也就是標準使用者或系統管理員\)的這部分的程序並不重要。 
...
10. 按一下 [**確定**關閉範本屬性] 對話方塊。

11. 關閉 [網路連線] 視窗。

12. 在設定中，按一下以測試 VPN**樣板**，然後按一下**Connect**。

>[!IMPORTANT]
>請確定範本至您的 VPN 伺服器的 VPN 連線會成功。 這麼做可確保 EAP 設定正確，才能在下一個範例中使用它們。 您必須連接至少一次，再繼續進行;否則，設定檔不會包含所有 vpn 連線所需的資訊。

## <a name="bkmk_ProfileXML"></a>建立 ProfileXML 設定檔

在之前完成本節中，請確定您已建立並測試範本的 VPN 連線的一節[手動建立範本的連線設定檔](#bkmk_profile)描述。 測試 VPN 連線是為了確保設定檔包含連接至 VPN 所需的所有資訊。
...
1. 包含具有相同使用者的 VPN 設定檔範本的已加入網域的用戶端電腦登入帳戶的一節 < 手動建立範本的連線設定檔 > 所述。

2. 貼入 Windows PowerShell 整合式指令碼環境中的列表 1 \(ISE\)，和自訂的註解中所述的參數。 這些是 $Template、 $ProfileName、 $Servers、 $DnsSuffix、 $DomainName、 $TrustedNetwork 和 $DNSServers。 每個設定的完整描述是註解中。
...
>[!NOTE]
>若要檢視完整的範例指令碼，請參閱下節[MakeProfile.ps1 完整的指令碼](#bkmk_fullscript)。
...
## <a name="vpn-deploy-client-sccm"></a>使用 System Center Configuration Manager 中設定 VPN 用戶端

System Center Configuration Manager 中，可以使用 [ProfileXML CSP] 節點中，就像在 Windows PowerShell 來部署 VPN 設定檔。 這裡，您可以使用您在一節中建立的 VPN_Profile.ps1 Windows PowerShell 指令碼[建立 ProfileXML 設定檔](#bkmk_ProfileXML)。
...
您建立使用者群組，以接收 VPN 設定檔之後，您可以建立套件和程式以部署一節建立的 Windows PowerShell 組態指令碼[建立 ProfileXML 設定檔](#bkmk_ProfileXML)。
...
4.  在 [套件] 頁面上完成下列步驟：
    ...
    c.  在 設定來源資料夾 對話方塊中，按一下**瀏覽**，選取 檔案共用，其中包含 VPN_Profile.ps1，然後按一下**確定**。<p>請確定您選取的網路路徑，而不是本機路徑。 換句話說，路徑必須是類似 *\\fileserver\\vpnscript*，而非*c:\\vpnscript*。
...
若要使用 Intune 來部署 Windows 10 的遠端存取一律在 VPN 設定檔，您可以使用您建立一節中的 VPN 設定檔設定 ProfileXML CSP 節點[建立 ProfileXML 設定檔](#bkmk_ProfileXML)，或者您可以使用基底的 EAP下面提供的 XML 範例。
...
5.   底下**平台**，選取**Windows 10 或更新版本**，然後選擇**VPN**從設定檔類型 下拉式清單。
...
8.  取代 **\<TrustedRootCA > 5a 89 fe cb 5b 49 a7 0b 1a 52 63 b7 35 ee d7 1c c2 68 是 4b <\/TrustedRootCA >** 在範例中，您在內部部署環境的根憑證授權單位的憑證指紋在兩個位置中。
...
10. 取代 **\<ServerNames > NPS.contoso.com\</ServerNames >** 在範例 XML 的已加入網域的 NPS 驗證發生的其中的 fqdn。 

11. 複製修改過的 XML 字串並貼入**EAP Xml**下方 基底 VPN 索引標籤，然後按一下 **確定**。<p>在 Intune 中，會建立使用 EAP 一律在 VPN 裝置組態原則。
