...
> [!NOTE]
> 如果您有一或多個網域成員檔案伺服器，您可以將它們新增至 Active Directory Domain Services 中的組織單位 (OU) ，然後使用群組原則來一次設定所有檔案伺服器的雜湊發行，而不是個別設定每一部檔案伺服器。 如需詳細資訊，請參閱[啟用網域成員檔案伺服器的雜湊發行](../../branchcache/deploy/Enable-Hash-Publication-for-Domain-Member-File-Servers.md)。
