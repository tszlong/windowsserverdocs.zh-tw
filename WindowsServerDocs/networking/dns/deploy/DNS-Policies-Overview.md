...
您可以使用本主題來瞭解 DNS 原則，這是 Windows Server 2016 中的新功能。 您可以將 DNS 原則用於地理位置型流量管理、以一天的時間為基礎的智慧型 DNS 回應、管理設定為分割\-大腦部署的單一 DNS 伺服器、在 DNS 查詢上套用篩選等等。 下列專案提供有關這些功能的詳細資料。
...
|              名稱               |                                         描述                                          |                                                                                                                               範例值                                                                                                                               |
|---------------------------------|----------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|        **用戶端子網**        | 預先定義的用戶端子網名稱。 用來驗證傳送查詢的子網。 |                             -   **EQ，西班牙，法國**-如果子網已識別為西班牙或法國，則會解析為 true<br />-   **NE，加拿大，墨西哥**-如果用戶端子網是加拿大和墨西哥以外的任何子網，則解析為 true                             |
