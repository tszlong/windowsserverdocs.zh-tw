...
如需跨網路的一致性，建議伺服器、印表機及其他裝置使用一致的名稱。 您可以使用電腦名稱協助使用者與系統管理員輕鬆識別伺服器、印表機或其他裝置的目的與位置。 例如，如果您有三部 DNS 伺服器，一個在三藩市，一個在洛杉磯，另一個在芝加哥，則您可以使用命名慣例*伺服器*函式 - *位置* - *編號*：
...
您可以使用本主題中的程式，針對執行 Windows Server 2016 的電腦，使用靜態 IP 位址來設定網路連線的網際網路通訊協定第4版 (IPv4) 屬性。
...
您可以使用下列其中一個程式，將 Active Directory Domain Services 安裝 (AD DS) 和 DNS，並在新樹系中建立新的網域。
...
網路原則伺服器 (NPS) 可讓您使用下列功能集中設定和管理網路原則：遠端驗證撥入使用者服務 (RADIUS) 伺服器和 RADIUS proxy。

NPS 是核心網路的選用元件，但是如果下列任一項為真，您應該安裝 NPS：

- 您打算擴充您的網路，使其包含與 RADIUS 通訊協定相容的遠端存取服務器，例如執行 Windows Server 2016、Windows Server 2012 R2、Windows Server 2012、Windows Server 2008 R2 或 Windows Server 2008 以及路由及遠端存取服務、終端機服務閘道或遠端桌面閘道的電腦。
...
您可以使用此程式，使用 [新增角色及功能] Wizard (NPS) 安裝網路原則伺服器。 NPS 是網路原則與存取服務伺服器角色的角色服務。
...
> [!NOTE]
> 若要使用網路介面 (Netsh) Windows PowerShell 中的命令來執行此程式，請開啟 PowerShell 並輸入下列命令，然後按 ENTER。
>
...
Windows Server 2016 中的 Web 服務器 (IIS) 角色提供安全、容易管理、模組化且可擴充的平臺，讓您能夠可靠地裝載網站、服務和應用程式。 透過 Internet Information Services (IIS) ，您可以與網際網路、內部網路或外部網路上的使用者共用資訊。 IIS 是整合的 web 平臺，可將 IIS、ASP.NET、FTP 服務、PHP 及 Windows Communication Foundation (WCF) 。

除了可讓您發佈 CRL 以供網域成員電腦存取之外，網頁伺服器 (IIS) 伺服器角色可讓您設定和管理多個網站、Web 應用程式和 FTP 網站。 IIS 也提供下列優點：
...
5.  在 [**選取伺服器角色**] 頁面上，流覽至 [ **WEB 伺服器 (IIS) **]，然後加以選取。 此時會開啟 [**網頁伺服器 (IIS) 所需的新增功能**] 對話方塊。 按一下 [新增功能]  ，然後按 [下一步]  。
