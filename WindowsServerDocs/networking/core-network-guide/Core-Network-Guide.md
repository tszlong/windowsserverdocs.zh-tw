---
title: 核心網路元件
description: 本指南說明如何規劃和部署全功能網路與 Windows Server 2016 的新樹系中的新 Active Directory 網域所需之核心元件
manager: brianlic
...
ms.locfileid: "59850539"
---
# <a name="core-network-components"></a>核心網路元件

>適用於：Windows Server （半年通道），Windows Server 2016

本指南提供有關如何規劃和部署全功能網路與新樹系中的新 Active Directory 網域所需的核心元件的指示。

> [!NOTE]
> 本指南可從 TechNet 組件庫的 Microsoft Word 格式下載。 如需詳細資訊，請參閱 <<c0> [ 核心網路指南 for Windows Server 2016](https://gallery.technet.microsoft.com/Core-Network-Guide-for-9da2e683)。
...
-   電腦與其他傳輸控制通訊協定/網際網路通訊協定 (TCP/IP) 相容裝置之間網路連線能力的核心通訊協定。 TCP/IP 是一組標準的通訊協定，用於連線電腦與建置網路。 TCP/IP 是網路通訊協定所提供的軟體 Microsoft Windows 作業系統，實作和支援 TCP/IP 通訊協定套件。
...
> [!NOTE]
> 執行 Windows 用戶端作業系統的電腦會設定預設為接收來自 DHCP 伺服器 IP 位址租用。 因此，不需要用戶端電腦的其他 DHCP 或網際網路通訊協定第 4 版 (IPv4) 設定。

## <a name="technology-overviews"></a>技術概觀
下列幾節提供為建立核心網路所部署之必要技術的簡短概觀。

### <a name="active-directory-domain-services"></a>Active Directory Domain Services
目錄是階層結構，可以儲存網路上物件的相關資訊，例如，使用者和電腦。 目錄服務，例如 AD DS 中，提供方法來儲存目錄資料，並且讓這項資料可供網路使用者和系統管理員。 例如，AD DS 會儲存使用者帳戶，包括名稱、 電子郵件地址、 密碼和電話號碼的相關資訊，並讓其他授權的使用者在相同的網路存取這項資訊。
...
這兩個名為 DC1，提供目錄的這個伺服器上安裝 Active Directory 網域服務 (AD DS) 和網域名稱系統 (DNS)，以及名稱解析服務，所有的電腦與網路上的裝置。
...
根據預設會設定執行 Windows 用戶端作業系統的電腦，做為 DHCP 用戶端，從 DHCP 伺服器自動取得 IP 位址及 DHCP 選項。
...
> [!NOTE]
> 如需協助規劃您的部署，另請參閱[附錄 E-核心網路規劃準備表](#BKMK_E)。
...
此外，若要使用靜態 IP 位址來設定網路的伺服器，必須確定您要用於核心網路伺服器所在之子網路的 IP 位址範圍。 本指南中，私人 IP 位址範圍 10.0.0.1-10.0.0.254 和 10.0.1.1-10.0.1.254 做為範例，但您可以使用您偏好的任何私人 IP 位址範圍。
...
如需跨網路的一致性，建議伺服器、印表機及其他裝置使用一致的名稱。 您可以使用電腦名稱協助使用者與系統管理員輕鬆識別伺服器、印表機或其他裝置的目的與位置。 比方說，如果您有三部 DNS 伺服器，分別於舊金山、 一部位於洛杉磯，和在芝加哥，您可能會使用命名慣例*server 函式*-*位置*-*數字*:
...
下面是之前安裝 Active Directory 網域服務 (AD DS) 的主要規劃步驟與在 DC1 上的 DNS。

#### <a name="planning-the-name-of-the-forest-root-domain"></a>規劃樹系根網域的名稱
AD DS 設計處理序中的第一個步驟是判斷您的組織需要多少樹系。 樹系是最上層的 AD DS 容器，並包含一或多個網域共用通用的結構描述和通用類別目錄。 組織可以擁有多個樹系，但單一樹系設計是大多數組織慣用的模型，而且最容易管理。
...
在安裝 AD DS，您必須選擇您想要使用的樹系功能等級。 網域和樹系功能導入 Windows Server 2003 Active directory，提供啟用網域或樹系-寬的網路環境中的 Active Directory 功能的方法。 視您的環境而定，提供了不同等級的網域功能和樹系功能。

樹系功能會啟用樹系中跨所有網域的功能。 樹系功能等級有下列幾種：

-    Windows Server 2008。 此樹系功能等級支援執行 Windows Server 2008 和更新版本的 Windows Server 作業系統的網域控制站。

-    Windows Server 2008 R2。 此樹系功能等級支援 Windows Server 2008 R2 網域控制站和網域控制站執行 Windows Server 作業系統的更新版本。

-    Windows Server 2012。 此樹系功能等級支援 Windows Server 2012 網域控制站和網域控制站執行 Windows Server 作業系統的更新版本。

-    Windows Server 2012 R2。 此樹系功能等級支援 Windows Server 2012 R2 網域控制站和網域控制站執行 Windows Server 作業系統的更新版本。

-    Windows Server 2016. 此樹系功能等級只支援 Windows Server 2016 網域控制站和網域控制站執行 Windows Server 作業系統的更新版本。

如果您要部署新樹系中新的網域，而且所有網域控制站都執行 Windows Server 2016，建議使用 Windows Server 2016 樹系功能等級設定 AD DS，在 AD DS 安裝期間。

> [!IMPORTANT]
> 在提高樹系功能等級之後，便無法將執行舊版作業系統的網域控制站引入樹系。 例如，如果您的樹系功能等級提高至 Windows Server 2016，執行 Windows Server 2012 R2 或 Windows Server 2008 網域控制站無法新增至樹系。

下表提供適用於 AD DS 的範例設定項目。
...
In-addr.arpa 網域適用於所有的 TCP/IP 網路為基礎的網際網路通訊協定第 4 版 (IPv4) 定址。 當您建立新的反向對應區域時，新增區域精靈會自動假設您要使用這個網域。
...
若要登入網域，電腦必須是網域成員電腦，必須先登入嘗試的 AD DS 中建立使用者帳戶。
...
|[新增領域] 對話方塊值<br /><br />1.領域名稱<br />2.起始 IP 位址<br />3.結束 IP 位址<br />4.子網路遮罩<br />5.預設閘道 (選擇性)<br />6.租用期間|1.主要子網路<br />2.  10.0.0.1<br />3.  10.0.0.254<br />4.  255.255.255.0<br />5.  10.0.0.1<br />6.8 天|
...
5.  [用戶端電腦加入網域並登入](#BKMK_joinlogclients)
...
您可以使用本節中的程序，若要變更電腦的名稱。 當作業系統已自動建立您不想使用的電腦名稱時，將電腦重新命名就很有用。
...
> [!NOTE]
> 如需如何重新命名執行其他 Microsoft 作業系統的電腦資訊，請參閱[附錄 A-重新命名電腦](#BKMK_A)。

#### <a name="BKMK_ip"></a>設定靜態 IP 位址
您也可以在本主題中使用的程序，設定網際網路通訊協定第 4 版 (IPv4) 執行 Windows Server 2016 的電腦位址的靜態 ip 位址的網路連線的屬性。
...
2.  在 **網路和共用中心**，按一下**變更介面卡設定**。 [網路連線] 資料夾隨即開啟，並顯示可用的網路連線。
...
> [!NOTE]
> 如需如何執行其他 Microsoft 作業系統的電腦上設定靜態 IP 位址資訊，請參閱[附錄 B-設定靜態 IP 位址](#BKMK_B)。

#### <a name="BKMK_deployADDNS01"></a>部署 DC1
若要部署 DC1，這是電腦執行 Active Directory 網域服務 (AD DS) 和 DNS，您必須完成下列步驟順序如下：

-   執行[設定所有伺服器](#BKMK_configuringAll)一節中的步驟。

-   [安裝 AD DS 與 DNS 的新樹系](#BKMK_installAD-DNS)

-   [在 Active Directory 使用者和電腦建立使用者帳戶](#BKMK_createUA)
...
如果您有較大的組織具備多位系統管理員，請參閱 AD DS 文件，以決定組織員工的最佳的群組成員資格。

**網域使用者帳戶和本機電腦上的使用者帳戶之間的差異**
...
#### <a name="BKMK_installAD-DNS"></a>安裝 AD DS 與 DNS 的新樹系

您可以使用下列程序的其中一個選項來安裝 Active Directory 網域服務 (AD DS) 和 DNS 和新樹系中建立新的網域。 

第一個程序提供第二個程序將說明如何使用伺服器管理員安裝 AD DS 與 DNS 時，使用 Windows PowerShell 來執行這些動作的指示。

>[!IMPORTANT]
>完成此程序中執行的步驟之後，會自動重新啟動電腦。

**安裝 AD DS 與 DNS 使用 Windows PowerShell**

您可以使用下列命令來安裝和設定 AD DS 與 DNS。 您必須在此範例中的網域名稱取代您想要用於您的網域值。

>[!NOTE]
>如需有關這些 Windows PowerShell 命令的詳細資訊，請參閱下列參考主題。
...
- 系統管理員身分執行 Windows PowerShell，輸入下列命令，，然後按 ENTER:  

`Install-WindowsFeature AD-Domain-Services -IncludeManagementTools`

當安裝順利完成時，是在 Windows PowerShell 中顯示下列訊息。
...
- 在 Windows PowerShell 中，輸入下列命令，取代文字**corp.contoso.com**與您的網域名稱，然後按 ENTER 鍵：

````
Install-ADDSForest -DomainName "corp.contoso.com"
````

- 安裝和設定程序期間，會顯示在頂端的 [Windows PowerShell] 視窗，顯示下列提示。 它出現之後，請輸入密碼，然後按 ENTER 鍵。

    **SafeModeAdministratorPassword:**

- 您輸入密碼，然後按 ENTER 之後，便會出現下列確認提示。 輸入相同的密碼，然後按 ENTER 鍵。

    **確認 SafeModeAdministratorPassword:**

- 以下的提示出現時，請輸入字母**Y**然後按 ENTER 鍵。

    
    目標伺服器會設定為網域控制站，並重新啟動此作業完成時。
    若要繼續此作業嗎？
    [Y] 是 [A] 是所有 [n] 否至所有 [S] 沒有 [L] 暫停 [？]說明 （預設值為"Y"）：
    
- 如果您想要您可以讀取的 AD DS 與 DNS 的一般的成功安裝期間會顯示警告訊息。 這些訊息都是正常，而且不會安裝失敗的指示。

- 安裝成功之後，會出現訊息指出您將要記錄移出電腦，以便可以重新啟動電腦。 如果您按一下**關閉**、 立即登出電腦，並重新啟動電腦。 如果您沒有按一下**關閉**，預設會段時間後重新啟動電腦。

- 重新啟動伺服器之後，您可以確認安裝成功的 Active Directory 網域服務和 DNS。 開啟 Windows PowerShell 中，輸入下列命令，然後按 ENTER。

````
Get-WindowsFeature
````

此命令的結果會顯示在 Windows PowerShell 中，並應該會類似下面的影像中的結果。 適用於已安裝的技術，在方括號左邊的技術名稱包含字元**X**，而**安裝狀態**會**已安裝**。

![Get-windowsfeature 命令的結果](../media/Core-Network-Guide/server-roles-installed.jpg)

**安裝 AD DS 與 DNS 使用伺服器管理員**
...
15. 如果想要將設定匯出至 Windows PowerShell 指令碼，請按一下 [檢視指令碼]。 隨即會在 [記事本] 開啟指令碼，您可以將它儲存到所需的資料夾位置。 按一下 [下一步] 。 在 [先決條件檢查] 中，驗證您的選取項目。 檢查完成時，按一下 [安裝]。 當 Windows 提示時，按一下 [關閉]。 在伺服器重新啟動以完成安裝 AD DS 與 DNS。

16. 若要確認安裝成功，檢視 [伺服器管理員] 主控台，在伺服器重新啟動之後。 AD DS 與 DNS 應該會出現在左窗格中，像下圖中反白顯示的項目。

![AD DS 與 DNS 伺服器管理員 中](../media/Core-Network-Guide/server-roles-installed-sm.jpg)

##### <a name="BKMK_createUA"></a>在 Active Directory 使用者和電腦建立使用者帳戶
...
2.  在詳細資料窗格中，按兩下包含您要新增成員之群組的資料夾。
    ...
    -   **Active Directory 使用者和電腦**/*網域節點*/*包含群組的資料夾*
...
> [!NOTE]
> -   對於中型和大型組織而言，建議您設定和使用 DNSAdmins 群組，在 Active Directory 使用者和電腦。 如需詳細資訊，請參閱[其他技術資源](#BKMK_resources)。
> -   若要使用 Windows PowerShell 執行這個程序，請開啟 PowerShell，將下列 Cmdlet 輸入一行，然後按 ENTER。 您也必須以您要使用的值取代這個範例中的 DNS 反向對應區域和區域檔案名稱。 請確定您會反轉反向對應區域名稱的網路識別碼。 例如，如果網路識別碼為 192.168.0，請建立反向對應區域名稱**0.168.192.in-addr.arpa**。
>
...
您已安裝 Active Directory 網域服務 (AD DS)，並建立一或多個使用者帳戶具有將電腦加入網域的權限之後，您可以將核心網路伺服器加入網域並登入伺服器才能安裝其他技術，例如動態主機設定通訊協定 (DHCP)。

您要部署，除了執行 AD DS 伺服器的所有伺服器上執行下列作業：
...
###### <a name="to-join-computers-running--windows-server-2016--windows-server-2012-r2--and--windows-server-2012--to-the-domain"></a>若要將執行 Windows Server 2016、 Windows Server 2012 R2 和 Windows Server 2012 網域的電腦
...
> [!NOTE]
> 如需如何加入至定義域執行其他 Microsoft 作業系統的電腦資訊，請參閱[附錄 C-將電腦加入網域](#BKMK_C)。

###### <a name="to-log-on-to-the-domain-using-computers-running-windows-server-2016"></a>若要使用執行 Windows Server 2016 的電腦為網域登入
...
> [!NOTE]
> 如需如何使用執行其他 Microsoft 作業系統的電腦為網域登入資訊，請參閱[附錄 D-登入網域](#BKMK_D)。
...
5.  在 **選取伺服器角色**，請在**角色**，選取**DHCP 伺服器**。 在 [新增 DHCP 伺服器所需的功能] 中，按一下 [新增功能]。 按一下 [下一步] 。

6.  在 [選取功能] 中，按 [下一步]，然後在 [DHCP 伺服器]中，檢閱提供的資訊，接著按 [下一步]。

7.  在 [確認安裝選項] 中，按一下 [必要時自動重新啟動目的地伺服器]。 提示您確認這個選取項目時，按一下 [是]，然後按一下 [安裝]。 **Průběh instalace**頁面會顯示安裝程序期間的狀態。 此程序完成時，顯示 「 需要的設定。 在上安裝成功*ComputerName*"的訊息，其中*ComputerName*是您安裝 DHCP 伺服器電腦的名稱。 在訊息視窗中，按一下 [完成 DHCP 設定]。 [DHCP 後續安裝設定精靈] 隨即開啟。 按一下 [下一步] 。
...
2.  在  **DHCP**，展開伺服器名稱。 例如，如果 DHCP 伺服器名稱為 DHCP1.corp.contoso.com，請按一下 向下箭號旁**DHCP1.corp.contoso.com**。

3.  下伺服器名稱，以滑鼠右鍵按一下**IPv4**，然後按一下**新領域**。 此時會開啟 [新增領域精靈]。
...
### <a name="BKMK_joinlogclients"></a>用戶端電腦加入網域並登入
...
##### <a name="to-join-computers-running-windows-10-to-the-domain"></a>若要將執行 Windows 10 網域的電腦

1.  使用本機 Administrator 帳戶登入電腦。

2.  在 **搜尋 web 和 Windows**，型別**系統**。 在搜尋結果中，按一下**系統 （控制台）**。 [系統] 對話方塊隨即開啟。

3.  在 **系統**，按一下**進階系統設定**。 [系統內容] 對話方塊隨即開啟。 按一下 [**電腦名稱**] 索引標籤。

4.  在 **電腦名稱**，按一下**變更**。 [電腦名稱/網域變更] 對話方塊隨即開啟。

5.  在**電腦名稱/網域變更**，請在**隸屬**，按一下 **網域**，然後輸入您想要加入的網域名稱。 例如，如果網域名稱是 corp.contoso.com，請輸入 **corp.contoso.com**。
...
##### <a name="to-join-computers-running-windows-81-to-the-domain"></a>若要將執行 Windows 8.1 加入網域的電腦
...
##### <a name="to-log-on-to-the-domain-using-computers-running-windows-10"></a>若要使用執行 Windows 10 電腦的網域登入
...
### <a name="BKMK_optionalfeatures"></a>部署網路存取授權和 Web 服務的選用功能
如果您想要部署網路存取伺服器，例如無線存取點或 VPN 伺服器，請安裝核心網路之後，建議您部署 NPS 和網頁伺服器。 對於網路存取部署，建議使用安全憑證式驗證方法。 您可以使用 NPS 管理網路存取原則以及部署安全驗證方法。 您可以使用網頁伺服器發佈憑證授權單位 (CA) 的憑證撤銷清單 (CRL)，為安全驗證提供憑證。

> [!NOTE]
> 您可以使用核心網路附屬指南來部署伺服器憑證和其他額外功能。 如需詳細資訊，請參閱[其他技術資源](#BKMK_resources)。

下圖顯示已新增的 NPS 和網頁伺服器的 Windows Server 核心網路拓撲。

![具有新增的 NPS 和網頁伺服器的 Windows Server 核心網路拓撲](../media/Core-Network-Guide/cng16_overview_2.jpg)
...
網路原則伺服器 (NPS) 可讓您集中設定和管理網路原則與下列功能：遠端驗證撥號使用者服務 (RADIUS) 伺服器與 RADIUS proxy。

NPS 是核心網路的選用元件，但是如果下列任一項為真，您應該安裝 NPS：

-   您計畫擴充網路以包含相容於 RADIUS 通訊協定，例如執行 Windows Server 2016、 Windows Server 2012 R2、 Windows Server 2012、 Windows Server 2008 R2 或 Windows Server 2008 電腦的遠端存取伺服器和路由及遠端存取服務、 終端機服務閘道或遠端桌面閘道。


-   您想要部署 802.1x 驗證的有線或無線存取。

部署這個角色服務之前，您必須執行下列步驟，您會設定為 NPS 在電腦上。
...
-   [計畫 NPS1 的部署規劃](#bkmk_NetFndtn_Pln_NPS-01)

-   [安裝網路原則伺服器 (NPS)](#BKMK_installNPS)

-   [在 預設網域中登錄 NPS](#BKMK_registerNPS)

> [!NOTE]
> 本指南提供部署 NPS 在獨立伺服器上的指示，或 VM 命名為 nps1 的。  另一個建議的部署模型是在網域控制站上的安裝 NPS。 如果您想在獨立伺服器上而不是網域控制站上安裝 NPS，請在 DC1 上安裝 NPS。

##### <a name="bkmk_NetFndtn_Pln_NPS-01"></a>計畫 NPS1 的部署規劃
...
- 規劃使用者帳戶資料庫。 根據預設，如果您加入至 Active Directory 網域，執行 NPS 的伺服器 NPS 會執行使用 AD DS 使用者帳戶資料庫驗證和授權。 在某些狀況下，例如使用 NPS 當作 RADIUS Proxy 將連線要求轉送到其他 RADIUS 伺服器的大型網路，您可能要在非網域成員電腦上安裝 NPS。
...
若要使用 [新增角色及功能精靈] 安裝網路原則伺服器 (NPS)，您可以使用此程序。 NPS 是網路原則與存取服務伺服器角色的角色服務。
...
5.  在 **選取伺服器角色**，請在**角色**，選取**網路原則與存取服務**。 會開啟一個對話方塊，要求是否網域控制站應該加入所需的網路原則與存取服務的功能。 按一下 [新增功能]，然後按 [下一步]。
...
##### <a name="BKMK_registerNPS"></a>在 預設網域中登錄 NPS
...
> [!NOTE]
> 使用網路殼層 (Netsh) 命令，在 Windows PowerShell 執行此程序，請開啟 PowerShell 並輸入下列命令，並然後按 ENTER 鍵。
>
...
如需有關網路原則伺服器的詳細資訊，請參閱 <<c0> [ 網路原則伺服器 (NPS)](../technologies/nps/nps-top.md)。

#### <a name="BKMK_IIS"></a>部署 WEB1

Windows Server 2016 中的網頁伺服器 (IIS) 角色提供安全、 容易管理、 模組化且可擴充的平台能夠可靠地主控網站、 服務和應用程式。 使用網際網路資訊服務 (IIS) 中，您可以共用網際網路、 內部或外部網路與使用者資訊。 IIS 是一個整合 IIS、 ASP.NET、 FTP 服務、 PHP 和 Windows Communication Foundation (WCF) 的統合式的 web 平台。

除了讓您將存取的 CRL 發佈由網域成員電腦、 網頁伺服器 (IIS) 伺服器角色可讓您設定和管理多個 web sites、 web 應用程式和 FTP 站台。 IIS 也會提供下列優點：
...
4.  在 [**選取目的地伺服器**頁面上，確定本機電腦已選取，然後按一下**下一步]**。

5.  在 **選取伺服器角色**頁面上，捲動至並選取**網頁伺服器 (IIS)**。 **新增所需的網頁伺服器 (IIS) 功能**對話方塊隨即開啟。 按一下 [新增功能]，然後按 [下一步]。
...
-   [在 Windows Server 2016 中的 Active Directory 網域服務 (AD DS) 中最新消息](https://technet.microsoft.com/library/mt163897.aspx)

-   [Active Directory 網域服務概觀](https://technet.microsoft.com/library/hh831484.aspx)在 https://technet.microsoft.com/library/hh831484.aspx。

-   [網域名稱系統 (DNS) 概觀](https://technet.microsoft.com/library/hh831667.aspx)在 https://technet.microsoft.com/library/hh831667.aspx。

-   [實作 DNS Admins 角色](https://technet.microsoft.com/library/cc756152(WS.10).aspx)

-   [動態主機設定通訊協定 (DHCP) 概觀](https://technet.microsoft.com/library/hh831825.aspx)在 https://technet.microsoft.com/library/hh831825.aspx。

-   [網路原則與存取服務概觀](https://technet.microsoft.com/library/hh831683.aspx)在 https://technet.microsoft.com/library/hh831683.aspx。

-   [Web 伺服器 (IIS) 概觀](https://technet.microsoft.com/library/hh831725.aspx)在 https://technet.microsoft.com/library/hh831725.aspx。

## <a name="BKMK_appendix"></a>附錄 A 到 E
下列各節包含執行非 Windows Server 2016 中，Windows 10，Windows Server 2012 和 Windows 8 作業系統的電腦的其他組態資訊。 此外，網路準備表可協助您進行部署。
...
您可以使用本節中的程序，以提供不同的電腦名稱與執行 Windows Server 2008 R2、 Windows 7、 Windows Server 2008 和 Windows Vista 的電腦。
...
2.  在 [電腦名稱、網域及工作群組設定] 中，按一下 [變更設定]。 [系統內容] 對話方塊隨即開啟。
    ...
    > [!NOTE]
    > 在電腦上執行 Windows 7 之前**系統屬性** 對話方塊隨即開啟，**使用者帳戶控制**對話方塊隨即開啟，要求權限以繼續。 按一下 [繼續] 繼續。
...
##### <a name="to-rename-computers-running-windows-server-2008-and-windows-vista"></a>若要重新命名執行 Windows Server 2008 和 Windows Vista 的電腦
...
2.  在 [電腦名稱、網域及工作群組設定] 中，按一下 [變更設定]。 [系統內容] 對話方塊隨即開啟。
    ...
    > [!NOTE]
    > 在電腦上執行 Windows Vista 之前**系統屬性** 對話方塊隨即開啟，**使用者帳戶控制**對話方塊隨即開啟，要求權限以繼續。 按一下 [繼續] 繼續。
...
您可以使用這些程序加入到網域中執行 Windows Server 2008 R2、 Windows 7、 Windows Server 2008 和 Windows Vista 的電腦。
...
##### <a name="to-join-computers-running-windows-server-2008-r2-and-windows-7-to-the-domain"></a>若要加入的網域中執行 Windows Server 2008 R2 和 Windows 7 的電腦
...
##### <a name="to-join-computers-running-windows-server-2008-and-windows-vista-to-the-domain"></a>若要加入的網域中執行 Windows Server 2008 和 Windows Vista 的電腦
...
您可以使用這些程序來使用執行 Windows Server 2008 R2、 Windows 7、windows Server 2008 和 Windows Vista 的電腦為網域登入。
...
##### <a name="log-on-to-the-domain-using-computers-running-windows-server-2008-r2-and-windows-7"></a>使用執行 Windows Server 2008 R2 和 Windows 7 的電腦為網域登入
...
##### <a name="log-on-to-the-domain-using-computers-running-windows-server-2008-and-windows-vista"></a>使用執行 Windows Server 2008 和 Windows Vista 的電腦為網域登入
...
## <a name="BKMK_E"></a>附錄 E-核心網路規劃準備表
...
1.  [安裝 Active Directory 網域服務和 DNS](#BKMK_FndtnPrep_InstallAD)
...
### <a name="BKMK_FndtnPrep_InstallAD"></a>安裝 Active Directory 網域服務和 DNS
在本節中的表格列出預先安裝及安裝 Active Directory 網域服務 (AD DS) 的設定項目和 DNS。
...
|設定項目|範例值|值|
|-----------------------|------------------|----------|
|區域類型：|-主要區域<br />-次要區域<br />虛設常式區域||
|區域類型<br /><br />**將區域存放在 Active Directory**|選取<br />-未選取||
|Active Directory 區域複寫領域|-若要在這個樹系中的所有 DNS 伺服器<br />-若要在這個網域中的所有 DNS 伺服器<br />-若要在這個網域中的所有網域控制站<br />-若要指定此目錄磁碟分割的範圍內的所有網域控制站||
...
|租用期間<br /><br />日<br /><br />小時<br /><br />分|-   8<br />-   0<br />-   0||
...
如需 Windows Server 核心網路 NPS 部署程序的設定項目[安裝網路原則伺服器 (NPS)](#BKMK_installNPS)並[預設網域中登錄 NPS](#BKMK_registerNPS)。
