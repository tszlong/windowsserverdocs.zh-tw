...
如需跨網路的一致性，建議伺服器、印表機及其他裝置使用一致的名稱。 您可以使用電腦名稱協助使用者與系統管理員輕鬆識別伺服器、印表機或其他裝置的目的與位置。 例如，如果您有三部 DNS 伺服器，一個位於三藩市，一個在洛杉磯，另一個在芝加哥，則您可以使用命名慣例*伺服器*函式 -*位置*-*號碼*：
...
|設定項目：|範例值：|
|------------------------|-------------------|
|完整 DNS 名稱|例如：<br /><br />-corp.contoso.com<br />-example.com|
...
|Active Directory 網域服務資料庫資料夾位置|E:\Configuration @ no__t-0<br /><br />或接受預設位置。|
|Active Directory 網域服務記錄檔資料夾位置|E:\Configuration @ no__t-0<br /><br />或接受預設位置。|
|Active Directory 網域服務 SYSVOL 資料夾位置|E:\Configuration @ no__t-0<br /><br />或接受預設位置|
|目錄還原模式系統管理員密碼|**J @ no__t-1p2leO4 $ F**|
...
- 重新開機伺服器之後，您可以確認 Active Directory Domain Services 和 DNS 的安裝成功。 開啟 [Windows PowerShell]，輸入下列命令，然後按 ENTER。
...
2.  在詳細資料窗格中，按兩下包含您要新增成員之群組的資料夾。
    ...
    -   Active Directory*包含群組*的**使用者和電腦**/ 個*網域節點*/ 個資料夾
...
5.  在 [**選取伺服器角色**] 頁面上，流覽至並選取 [**網頁伺服器（IIS）** ]。 [**新增網頁伺服器（IIS）所需的功能**] 對話方塊隨即開啟。 按一下 [新增功能]，然後按 [下一步]。
...
-   [Active Directory Domain Services](https://technet.microsoft.com/library/hh831484.aspx)在 https://technet.microsoft.com/library/hh831484.aspx 的總覽。

-   [網域名稱系統（DNS）總覽](https://technet.microsoft.com/library/hh831667.aspx)，位於 https://technet.microsoft.com/library/hh831667.aspx 。

-   [執行 DNS 管理員角色](https://technet.microsoft.com/library/cc756152(WS.10).aspx)

-   [動態主機設定通訊協定（DHCP）總覽](https://technet.microsoft.com/library/hh831825.aspx)于 https://technet.microsoft.com/library/hh831825.aspx 。

-   [網路原則與存取服務總覽](https://technet.microsoft.com/library/hh831683.aspx)，位於 https://technet.microsoft.com/library/hh831683.aspx 。
...
2.  在 [電腦名稱、網域及工作群組設定] 中，按一下 [變更設定]。 [系統內容] 對話方塊隨即開啟。
    ...
    > [!NOTE]
    > 在執行 Windows 7 的電腦上，在 [**系統**內容] 對話方塊開啟之前，會開啟 [**使用者帳戶控制**] 對話方塊，要求許可權以繼續。 按一下 [繼續] 繼續。
...
2.  在 [電腦名稱、網域及工作群組設定] 中，按一下 [變更設定]。 [系統內容] 對話方塊隨即開啟。
    ...
    > [!NOTE]
    > 在執行 Windows Vista 的電腦上，在 [**系統**內容] 對話方塊開啟之前，會開啟 [**使用者帳戶控制**] 對話方塊，要求許可權以繼續。 按一下 [繼續] 繼續。
...
1.  [安裝 Active Directory Domain Services 和 DNS](#BKMK_FndtnPrep_InstallAD)
...
Windows Server 核心網路 NPS 部署程式的設定專案會[安裝網路原則伺服器（nps）](#BKMK_installNPS) ，並[在預設網域中註冊 NPS](#BKMK_registerNPS)。
