...
>[!IMPORTANT]
>如果您在連線要求原則中設定的驗證方法，比您在網路原則中設定的驗證方法低，則會覆寫您在網路原則中設定的更安全的驗證方法。 例如，如果您有一個需要使用受保護的可延伸驗證通訊協定的網路原則-Microsoft 挑戰交握驗證通訊協定第2版 \(PEAP-MS-CHAP v2\)（這是用於安全無線的密碼型驗證方法），而且您也會設定連線要求原則來允許未經驗證的存取，因此不需要用戶端使用 PEAP-ms-chap v2 進行驗證。 在此範例中，所有連線到您網路的用戶端都會被授與未經驗證的存取權。
