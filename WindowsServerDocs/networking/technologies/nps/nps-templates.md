...
網路原則伺服器 \(NPS @ no__t-1 範本可讓您建立設定元素，例如遠端驗證撥入使用者服務 \(RADIUS @ no__t-3 用戶端或共用密碼，您可以在本機 NPS 上重複使用並匯出以用於其他Nps.
...
例如，如果您在 NPS 主控台的 [RADIUS 用戶端和伺服器] 底下設定 RADIUS 用戶端，則您已改變 NPS 設定，並在設定 NPS 與其中一個網路存取伺服器通訊時，採取一個步驟 \(NAS's @ no__t-1。 \(The 下一步是設定 NAS 與 NPS 通訊。 \)不過，如果您在 NPS 主控台的 [**範本管理**] 底下設定新的 Radius 用戶端範本，而不是在 [ **RADIUS 用戶端和伺服器**] 底下建立新的 radius 用戶端，則您已建立範本，但尚未變更 NPS還有功能。 若要改變 NPS 功能，您必須從 NPS 主控台中正確的位置選取範本。
