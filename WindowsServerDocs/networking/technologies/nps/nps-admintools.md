...
- 在本機上，使用 NPS Microsoft Management Console \(MMC @ no__t-1 嵌入式管理單元、[系統管理工具]、[Windows PowerShell 命令] 中的靜態 NPS 主控台，或適用于 NPS 的 [網路命令介面] \(Netsh @ no__t-3 命令。
...
>[!NOTE]
>在 Windows Server 2016 中，您可以使用 NPS 主控台來管理本機 NPS。 若要同時管理遠端和本機 Nps，您必須使用 NPS MMC 貼齊 @ no__t-0in。
...
2. 在 NPS 主控台中，按一下 [NPS \(Local @ no__t-1]。 在詳細資料窗格中，選擇 [**標準**設定] 或 [ **Advanced configuration**]，然後根據您的選擇執行下列其中一項：
...
## <a name="manage-multiple-npss-by-using-the-nps-mmc-snap-in"></a>使用 NPS MMC Snap @ no__t-0in 管理多個 Nps

您可以使用此程式，使用 NPS MMC 貼齊 @ no__t-0in 來管理本機 NPS 和多個遠端 Nps。

執行下列程式之前，您必須在本機電腦和遠端電腦上安裝 NPS。

根據網路狀況，以及您使用 NPS MMC 貼齊 @ no__t-0in 管理的 Nps 數目，MMC 貼齊 @ no__t-1in 的回應可能會變慢。 此外，NPS 設定流量會在遠端系統管理會話期間透過網路傳送，方法是使用 NPS snap @ no__t-0in。 請確定您的網路實體安全，而且惡意使用者沒有此網路流量的存取權。

**系統管理認證** 

若要完成此程式，您必須是 Administrators 群組的成員。

### <a name="to-manage-multiple-npss-by-using-the-nps-snap-in"></a>使用 NPS snap @ no__t-0in 管理多個 Nps

1. 若要開啟 MMC，請以系統管理員身分執行 Windows PowerShell。 在 Windows PowerShell 中，輸入**mmc**，然後按 enter。 Microsoft Management Console 便會隨即開啟。
2. 在**MMC 的 [檔案] 功能表上**，按一下 [**新增/移除貼齊 @ no__t-2in**]。 [**新增或移除嵌入式管理單元 @ no__t-1ins** ] 對話方塊隨即開啟。
3. 在 [**新增或移除貼齊 @ no__t-1ins**] 的 [**可用的貼齊 @ no__t-3ins**] 中，依序向下清單、[**網路原則伺服器**]，然後按一下 [**新增**]。 [**選取電腦**] 對話方塊隨即開啟。
4. 在 [**選取電腦**] 中，確認已選取執行**此主控台的本機電腦 @no__t 2the 電腦 @ no__t-3** ，然後按一下 **[確定]** 。 本機 NPS 的貼齊 @ no__t-0in 會新增至**所選貼齊 @ no__t-2ins**中的清單。
5. 在 [**新增或移除貼齊 @ no__t-1ins**] 的 [**可用的貼齊 @ no__t-3ins**] 中，確定 [**網路原則伺服器**] 仍為選取狀態，然後按一下 [**新增**]。 [**選取電腦**] 對話方塊隨即開啟。
6. 在 [**選取電腦**] 中，按一下 [**另一台電腦**]，然後輸入您想要使用 NPS snap @ no__t-4 英吋管理的遠端 NPS 的 IP 位址或完整功能變數名稱 \(FQDN @ no__t-3。 （選擇性）您可以按一下 **[流覽]** ，詳閱您想要新增之電腦的目錄。 按一下 [確定]。
7. 重複步驟5和6，將更多 Nps 新增至 NPS snap @ no__t-0in。 當您已新增所有想要管理的 Nps 時，請按一下 **[確定]** 。
8. 若要儲存 NPS 嵌入式管理單元以供稍後**使用，請按一下 [** 檔案]，然後按一下 [**儲存**]。 在 **另存**新檔 對話方塊中，流覽至您要儲存檔案的硬碟位置，輸入您的 Microsoft Management Console 名稱，\( @ no__t-2 檔案，然後按一下 **儲存**。 
...
2. 使用 [遠端桌面] 連線到一個 NPS，然後在該伺服器上使用 NPS MMC 來管理其他遠端伺服器。 如需詳細資訊，請參閱上一節**使用 NPS Mmc 貼齊 @ no__t-1In 管理多個 nps**。
...
2. 在 [**遠端桌面**] 中，按一下 [**允許此電腦的遠端連線**]。 [**遠端桌面連線**] 對話方塊隨即開啟。 執行下列其中一項。
