---
title: 主機計算網路 (HCN) 案例
description: 此範例顯示如何使用主機計算網路服務 API，在主機上建立主機計算網路，以用來將虛擬 NIC 連線到虛擬機器或容器。
ms.author: daschott
author: daschott
ms.date: 11/05/2018
ms.topic: article
...
此範例示範如何使用主機計算網路服務 API，在主機上建立可用於將虛擬 NIC 連線至虛擬機器或容器的主機計算網路。
...
此範例示範如何使用 Host Compute Network Service API 建立主機計算網路端點，然後將它熱新增至虛擬機器或容器。
```C++
...
此範例說明如何使用主機計算網路服務 API 來刪除主機計算網路端點。
```C++
    wil::unique_cotaskmem_string errorRecord;
...
```
### <a name="modify-and-endpoint"></a>Modify 和 endpoint
...
此範例示範如何使用主機計算網路服務 API，在可用於連接端點和容器的主機上建立主機計算網路命名空間。
```C++
...
此範例說明如何使用主機計算網路服務 API 來刪除主機計算網路命名空間。
```C++
    wil::unique_cotaskmem_string errorRecord;
...
此範例示範如何使用主機計算網路服務 API，在主機上建立主機計算網路 Load Balancer，可用來跨計算進行端點的負載平衡。
```C++
...
此範例示範如何使用主機計算網路服務 API 來刪除主機計算網路平衡。
```C++
    wil::unique_cotaskmem_string errorRecord;
...
此範例說明如何使用主機計算網路服務 API 來列舉所有主機計算網路 Load Balancer。
```C++
...
此範例示範如何使用主機計算網路服務 API 來查詢主機計算網路平衡的屬性。
```C++
    unique_hcn_loadbalancer handle;
    GUID lbGuid; // Initialize it to appropriate loadbalancer guid value
    HRESULT hr = HcnOpenLoadBalancer(lbGuid, &handle, &errorRecord);
...
此範例示範如何使用 Host Compute Network Service API 來註冊和取消註冊整個服務的通知。 這可讓呼叫端透過在註冊期間所指定的回呼函式接收通知 () 在發生新網路建立事件等整個服務作業時。
```C++
