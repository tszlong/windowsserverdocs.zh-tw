...
---
# <a name="network-shell-netsh-example-batch-file"></a>Network Shell \(Netsh\) 範例批次檔
...
您可以在批次檔中使用適用于 Windows 網際網路名稱服務的 Netsh 命令 \(WINS\) 和其他腳本，以將工作自動化。 下列批次檔範例示範如何使用適用于 WINS 的 Netsh 命令來執行各種相關的工作。

在此範例批次檔中，WINS\-A 是具有 IP 位址192.168.125.30 的 WINS 伺服器，而 WINS\-B 是 IP 位址為192.168.0.189 的 WINS 伺服器。

範例批次檔會完成下列工作。

- 將具有 IP 位址192.168.0.205 的動態名稱記錄（我的\_記錄 \[04h\]）新增至 WINS\-
- 將 WINS\-B 設定為 WINS\-A 的推送/提取複寫協力電腦
- 連接到 WINS\-B，然後將 WINS\-設定為 WINS\-B 的推送/提取複寫協力電腦
- 起始 WINS 的推送複寫\-A 到 WINS\-B
- 連接到 WINS\-B，以確認是否已成功複寫新的記錄（我的\_記錄）
...
- **伺服器**。 將目前的 WINS 命令\-行內容移至其名稱或 IP 位址所指定的伺服器。
