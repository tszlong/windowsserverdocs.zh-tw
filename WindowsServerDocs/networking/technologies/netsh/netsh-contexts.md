...
Netsh 會使用動態 @ no__t-0link library \(DLL @ no__t-2 檔案，與其他作業系統元件互動。 
...
若要顯示您可以在內容中使用的命令和子上下文清單，請在 netsh 提示字元中輸入內容名稱，然後輸入 **/？** 或 **[** 說明]。 例如，若要顯示您可以在路由內容中使用的子類別和命令清單，請在 netsh 提示字元 \(that 是， **netsh @ no__t-2**\)，請輸入下列其中一項：
...
- 文字後面接著省略號 \( ... \) 是可在命令列中重複多次的參數。
- 在括弧 [&nbsp;] 之間的文字是選擇性的專案。
- 介於大括弧 {&nbsp;} 之間的文字與以管道分隔的選項會提供一組您必須只選取一個的選項，例如 `{enable|disable}`。
...
Netsh 是命令列腳本公用程式，可讓您在本機或遠端顯示或修改目前執行中電腦的網路設定。 使用時不含參數， **netsh**會開啟 netsh 命令提示字元 \( 是， **netsh @ no__t-3**\)。

#### <a name="syntax"></a>語法

**netsh**\[ **-a**&nbsp;*AliasFile*\] \[ **-c**&nbsp;*內容*0 1 **-r**3*遠端電腦*5 6 **-u** 8 *DomainName @ no__t-20* 1 *UserName* 3 4 **-p**6*密碼*8 @ no__t-29 @ No__t-30 1 {*NetshCommand*3 **-f**5*ScriptFile*}7

#### <a name="parameters"></a>參數

**`-a`**

選擇性。 指定在執行*AliasFile*之後，您會回到**netsh**提示字元。

**`AliasFile`**

選擇性。 指定包含一或多個**netsh**命令之文字檔的名稱。

**`-c`**

選擇性。 指定 netsh 進入指定的**netsh**內容。

**`Context`**

選擇性。 指定您想要輸入的**netsh**內容。 
...
選擇性。 指定您想要在使用者帳戶下執行 netsh 命令。

***`DomainName\\`***

選擇性。 指定使用者帳戶所在的網域。 如果未指定*DomainName @ no__t-1* ，則預設值為本機網域。
...
選擇性。 指定以 **-u** *UserName*指定之使用者帳戶的密碼。

***`NetshCommand`***

選擇性。 指定您想要執行的**netsh**命令。

**`-f`**

選擇性。 執行您以*ScriptFile*指定的腳本後，結束**netsh** 。
...
選擇性。 在 netsh 提示字元中顯示說明。

> [!NOTE]
> 如果您指定後面接著另一個命令的 **`-r`** ， **netsh**會在遠端電腦上執行命令，然後返回 cmd.exe 命令提示字元。 如果您指定不含另一個命令的 **`-r`** ， **netsh**會在遠端模式中開啟。 此程式類似于在 Netsh 命令提示字元中使用**set machine** 。 當您使用 **`-r`** 時，只會為目前的**netsh**實例設定目的電腦。 在您結束並重新輸入**netsh**之後，目的電腦會重設為本機電腦。 您可以在遠端電腦上執行**netsh**命令，方法是指定存放在 WINS 中的電腦名稱稱、UNC 名稱、DNS 伺服器要解析的網際網路名稱，或 IP 位址。
