...
> [!TIP]
> 僅包含一個網路介面卡的 NIC 小組無法提供負載平衡和容錯移轉。 不過，若使用一個網路介面卡，當您同時使用虛擬區域網路絡 (Vlan) 時，您可以使用 NIC 小組來分隔網路流量。

當您將網路介面卡設定為 NIC 小組時，它們會連線到 NIC 小組解決方案通用核心，然後呈現一或多個虛擬配接器 (也稱為「小組 Nic」（tNICs）或「小組介面」) 到作業系統。

由於 Windows Server 2016 支援每個小組最多有32個小組介面，因此有各種不同的演算法可散發輸出流量， (在 Nic 之間載入) 。  下圖描述具有多個 tNICs 的 NIC 小組。
...
您可以使用已通過 Windows 硬體合格和標誌測試 (WHQL 測試的任何 Ethernet NIC，) Windows Server 2016 中的 NIC 小組。
...
-   Hyper-v 虛擬網路介面卡，這是在主機磁碟分割中公開為 Nic 的 Hyper-v 虛擬交換器埠。
    ...
    > [!IMPORTANT]
    > 請勿將在主機分割區中公開的 Hyper-v 虛擬 Nic 放 (Vnic) 在小組中。 任何設定都不支援主機分割區內的 Vnic 小組。 如果發生網路失敗，嘗試進行小組 Vnic 可能會導致通訊完全中斷。

-   核心 debug 網路介面卡 (KDNIC) 。

-   用於網路開機的 Nic。

-   使用乙太網路以外技術的 Nic （例如，WWAN、WLAN/Wi-fi、Bluetooth 和不等），包括透過未通過的網際網路通訊協定 (IPoIB) Nic。

## <a name="compatibility"></a>相容性
NIC 小組與 Windows Server 2016 中的所有網路技術相容，但有下列例外狀況。

-   **單一根目錄 i/o 虛擬化 (sr-iov) **。 針對 SR-IOV，資料會直接傳遞至 NIC，而不會透過網路堆疊 (在主機作業系統中傳遞，萬一虛擬化) 。 因此，NIC 小組不可能檢查或將資料重新導向至小組中的另一個路徑。
...
若要瞭解如何在虛擬機器中使用 NIC 小組 (在 Hyper-v 主機上執行的 Vm) ，請參閱[在主機電腦或 VM 上建立新的 NIC 小組](Create-a-New-NIC-Team-on-a-Host-Computer-or-VM.md)。

## <a name="virtual-machine-queues-vmqs"></a> (Vmq 數量的虛擬機器佇列) 
...
視交換器設定模式和負載分佈演算法而定，「NIC 小組」會以最小的可用和支援的佇列數來呈現團隊中的任何介面卡 ([最小佇列] 模式) 或所有小組成員 ([佇列總計] 模式) 中可用的佇列總數。

如果小組處於 [交換器獨立小組] 模式，而您將負載分佈設定為 [Hyper-v 埠模式] 或 [動態模式]，則報告的佇列數目是小組成員可用的所有佇列的總和 ([佇列總計] 模式) 。 否則，報告的佇列數目是小組中任何成員所支援的最小佇列數 ([最小佇列] 模式) 。

原因如下：

-   當交換器獨立的小組處於 Hyper-v 埠模式或動態模式時，Hyper-v 交換器埠的輸入流量 (VM) 一律會抵達相同的小組成員。 主機可以預測/控制哪一個成員接收特定 VM 的流量，因此，NIC 小組可以更仔細地瞭解哪些 VMQ 佇列要在特定小組成員上配置。 NIC 小組使用 Hyper-v 交換器時，會將虛擬機器的 VMQ 設定為精確的一個小組成員，並知道輸入流量會達到該佇列。

-   當小組處於任何切換相依模式 (靜態小組或 LACP 小組) 時，團隊所連線的參數會控制輸入流量分佈。 主機的 NIC 小組軟體無法預測哪個團隊成員取得 VM 的輸入流量，而且可能是交換器會將 VM 的流量分散到所有小組成員。 由於 NIC 小組軟體的結果，使用 Hyper-v 交換器時，會在每個小組成員（而不只是一個小組成員）上，為 VM 進行佇列的程式。

-   當小組處於切換獨立模式並使用位址雜湊負載平衡時，輸入流量一律會傳入一個 NIC (主要小組成員) -全部都在一個小組成員上。 由於其他小組成員不會處理輸入流量，因此會使用與主要成員相同的佇列來進行程式設計，因此，如果主要成員失敗，則任何其他小組成員都可以用來挑選輸入流量，而佇列也已準備就緒。
...
-   在理想的情況下，每個 NIC 的 * RssBaseProcNumber 都應該設為大於或等於兩個 (2) 的偶數數位。 第一個實體處理器（核心0） (邏輯處理器0和 1) ，通常會執行大部分的系統處理，因此網路處理應該會脫離此實體處理器。 某些機器架構沒有每個實體處理器有兩個邏輯處理器，因此，針對這類機器，基底處理器應該大於或等於1。 如果有疑問，請假設您的主機使用每個實體處理器架構2個邏輯處理器。

-   如果小組是在佇列的總和模式中，則小組成員的處理器應該不重迭。 例如，在4核心主機 (8 個邏輯處理器) 與2個 10Gbps Nic 的小組合作，您可以設定第一個使用基底處理器2，並使用4個核心;第二個設定為使用基本處理器6，並使用2個核心。

-   如果小組處於最小佇列模式，則小組成員所使用的處理器集必須完全相同。


## <a name="hyper-v-network-virtualization-hnv"></a>Hyper-V 網路虛擬化 (HNV)
NIC 小組與 Hyper-v 網路虛擬化完全相容 (HNV) 。  HNV 管理系統提供 NIC 小組驅動程式的資訊，可讓 NIC 小組以優化 HNV 流量的方式散發負載。

## <a name="live-migration"></a>即時移轉
Vm 中的 NIC 小組不會影響即時移轉。 無論是否在 VM 中設定 NIC 小組，都有相同的規則即時移轉。


## <a name="virtual-local-area-networks-vlans"></a> (Vlan) 的虛擬區域網路絡
當您使用 NIC 小組時，建立多個團隊介面可以讓主機同時連接到不同的 Vlan。
使用下列指導方針來設定您的環境：
...
-    如果 VM 有多個 SR-IOV 虛擬函式 (VFs) ，請先確定它們位於相同的 VLAN 上，然後再將它們小組在 VM 中。 您可以輕鬆地將不同的 VFs 設定在不同的 Vlan 上，這樣做會造成網路通訊問題。
...
每個 VM 都可以有虛擬函式 (VF 從一或兩個 SR-IOV Nic) ，而在 NIC 中斷連線時，從主要 VF 容錯移轉至備份介面卡 (VF) 。 或者，VM 可能會有一個 NIC 的 VF，以及連接到另一個虛擬交換器的非 VF vmNIC。 如果與 VF 關聯的 NIC 中斷連線，流量就可以容錯移轉至另一個交換器，而不會失去連線能力。
...
- 在[主機電腦或 VM 上建立新的 Nic 小組](Create-a-New-NIC-Team-on-a-Host-Computer-or-VM.md)：在本主題中，您會在主機電腦或 hyper-v 虛擬機器中建立新的 nic 小組， (VM) 執行 Windows Server 2016。
