---
title: 網路負載平衡
description: 在本主題中，我們提供您的網路負載平衡概觀\(NLB\) Windows Server 2016 中的功能。 您可以使用 NLB 來管理與單一虛擬叢集的兩個或多個伺服器。 NLB 可增強的可用性和延展性的網際網路伺服器應用程式，例如，用於 web、 FTP、 防火牆、 proxy、 虛擬私人網路\(VPN\)，和其他關鍵\-重要的伺服器。
manager: dougkim
...
NLB 特別適合用於確保無狀態應用程式，例如執行 Internet Information Services 網頁伺服器\(IIS\)，可用最短的停機時間，以及它們是可擴充的\(加上額外的伺服器，做為在負載增加時\)。 下列各節說明 NLB 如何支援執行這些應用程式的叢集伺服器之高可用性、延展性和管理性。  
  
### <a name="high-availability"></a>高可用性  
高可用性的系統能可靠地以最短的停機時間提供可接受的服務等級。 若要提供高可用性，NLB 包含內建\-功能可自動在：  
...
-   整個 NLB 叢集平衡負載的要求，個別的 tcp\/IP 服務。  
  
-   在單一叢集中最多支援 32 部電腦。  
  
-   平衡多個伺服器負載要求\(來自相同的用戶端，或從數個用戶端\)跨叢集中的多部主機。  
...
-   管理和設定使用 NLB 管理員的多個 NLB 叢集與叢集主機從單一電腦或[Windows PowerShell 中的網路負載平衡 (NLB) Cmdlet](https://technet.microsoft.com/library/hh801274.aspx)。
  
-   使用連接埠管理規則來指定單一 IP 連接埠或連接埠群組的負載平衡行為。  
  
-   為每個網站定義不同的連接埠規則。 如果您使用同一組負載\-平衡多個應用程式或網站、 連接埠規則的伺服器會根據目的地虛擬 IP 位址\(使用虛擬叢集\)。  

-   所有用戶端將要求導向到單一主機使用的選擇性、 單一\-裝載規則。 NLB 會將用戶端要求路由傳送到執行特定應用程式的特定主機。  

-   封鎖特定 IP 連接埠不需要的網路存取權。  

-   啟用網際網路群組管理通訊協定\(IGMP\)支援的叢集主機，以控制交換器連接埠流量\(連入網路封包傳送到交換器上的所有連接埠的何處\)中作業時多點傳送的模式。  
...
NLB 會安裝為標準的 Windows Server 網路驅動程式元件。 其作業而言是透明的 TCP\/IP 網路堆疊。 下圖顯示典型的組態中的 NLB 與其他軟體元件之間的關聯性。  
  
![網路負載平衡與其他軟體元件](../media/NLB/nlb.jpg)  
...
- 可讓用戶端使用單一邏輯網際網路名稱和虛擬 IP 位址，也就是叢集 IP 位址存取叢集\(保留為每部電腦的個別名稱\)。 NLB 允許多重主目錄伺服器使用多個虛擬 IP 位址。  
  
> [!NOTE]  
> 當您部署為虛擬叢集的 Vm 時，NLB 不需要有多個虛擬 IP 位址的多重主目錄伺服器。  
...
-   如果您使用的單點傳播模式，用來處理用戶端的網路介面卡\-要\-叢集流量必須支援變更其媒體存取控制\(MAC\)位址。  
  
## <a name="software-requirements"></a>軟體需求  
以下是執行 NLB 叢集的軟體需求。  
  
-   只有 TCP\/IP 可用的每個主機啟用 NLB 的介面卡上。 未新增任何其他通訊協定\(，例如 IPX\)給此配接器。  
...
您可以使用 nlb 的 伺服器管理員 或 Windows PowerShell 命令來安裝 NLB。

您也可以選擇性地安裝網路負載平衡工具來管理本機或遠端 NLB 叢集。 這些工具包括網路負載平衡管理員與 NLB 的 Windows PowerShell 命令。

### <a name="installation-with-server-manager"></a>使用 伺服器管理員安裝

在 [伺服器管理員] 中，您可以使用 [新增角色及功能精靈] 以新增**網路負載平衡**功能。 當您完成精靈時，已安裝 NLB，並不需要重新啟動電腦。


### <a name="installation-with-windows-powershell"></a>使用 Windows PowerShell 的安裝  

若要使用 Windows PowerShell 安裝 NLB，執行下列命令在提升權限的 Windows PowerShell 提示字元的電腦上您要安裝 NLB。
...
|內容類型|參考|  
|----------------|--------------|  
|部署|[網路負載平衡部署指南](https://technet.microsoft.com/library/cc754833(WS.10).aspx) &#124; [以終端機服務設定網路負載平衡](https://technet.microsoft.com/library/cc771300(v=WS.10).aspx)|  
|操作|[管理網路負載平衡叢集](https://technet.microsoft.com/library/cc753954(WS.10).aspx) &#124; [設定網路負載平衡參數](https://technet.microsoft.com/library/cc731619(WS.10).aspx) &#124; [控制網路負載平衡叢集上的主機](https://technet.microsoft.com/library/cc770870(WS.10).aspx)|  
|疑難排解|[疑難排解網路負載平衡叢集](https://technet.microsoft.com/library/cc732592(WS.10).aspx) &#124; [NLB 叢集事件與錯誤](https://technet.microsoft.com/library/cc731678(WS.10).aspx)|
|工具及設定|[網路負載平衡的 Windows PowerShell cmdlet](https://go.microsoft.com/fwlink/p/?LinkId=238123)|
