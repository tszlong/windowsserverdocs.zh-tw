...
ms.locfileid: "71405273"
---
# <a name="windows-powershell-commands-for-rss-and-vrss"></a>適用于 RSS 和 vRSS 的 Windows PowerShell 命令

>適用於：Windows Server (半年度管道)、Windows Server 2016

在本主題中，您將瞭解如何快速找出有關接收端調整的 Windows PowerShell 命令的技術參考資訊 \(RSS @ no__t-1 和虛擬 RSS \(vRSS @ no__t-3。

使用下列 RSS 命令，在具有多個處理器或多個核心的實體電腦上設定 RSS。 您可以使用相同的命令，在執行支援作業系統的虛擬機器上設定 vRSS \(VM @ no__t-1。 如需詳細資訊，請參閱[Windows PowerShell 中的網路介面卡 Cmdlet](https://docs.microsoft.com/powershell/module/netadapter/?view=win10-ps)。
...
使用下列 PowerShell 命令來設定原生主機上的 RSS，以及管理 VM 或主機虛擬 NIC （vNIC）上的 RSS。 這些命令的某些參數可能也會影響 Hyper-v 主機中的虛擬機器佇列 \(VMQ @ no__t-1。  
...
## <a name="enable-vrss-on-the-hyper-v-virtual-switch-port"></a>在超 no__t-0V 虛擬交換器埠上啟用 vRSS

除了在 VM 中啟用 RSS，vRSS 也會要求您在超 no__t-0V 虛擬交換器埠上啟用 vRSS。 
