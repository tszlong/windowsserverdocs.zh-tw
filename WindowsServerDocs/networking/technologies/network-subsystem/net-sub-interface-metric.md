...
這與舊版的 Windows 和 Windows Server 不同，它允許您使用使用者介面或命令**INetCfgComponentBindings：： MoveBefore** **來設定網路介面卡的系結順序。INetCfgComponentBindings：： MoveAfter**。 這兩種用於排序網路介面的方法，在 Windows Server 2016 和 Windows 10 中都無法使用。
...
當選擇網路流量路由，而且您已**設定 get-netipinterface**命令的**InterfaceMetric**參數時，用來判斷介面喜好設定的整體計量是路由計量的總和，以及介面度量。 一般而言，介面計量會將喜好設定提供給特定的介面，例如，如果有線和無線都可以使用，就可以使用有線。
