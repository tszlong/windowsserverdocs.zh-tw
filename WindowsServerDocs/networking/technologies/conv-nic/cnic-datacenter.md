...
在本主題中，我們將提供指示，說明如何使用交換器內嵌小組集合，在組合的 NIC 設定中部署聚合式 NIC \( \) 。
...
確定實體 NIC 可以連接到目的地主機。  這項測試會使用第3層 \( L3 \) 或 IP 層，以及第2層 \( L2 \) 虛擬區域網路絡 \( VLAN \) 來示範連線能力。
...
在此步驟中，Nic 處於**存取**模式。 不過，當您 \( 稍後在本指南中建立 Hyper-v 虛擬交換器 vSwitch 時 \) ，會在 vSwitch 埠層級套用 VLAN 屬性。
...
>[!TIP]
>根據美國電氣和電子公司工程師 \( IEEE \) 網路標準， \( 實體 NIC 中的服務 QoS 屬性品質， \) 會在 \( \) 您設定 VLAN ID 時，在 802.1 q VLAN 標頭中內嵌的 802.1 p 標頭上執行動作。
...
1. \( \) 在每個 hyper-v 主機上安裝資料中心橋接 DCB。

   - **選用**，適用于使用 iWarp 的網路設定。
   - **Required**對於使用 ROCE \( 任何 RDMA 服務版本的網路設定而言是必要的 \) 。
...
## <a name="step-5-verify-the-rdma-configuration-mode-1"></a>步驟 5。 確認 RDMA 設定 \( 模式1\)

您想要確保在建立 vSwitch 並轉換成 RDMA 模式2之前，已正確設定網狀架構 \( \) 。
...
3. 下載[DiskSpd.exe 公用程式](https://aka.ms/diskspd)，並將其解壓縮至 C:\TEST\.
...
5. 執行**Test-Rdma.ps1** PowerShell 腳本，將 ifIndex 值傳遞至腳本，以及相同 VLAN 上第一個遠端介面卡的 IP 位址。<p>在此範例中，腳本會在遠端網路介面卡 IP 位址192.168.1.5 上傳遞14的**ifIndex**值。
...
6. 執行**Test-Rdma.ps1** PowerShell 腳本，將 ifIndex 值傳遞至腳本，以及相同 VLAN 上第二個遠端介面卡的 IP 位址。<p>在此範例中，腳本會在遠端網路介面卡 IP 位址192.168.2.5 上傳遞**ifIndex**值13。
...
11. 測試遠端系統的連線，因為這兩個主機 Vnic 都位於相同的子網上，而且具有相同的 VLAN ID \( 102 \) 。
