...
確定實體 NIC 可以連接到目的地主機。  這項測試會使用第3層 \(L3\) 或 IP 層，以及第2層 \(L2\) 虛擬區域網路 \(VLAN\)來示範連線能力。
...
1. 查看第一個網路介面卡屬性。
   ...
   |------------|------------------------------------------|---------|--------|-------------------|-----------|
   | 測試-40G-1 | Mellanox ConnectX-3 Pro 乙太網路介面卡 |   11    |   上移   | E4-1D-2D-07-43-D0 |  40 Gbps  |
...
3. 查看第二張網路介面卡屬性。
   ...
   |    名稱    |          InterfaceDescription           | IfIndex | 狀態 |    MacAddress     | LinkSpeed |
   |------------|-----------------------------------------|---------|--------|-------------------|-----------|
   | 測試-40G-2 | Mellanox ConnectX-3 Pro Ethernet A ... #2 |   13    |   上移   | E4-1D-2D-07-40-70 |  40 Gbps  |
...
5. 確認其他 NIC 小組或設定成員 pNICs 具有有效的 IP 位址。<p>使用個別的子網，\(xxx.xxx。**2**. xxx 與 xxx.xxx。**1**. xxx\)，可協助從這個介面卡傳送到目的地。 否則，如果您在相同的子網上找到這兩個 pNICs，則 Windows TCP/IP 堆疊會在介面之間進行負載平衡，而簡單的驗證會變得更複雜。
...
>[!TIP]
>根據電氣和電子公司工程師 \(IEEE\) 網路標準，實體 NIC 中的服務品質 \(QoS\) 屬性會在您設定 VLAN 識別碼時，在 802.1 Q \(VLAN\) 標頭內內嵌的 802.1 p 標頭上採取行動。
...
3. 確定狀態為 [已**啟動**]。
   ...
   |    名稱    |          InterfaceDescription           | IfIndex | 狀態 |    MacAddress     | LinkSpeed |
   |------------|-----------------------------------------|---------|--------|-------------------|-----------|
   | 測試-40G-1 | Mellanox ConnectX-3 Pro Ethernet 的 Ada 。 |   11    |   上移   | E4-1D-2D-07-43-D0 |  40 Gbps  |
...
6. 確定狀態為 [已**啟動**]。
   ...
   |    名稱    |          InterfaceDescription           | IfIndex | 狀態 |    MacAddress     | LinkSpeed |
   |------------|-----------------------------------------|---------|--------|-------------------|-----------|
   | 測試-40G-2 | Mellanox ConnectX-3 Pro Ethernet 的 Ada 。 |   11    |   上移   | E4-1D-2D-07-43-D1 |  40 Gbps  |
...
您想要確定在建立 vSwitch 並轉換至 RDMA \(模式 2\)之前，已正確設定網狀架構。
...
3. 顯示主機 vNIC 的兩個視圖
   ...
   |        名稱         |        InterfaceDescription         | IfIndex | 狀態 |    MacAddress     | LinkSpeed |
   |---------------------|-------------------------------------|---------|--------|-------------------|-----------|
   | vEthernet （VMSTEST） | Hyper-v 虛擬乙太網路介面卡 #2 |   28    |   上移   | E4-1D-2D-07-40-71 |  80 Gbps  |
...
5. 查看其他 NIC 屬性。
   ...
   |      名稱       |        InterfaceDescription         | IfIndex | 狀態 |    MacAddress     | LinkSpeed |
   |-----------------|-------------------------------------|---------|--------|-------------------|-----------|
   | vEthernet （進行中） | Hyper-v 虛擬乙太網路介面卡 #2 |   28    |   上移   | E4-1D-2D-07-40-71 |  80 Gbps  |
...
3. 為介面 vEthernet 加入新的 IP 位址 \(SMB2\)。
...
11. 測試遠端系統的連線，因為這兩個主機 Vnic 都位於相同的子網上，而且 \(102\)的 VLAN 識別碼相同。
...
您想要將來自遠端系統的 RDMA 功能，驗證到 vSwitch 集小組成員的本機系統（具有 vSwitch）。<p>因為主機 Vnic \(的 SMB1 和 SMB2\) 都指派給 VLAN 102，所以您可以選取遠端系統上的 VLAN 102 介面卡。 <p>在此範例中，NIC 測試-40G-2 會執行 RDMA 至 SMB1 （192.168.2.111）和 SMB2 （192.168.2.222）。
...
在此輸出中的最後一行「RDMA 流量測試成功： RDMA 流量已傳送至192.168.2.5」，顯示您已成功在介面卡上設定聚合式 NIC。
