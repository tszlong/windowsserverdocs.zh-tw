...
網路虛擬化提供類似的功能，在相同的實體網路基礎結構上執行多個虛擬網路（可能會有重迭的 IP 位址），而每個虛擬網路的運作方式，就像是唯一在上執行的虛擬網路一樣。共用的網路基礎結構。 圖 1 顯示此關係。  
...
-   **HNVv1**  
    ...
    HNVv1 與 Windows Server 2012 R2 和 System Center 2012 R2 Virtual Machine Manager （VMM）相容。 HNVv1 的設定會依賴 WMI 管理和 Windows PowerShell Cmdlet （透過 System Center VMM 提供）來定義隔離設定和客戶位址（CA）-虛擬網路對實體位址（PA）對應和路由。 Windows Server 2016 中的 HNVv1 沒有新增任何功能，而且沒有規劃任何新功能。  

    *   設定小組和 HNV V1 與平臺不相容。

    o 若要使用 HA NVGRE 閘道，使用者必須使用 LBFO 小組或不是小組。 或
...
-   **HNVv2**  
    ...
    HNVv2 中包含大量的新功能，這是使用 Hyper-v 交換器中的 Azure 虛擬篩選平臺（VFP）轉送延伸模組來執行。 HNVv2 已與 Microsoft Azure Stack 完全整合，其中包含軟體定義網路（SDN）堆疊中的新網路控制站。  虛擬網路原則是透過 Microsoft[網路控制](../../../sdn/technologies/network-controller/Network-Controller.md)站，使用 RESTful NORTHBOUND （NB） API 來定義，並經由多個 SouthBound INTEFACES （為）（包括 OVSDB）向主機代理程式進行檢測。 主機代理程式會在其強制執行所在的 Hyper-v 交換器的 VFP 延伸模組中套用原則。  
...
在 [圖 2] 中，VSID 5001 中的虛擬機器可以透過 Hyper-v 交換器將其封包轉送到也在 VSID 5001 中的虛擬機器。 來自 VSID 5001 中虛擬機器的連入封包，會傳送至 Hyper-v 交換器上的特定 VPort。 這些封包的 Hyper-v 交換器會套用輸入規則（例如 encap）和對應（例如封裝標頭）。 封包接著會轉送到 Hyper-v 交換器上的不同 VPort （如果目的地虛擬機器已連接到相同的主機）或不同主機上的不同 Hyper-v 交換器（如果目的地虛擬機器位於不同的主機上）。  
...
每個虛擬子網路都會定義第 3 層 IP 子網路，而第 2 層 (L2) 廣播網域界限會與 VLAN 類似。 當虛擬機器廣播封包時，HNV 會使用單播複寫（UR）來建立原始封包的複本，並將目的地 IP 和 MAC 取代為相同 VSID 中每個 VM 的位址。  

> [!NOTE]  
> 當 Windows Server 2016 發行時，將會使用單播複寫來執行廣播和子網多播。 不支援跨子網多播路由和 IGMP。  

除了做為廣播網域之外，VSID 還提供隔離功能。 HNV 中的虛擬網路介面卡會連線至 Hyper-v 交換器埠，而此埠會將 ACL 規則直接套用至埠（virtualNetworkInterface REST 資源）或其所屬的虛擬子網（VSID）。  

Hyper-v 交換器埠必須套用 ACL 規則。 此 ACL 可能會允許全部、全部拒絕或更明確，只允許以5個元組（來源 IP、目的地 IP、來源埠、目的地埠、通訊協定）比對為基礎的特定類型流量。  

> [!NOTE]  
> Hyper-v 交換器擴充功能無法與新的軟體定義網路（SDN）堆疊中的 HNVv2 搭配使用。 HNVv2 是使用 Azure 虛擬篩選平臺（VFP）交換器擴充功能來執行，無法與其他任何協力廠商交換器擴充功能搭配使用。  

## <a name="switching-and-routing-in-hyper-v-network-virtualization"></a>Hyper-v 網路虛擬化中的切換和路由  
HNVv2 會執行正確的第2層（L2）切換和第3層（L3）路由語義，使其如同實體交換器或路由器可正常作用。 當連線到 HNV 虛擬網路的虛擬機器嘗試與相同虛擬子網（VSID）中的另一部虛擬機器建立連接時，必須先瞭解遠端虛擬機器的 CA MAC 位址。 如果來源虛擬機器的 ARP 資料表中有目的地虛擬機器 IP 位址的 ARP 專案，則會使用此專案中的 MAC 位址。 如果專案不存在，則來源虛擬機器將會傳送 ARP 廣播，其中包含對應至目的地虛擬機器 IP 位址的 MAC 位址要求。 Hyper-v 交換器會攔截此要求，並將它傳送至主機代理程式。 主機代理程式會在其本機資料庫中，尋找所要求目的地虛擬機器 IP 位址的對應 MAC 位址。  
...
如果連線到 HNV 虛擬網路的虛擬機器想要建立與不同虛擬子網（VSID）中虛擬機器的連接，則必須據以路由封包。 HNV 會假設在 CA 空間中只有一個 IP 位址，用來做為下一個躍點來連線到所有 IP 首碼（也就是說，一個預設路由/閘道）。 目前，這會強制執行單一預設路由的限制，而非預設路由則不受支援。  

### <a name="routing-between-virtual-subnets"></a>在虛擬子網路間路由  
在實體網路中，IP 子網是第2層（L2）網域，其中的電腦（虛擬和實體）可以直接彼此通訊。 L2 網域是一個廣播網域，其中 ARP 專案（IP： MAC 位址對應）是透過在所有介面上廣播的 ARP 要求來學習，而 ARP 回應則會傳回給要求的主機。 電腦會使用從 ARP 回應中學習到的 MAC 資訊來完整地構造 L2 框架，包括 Ethernet 標頭。 不過，如果 IP 位址位於不同的 L3 子網中，則 ARP 要求不會跨越此 L3 界限。 相反地，具有來源子網中 IP 位址的 L3 路由器介面（下一個躍點或預設閘道）必須使用自己的 MAC 位址來回應這些 ARP 要求。  

在標準 Windows 網路中，系統管理員可以建立靜態路由，並將它們指派給網路介面。 此外，「預設閘道」通常會設定為介面上的下一個躍點 IP 位址，而該介面上會傳送以預設路由（0.0.0.0/0）為目的地的封包。 如果沒有特定的路由存在，封包就會傳送到這個預設閘道。 這通常是實體網路的路由器。  HNV 使用屬於每部主機的內建路由器，並在每個 VSID 中具有介面，以建立虛擬網路的分散式路由器。  
...
相較于為每個虛擬子網（VSID）配置一個 PA IP 位址的 HNVv1，HNVv2 現在會針對每個交換器使用一個 PA IP 位址-內嵌小組（SET） NIC 團隊成員。 預設部署假設有兩個 NIC 的小組，並為每個主機指派兩個 PA IP 位址。 單一主機具有從相同 VLAN 上的相同提供者（PA）邏輯子網指派的 PA Ip。 相同虛擬子網中的兩個租使用者 Vm 可能確實位於連線到兩個不同提供者邏輯子網的兩部不同主機上。 HNV 會根據 CA-PA 對應來為封裝的封包，建立外部 IP 標頭。 不過，它會依賴主機 TCP/IP 堆疊到預設 PA 閘道的 ARP，然後根據 ARP 回應來建立外部乙太網路標頭。 一般來說，此 ARP 回應是來自實體交換器或主機連線的 L3 路由器上的 SVI 介面。 因此，HNV 會依賴 L3 路由器來路由提供者邏輯子網/Vlan 之間封裝的封包。  

### <a name="routing-outside-a-virtual-network"></a>在虛擬網路外路由  
多數的客戶部署將需要從 HNV 環境與不屬於 HNV 環境的資源通訊。 這樣必須要有網路虛擬化閘道，才允許在這兩種環境之間通訊。 需要 HNV 閘道的基礎結構包括私用雲端和混合式雲端。 基本上，在內部和外部（實體）網路（包括 NAT）之間，或在使用 IPSec VPN 或 GRE 通道的不同網站與/或雲端（私人或公用）之間，第3層路由需要 HNV 閘道。  

閘道可以有不同的實際規格。 它們可以建置於 Windows Server 2016 上，併入作為 VXLAN 閘道的頂尖機架（TOR）交換器中，透過負載平衡器所通告的虛擬 IP （VIP）來存取、放入其他現有的網路設備，或者可以是新的獨立網路應用裝置.  
...
-   客戶**位址**（CA）客戶根據其內部網路基礎結構指派的 IP 位址。 此位址可讓客戶與虛擬機器切換式網路流量，就像它尚未移到公用或私用雲端一樣。 虛擬機器可以看見 CA，而客戶也可以連線至該 CA。  
...
HNV 會使用網路虛擬化一般路由封裝（NVGRE）或虛擬可擴充區域網路（VXLAN）來執行重迭租使用者網路。  VXLAN 是預設值。  

### <a name="virtual-extensible-local-area-network-vxlan"></a>虛擬可擴充區域網路（VXLAN）  
虛擬擴充區域網路（VXLAN）（[RFC 7348](https://www.rfc-editor.org/info/rfc7348)）通訊協定已在市場上廣泛採用，並提供來自 Cisco、織錦、Arista、DELL、HP 等廠商的支援。 VXLAN 通訊協定會使用 UDP 做為傳輸。 適用于 VXLAN 的 IANA 指派 UDP 目的地埠是4789，而 UDP 來源埠應該是要用於 ECMP 散佈之內部封包中的資訊雜湊。 在 UDP 標頭之後，會將 VXLAN 標頭附加至封包，其中包含保留的4位元組欄位，後面接著 VXLAN 網路識別碼（VNI）-VSID 的3位元組欄位，後面接著另一個保留的1位元組欄位。 在 VXLAN 標頭之後，會附加原始 CA L2 框架（不含 CA 乙太網路框架）。  
...
此網路虛擬化機制會使用一般路由封裝（NVGRE）做為通道標頭的一部分。 在 NVGRE 中，虛擬機器的封包會封裝在另一個封包內。 這個新封包的標頭除了虛擬子網路識別碼之外，還擁有適當的來源與目的地 PA IP 位址，虛擬子網路識別碼儲存於 GRE 標頭的索引鍵欄位中，如圖 7 所示。  
...
我們會假設主機服務提供者先前已透過網路控制站建立提供者（PA）邏輯網路，以對應至其實體網路拓撲。 網路控制站會從邏輯子網的 IP 首碼（主機連線的位置）配置兩個 PA IP 位址。 網路控制卡也會指出適當的 VLAN 標記以套用 IP 位址。  

使用網路控制站，Contoso Corp 和 Fabrikam Corp 會建立其虛擬網路和子網，並由主機服務提供者指定的提供者（PA）邏輯網路所支援。 Contoso Corp 和 Fabrikam Corp 都各自將 SQL Server 和網頁伺服器移到相同裝載提供者的共用 IaaS 服務，且剛好都在 Hyper-V 主機 1 上執行 **SQL** 虛擬機器，以及在 Hyper-V 主機 2 上執行 **Web** (IIS7) 虛擬機器。 所有虛擬機器都會保留原始的內部網路 IP 位址 (它們的 CA)。  
...
當 Hyper-v 主機2上的 Contoso Corp Web 虛擬機器（10.1.1.12）在10.1.1.11 建立與 SQL Server 的 TCP 連線時，將會發生下列情況：  
...
-   VSwitch 中的 VFP 轉送延伸模組會透過 VFP 層（如下所述）來處理此封包，並將其指派給接收封包的來源 vSwitch 埠，並在 VFP 整合流程資料表中建立新的流程專案。  

-   VFP 引擎會根據 IP 和乙太網路標頭，針對每個層級（例如虛擬網路層）執行規則比對或流程資料表查閱。  
...
-   在 VXLAN 封裝的情況下，會在 VXLAN 標頭中以5001的 VSID 來建立外部 UDP 標頭。  
    外部 IP 標頭會使用根據 SDN 主機代理程式的原則存放區，分別指派給 Hyper-v 主機2（192.168.2.20）和 Hyper-v 主機1（192.168.1.10）的來源和目的地 PA 位址進行結構化。  
...
在 Windows Server 2016 中，HNVv2 是使用 Azure 虛擬篩選平臺（VFP）來執行，這是 Hyper-v 交換器內的 NDIS 篩選延伸模組。 VFP 的重要概念是，符合動作流程引擎與內部 API 公開給 SDN 主機代理程式，以進行網路原則的設計。 SDN 主機代理程式本身會透過 OVSDB 和 WCF SouthBound 通道，從網路控制站接收網路原則。 虛擬網路原則（例如 CA-PA 對應）不只是使用 VFP 進行設計，而是以其他原則（例如 Acl、QoS 等等）做為架構。  
...
在 VFP 中，會根據原則類型（例如虛擬網路）建立圖層，而且是一組一般的規則/流程資料表。 在特定規則指派給該層來執行這類功能之前，它不會有任何內建函式。 系統會為每個圖層指派優先順序，而層會依遞增優先順序指派給埠。 規則會組織成主要根據方向和 IP 位址系列的群組。 群組也會被指派優先順序，而且最多可以有一個來自群組的規則符合指定的流程。  
...
VFP 延伸模組具有慢速路徑和快速路徑來進行封包傳送。 流程中的第一個封包必須跨越每一層中的所有規則群組，並執行規則查詢，這是一項昂貴的作業。 不過，一旦使用動作清單（根據符合的規則）在整合的流程資料表中註冊流程之後，所有後續封包都會根據統一的流程資料表專案進行處理。  
