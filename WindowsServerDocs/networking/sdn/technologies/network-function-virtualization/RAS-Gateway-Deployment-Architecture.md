...
您可以使用本主題來瞭解 (CSP) 部署 RAS 閘道的雲端服務提供者，包括 RAS 閘道集區、路由反映程式，以及為個別租使用者部署多個閘道。
...
-   [ (CA 新增租使用者和客戶位址) 空間 EBGP 對等互連](#bkmk_tenant)
...
邊界閘道協定的 (BGP) 路由反映程式功能現在隨附于 RAS 閘道，並提供 BGP 完整網狀拓朴的替代方案，通常是在路由器之間路由同步處理的必要項。 透過完整網狀同步處理，所有 BGP 路由器都必須與路由拓撲中的所有其他路由器連接。 不過，當您使用路由反射器時，路由反映是唯一與所有其他路由器（稱為 BGP 路由反映程式用戶端）連接的路由器，因此可簡化路由同步處理並減少網路流量。 路由反映會學習所有路由、計算最佳路由，並將最佳路由轉散發至其 BGP 用戶端。
...
網路控制站會將 Hyper-v 網路虛擬化原則推送至 Contoso 和 Woodgrove 虛擬網路，以及 ras 原則至 RAS 閘道，並將原則負載平衡至設定為軟體負載平衡集區的 Multiplexers ( (Mux) 。

## <a name="adding-new-tenants-and-customer-address-ca-space-ebgp-peering"></a><a name="bkmk_tenant"></a> (CA 新增租使用者和客戶位址) 空間 eBGP 對等互連
...
同樣地，使用 Hyper-v 網路虛擬化原則時，租使用者虛擬網路會將封包直接路由傳送至 RAS 閘道 Vm (，而不需要知道路由反射器) 然後透過站對站 VPN 通道的企業網站。

此外。 將來自租使用者虛擬網路的流量傳回遠端租使用者企業網站會略過 SLBs，這是一種稱為「直接伺服器回傳」 (DSR) 的程式。
...
由於您可以視需要在 RAS 閘道集區中新增任意數目的 RAS 閘道 Vm，因此您可以輕鬆地調整您的 RAS 閘道部署，以優化效能和容量。 當您將 Vm 新增至集區時，您可以使用任何種類的站對站 VPN 連線來設定這些 RAS 閘道， (IKEv2、L3、GRE) 、排除無停機時間的容量瓶頸。
