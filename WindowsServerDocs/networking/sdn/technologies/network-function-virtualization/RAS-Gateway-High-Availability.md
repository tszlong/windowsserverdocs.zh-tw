---
title: RAS 閘道高可用性
description: 您可以使用本主題來瞭解適用于軟體定義網路之 RAS 多租使用者閘道的高可用性設定， (Windows Server 2016 中的 SDN) 。
manager: grcusanz
...
ms.locfileid: "87969555"
---
# <a name="ras-gateway-high-availability"></a>RAS 閘道高可用性

>適用於：Windows Server (半年度管道)、Windows Server 2016

您可以使用本主題來瞭解適用于軟體定義網路之 RAS 多租使用者閘道的高可用性設定 (SDN) 。
...
當您部署多個可提供高可用性和容錯移轉的 RAS 閘道 Vm 實例時，您會部署閘道集區。 在 Windows Server 2012 R2 中，所有閘道 Vm 形成一個單一集區，讓閘道部署的邏輯區隔有點困難。  Windows Server 2012 R2 閘道為閘道 Vm 提供1:1 個冗余部署，導致無法使用站對站 (S2S) VPN 連線的可用容量。
...
-   集區可以執行任何個別的閘道功能，網際網路金鑰交換版本 2 (IKEv2) S2S、第3層 (L3) 和一般路由封裝 (GRE) ，或集區可以執行所有這些功能。
...
-   如果您是透過合作夥伴 (轉銷商) 頻道來銷售雲端服務，您可以為每個轉售商建立一組個別的集區。
...
下圖說明 (CSP) 部署 RAS 閘道的典型雲端服務提供者。

![RAS 閘道部署總覽](../../../media/RAS-Gateway-High-Availability/ras_csp_deploy.png)

使用這種類型的部署時，閘道集區會部署在軟體 Load Balancer (SLB) 後面，讓 CSP 能夠為整個部署指派單一公用 IP 位址。 租使用者的多個閘道連線可在多個閘道集區上終止，也可以在集區中的多個閘道上結束 這會透過上圖中的 IKEv2 S2S 連線加以說明，但同樣也適用于其他閘道功能，例如 L3 和 GRE 閘道。

在此圖中，MT BGP 裝置是具有 BGP 的 RAS 多租使用者閘道。 多租使用者 BGP 會用於動態路由。 租使用者的路由是集中式-單一點，稱為路由反射器 (RR) 會處理所有租使用者網站的 BGP 對等互連。 RR 本身會分散到集區中的所有閘道。 這會產生一個設定，讓租使用者的連線 (資料路徑) 在多個閘道上終止，但是租使用者 (BGP 對等點控制路徑) 的 RR 只會在其中一個閘道上。
...
-   [ (IKEv2、L3 和 GRE 的閘道連線布建和負載平衡) ](#bkmk_provisioning)
...
### <a name="provisioning-and-load-balancing-of-gateway-connections-ikev2-l3-and-gre"></a><a name="bkmk_provisioning"></a> (IKEv2、L3 和 GRE 的閘道連線布建和負載平衡) 
...
在網路控制卡選取連線的 RAS 閘道之後，網路控制卡會為閘道上的連線布建必要的設定。 如果連線是 IKEv2 S2S 連線，網路控制卡也會在 SLB 集區上布建網路位址轉譯 (NAT) 規則;SLB 集區上的此 NAT 規則會將來自租使用者的連接要求導向至指定的閘道。 租使用者是由來源 IP 來區分，這應該是唯一的。

> [!NOTE]
> L3 和 GRE 連線會略過 SLB，並直接與指定的 RAS 閘道連接。  這些連線要求遠端端點路由器 (或其他協力廠商裝置) 必須正確設定，才能與 RAS 閘道連接。

如果已針對連線啟用 BGP 路由，則會由 RAS 閘道起始 BGP 對等互連，而路由會在內部部署與雲端閘道之間交換。 BGP 所瞭解的路由 (或靜態設定的路由（如果未使用 BGP) 會傳送至網路控制站）。 然後，網路控制站會向下連接至安裝租使用者 Vm 的 Hyper-v 主機。 此時，可以將租使用者流量路由傳送至正確的內部部署網站。 網路控制站也會建立相關聯的 Hyper-v 網路虛擬化原則，以指定閘道位置，並向下連接至 Hyper-v 主機。
...
網路控制站的 RAS 閘道容錯移轉回應的程式（包括失敗偵測、將連線和路由設定複製到待命閘道）、受影響連線的 BGP/靜態路由的容錯移轉 (包括在計算) 主機上的路由提款和重新設定，以及計算主機上的 Hyper-v 網路虛擬化原則-對於 GRE 閘道和連線而言是相同的。 不過，重新建立 GRE 連線的方式會有所不同，但 GRE 的高可用性解決方案則有一些額外的需求。

![GRE 的高可用性](../../../media/RAS-Gateway-High-Availability/ras_ha.png)

在閘道部署時，會將動態 IP 位址指派給每個 RAS 閘道 VM (DIP) 。 此外，也會為每個閘道 VM 指派一個虛擬 IP 位址， (在 GRE 高可用性的 VIP) 。 Vip 只會指派給可接受 GRE 連線的集區中的閘道，而非 GRE 集區。 指派的 Vip 會公告至機架的頂端 (TOR 使用 BGP 的) 交換器，然後進一步將 Vip 通告到雲端實體網路。 這讓閘道可以從 GRE 連線另一端所在的遠端路由器或協力廠商裝置進行連線。 此 BGP 對等互連與租使用者路由交換的租使用者層級 BGP 對等不同。
...
當新的租使用者建立新的 L3 閘道時，網路控制卡閘道 Service Manager 會選取可用的閘道 VM，並使用高可用性客戶位址設定新的租使用者介面， (CA) 空間 IP 位址 (來自租使用者的 VLAN 標記邏輯網路) 。 IP 位址會用來做為遠端 (實體網路) 閘道上的對等 IP 位址，而是下一個躍點以連線到租使用者的 Hyper-v 網路虛擬化網路。
