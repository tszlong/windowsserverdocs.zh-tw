...
下列清單代表在 Windows Server 2012 R2 中，從市場生產環境部署的 Hyper-v 網路虛擬化（HNVv1）最常看到的問題類別，並以許多方式與 Windows Server 2016 HNVv2 中出現的相同問題類型搭配新的軟體定義網路（SDN）堆疊。

大部分的錯誤都可以分類成一小部分的類別：
* **無效或不支援的**設定使用者未正確地叫用 NorthBound API，或使用了不正確原則。

* **原則應用程式發生錯誤**來自網路控制站的原則未傳遞至 Hyper-v 主機，在所有 Hyper-v 主機上明顯延遲及/或不是最新狀態（例如，在即時移轉之後）。
* 設定**漂移或軟體錯誤**導致丟棄封包的資料路徑問題。

* 與**NIC 硬體/驅動程式或 underlay 網路網狀架構相關的外部錯誤**不正常的工作卸載（例如 VMQ）或 underlay 的網路網狀架構設定錯誤（例如 MTU）
...
> 系統中有第二個錯誤，閘道 HNV 提供者 VM Nic 的網路介面資源處於失敗狀態，錯誤為「虛擬交換器-PortBlocked」。 如果 VM NIC 資源中的 IP 設定設為 null （依設計），也可以放心地忽略此錯誤。
...
請檢查 hyper-v 主機（NC 主機代理程式服務）與網路控制站節點之間（SouthBound）通訊所使用的 x.509 憑證指紋（主機名稱將是證書的主體名稱）是否相同。 此外，請檢查網路控制站的 REST 憑證是否具有 [ *CN = <FQDN or IP> *] 的 [主體名稱]。
...
除此之外，從我們到目前為止所見到的問題（特別是在以 SDNExpress 為基礎的部署），在 GW Vm 上不會設定租使用者區間的最常見原因，就是與使用者嘗試指派給 TenantConfig.psd1 中的網路連線（S2S 通道）的使用者而言，FabricConfig.psd1 中的 GW 容量較少。 這可以藉由比較下列命令的輸出來輕鬆檢查：
...
- **移至本機記錄**。  如果您已提供集中位置來儲存記錄檔，您可以使用參數回到網路控制卡節點上的本機登入 ``UseLocalLogLocation`` （由於磁碟空間需求很大，因此不建議這麼做）。
...
2.  在其中一個網路控制站節點 Vm （最好是主要網路控制站節點-NetworkControllerReplica）上驗證 PerfMon 中的*SDNSLBMPerfCounters* ：
    ...
    4.  SLBM 是否已連線至 Mux？ （*Mux 已連線*  == *Mux 在 SLBM*  ==  上狀況良好*Mux 報告狀況良好*= # SLB mux vm）。
