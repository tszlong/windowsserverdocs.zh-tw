...
大部分的錯誤都可以分類成一小部分的類別：
* **無效或不支援的**設定使用者未正確地叫用 NorthBound API，或使用了不正確原則。

* **原則應用程式發生錯誤**來自網路控制站的原則未傳遞至 Hyper-v 主機，在所有 Hyper-v 主機上明顯延遲及/或不是最新狀態 (例如，在即時移轉) 之後。
* 設定**漂移或軟體錯誤**導致丟棄封包的資料路徑問題。

* 與**NIC 硬體/驅動程式或 underlay 網路網狀架構相關的外部錯誤**錯誤的工作卸載 (例如 VMQ) 或 underlay 的網路網狀架構設定不正確 (例如 MTU) 
...
若要使用網路控制器 (控制路徑) 診斷工具，您必須先安裝 RSAT NetworkController 功能，並匯入 ``NetworkControllerDiagnostics`` 模組：
...
若要使用 HNV Diagnostics (資料路徑) 診斷工具，您必須匯入 ``HNVDiagnostics`` 模組：
...
在數個網路控制站資源中，有一個名為「設定_狀態_」的內嵌資源。 設定狀態提供系統健康情況的相關資訊，包括網路控制站的設定與 Hyper-v 主機上執行) 狀態的實際 (之間的一致性。
...
>
>只有在 VMM 部署) 中的網路控制卡使用 Kerberos 驗證 (時，才需要指定*Credential*參數。 認證必須是「網路控制站管理」安全性群組中的使用者。
...
>[!NOTE]
> 系統中有一個錯誤，SLB Mux 傳輸 VM NIC 的網路介面資源處於失敗狀態，錯誤為「虛擬交換器-主機未連線到控制器」。 如果 VM NIC 資源中的 IP 設定是從傳輸邏輯網路的 IP 集區設為 IP 位址，則可以放心地忽略此錯誤。
> 系統中有第二個錯誤，閘道 HNV 提供者 VM Nic 的網路介面資源處於失敗狀態，錯誤為「虛擬交換器-PortBlocked」。 如果將 VM NIC 資源中的 IP 設定設為 null (依設計) ，也可以放心地忽略此錯誤。
...
| PolicyConfigurationFailure | 因為憑證錯誤或連線錯誤，所以無法推送 VmNic 的 VNet 原則 | 檢查是否已部署適當的憑證 (憑證主體名稱必須符合主機) 的 FQDN。 另請確認主機與網路控制站的連線能力 |
| PolicyConfigurationFailure | 因為憑證錯誤或連線錯誤，所以無法推送 VmNic 的 vSwitch 原則 | 檢查是否已部署適當的憑證 (憑證主體名稱必須符合主機) 的 FQDN。 另請確認主機與網路控制站的連線能力 |
| PolicyConfigurationFailure | 因為憑證錯誤或連線錯誤，所以無法針對 VmNic 推送防火牆原則 | 檢查是否已部署適當的憑證 (憑證主體名稱必須符合主機) 的 FQDN。 另請確認主機與網路控制站的連線能力 |
...
| HostUnreachable | MUX 狀況不良 (常見的案例是 BGPRouter 中斷連線)  | RRAS (BGP 虛擬機器上的 BGP 對等體) 或機架 (ToR) 交換器無法連線或未成功。 檢查軟體 Load Balancer 多工器資源和 BGP 對等 (ToR 或 RRAS 虛擬機器上的 BGP 設定)  |
| HostNotConnectedToController | SLB 主機代理程式未連線 | 檢查 SLB 主機代理程式服務是否正在執行;請參閱 SLB 主機代理程式記錄 (自動執行) 的原因，例如，當 SLBM (NC) 拒絕主機代理程式執行狀態所出示的憑證時，將會顯示差別細微資訊 |
...
執行下面的*netstat*命令，以驗證 NC 主機代理程式與網路控制站節點之間是否有三個已建立的連線 (s) 以及 hyper-v 主機上的一個接聽通訊端
- 接聽埠 TCP： Hyper-v 主機 (NC 主機代理程式服務) 上的6640
- 從埠6640上的 Hyper-v 主機 IP 連線到暫時埠上的 NC 節點 IP ( # A0 32000) 的兩個建立連接
...
*補救*如果使用 SDNExpress 腳本或手動部署，請更新登錄中的 HostId 金鑰，以符合伺服器資源的實例識別碼。 重新開機 Hyper-v 主機上的網路控制站主機代理程式 (實體伺服器) 如果使用 VMM，請從 VMM 刪除 Hyper-v 伺服器，並移除 [HostId] 登錄機碼。 然後，透過 VMM 重新新增伺服器。

請檢查 Hyper-v 主機 (主機名稱所使用的 x.509 憑證指紋，是否為 (SouthBound) 在 Hyper-v 主機 (NC 主機代理程式服務) 和網路控制站節點之間進行通訊的憑證的主體名稱) 。 此外，請檢查網路控制站的 REST 憑證是否具有 [ *CN = <FQDN or IP> *] 的 [主體名稱]。
...
您也可以檢查每個憑證的下列參數，確定主體名稱是預期的 (主機名稱或 NC REST FQDN 或 IP) 、憑證尚未到期，以及憑證鏈中的所有憑證授權單位單位都包含在受信任的根授權單位中。
...
SLB 設定狀態可以判斷為 NetworkController Cmdlet 輸出的一部分。 此 Cmdlet 也會輸出 JSON 檔案中的目前網路控制站資源集、來自每個 Hyper-v 主機的所有 IP 設定 (server) 和來自主機代理程式資料庫資料表的區域網路原則。
...
>[!NOTE]
>預設輸出位置會是 <working_directory> \NCDiagnostics\ 目錄。 您可以使用參數來變更預設輸出目錄 `-OutputDirectory` 。

SLB 設定狀態資訊可以在此目錄中的檔案_diagnostics-slbstateResults.Js_中找到。
...
除此之外，從我們到目前為止所見到的問題 (特別是針對以 SDNExpress 為基礎的部署) ，在 GW Vm 上不會設定租使用者區間的最常見原因，就是 FabricConfig.psd1 中的 GW 容量相較于使用者嘗試指派給) 1 中的 S2S 通道 TenantConfig.psd的 (網路連線。 這可以藉由比較下列命令的輸出來輕鬆檢查：
...
在 Hyper-v 主機上執行的第一個來賓 VM 已連線到租使用者虛擬網路之後，網路控制站將會指派兩個 HNV 提供者 IP 位址 (PA IP 位址) 至 Hyper-v 主機。 這些 Ip 會來自 HNV 提供者邏輯網路的 IP 集區，並由網路控制卡管理。  若要瞭解這兩個 HNV 的 IP 位址是什麼
...
這些 HNV 提供者 IP 位址 (PA Ip) 會指派給在個別 TCPIP 網路區間中建立的 Ethernet 介面卡，且介面卡名稱為_VLANX_ ，其中 X 是指派給 HNV 提供者 (傳輸) 邏輯網路的 VLAN。

使用 HNV 提供者邏輯網路的兩部 Hyper-v 主機之間的連線，可以透過具有額外區間 (-c Y) 參數的 ping 來完成，其中 Y 是 PAhostVNICs 建立所在的 TCPIP 網路區間。 這個區間可以藉由執行下列方式來判斷：
...
>[!NOTE]
> PA 主機 vNIC 介面卡不會在資料路徑中使用，因此不會有指派給 "vEthernet (PAhostVNic) adapter" 的 IP。

比方說，假設 Hyper-v 主機1和2具有 HNV 提供者 (PA) 的 IP 位址：
...
*補救*
* 將實體交換器埠上的 MTU 大小調整為至少 1674B (包括 14B Ethernet 標頭和尾端) 
* 如果您的 NIC 卡不支援 EncapOverhead 關鍵字，請將 JumboPacket 關鍵字調整為至少1674B

#### <a name="check-tenant-vm-nic-connectivity"></a>檢查租使用者 VM NIC 連線能力
指派給來賓 VM 的每個 VM NIC 在私人客戶位址 (CA) 和 HNV 提供者位址 (PA) 空間之間，都有一個 CA PA 對應。 這些對應會保留在每部 Hyper-v 主機上的 OVSDB 伺服器資料表中，而且可以藉由執行下列 Cmdlet 來找到。
...
在「綠色 Web VM 1」與主機 "sa18n30-2.sa18.nttest.microsoft.com" 上192.168.1.4 的 SenderCA IP （具有10.127.132.153 的管理 IP）中建立 CA-ping，並將 ListenerCA 的 IP 為 192.168.1.5) 4114 的虛擬子網 (
...
1. 出租檢查虛擬子網或 VM 網路介面上未指定任何分散式防火牆原則，這會封鎖流量。
...
1. Hoster.config在 ``Get-ProviderAddress`` 裝載有兩個租使用者虛擬機器的 hyper-v 主機上執行，然後 ``Test-LogicalNetworkConnection`` ``ping -c <compartment>`` 從 hyper-v 主機執行或，以驗證 HNV 提供者邏輯網路上的連線能力
2.  Hoster.config請確定 hyper-v 主機上的 MTU 設定是否正確，以及 Hyper-v 主機之間的任何第2層切換裝置。 ``Test-EncapOverheadValue``在有問題的所有 hyper-v 主機上執行。 此外，請檢查中的所有第2層交換器是否將 MTU 設定為至少1674個位元組，以考慮160個位元組的最大額外負荷。
...
網路控制站可以自動收集偵錯工具記錄檔，並將它們儲存在集中式位置。 當您第一次或稍後部署網路控制站時，可以啟用記錄檔收集。 記錄檔是從網路控制站收集而來，而網路控制卡則是由網路控制站管理：主機電腦、軟體負載平衡器 (SLB) 和閘道電腦。

這些記錄包含網路控制站叢集、網路控制站應用程式、閘道記錄、SLB、虛擬網路和分散式防火牆的「偵錯工具記錄」。 每當有新的主機/SLB/閘道新增至網路控制站時，就會在這些機器上開機記錄。
同樣地，從網路控制站移除主機/SLB/閘道時，這些機器上的記錄就會停止。

#### <a name="enable-logging"></a>啟用記錄

當您使用**NetworkControllerCluster** Cmdlet 安裝網路控制站叢集時，會自動啟用記錄功能。 根據預設，記錄檔會在網路控制站節點的本機上收集，網址為 *%systemdrive%\SDNDiagnostics*。 **強烈建議**您將此位置變更為遠端檔案共用， (不是本機) 。
...
使用預設設定時，建議您在中央位置至少有 75 GB 的可用空間，而且本機節點上必須有 25 GB， (如果不使用中央位置) 適用于3節點的網路控制站叢集。

#### <a name="change-logging-settings"></a>變更記錄設定

您可以使用 Cmdlet 隨時變更記錄設定 ``Set-NetworkControllerDiagnostic`` 。 可以變更下列設定：

- **集中式記錄位置**。  您可以使用參數來變更儲存所有記錄檔的位置 ``DiagnosticLogLocation`` 。
- **存取記錄位置的認證**。  您可以使用參數來變更認證，以存取記錄位置 ``LogLocationCredential`` 。
- **移至本機記錄**。  如果您已提供集中位置來儲存記錄，您可以使用參數在本機的網路控制卡節點上進行登入， ``UseLocalLogLocation`` 因為) 的磁碟空間需求很大，因此不建議 (。
...
如果使用者已執行 NetworkController 指令_程式_，則每個已在網路控制卡中指定了伺服器資源的 hyper-v 主機，都可以使用其他記錄檔。 如果已啟用) 會保留在 C:\NCDiagnostics 中，這些記錄 (和追蹤

### <a name="slb-diagnostics"></a>SLB 診斷

#### <a name="slbm-fabric-errors-hosting-service-provider-actions"></a> (裝載服務提供者動作) 的 SLBM Fabric 錯誤

1.  檢查 (SLBM) 的軟體 Load Balancer 管理員是否正常運作，以及協調流程層可以彼此溝通： SLBM-> SLB Mux 和 SLBM > SLB 主機代理程式。 從任何可存取網路控制站 REST 端點的節點執行[DumpSlbRestState](https://github.com/Microsoft/SDN/blob/master/Diagnostics/DumpSlbRestState.ps1) 。
2.  在其中一個網路控制站節點 Vm 上驗證 PerfMon 中的*SDNSLBMPerfCounters* (最好是主要網路控制站節點 NetworkControllerReplica) ：
    1.  Load Balancer (LB) 引擎連線到 SLBM 嗎？  (*SLBM LBEngine 設定總計*> 0) 
    2.  SLBM 至少會知道自己的端點嗎？  (*VIP 端點總計*>= 2 ) 
    3.  Hyper-v (DIP 是否) 主機連線到 SLBM？  (的*HP 用戶端連線*= = 伺服器數目) 
    4.  SLBM 是否已連線至 Mux？ *Muxes Connected*  ==  *在 SLBM*  ==  *mux 回報狀況*良好 = # SLB mux vm) 上， (mux 連線 mux 狀況良好。
3.  請確定設定的 BGP 路由器已成功與 SLB MUX 對等互連
    1.  如果使用 RRAS 與遠端存取 (亦即 BGP 虛擬機器) ：
        1.  Bgp 應該會顯示 [已連線]
        2.  BgpRouteInformation 應該至少顯示 SLBM 自我 VIP 的路由
    2.  如果使用實體的最大機架 (ToR) 切換為 BGP 對等，請參閱您的檔
        1.  例如： # show bgp 實例
4.  在 SLB Mux VM 上驗證 PerfMon 中的*SlbMuxPerfCounters*和*SLBMUX*計數器
5.  檢查軟體 Load Balancer 管理員資源中的設定狀態和 VIP 範圍
    1.  NetworkControllerLoadBalancerConfiguration-ConnectionUri <HTTPs:// <FQDN or IP> | convertto-html-json-深度 8 (檢查 IP 集區中的 VIP 範圍，並確保 SLBM 的自我 VIP (*LoadBalanacerManagerIPAddress*) 和任何租使用者對應的 vip 都在這些範圍內) 
...
**補救**根據呈現的下列診斷資訊，修正下列問題：
* 確保 SLB Multiplexers (已連線
  * 修正憑證問題
  * 修正網路連線問題
* 確保已成功設定 BGP 對等互連資訊
* 請確認登錄中的主機識別碼符合伺服器資源 (參考附錄中的伺服器實例識別碼，以取得*HostNotConnected*錯誤碼) 
* 收集記錄
...
3.  Hoster.config如果未探索或連接 DIP 端點：
    1.  檢查*NetworkControllerConfigurationState*
        1.  驗證 NC 和 SLB 主機代理程式已使用成功連線到網路控制站事件協調器``netstat -anp tcp |findstr 6640)``
    2.  檢查*nchostagent*服務登錄中的*HostId* (參照*HostNotConnected*錯誤碼，附錄) 符合對應的伺服器資源的實例識別碼 (``Get-NCServer |convertto-json -depth 8``) 
